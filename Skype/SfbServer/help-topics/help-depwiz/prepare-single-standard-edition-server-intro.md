...
若要开始安装 Skype for Business Server 2015 Standard Edition Server，该服务器将保留中央管理存储以及您选择的其他并并的服务，您必须以将成为 Standard Edition Server 的服务器上本地 管理员组 的成员身份登录。 “准备单个 Standard Edition Server”页将详细阐述初始安装要求。 计算机必须是要部署在其中的域的成员，并且必须已成功为林完成架构、林和域准备。
