...
- 为所有边缘服务（包括访问边缘服务、Web 会议边缘服务和 A/V 边缘服务）使用单个完全限定的域名 (FQDN) 和 IP 地址。 如果不选择使用一个 FQDN 和 IP 地址的选项，则需要在部署过程中分别为这三个边缘服务指定单独的 FQDN 和 IP 地址。 有关边缘服务的详细信息，请参阅规划文档中的[所需的外部用户访问组件](https://technet.microsoft.com/library/2d0f9817-14e7-4109-95dc-62420e3c29e2.aspx)。
...
- 支持联盟。 如果要支持内部用户与组织外受信任域中的用户（包括支持的公共即时消息 (IM) 提供商的任何用户）之间的通信，则选择此选项。 如果选择此选项，则需要配置对要支持的特定联盟域和公共 IM 连接服务提供商的支持。 有关配置联合身份验证和其他类型的外部用户访问支持的详细信息，请参阅边缘服务器部署文档中的[配置支持外部用户访问](https://technet.microsoft.com/library/f8424f8c-f965-4414-8485-30f07e10214a.aspx)。
