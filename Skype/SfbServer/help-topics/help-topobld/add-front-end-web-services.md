...
无法覆盖 Standard Edition Server 的内部 Web 服务池完全限定域名 (FQDN)。 如果要为 Enterprise Edition 前端池配置域名系统 (DNS) 负载平衡，可以指定一个不同的内部基 URL，该 URL 必须与池 FQDN (例如 \<your base URL\> ，internal-) 。
