...
3. 在“**定义新文件存储**”中的“**文件服务器 FQDN**”下，键入文件服务器的完全限定域名 (FQDN)。在“**文件共享**”下，键入新文件共享的文件夹名称，然后单击“**确定**”。
    ...
    > [!IMPORTANT]
    > 此步骤在拓扑生成器中定义为使用新文件存储。 只需定义一次，无需针对每台服务器定义。 发布拓扑之前，必须在定义的文件服务器上创建定义的文件共享。 有关详细信息，请参阅[定义前端的文件存储区](http://technet.microsoft.com/library/90994400-c4e5-4509-af41-121ac716fbca.aspx)。 
...
9. 发布拓扑，检查复制状态，并根据需要业务 Server 部署向导运行 Skype。 有关详细信息，请参阅[Removing Lync Servers and 组件的常见过程](http://technet.microsoft.com/library/5438ce1e-57fa-4031-8bdb-3af6581d901b.aspx)。
...
14. 从拓扑中删除旧文件存储，然后发布拓扑。 有关详细信息，请参阅[删除文件存储](http://technet.microsoft.com/library/1ba7eb15-5c87-4357-b4d8-f59409ac7f71.aspx)。
