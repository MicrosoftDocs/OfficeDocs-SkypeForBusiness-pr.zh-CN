...
---
# <a name="move-file-store-data-to-a-new-file-store-in-skype-for-business-server-2015"></a>在 Skype for Business Server 2015 中将文件存储数据移动到新文件存储

如果需要删除当前充当 Skype for Business Server 2015 部署的文件存储的文件服务器，或者需要进行其他会使当前文件存储不可用的更改，则首先需要创建新的共享。 然后，需要执行以下步骤：
...
1. 以 RTCUniversersalServerAdmins 或 CsServerAdministrator 组（其中安装了 Skype for Business Server 2015 管理工具）成员登录到计算机。
...
6. 启动拓扑生成器 **：单击"** 开始"，**单击"** 所有程序"，再单击 **"Skype for Business Server 2015"，** 然后单击 **"Skype for Business Server 2015Topology Builder"。**
