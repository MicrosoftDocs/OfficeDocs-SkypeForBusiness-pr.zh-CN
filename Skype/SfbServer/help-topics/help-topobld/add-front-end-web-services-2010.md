...
不能覆盖 Standard Edition Server 的内部 Web 服务池 (FQDN) FQDN。 如果要为 Enterprise Edition 前端池配置域名系统 (DNS) 负载平衡，可以指定一个不同的内部基 URL (该 URL 必须不同于池 FQDN，例如，可以是内部 url) 。 \<your base URL\>
