...
- 如果部署包含持久聊天，则规划工具将添加一台额外的服务器，但不增加池数量。 例如，如果部署已包含四台服务器，规划工具将建议添加另一台服务器 (一个服务器) 维护一个池。 
    
规划工具还会添加所有SQL数据库的镜像数据库。 例如，如果有前端 SQL Server 数据库，规划工具将添加另一个数据库作为此数据库的镜像数据库，并将其名称为"前端镜像SQL数据库。
