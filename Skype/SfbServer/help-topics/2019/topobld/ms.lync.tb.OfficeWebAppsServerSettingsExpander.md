...
编辑配置的 Office Web Apps Server 的属性。 可编辑以下属性：

 **Office Web Apps Server FQDN：** 此属性定义 Office Web Apps Server 的完全限定域名，如果 IPv6 用于) 记录，则应该匹配域名系统 (DNS) 主机 A 或 AAAA (。

 **Office Web Apps Server** 发现 URL：统一资源定位器 (URL) ，对于客户端访问 Office Web Apps Server，如果服务器位于除内部网络外的其他网络区域中，可能需要编辑默认地址。
