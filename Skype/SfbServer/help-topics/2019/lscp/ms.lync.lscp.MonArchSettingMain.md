...
- 与 Exchange 存储集成，适用于位于 Exchange 上的用户
...
> [!NOTE]
> 存档配置适用于 Skype for Business Server 上用户，如果使用 Exchange 将存档数据存储在 Microsoft Exchange 中，则存档配置适用于位于 Exchange 上但对 Exchange 上用户实现略有不同的用户。 将在下节中介绍这些差异。 
