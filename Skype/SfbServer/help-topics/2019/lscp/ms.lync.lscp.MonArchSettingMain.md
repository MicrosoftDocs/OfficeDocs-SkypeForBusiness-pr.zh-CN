...
> [!NOTE]
> 存档配置适用于位于 Skype for Business Server 上的用户;如果使用 Exchange 在 Microsoft Exchange 中存储存档数据，则存档配置适用于位于 Exchange 上的用户，但对于位于 Exchange 上的用户，其实现方式略有不同。 将在下节中介绍这些差异。 
