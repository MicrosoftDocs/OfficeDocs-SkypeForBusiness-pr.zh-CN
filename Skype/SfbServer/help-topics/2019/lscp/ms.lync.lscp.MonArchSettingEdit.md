...
> [!NOTE]
> 存档配置适用于 Skype for Business Server 上的用户，如果启用 Microsoft Exchange 集成选项以使用 Exchange 在 Microsoft Exchange 中存储存档数据，则存档配置适用于 Exchange 上的用户。 但是，某些选项的实现方式与在 Exchange稍有不同，如下一节中所述。 
