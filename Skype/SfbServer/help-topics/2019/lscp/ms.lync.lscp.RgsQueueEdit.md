...
- **启用队列退出** 选中此复选框可指定呼叫者在代理应答呼叫之前等待等待的最大时间。 如果选择此选项，则还需要指定以下内容：
  ...
  - **转发到 SIP 地址** 选择此选项以将呼叫转发给其他用户。 在 **"SIP** 地址"中，以 sip：\<username>@\<domainname> 格式键入用户的 URI。
...
- **启用队列溢出** 选中此复选框可指定队列可以保留的最大呼叫数。 如果选择此选项，则还需要指定以下内容：
  ...
  - **转发到语音邮件** 如果选择此选项，请在 **"SIP** 地址"中，键入 sip：\<username>@\<domainname> 格式的语音邮件地址 (例如，sip:bob@contoso.com) 。
...
有关响应组特性和功能的详细信息，请参阅规划文档中[Skype for Business Server中的规划](../../../plan-your-deployment/enterprise-voice-solution/response-group.md)响应组应用程序。 有关使用队列的详细信息，请参阅操作文档中的[Managing Response Group Queues](/previous-versions/office/lync-server-2013/lync-server-2013-managing-response-group-queues)。