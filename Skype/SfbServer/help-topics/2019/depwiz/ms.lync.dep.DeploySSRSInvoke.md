...
> [!IMPORTANT]
> 若要成功部署 Reporting Services 报告，且在部署完成后访问报告，必须具有 TCP/IP 端口 80 (，并且（可选）TCP 端口 443（对于 SSL）（如果向 SQL Server 上高级安全 Windows 防火墙中打开的 Reporting Services) 分配证书）。 有关详细信息，请参阅[Configure the Windows Firewall to Allow SQL Server Access](/sql/sql-server/install/configure-the-windows-firewall-to-allow-sql-server-access) for Microsoft SQL Server 2008 R2。
