...
> [!NOTE]
> 存档策略仅适用于位于 Skype for Business Server 上的用户。 如果使用 Exchange 集成在 Microsoft Exchange 中存储存档数据，则 Exchange 2013 策略将控制 Exchange 2013 上用户存档。 若要为这些用户启用存档，必须将用户的邮箱置于In-Place保留状态。 
