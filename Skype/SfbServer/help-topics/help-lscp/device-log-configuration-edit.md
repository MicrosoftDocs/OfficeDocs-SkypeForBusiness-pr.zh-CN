...
可以将设备日志配置添加到"编辑日志设置"页，该页确定在清除 日志文件 之前保留的最大日志缓存大小、最大 日志文件 大小或保留时间长度。 您可以根据组织的要求更改这些设置。
...
- **最大文件大小 (字节数)** 可以指定在清除日志文件大小。 默认值为 1，024，000 字节 (1 MB) 。
    
- **最大缓存大小 (字节数)** 可以指定在必须清除缓存 (数据写入缓存之前) 在 日志文件 缓存中保留的最大信息量（以字节日志文件）。 默认值为 512，000 字节 (0.5 MB) 。
