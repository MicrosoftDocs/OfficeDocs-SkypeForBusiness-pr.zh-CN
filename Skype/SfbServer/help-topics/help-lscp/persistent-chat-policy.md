...
您可以使用“**持久聊天**”组的“**持久聊天策略**”页来管理全局级、池级、站点级或用户级的策略，包括为部署配置默认全局策略以及创建一个或多个其他用户和站点策略。 如果持久聊天服务器启用了用户策略，则在其客户端不会出现持续聊天服务器环境。
...
如果用户已启用的 Skype 业务服务器，您可以对特定用户启用或禁用它们持久聊天服务器应用适当的政策。
