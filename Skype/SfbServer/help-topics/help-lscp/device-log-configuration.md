...
> [!NOTE]
> 您还可以配置一天中您希望设备更新 Web 服务自动删除超过您配置为默认情况下保留日志文件 (的天数的日志文件（即超过) 年 10 天的日志文件）的时间。 无法使用"控制面板"Skype for Business Server修改此设置。 您必须使用命令行管理Skype for Business Server命令行管理程序。 若要指定删除过期日志文件的时间，请使用 **New-CsDeviceUpdateConfiguration** cmdlet 和 -LogCleanUpTimeOfDay 参数。 有关详细信息，请参阅 [New-CsDeviceUpdateConfiguration](/powershell/module/skype/new-csdeviceupdateconfiguration?view=skype-ps)。
