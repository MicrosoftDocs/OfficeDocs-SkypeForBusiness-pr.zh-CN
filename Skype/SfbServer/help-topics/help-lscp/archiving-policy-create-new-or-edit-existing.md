...
> [!NOTE]
> 如果使用Exchange集成在 Microsoft Exchange 中存储存档数据，Exchange 2013 策略将控制 2013 年 2013 Exchange存档。 若要为这些用户启用存档，必须将用户的邮箱置于In-Place保留状态。 
...
- **存档外部通信** 选中此复选框可启用包括外部用户（如远程用户、 (匿名用户和 PIC 设置用户) 联盟伙伴）的通信的存档。 默认情况下，任何策略中都未启用此功能。
