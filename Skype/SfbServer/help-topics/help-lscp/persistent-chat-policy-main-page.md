...
> [!NOTE]
> 在拓扑中，持久聊天服务器站点策略全局应用、每用户池、每用户站点或每用户应用。 
...
在Skype for Business Server控制面板中，定义可分配给"用户"中的用户 **的用户策略**。
...
如果为用户启用了聊天Skype for Business，您可以将相应的策略应用于特定用户，以对持久聊天服务器启用或禁用这些策略。
...
5. 在 **"编辑 Lync Server 用户"** 中的"持久 **聊天策略"下**，选择要应用持久聊天用户策略。
