...
了解如何准备云连接器设备以进行部署，并将其与电话系统 (云 PBX) 配合使用。
...
> [!NOTE]
> 仅对部署中的第一台设备执行一些步骤：为网站目录创建共享、下载 bits 并准备虚拟硬盘 (VHDX) 文件从 Windows Server ISO 映像。 
...
2. Vhd、SfBBits 和 VersionInfo 文件将存储在 **网站目录**中。
   ...
   ```
    ...
    如果要为站点部署高可用性 (HA) ，请确保运行 cmdlet 以将 **网站目录** 设置为站点中每台主机服务器上的相同位置。
...
若要更新该文件，请先运行以下 cmdlet 以获取 ( # A0) 的示例模板：
...
在使用环境的值对其进行更新之后，请将该文件另存为 **设备目录**中的 CloudConnector.ini。 您可以运行 **set-ccappliancedirectory** 以确定 **设备目录**的路径。

更新 .ini 文件时，请考虑以下事项：

> [!NOTE]
> 并非 .ini 文件的所有值都在此部分中进行讨论，此处仅介绍了那些具有特殊注意事项的值。 有关完整列表，请参阅[Plan For Skype For Business Cloud Connector Edition](plan-skype-for-business-cloud-connector-edition.md)主题中的 "[确定部署参数](plan-skype-for-business-cloud-connector-edition.md#BKMK_SiteParams)" 部分。 有关需要为其他设备或新网站更改的值的详细信息，请参阅 " [具有高可用性的单个站点 (HA) ](deploy-multiple-sites-in-cloud-connector.md#BKMK_SingleSitecomparedtomulti-site) 与主题" 在 [云连接器中部署多个站点](deploy-multiple-sites-in-cloud-connector.md)"中的多站点部署。 
...
此步骤将从 Windows Server 2012 ISO 映像准备虚拟硬盘 (VHDX) 文件。 VHDX 将用于在部署过程中创建虚拟机。 将创建 (基本 VM) 的临时虚拟机，并将从 ISO 文件安装 Windows Server 2012。 创建虚拟机后，将会安装一些必要的组件，并将应用最新的 Windows 更新。 最终，基本 VM 将 (sysprep) 进行一般化，并进行清理，只留下生成的虚拟磁盘文件。

> [!NOTE]
> 只需为第一台设备执行此步骤。 

在继续执行此步骤之前，请确保已创建公司网络交换机。 此外，请确认在 CloudConnector.ini 文件中正确配置了以下设置：
...
如果要部署多站点部署，则无需将 ISO 转换为每个站点的 VHD。 您可以将为第一个站点创建的 VHDX 复制到第二个站点的主机服务器。
 ...
 在主机服务器上将文件复制到同一文件位置 ( 的 " **网站目录** \Bits\VHD" 文件夹) ，并在其他网站上使用相同的文件名。
...
## <a name="change-local-group-policy-on-the-host-machine-versions-141-and-earlier"></a>在主机上更改本地组策略 (版本1.4.1 及更早版本) 

> [!NOTE]
> 云连接器版本1.4.2 和更高版本不需要执行此任务。 

在 Skype for Business 云连接器版本部署过程中，将创建 CceService 帐户。 它运行云连接器管理服务，并需要具有卸载 cloudconnector.msi 的权限。 您必须更改云连接器主机上的组策略设置，以指定用户注销时不应卸载用户注册表。 请按照以下步骤操作：
