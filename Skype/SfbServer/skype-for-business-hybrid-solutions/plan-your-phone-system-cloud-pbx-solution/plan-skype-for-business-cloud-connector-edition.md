...
查找有关 Skype for Business 云连接器版本的信息，这是一组打包的虚拟机（Vm），用于实现与 Office 365 （云 PBX）中的电话系统的本地 PSTN 连接。

如果尚不存在现有的 Lync Server 或 Skype for Business Server 部署，则云连接器版本可能是组织的合适解决方案。 如果你仍在调查 Office 365 解决方案中的哪个电话系统适合你的企业，请参阅[Microsoft 电话解决方案](https://docs.microsoft.com/SkypeForBusiness/hybrid/msft-telephony-solutions)。
...
云连接器版本2.1 现已推出。 如果您尚未升级到2.1，请参阅[升级到新版本的云连接器](upgrade-to-a-new-version-of-cloud-connector.md)。 您可以在[https://aka.ms/CloudConnectorInstaller](https://aka.ms/CloudConnectorInstaller)中找到安装文件。
...
云连接器将提供驻留在本地和联机用户之间的路由，你可以选择将自己的 PSTN 提供商与 Office 365 中的电话系统配合使用。
...
- 云连接器不需要完整的本地 Skype for business Server 部署。
    ...
    目前，云连接器不能与 Lync 或 Skype for Business 本地服务器共存。 如果要将现有 Lync 或 Skype for Business 用户移动到 Office 365 并为用户持续提供本地电话，请考虑使用现有 Skype for Business Server 部署的 Office 365 中的电话系统与本地连接。 有关详细信息，请参阅在 office [365 （云 PBX）解决方案中规划电话系统](plan-your-phone-system-cloud-pbx-solution.md)和在[office 365 中规划使用本地 PSTN 连接的 office 中的电话系统（在 Skype for business Server 中](plan-phone-system-with-on-premises-pstn-connectivity.md)）。

- 如果您以前使用的是 Skype for Business 或 Lync Server 部署，并且您扩展了架构，则无需清理云连接器部署的架构，只要您从环境中删除了所有 Skype for Business 或 Lync Server 组件。

- 你的用户已联机托管。

- 如果您的组织已配置目录同步（DirSync），则必须先在本地部署中创建规划其混合语音的所有用户帐户，然后再将其同步到云。

- 如果需要，你可以保留当前的 PSTN 运营商。

- 如果要向托管在云连接器上的用户提供电话拨入式会议，则可以购买来自 Microsoft 的音频会议的 PSTN 会议许可证或付费产品。

- 呼叫升级还需要音频会议许可证（或 "即付即用" 提供）。 如果 Skype for Business 用户收到来自外部 PSTN 用户的呼叫，并且想要向该呼叫添加一个更多参与者（将呼叫升级到会议），则将通过 Microsoft 音频会议服务执行升级。

- 云连接器2.0 及更高版本现在支持媒体旁路。 媒体旁路允许客户端直接向公用电话交换网（PSTN）的下一个跃点（网关或会话边界控制器（SBC））发送媒体，并从媒体路径中消除云连接器版本组件。 有关详细信息，请参阅[在云连接器版本中规划媒体旁路](plan-for-media-bypass-in-cloud-connector-edition.md)。

- 云连接器2.1 及更高版本支持使用 Operations Management Suite （OMS）监视云连接器。 有关详细信息，请参阅[使用 Operations Management Suite （OMS）监视云连接器](monitor-cloud-connector-using-operations-management-suite-oms.md)
...
- **中央管理存储（CMS）角色**-包括拓扑组件的配置存储，其中包括 CMS 文件传输。
...
- 有两种类型的硬件配置使用云连接器进行了测试：
  ...
  - 较小的版本用于在低端硬件上运行，可用于评估目的或较低呼叫量的站点。 如果部署较小版本的云连接器，您仍需要注意生产级的硬件要求（如双电源）。

- 如果您有云连接器版本2.0 或更高版本，并且您部署16个设备的最大配置（使用较大的硬件），则 PSTN 站点可以处理多达8000个同时进行的呼叫。 如果部署较小版本，则支持的限制为800。

    您还需要将一些设备专用于高可用性。 最小建议是，应保留一个设备以实现高可用性。

  - 在第2版中，如果部署 15 + 1 配置，PSTN 站点可以处理多达7500个并发呼叫。

  - 如果您具有早期版本，并部署最大 3 + 1 配置（使用较大的硬件），则 PSTN 站点可以处理多达1500个并发呼叫。 如果部署较小版本，则支持的限制为150。
...
- 为部署中的每个云连接器版本支持安装4个虚拟机所需的硬件。 建议使用以下配置：
  ...
  - 64位双处理器、六核（12个实核）、2.50 千兆位（GHz）或更高版本
...
- 如果选择部署支持多达50同时呼叫的较小版本的云连接器版本，将需要以下硬件：
  ...
  - Intel i7 4790 四核处理器，带英特尔4600图形（无需高端显卡）
...
- 如果用于浏览 Internet 的主机上需要代理服务器，则必须进行以下配置更改：
  ...
  - 若要绕过代理，请指定您的代理服务器设置的 WinHTTP 代理设置以及包含 "192.168.213" 的旁路列表。\*在 cloudconnector.ini 文件中定义的云连接器管理服务和 Skype for business 网络网络子网所使用的网络。 否则，管理连接将失败并阻止云连接器的部署和自动恢复。 以下是 winhttp 配置命令示例： netsh winhttp set proxy "10.10.10.175： 8080" 旁路-list = "local;\*1"。\*; 172.20。\*; 192.168.218.。\*"\<local\>"。
  ...
  - **组策略：** 计算机\>管理模板\>Windows 组件\> Internet Explorer：按每台计算机（而不是按用户）进行代理设置

- 合格的 PBX/中继或合格的 SBC/网关（建议至少使用两个网关）。

    云连接器支持为 Skype for business 认证的相同的会话边界控制器（SBCs）。 有关详细信息，请参阅[Skype for business 的电话基础结构](https://docs.microsoft.com/SkypeForBusiness/certification/infra-gateways)。
...
- Office 365 Skype for Business 管理员凭据，用于运行远程 PowerShell。
...
> [!NOTE]
> 运行云连接器的最低硬件指南基于基本硬件容量（内核、MHz、千兆字节等），其中一些缓冲区可适应任何计算机的体系结构中的无形性能障碍。 Microsoft 已在商业可用硬件上运行最糟糕的事例负载测试，这是最低指导。 验证媒体质量和系统性能。 Microsoft 官方的云连接器设备合作伙伴具有特定的云连接器硬件实施，它们具有独立测试的性能，并且其硬件适用性符合负载和质量要求。
...
- 定义端口范围（请参阅[端口和协议](plan-skype-for-business-cloud-connector-edition.md#BKMB_Ports)）。
...
|云连接器中介组件  <br/> |内部客户端  <br/> |TCP 49 152-57 500\*  <br/> |TCP 50000-50019  <br/> Optional  <br/> |
...
例如，以下命令将中介组件用于媒体流量的端口数限制为 50 000-51 000 用于音频（入和出）。 中介组件将能够使用此配置处理250并发呼叫。 请注意，您还可能希望在 SBC/PSTN 网关上限制此范围：
...
在这种情况下，到 internet 的所有媒体流量都将通过联机边缘流动，如下所示：用户\>终结点-在线\>边缘--云连接器边缘：
...
|云连接器边缘外部接口  <br/> |任意  <br/> |UDP 3478  <br/> |UDP 3478  <br/> |
|任意  <br/> |云连接器边缘外部接口  <br/> |TCP 50000-59999  <br/> |TCP 443  <br/> |
|任意  <br/> |云连接器边缘外部接口  <br/> |UDP 3478  <br/> |UDP 3478  <br/> |
|云连接器边缘外部接口  <br/> |任意  <br/> |TCP 50000-59999  <br/> |TCP 443  <br/> |
...
|云连接器边缘外部接口  <br/> |任意  <br/> |任意  <br/> |UDP 53  <br/> |
|云连接器边缘外部接口  <br/> |任意  <br/> |任意  <br/> |TCP 53  <br/> |
|云连接器边缘外部接口  <br/> |任意  <br/> |TCP 50000-59999  <br/> |任意  <br/> |
|云连接器边缘外部接口  <br/> |任意  <br/> |UDP 3478;UDP 50000-59999  <br/> |任意  <br/> |
|任意  <br/> |云连接器边缘外部接口  <br/> |任意  <br/> |TCP 443;TCP 50000-59999  <br/> |
|任意  <br/> |云连接器边缘外部接口  <br/> |任意  <br/> |UDP 3478;UDP 50000-59999  <br/> |
...
对于您计划部署的每个 PSTN 站点，您需要在开始部署之前收集以下信息。 更新 Cloudconnector.ini 文件时，需要提供此信息。
...
Optional若要限制出站呼叫号码，请更新 LocalRoute 值。
...
|O365 Online 服务 Fqdn  <br/> |对于世界范围的 O365 实例，大多数情况下都必须是默认值。  <br/> .Ini 文件中的名称： "OnlineSipFederationFqdn"  <br/> ||
...
|CMS 计算机 IP 地址  <br/> |CMS 服务器的 IP 地址（外围网络中的内部）。  <br/> INI 文件中的名称： "主要中央管理服务的参数" 下的 "IP"  <br/> ||
|文件共享名称  <br/> |要在 CMS 服务器上为 Skype for Business 复制数据创建的文件共享名称（例如，CmsFileStore）。  <br/> 在大多数情况下，可以使用默认建议的值。  <br/> .Ini 文件中的名称： "主要中央管理服务的参数" 下的 "CmsFileStore"  <br/> ||
...
|中介组件计算机 IP 地址  <br/> |中介组件的内部公司网络 IP （外围网络中的内部）。  <br/> .Ini 文件中的名称： "中介服务器池的参数" 下的 "IP"  <br/> ||
...
|访问边缘的外部 IP  <br/> |边缘组件的外部 IP （如果没有可用的 NAT，则为公用 IP）; 或者已转换 IP （如果映射，请指定两个地址）。  <br/> .Ini 文件中的名称： "边缘服务器池的参数" 下的 "ExternalSIPIPs"  <br/> ||
...
|媒体中继边缘的外部 IP  <br/> |目前仅支持一个 IP，因此这是与访问边缘相同的 IP （公用或映射的 IP）（如果映射，请同时指定两个地址）。 可以是与访问边缘的边缘组件外部 IP 相同的地址。 注释如果边缘位于 NAT 后面，还需要指定下一个参数的值。  <br/> .Ini 文件中的名称： "边缘服务器池的参数" 下的 "ExternalMRIPs"  <br/> ||
|媒体中继边缘的外部 IP （如果边缘位于 NAT 之后）  <br/> |如果你的边缘位于 NAT 后面，你还需要指定 NAT 设备的公用地址。  <br/> .Ini 文件中的名称： "边缘服务器池的参数" 下的 "ExternalMRPublicIPs"  <br/> ||
...
|语音网关2品牌和型号（如果网关数超过2个，请复制此行）  <br/> |指定语音网关的品牌和型号。 请注意，您可以从的已测试设备列表中连接设备[https://technet.Microsoft.com/UCOIP](https://technet.Microsoft.com/UCOIP)。  <br/> ||
|语音网关1名称  <br/> |用于生成 AD 域的计算机 FQDN。 如果在中介组件和语音网关之间使用 TLS，则是必需的。 如果您不打算使用 FQDN，例如，TLS 不是必需的，或者语音网关不支持使用 FQDN （仅 IP）的连接—请指定。  <br/> ||
|语音网关2名称（如果网关数超过2个，请复制此行）  <br/> |用于生成 AD 域的计算机 FQDN。 如果在中介组件和语音网关之间使用 TLS，则是必需的。 如果您不打算使用 FQDN，例如，TLS 不是必需的，或者语音网关不支持使用 FQDN （仅 IP）的连接—请指定。  <br/> ||
|语音网关 1 IP 地址  <br/> |语音网关的 IP 地址。  <br/> ||
|语音网关 2 IP 地址（如果网关数超过2个，请复制此行）  <br/> |语音网关的 IP 地址。  <br/> ||
|语音网关1端口号（如果网关数超过2个，请复制此行）  <br/> |语音网关 SIP 中继将侦听的端口，例如5060。  <br/> ||
|语音网关2端口#  <br/> |语音网关 SIP 中继将侦听的端口，例如5060。  <br/> ||
|SIP 流量的语音网关1协议  <br/> |TCP 或 TLS。  <br/> ||
|适用于 SIP 流量的语音网关2协议（如果网关数超过2个，请复制此行）  <br/> |TCP 或 TLS。  <br/> ||
|进出边缘组件的流量的外部媒体端口范围  <br/> |与边缘的外部接口之间的媒体流量的 TCP/UDP 端口范围。 必须始终从 50 000 开始。 有关详细信息，请参阅 "端口和协议"。  <br/> |50000-59 999  <br/> |
|通过内部防火墙与中介组件进行通信的媒体端口范围  <br/> |中介组件将用来与客户端和网关通信的 UDP 端口范围（建议每个呼叫4个端口）。  <br/> ||
...
|云连接器域管理员密码  <br/> 仅限版本1.4。2  <br/> |云连接器域管理员的密码（与生产域不同）。 User name 为 Administrator。 您不能更改用户名。  <br/> ||
...
|Office 365 租户管理  <br/> | 云连接器使用该帐户来更新和管理云连接器的租户设置： <br/>  版本2.0 及更高版本：具有 Skype for Business 管理员权限的专用 Office 365 帐户的凭据。 <br/>  2.0 之前的版本：具有全局租户管理员权限的专用 Office 365 帐户的凭据。 <br/> ||
...
|ForwardCallHistory  <br/> 版本2.0 及更高版本  <br/> | 此参数用于打开用于在同时响铃、呼叫转接和呼叫转移方案中报告初始呼叫者的 SIP 标头。 将此参数设置为 True 将启用两个 SIP 标头： <br/>  历史记录-信息 <br/>  引用者 <br/>  History-Info 标头用于对 SIP 请求进行重新设定，"提供（s）用于捕获请求历史记录信息的标准机制" 为网络和最终用户启用各种服务（[RFC 4244-Section 1.1](http://www.ietf.org/rfc/rfc4244.txt)）。 对于云连接器中继接口，这在同时和呼叫转发方案中使用。  <br/>  值可以为 True 或 False。 默认值为 False。 <br/> ||
...
每个边缘组件都需要来自公共证书颁发机构的证书。 证书必须具有可导出的私钥才能在边缘组件之间进行复制。 若要满足证书要求，您需要在以下选项之间做出决定，并提供证书的使用者名称（SN）和使用者可选名称（SAN）。
...
> [!NOTE]
> 您不能为 sip 创建外部 DNS 条目。\<sipdomain\>，因为此名称属于 Office 365 部署。
...
您需要为每个 SIP 域添加 sip.sipdomain.com 和每个域的访问边缘池的名称（可以是一个物理池，但具有不同的名称）。 下面是一个多 sip 域方案中 SN 和 SAN 条目的示例：
...
部署用于高可用性的云连接器版本时，至少要部署两个充当彼此的备份的设备。 每个设备由四个组件组成：边缘、中介、中央管理存储（CMS）和域控制器。
...
4. 将呼叫路由到边缘组件（SIP 和媒体流先通过在线边缘;媒体将通过内部防火墙转到中介组件）。
