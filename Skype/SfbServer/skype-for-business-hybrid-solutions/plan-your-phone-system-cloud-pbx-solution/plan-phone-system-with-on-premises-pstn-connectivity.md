...
- 你拥有一些本地Skype for Business用户，另一些用户则位于 Skype for Business Online 中。 现在可以为 电话系统 Online 中的用户启用 Skype for Business 功能，但继续使用本地 PSTN 连接。

- 你拥有本地部署，并且希望将部分或所有用户移动到 Skype for Business Online，但继续使用本地 PSTN 连接。

    > [!IMPORTANT]
    > 若要为用户电话系统本地 PSTN 连接，其 SIP 地址必须位于您自己的域中。 不支持将默认域用于Microsoft 365或Office 365 onmicrosoft.com，不支持使用默认域。 

若要了解有关服务电话系统，包括许可和计划，请参阅[pstN Calling plans for Skype for Business](https://support.office.com/article/PSTN-Calling-plans-for-Skype-for-Business-f47c6a97-bc8b-42e6-b5d4-ce6b41ed1918)。
...
部署云托管服务（如电话系统本地 PSTN 连接）时，必须记住以下事项。 在纯本地 Skype for Business Server 2015 企业语音部署中，所有基础结构和客户端都位于企业自己的网络中。 此网络的质量和性能（对于高质量音频和视频至关重要）由企业员工直接控制。 使用电话系统 PSTN 连接时，涉及三个网络，其中两个网络由客户负责，但企业员工只能直接控制其中一个网络：
...
## <a name="prerequisites-for-using-phone-system-with-on-premises-pstn-connectivity"></a>使用本地 PSTN 电话系统 PSTN 连接的先决条件

在可以配置电话系统 PSTN 连接和将用户移动到 Skype for Business Online 之前，您必须确认已满足以下先决条件：
...
| 下一个跃点联盟路由内部池服务器  <br/> | Skype for Business Server 2015 年 3 月，2016 年 3 月累积更新 6.0.9319.235 或更高版本 (前端或控制器)   <br/> |
...
- **为本地企业语音配置** 和测试本地用户这包括 PSTN 连接组件。 有关详细信息，请参阅以下主题（如果您使用 Skype for Business Server 2015，请参阅在[Skype for Business Server 2015](../../plan-your-deployment/enterprise-voice-solution/enterprise-voice.md)中规划 企业语音 和 Deploy 企业语音 in [Skype for Business Server 2015。](../../deploy/deploy-enterprise-voice/deploy-enterprise-voice.md).
...
- **Active Directory 同步** 必须使用 Active Directory 同步Azure AD 连接。 有关详细信息，[请参阅Azure AD 连接。](/azure/active-directory/hybrid/how-to-connect-install-custom)

    > [!NOTE]
    > 使用的版本AAD 连接版本必须为 1.0.9125.0 或更高版本。 如果你使用的是早期版本的 AAD 连接 工具或 DirSync，请升级到受支持的版本。 您可以升级当前安装并维护环境中定义的任何自定义规则。 

- **配置混合部署** 无论是所有 Skype for Business 用户当前在线还是本地，或者如果您当前拥有混合环境，都必须完成配置 Skype for Business Server 或 Lync Server 2013 的混合部署的步骤，如D部署 Skype for Business Server 和 [Office 365](../../../SfbHybrid/hybrid/configure-hybrid-connectivity.md?bc=%2fSkypeForBusiness%2fbreadcrumb%2ftoc.json&toc=%2fSkypeForBusiness%2ftoc.json)之间的混合连接所述。 有关混合部署的更多背景信息，请参阅在混合部署[Skype for Business Server Office 365。](../../../SfbHybrid/hybrid/plan-hybrid-connectivity.md?bc=%2fSkypeForBusiness%2fbreadcrumb%2ftoc.json&toc=%2fSkypeForBusiness%2ftoc.json) 
