...
具有本地 PSTN 连接的云 PBX 不提供与完全本地 PSTN 连接解决方案相同的企业语音集。 为了帮助你确定具有本地 PSTN 连接的云 PBX 是否将为组织提供正确的功能集，请参阅以下是使用云 [PBX 获取的功能](/microsoftteams/here-s-what-you-get-with-phone-system?bc=%2fskypeforbusiness%2fbreadcrumb%2ftoc.json&toc=%2fskypeforbusiness%2ftoc.json)。
...
部署云托管服务（如具有本地 PSTN 连接的电话系统）时，必须记住以下事项。 在纯本地 Skype for Business Server 2015 企业语音部署中，所有基础结构和客户端都位于企业自己的网络中。 此网络的质量和性能（对于高质量音频和视频至关重要）由企业员工直接控制。 使用具有本地 PSTN 连接的电话系统，涉及三个网络，其中两个网络由客户负责，但只有一个企业员工直接控制：
...
在可以使用本地 PSTN 连接配置电话系统以及将用户移动到 Skype for Business Online 之前，必须确认你已满足以下先决条件：
...
- **Active Directory 同步** 必须使用 Azure AD Connect 配置 Active Directory 同步。 有关详细信息，请参阅[Azure AD Connect。](https://docs.microsoft.com/azure/active-directory/hybrid/how-to-connect-install-custom)

    > [!NOTE]
    > 使用的 AAD Connect 版本必须为版本 1.0.9125.0 或更高版本。 如果你使用的是较早版本的 AAD Connect 工具或 DirSync，请升级到受支持的版本。 您可以升级当前安装并维护环境中定义的任何自定义规则。 
