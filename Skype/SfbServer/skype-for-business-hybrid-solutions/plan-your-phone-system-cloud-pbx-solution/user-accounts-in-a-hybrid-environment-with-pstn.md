...
用户必须对业务的 Skype 启用企业语音使用在本地和 Skype 的业务用户管理工具。 为用户启用的 Skype 业务联机仅**不受支持**。 请有关如何为用户启用 for Business 的 Skype 混合配置，参阅[本文](enable-the-users-for-enterprise-voice-on-premises.md#special-considerations-when-enabling-users-for-enterprise-voice-on-premises)的详细信息。