...
- [媒体流量： IP 地址和端口范围](#media-traffic-ip-addresses-and-port-ranges)
- [支持的 Sbc](#supported-session-border-controllers-sbcs)

有关配置直接路由的详细信息，请参阅[配置直接路由](configure-direct-routing.md)。
...
## <a name="media-traffic-ip-addresses-and-port-ranges"></a>媒体流量： IP 地址和端口范围

媒体流量排列与单独的 Microsoft 云服务。 下表中显示的媒体处理器 IP 范围：
...
[配置直接路由](configure-direct-routing.md)
