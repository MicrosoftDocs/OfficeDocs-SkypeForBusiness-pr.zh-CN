...
2. 转到 "设置\>"-\> "数据-Windows 性能计数器", 然后为以下内容添加性能计数器: 
   ...
   - LS: MediationServer-入站通话 (_Total\- ) 当前 

   - LS: MediationServer-当前出站通话 (\- _total) 

     总活动媒体绕过通话:

   - LS: MediationServer-入站呼叫 (_Total\- ) 活动媒体绕过呼叫 

   - LS: MediationServer-出站呼叫 (_Total\- ) 活动媒体绕过呼叫 
...
- 针对错误警报的查询是:
  ...
  ```
    ...
    对于每台计算机, 查询将获取入站呼叫和出站呼叫的最后计数器, 并对这两个值求和。 如果 sum 值超过 500, 它将返回一个日志;如果不是, 它将返回 nothing。 简而言之, 该查询将返回其并发通话在时间窗口中过多的服务器的列表。
...
- 重置警报的查询是:
  ...
  ```
    ...
    重置查询与错误查询的内容完全相反。 对于每台计算机, 查询将获取入站呼叫和出站呼叫的最后计数器, 并对这两个值求和。 如果 sum 值小于 500, 它将返回一个日志;否则, 它将返回 nothing。
...
如果处理器使用率超过 90% 或服务被停止, 查询将从所有计算机获取所有处理器使用率计数器和服务停止事件, 并返回一个日志。 
