...
---
# <a name="get-ccappliancedirectory"></a>获得 CcApplianceDirectory
...
下面的示例显示当前文件夹的云接头组件的配置和虚拟机文件的存储位置：
...
获得 CcApplianceDirectory cmdlet 显示所有配置和虚拟机文件、 日志和外部证书都存储云接口装置。
  
每个云接头装置有四个组件： 中介服务器、 中央管理存储、 边缘服务器和域控制器。 默认的文件夹是 C:\Users\%userprofile%\CloudConnector\ApplianceRoot。 可以使用 Set-CCApplianceDirectory cmdlet 更改此文件夹。
