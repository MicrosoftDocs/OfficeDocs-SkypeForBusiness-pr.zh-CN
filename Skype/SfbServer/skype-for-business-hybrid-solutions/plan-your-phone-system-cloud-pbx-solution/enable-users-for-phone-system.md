...
所有用户都必须在本地 Active Directory 中创建，并Microsoft 365或Office 365 Azure AD 连接器的支持版本进行同步。 无法为直接在 Azure AD 电话系统Office 365用户启用登录。 如果要为在 Azure AD 中创建的用户启用具有本地 PSTN 连接的 电话系统，则需要在本地 AD 中为该用户创建新帐户，在本地配置帐户，然后使用受支持的 Azure AD 连接器工具版本同步该帐户。 
...
- [将用户同步到云并分配许可证](../../../SfbHybrid/hybrid/configure-hybrid-connectivity.md?bc=%2fSkypeForBusiness%2fbreadcrumb%2ftoc.json&toc=%2fSkypeForBusiness%2ftoc.json) (使用Office 365) 。
