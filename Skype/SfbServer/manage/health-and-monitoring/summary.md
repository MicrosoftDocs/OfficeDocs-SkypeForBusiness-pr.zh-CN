...
---
# <a name="media-quality-summary-report-in-skype-for-business-server-2015"></a>Skype for Business Server 2015 中的媒体质量摘要报告
 
**摘要：**了解媒体质量摘要报告在 Skype 业务服务器 2015年。
  
媒体质量摘要报告可能最适合用来分析组织内部的呼叫质量，此报告提供了分为以下几个类别的详细的用户体验质量 (QoE) 呼叫指标：
  
- UC 对等调用 （例如业务到业务电话的 Skype 的 Skype)
...
只要打开报告时，您就会看到有关所有这些类别的摘要信息。 无需离开该报表，您可以展开每个类别看如从 Office Communicator 2007 R2 到业务的 Skype 进行的调用的子类别。 此外，您还可以向下钻取这些子类别，以查看各子类别下进行的每个呼叫的详细信息。
  
在 Skype 的业务服务器 2015年介质质量摘要报告进一步将数据分成三种呼叫类型： 音频呼叫，视频电话和应用程序共享的调用。 每种呼叫类型在报告中都具有自己对应的部分，并具有自己的自定义呼叫指标集。
...
可从监控报告主页访问媒体质量摘要报告。 您可以通过单击以下指标深入[中业务服务器 2015年的 Skype 呼叫列表报告](call-list-report-0.md)：
...
|**性能降低 (MOS)** <br/> |否  <br/> |呼叫过程中遇到的性能降低的平均意见得分 (MOS) 的平均值。 性能降低值的范围介于 0.0 和 5.0 之间。 该值小于或等于 0.5 表示可接受的性能降低。 过去，平均意见得分是通过让用户对呼叫质量进行评级（范围为 1 到 5）来计算得出的。 在 Skype 业务服务器的一套算法预测用户将具有分级如何调用。  <br/> 高性能降低值可能是由拥塞、带宽不足、无线拥塞/干扰或媒体服务器或终结点超载造成的，从而导致音频失真或丢失。  <br/> |
