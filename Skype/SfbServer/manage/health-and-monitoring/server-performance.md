---
title: Skype for Business Server 中的服务器性能报告
ms.reviewer: ''
ms.author: v-cichur
author: cichur
manager: serdars
...
ms.locfileid: "49814272"
---
# <a name="server-performance-report-in-skype-for-business-server"></a>Skype for Business Server 中的服务器性能报告
 
**摘要：** 了解 Skype for Business Server 中的服务器性能报告。
  
服务器性能报告提供了遇到质量欠佳呼叫的百分比最高的 Skype for Business Server 服务器的列表。 该报告按服务器类型对服务器进行分解，并报告以下类型的单独统计信息：
...
值得注意的是，此报告中显示的排名是相对排名。 例如，假设性能最差的服务器在其 1，000 个已拨打的呼叫中拥有一个质量欠佳的呼叫。 这是一个高于可接受的 .1% 百分比。 但是，如果这是性能最差的服务器， (也就是说，如果所有其他服务器的通话百分比都低于 .1%) ，则服务器性能报告上仍将显示该服务器。
  
## <a name="accessing-the-server-performance-report"></a>访问服务器性能报告

服务器性能报告从监控报告主页访问。 可以通过单击以下任一指标向下钻取到 [Skype for Business Server](call-list-report-0.md) 中的呼叫列表报告：
...
服务器性能报告提供了多种筛选数据的方法;例如，可以筛选从有线连接进行的网络类型 (呼叫与从无线连接) 拨打的呼叫，以及从防火墙内拨打的 (呼叫与从防火墙外拨打的呼叫) 。 查看服务器性能报告以使用这些筛选器是一个好主意。 例如，假设有一个中介服务器，其质量欠佳的呼叫百分比为 3.24%。 如果您仅查看无线呼叫，则同一台服务器的较差呼叫百分比可能接近 20%。 这意味着服务器在无线呼叫方面遇到问题，此问题被部分掩盖，因为服务器没有有线呼叫问题。
  
## <a name="filters"></a>筛选器

利用筛选器，您可以返回一组针对性更强的数据或通过不同的方式查看返回的数据。 例如，服务器性能报告允许您执行诸如按服务器类型或网络类型（即有线或无线） (筛选返回的数据) 。 您还可以选择数据的分组方式。 在这种情况下，数据按小时、日、周或月进行分组。
...
|**服务器类型** <br/> |指示应报告其性能的服务器类型。 选择下列选项之一：  <br/> [全部]中介服务器 A/V 会议服务器 A/V 边缘服务器 |
|**前 N 个** <br/> |指示根据每个类别中 (质量欠佳的呼叫百分比) 服务器的数量。 例如，如果选择 **5，则显示 5** 个性能最差的服务器。 选择下列选项之一： <br/> [全部] 5 10 |
...
|**性能降低(MOS)** <br/> |是  <br/> |呼叫过程中遇到的性能降低的平均意见得分 (MOS) 的平均值。 性能降低值的范围介于 0.0 和 5.0 之间。 该值小于或等于 0.5 表示可接受的性能降低。 过去，平均意见得分是通过让用户对呼叫质量进行评级（范围为 1 到 5）来计算得出的。 在 Skype for Business Server 中，监控服务器使用一组算法来预测用户对呼叫的评分方式。  <br/> 高性能降低值可能是由拥塞、带宽不足、无线拥塞/干扰或媒体服务器或终结点超载造成的，从而导致音频失真或丢失。  <br/> |
|**数据包丢失** <br/> |是  <br/> |RTP 和数据包丢失 (传输) 平均速率。  (数据包（一种用于在 Internet 中传输音频和视频的协议）无法到达其目标位置时发生数据包丢失。) 高丢失率通常是由拥塞、带宽不足、无线拥塞或干扰或媒体服务器超载造成的。 数据包丢失通常导致音频失真或丢失。  <br/> |
...
|**出站数据包丢失** <br/> |否  <br/> |出站数据包的实时传输协议 (RTP) 数据包丢失率。  (数据包（一种用于在 Internet 中传输音频和视频的协议）无法到达其目标位置时，会发生数据包丢失。) 高丢失率通常是由拥塞造成的;带宽不足;无线拥塞或干扰;或媒体服务器超载。 数据包丢失通常导致音频失真或丢失。  <br/> |
