...
需要注意的是，此报告中显示的排名是相对排名。 例如，假设性能最差的服务器在 1，000 个拨打的呼叫中拥有一个质量欠佳的呼叫。 这是 0.1% 的可接受百分比。 但是，如果这是性能最差的服务器，则具有 (即，如果所有其他服务器具有甚至低于 .1% ) 的较差呼叫百分比，则该服务器仍将出现在"服务器性能报告"中。
...
|**数据包丢失** <br/> |是  <br/> |RTP 实时传输协议和数据包 () 的平均速率。  (当 RTP 数据包（一种用于在 Internet 中传输音频和视频的协议）无法到达其目标位置时发生数据包丢失。) 高丢失率通常是由拥塞、带宽不足、无线拥塞/干扰或媒体服务器超载造成的。 数据包丢失通常导致音频失真或丢失。  <br/> |
