...
设备报告可能更好地为麦克风和扬声器报告加了标题；这是因为，该设备报告检索按呼叫中使用的麦克风和扬声器分组的呼叫相关的指标（例如，质量欠佳的呼叫百分比、回声和语音切换时间）。 如果您对 IP 电话感兴趣， (通常称为"设备") ，请改为使用 电话[中的 IP](ip-phone-inventory-report.md) Skype for Business Server 清单报告。
...
通常可从监控报告主页访问该设备报告。 但是，如果正在查看呼叫详细信息[](call-detail-report.md)报告Skype for Business Server可以通过单击以下任一指标向下钻取到特定设备的设备报告：
