...
设备报告可能更好地为麦克风和扬声器报告加了标题；这是因为，该设备报告检索按呼叫中使用的麦克风和扬声器分组的呼叫相关的指标（例如，质量欠佳的呼叫百分比、回声和语音切换时间）。 如果您有兴趣 IP 电话 （通常也称为"设备"），改为使用[IP 电话业务服务器 2015年的 Skype 库存报表](ip-phone-inventory-report.md)。
...
通常可从监控报告主页访问该设备报告。 但是，如果您正在查看[中业务服务器 2015年的 Skype 呼叫详细信息报告](call-detail-report.md)您可以深化到特定设备的设备报告通过单击以下指标：
  
- 捕获设备
    
- 呈现设备
    
从设备报告您可以深化到该[业务服务器 2015年的 Skype 在调用列表报告](call-list-report-0.md)通过单击以下指标：
...
> [!NOTE]
> 请记住，捕获设备名称可能不同如果您正在运行的业务服务器 2015 Skype 的本地化的版本。 美国英语中名为 Aastra 6725ip 麦克风 (Aastra 6725ip)-V0 的设备在法语和西班牙语中可能会有不同的名称。 
...
|**语音切换原因** <br/> |呼叫必须切换为半双工模式才能防止回声的原因。在半双工模式中，通信一次只能在一个方向进行，与用户在使用对讲机进行通信时的轮流方式相似。选择下列选项之一：  <br/> [所有]无坏时间戳 Echo DNLP （动态非线性处理器） 低复杂性故障的设备状态自检 AEC 回响 （回声取消功能） |
|**回声原因** <br/> |在呼叫中检测到超出接受水平的回声的原因。（在长途通讯中，回声是声音的反射，与朝井底大喊时出现的现象一样。）选择下列选项之一：  <br/> [所有]无错误时间戳开机自检 AEC 回显 （回声抵消） ANLP （自适应非线性处理器） DNLP （动态非线性处理器） 麦克风剪辑 |
...
|**VPN** <br/> |指示外部客户端在发起呼叫时是否使用了虚拟专用网 (VPN) 连接。选择下列选项之一：  <br/> [所有]VPN 的非 VPN |
