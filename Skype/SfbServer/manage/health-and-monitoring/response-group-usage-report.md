---
title: 响应组中响应组Skype for Business Server
ms.reviewer: ''
ms.author: serdars
author: SerdarSoysal
manager: serdars
...
---
# <a name="response-group-usage-report-in-skype-for-business-server"></a>响应组中响应组Skype for Business Server

**摘要：** 了解应用程序中的响应组Skype for Business Server。

响应组应用程序提供了一种Skype for Business Server根据拨打的号码以及（可选）呼叫者对一系列问题的响应来应答和路由电话呼叫。 通常，响应组呼叫不路由到单个人员，而是路由到称为代理组的一组人员。 例如，如果有人呼叫技术支持的电话号码，Skype for Business Server可以将该呼叫自动路由到第一个可用的技术支持代理。 或者，Skype for Business Server遇到硬件问题 (请按 1"的一系列问题。 如果您有软件问题，请按 2。 如果您有网络问题，请按 3。") ，然后根据这些问题的回答将呼叫路由至最合适的技术支持代理。
...
可从监控报告主页访问响应组使用情况报告。 您可以通过单击以下[任何指标Skype for Business Server](call-list-report.md)中深入到响应组呼叫列表报告：
...
> [!CAUTION]
> 响应组工作流基本上是一组说明，用于确定Skype for Business Server拨打特定电话号码时执行哪些操作。 为此，每个工作流都唯一地与一个电话号码关联。 当有人呼叫该号码时，工作流将确定如何处理该呼叫。 例如，工作流可能会导致呼叫路由到一系列互动语音响应 (IVR) 问题，这些问题提示呼叫者输入其他信息 (按 1 获取硬件支持。 按 2 以获取软件支持”）。 或者，工作流可能导致将呼叫置于队列中，呼叫者将置于呼叫等待状态，直到代理可以应答呼叫。 工作流也可指示是否有代理能够应答呼叫：工作流用于配置工作时间（代理在一周中的哪几天、一天中的哪些时段能够应答呼叫）和假日（代理无法应答呼叫的日期）。 拨打属于响应组应用程序的电话号码时，实际上是在调用响应组工作流。 
