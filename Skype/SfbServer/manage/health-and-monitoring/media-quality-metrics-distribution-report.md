...
---
# <a name="the-media-quality-metrics-distribution-report-in-skype-for-business-server"></a>Skype for Business Server 中的媒体质量指标分布报告 
 
**摘要：** 了解 Skype for Business Server 中的媒体质量指标分布报告。
  
通过媒体质量指标分布报告，你可以查看显示用户体验质量指标（如抖动或数据包丢失）的分布值的图形。 例如，假设你的用户总共拨打了 10 次电话;这 10 次呼叫报告以下往返时间：
...
这些往返时间的平均时间是 500 毫秒 (5000 除以 10) 。 500 毫秒是一个非常大的往返时间;因此，您可能会认为网络拥塞是一个严重问题。  (往返时间过长通常是网络过载的结果。) 
  
当然，实际上，90% 的呼叫具有出色的往返时间;你只有一个扭曲总体结果的不良调用。 如果您仅查看平均往返时间，您可能会得出一个非常错误的结果。
  
媒体质量指标分布报告通过显示指定指标（如往返时间）的图形分布来帮助您避免 (错误) 。 这些图形可帮助你清楚地了解你拥有九个非常好的呼叫和一个非常错误的呼叫。 不可否认，您可能仍希望进一步调查该呼叫;但是，10 次呼叫中的 9 次非常好，表明没有理由对网络进行重大更改，至少此时没有变化。
...
|**以 x 轴表示的最小值** <br/> |要显示在图形的 X 轴上的最低值。  <br/> |
|**x 轴的最大数** <br/> |要显示在图形的 X 轴上的最高值。  <br/> |
