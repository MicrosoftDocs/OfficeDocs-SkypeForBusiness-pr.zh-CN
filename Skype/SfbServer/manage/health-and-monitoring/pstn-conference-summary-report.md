...
---
# <a name="pstn-conference-summary-report-in-skype-for-business-server"></a>Skype for Business Server 中的 PSTN 会议摘要报告
 
**摘要：** 了解 Skype for Business Server 中的 PSTN 会议摘要报告。
...
要确定包含电话拨入用户的所有会议的百分比，请将 PSTN 会议指标总计值与 [Skype for Business Server](conference-summary-report.md)中的会议摘要报告中找到的会议汇总指标进行比较。
  
如果您未看到所预计数目的 PSTN 会议，请记住，组织允许电话拨入式用户的会议的能力取决于已分配给用户的会议策略：如果仅允许几个用户主持 PSTN 会议，则可以明显看到 PSTN 会议的数目极少。 你可以从 Skype for Business Server 命令行管理程序 (运行以下命令，快速验证你的哪些会议策略 (如果有) 允许用户安排 PSTN 会议：
