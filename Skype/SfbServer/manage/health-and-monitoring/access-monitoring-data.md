...
在访问和分析监控数据时应考虑的一种工具是Skype for Business Server报告。 监控报告包含一组由 Microsoft SQL Server Reporting Service 发布的标准报告。 这些报告可通过 Web 浏览器进行访问，它们提供了使用情况、呼叫诊断信息和媒体质量信息，所有这些信息全部基于呼叫详细记录 (CDR) 和用户体验质量 (QoE) 数据库中存储的 CDR 和 QoE 记录。 监控报告随Skype for Business Server一起提供，在安装并配置监控Skype for Business Server之后，可以从 Skype for Business Server Skype for Business Server 部署向导安装监控报告。
