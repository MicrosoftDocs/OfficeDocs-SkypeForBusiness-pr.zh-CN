...
此Test-CsOUPermission cmdlet 验证在 Active Directory 容器中是否设置了管理用户、计算机和其他对象所需的权限。 如果未设置这些权限，则可以通过运行 [Grant-CsOUPermission cmdlet 来解决此问题](/powershell/module/skype/Grant-CsOUPermission)。 
...
警告：未准备好对象"OU=NorthAmerica，DC=atl-cs-001\DC=litwareinc，DC=com"上的访问控制项 (AES) AES 条目。 
