...
如果已设置所需的权限，Test-CsOUPermission返回一个单词响应：

True

如果未设置所需的权限，Test-CsOUPermission返回值 False。 您可能需要搜索一些时间以查找此值。 它通常嵌入多个附带的警告中。 例如：
...
如果Test-CsOUPermission失败，则通常意味着尚未将指定权限分配给 RTCUniversalUserAdmins 组。 您可以使用 cmdlet 来解决此问题，并分配所需的Grant-CsOUPermission权限。 例如，此命令为 RTCUniversalUserAdmins 组的用户、联系人和 inetOrgPersons 提供 OU 权限：
