...
使用本文中的步骤向 Skype for Business Server 中的现有 AlwaysOn 可用性组添加更多 Skype for Business Server 数据库，并查找在修补或升级作为 AlwaysOn 可用性组的一部分的后端服务器后所需的附加步骤Skype for Business Server。
...
2. 在拓扑生成器中，SQL Server AlwaysOn 可用性组的 FQDN 设置为该组的主节点的 FQDN。
   ...
   - 展开Skype for Business Server、展开拓扑和 SQL Server **Stores**。 右键单击SQL AlwaysOn 可用性组的新存储，然后单击"编辑 **属性"**。
...
2. 在 Skype for Business 命令行管理程序 (中运行以下 PowerShell 命令，该帐户使用适当权限的帐户登录，以将更改应用于 SQL AlwaysOn 数据库) 如下所示：
    ...
    其中，[sqlpool.contoso.com] 替换为 AlwaysOn 可用性 (FQDN) 完全限定的域名。
