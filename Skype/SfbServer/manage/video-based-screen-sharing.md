...
基于视频的屏幕共享（即 VbSS）退出 Lync 屏幕共享。 VbSS 与传统屏幕共享的区别与所使用的基础协议及其擅长内容有关。 屏幕共享使用远程桌面协议 (RDP) ，它非常能够创建数千个在用户计算机之间的一对一会话。 较新的技术 VbSS 将使用用户数据报协议 (UDP) 。
  
Skype for Business Server改进人们一对一及其一对多的多方 (对话和) 体验。 VbSS 利用媒体平台 (它依赖于 UDP 作为基础协议) ，目的是改进视频开始时间、观看 (尤其是当你正在观看的内容快速移动) 以及整体可靠性时。
  
改进屏幕共享的一部分是，在 VbSS 和 RDP 之间实现尽可能无缝的转换。 由于 VbSS 是针对 Skype for Business Server 的屏幕共享中使用的基础技术的更新，因此，除非正在查看网络流量中的 SIP 详细信息，或者您共享快速移动的内容或 3D 内容，否则可能很难检测您利用的技术。 例如，如果工作区中有许多旧版客户端，RDP 仍将作为会议和对话的故障安全。 Skype for Business Server使用内部逻辑决定在客户端连接时 (VbSS 或传统屏幕) 应用哪一种方法。 RDP 可以且将替换为 VbSS（当情况需要它时）以便你的查看体验不会中断。
...
> [!IMPORTANT]
> 这些设置是一个特例，必须在实现这两个功能时使用这些确切设置。 这将覆盖 [QoS 文档中的其他推荐设置](/previous-versions/office/lync-server-2013/lync-server-2013-managing-quality-of-service-qos)。 对于应用程序共享，除了定义这些端口值ASMCUSVC.exe还需要在 QoS GPO 中指定端口。 
...
也就是说，我们已在实验室中完成性能和压力测试，也应当根据使用情况考虑以下 (测试，当然) 。
  
假定：
  
- 在部署中使用的是 Skype for Business Server 2015 CU2 或更高版本。
    
- 环境中所有用户的屏幕Skype for Business Server分辨率高于 1920x1080。
    
达到完整容量 (如上所述，每个前端服务器总共有 375 个屏幕共享参与者，尽管每个会议) 只有 250 个参与者，但前端服务器可能使用大约 1 GB 网卡的 89%。 这是因为 Skype for Business Server CU2 (RDP) 中的现有屏幕共享技术以演示者电脑的本机分辨率传输屏幕内容。 因此，在考虑更高的屏幕分辨率后，你可能已经遇到与 Skype for Business Server CU2 进行屏幕共享的网络瓶颈。
...
基于视频的屏幕共享需要 Skype for Business Server 2015 CU3 或更高版本，以及适用于 Skype for Business 和会议支持的移动客户端功能比较中列出的支持客户端的当前[版本](../plan-your-deployment/clients-and-devices/desktop-feature-comparison.md#BKMK_Conferencing)。[](../plan-your-deployment/clients-and-devices/mobile-feature-comparison.md) 
  
在某些情况下，屏幕共享将转换为 RDP，如下所示：
  
- 如果你的帐户托管在 ASMCU 不满足支持 VbSS 的最低内部版本的环境中。
- 如果使用较旧版本的 Skype for Business 客户端的人加入会话，例如使用低于 16.0.6330.1000、Skype for Business 会议室系统设备或 Skype for Business 移动应用的任何 Windows 客户端版本的任何人。 
...
[2015 年 (VBSS) 基于视频Skype for Business Server共享](https://support.microsoft.com/kb/3170163)