...
基于视频的屏幕共享（即 VbSS）退出 Lync 屏幕共享。 VbSS 与传统屏幕共享的区别与所使用的基础协议及其擅长内容有关。 屏幕共享使用远程桌面协议 (RDP) ，这非常能够创建用户计算机之间的数千个一对一会话。 较新的技术 VbSS 将使用用户数据报协议 (UDP) 。
  
Skype for Business Server改进人们一对一及其一对多的多方 (对话和) 体验。 VbSS 利用媒体平台 (它依赖于 UDP 作为基础协议) ，目的是改善视频开始时间、观看 (尤其是当你正在观看的内容快速移动) 以及整体可靠性时。
  
改进屏幕共享的一部分是，在 VbSS 和 RDP 之间实现尽可能无缝的转换。 由于 VbSS 是针对 Skype for Business Server 的屏幕共享中使用的基础技术的更新，因此，除非正在查看网络流量中的 SIP 详细信息，或者您共享快速移动的内容或 3D 内容，否则可能很难检测到您利用的技术。 例如，如果工作区中有许多旧版客户端，RDP 仍将作为会议和对话的故障安全。 Skype for Business Server使用内部逻辑决定在客户端连接 (VbSS 或传统屏幕) 应用哪一种方法。 RDP 可以且将替换为 VbSS（当情况需要它时）以便你的查看体验不会中断。
...
如果为以下媒体端口启用了 QoS，并且还启用了 VbSS，那么在包含桌面共享的会议期间，AS MCU 将使用屏幕共享流量下方以粗体显示的视频端口设置。 
...
运行 Skype for Business Server 2015 累积更新 2 (CU2) 或更高版本的每个前端服务器最多支持 375 个参与者使用 RDP (但每个会议仅支持 250 个参与者) 。 引入和使用 VbSS 时，此容量在 CU3 后不会更改。
...
达到完整容量 (如上所述，每个前端服务器总共有 375 个屏幕共享参与者，尽管每个会议) 只有 250 个参与者，但前端服务器可能使用大约 1 GB 网卡的 89%。 这是因为 Skype for Business Server CU2 (RDP) 中的现有屏幕共享技术以演示者电脑的本机分辨率传输屏幕内容。 因此，在考虑更高的屏幕分辨率后，你可能已经遇到使用 Skype for Business Server 2015 CU2 进行屏幕共享的网络瓶颈。
...
- 通过为 VbSS (RDP) 最大带宽设置上限，限制用于 VbSS 和 RDP 的带宽比特率。
...
基于视频的屏幕共享需要 Skype for Business Server 2015 CU3 或更高版本，以及 Mobile client feature comparison [for Skype for Business and](../plan-your-deployment/clients-and-devices/mobile-feature-comparison.md) Meetings support 中列出的支持客户端[的当前版本](../plan-your-deployment/clients-and-devices/desktop-feature-comparison.md#BKMK_Conferencing)。 
  
在某些情况下，屏幕共享将转换为 RDP，如下所示：
  
- 如果你的帐户托管在 ASMCU 不满足支持 VbSS 的最低内部版本的环境中。
- 如果使用较旧版本的 Skype for Business 客户端的人加入会话，例如使用低于 16.0.6330.1000 的任何 Windows 客户端版本、Skype for Business 会议室系统设备或 Skype for Business 移动应用的任何人。 
- 如果用户从网站共享Skype for Business Web应用。
- 如果某人使用的是 Mac Skype for Business，但并非在 Skype for Business Online 或 Skype for Business Server 2015 上（包含 2018 年 7 月或 (年 7 月累积更新) ）。
...
[VBSS (2015) 提供基于视频Skype for Business Server共享功能](https://support.microsoft.com/kb/3170163)