...
## <a name="configure-a-quality-of-service-policy-in-skype-for-business-server-for-your-conferencing-application-and-mediation-servers"></a>为会议、应用程序和中介Skype for Business Server配置服务质量策略
...
6.  在"基于策略的 **QoS"** 对话框的打开页上，键入新策略的名称 (例如，Skype for Business Server **QoS**) "**框中**。 选择“指定 DSCP 值”，并将该值设置为“46”。 将“指定出站调节率”保留为未选中状态，然后单击“下一步”。
...
为音频流量创建 QoS 策略后，应为视频流量创建第二个策略 (（可选）第三个策略来管理应用程序共享流量) 。 要为视频创建策略，请按照您创建音频策略时遵循的相同基本过程，进行下列替换项：
  ...
  - 对视频流量使用以前配置的端口范围。 例如，如果为视频保留端口 57501 到 65535，则端口范围设置为 **：57501：65535。**
