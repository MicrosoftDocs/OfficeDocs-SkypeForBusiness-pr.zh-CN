...
默认情况下，当涉及通信会话时，Skype for Business 客户端应用程序可以使用端口 1024 和 65535 之间的任意端口;这是因为不会为客户端自动启用特定端口范围。 但是，若要使用服务质量，您将需要为一系列特定端口范围重新分配各种通信类型（音频、视频、媒体、应用程序共享和文件传输）。 可使用 Set-CsConferencingConfiguration cmdlet 完成此操作。

> [!NOTE]  
> 最终用户无法自行进行这些更改。 端口更改只能由管理员使用 Set-CsConferencingConfiguration cmdlet。


通过从 Skype for Business Server 命令行管理程序中运行以下命令，可以确定当前用于通信会话的端口范围：

    Get-CsConferencingConfiguration

假定自安装 Skype for Business Server 后，你未对会议设置进行任何更改，应返回包含以下属性值的信息：
...
这一点很重要，因为当此属性设置为 False 时，Skype for Business 客户端将在涉及通信会话时使用端口 1024 和 65535 之间的任何可用端口;无论使用何种其他端口设置， (，例如 ClientMediaPort 或 ClientVideoPort) 。 如果要将用法限制为一组指定的端口 (并且如果计划实施服务质量) ，则希望这样做，则必须先启用客户端媒体端口范围。 可以使用以下命令完成Windows PowerShell操作：
...
若要将上述端口范围分配给全局会议配置设置集合，可以使用以下 Skype for Business Server 命令行管理程序命令：
...
各个用户必须从 Skype for Business 注销，然后重新登录，这些更改才能实际生效。
...
除了指定供 Skype for Business 客户端使用的端口范围之外，还必须创建将应用于客户端计算机的单独的服务质量策略。  (为会议、应用程序和中介服务器创建的服务质量策略不应应用于客户端计算机。) 此信息仅适用于运行 Skype for Business 客户端和 Windows 10 的计算机。
...
若要为 Windows 10 计算机创建服务质量音频策略，请首先登录到安装了组策略管理的计算机。 打开"组策略 (，单击"开始"，指向"管理工具"，然后单击"组策略管理) "，然后完成以下过程： 
...
7.  On the next page， select **Only applications with this executable name，** enter **Lync.exe** as the name， and then click **Next**. 此设置指示策略仅设置来自 Skype for Business 客户端的匹配流量的优先级。
...
对于文件传输策略：
  ...
  - 使用以前为应用程序配置的端口范围。 例如，如果为应用程序共享保留端口 42020 至 42039，则端口范围设置为 **：42020：42039。**
...
如果您的计算机具有多个网络适配器，则有时可能会遇到 DSCP 值显示为 0x00 而不是配置的值的问题。 此错误通常会发生在一个或多个网络适配器无法访问 Active Directory 域的计算机上（例如，如果这些适配器用于专用网络）。 如果出现这种情况，则将 DSCP 值标记为使这些适配器可以访问该域，但不会标记为使这些适配器无法访问该域。
...
[在 Windows 10 中创建组策略对象](/windows/security/threat-protection/windows-firewall/create-a-group-policy-object)