...
若要允许远程用户登录到运行 Skype for Business Server 的服务器，还必须至少配置一个外部访问策略以支持远程用户访问。 有关详细信息，请参阅[配置策略以控制](../external-access-policies/configure-policies-to-control-remote-user-access.md)远程用户访问Skype for Business Server。
