...
## <a name="resetting-the-global-external-access-policy-by-using-windows-powershell-cmdlets"></a>使用 cmdlet 重置全局外部访问Windows PowerShell Cmdlet

全局外部访问策略可以使用 Windows PowerShell 和 Remove-CsExternalAccessPolicy cmdlet 重置。 此 cmdlet 可以从 Skype for Business Server 命令行管理程序或远程会话命令行管理程序Windows PowerShell。 
