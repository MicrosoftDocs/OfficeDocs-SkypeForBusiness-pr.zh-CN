...
当您安装 Skype for Business Server 时，将为您创建 SIP 中继配置设置的全局集合。 此外，管理员还可以在站点作用域或服务作用域（仅针对 PSTN 网关服务）内创建自定义设置集合。

为 Business Server 控制面板创建 SIP 中继配置设置 usingSkype 时，可以使用以下选项：
...
> [!Note]
> Skype for Business Server Remove-cstrunkconfiguration cmdlet 支持未在 Skype for Business Server 控制面板中显示的其他属性。 有关详细信息，请参阅[remove-cstrunkconfiguration](https://docs.microsoft.com/powershell/module/skype/New-CsTrunkConfiguration) cmdlet 的帮助主题。 

**使用 Skype for Business Server 控制面板创建新的中继配置设置的具体方法**
...
3. 在 "**选择站点**" 或 "**选择服务**" 对话框中（出现的对话框将取决于您是创建网站范围设置还是服务范围设置），选择新配置设置的位置，然后单击 **"确定"**。 如果对话框为空，则表示没有创建新设置的位置;例如，如果 "**选择站点**" 对话框为空，这意味着已为您的所有网站分配了中继配置网站的集合，并且每个网站（以及每个服务）只能承载一个这样的集合。 在此情况下，您可删除现有集合并创建新的集合，或只修改现有集合。
