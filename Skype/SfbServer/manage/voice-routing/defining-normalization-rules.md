...
- [使用 "**生成规范化规则**" 工具](#create-or-modify-a-normalization-rule-by-using-build-a-normalization-rule)来指定起始数字、长度、要删除的数字和要添加的数字的值, 然后让 Skype for business 服务器控制面板生成相应的匹配模式和转换规则给你的。
...
6. 在“建立规范化规则”**** 的以下字段中输入值：
    ...
    这些字段中输入的值将反映在“要匹配的模式”**** 和“转换规则”**** 中。 例如, 如果将开始的**数字**保留为空, 请在 "**长度**" 字段**** 中键入 " **7** ", 然后指定 "**要删除的数字**", 若要删除**0** , 则**模式**中的结果正则表达式如下所示:
