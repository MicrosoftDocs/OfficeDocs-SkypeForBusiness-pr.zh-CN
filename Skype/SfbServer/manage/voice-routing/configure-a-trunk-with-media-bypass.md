...
> [!NOTE]
> 媒体旁路将不会与 SBC (网关、IP-PBX 和会话边界控制器) 每个公用电话交换网 (互) 操作。 Microsoft 已经与认证合作伙伴一起测试了一组 PSTN 网关和 SDC，并且已经使用 Cisco IP-PBX 进行了一些测试。 媒体旁路仅受"电话基础结构 for [Skype for Business Server"页上列出的产品和版本](../../../SfbPartnerCertification/certification/infra-gateways.md)支持。 
...
12. **应选择"启用 RTP** 闭锁"，为网络地址转换（NAT (或防火墙以及支持闭锁的 SBC) 客户端启用旁路媒体。
...
配置中继后，继续通过选择全局媒体旁路选项来配置媒体旁路，如在 Skype for Business Server[中部署媒体旁路中所述](../../deploy/deploy-enterprise-voice/deploy-media-bypass.md)。
