...
> [!NOTE]
> 您可以查看 PIN 状态信息（如是否已设置 PIN 或上次更改 PIN 的时间），但无法通过查看 PIN 状态来查看当前 PIN。 如果用户丢失了 PIN，可以按照"在会议设置用户的电话拨入式会议 [PIN](set-a-user-s-dial-in-conferencing-pin.md)"中的过程重置Skype for Business Server
