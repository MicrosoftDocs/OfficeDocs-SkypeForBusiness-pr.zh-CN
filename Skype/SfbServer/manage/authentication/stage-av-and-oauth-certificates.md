...
|2  <br/> |2015/7/22 2：00：00 AM - 7/22/2015 5：59：59 AM  <br/> 对于此示例，在有效时间为 6：00：00 (4 小时的边缘服务器上设置证书为本示例，但使用 Set-CsCertificate -Type \<certificate usage type\> -Thumbprint \<thumbprint of new certificate\> -Roll -EffectiveDate) 可以更长 \<datetime string of the effective time for new certificate\>  <br/> |
|3  <br/> |7/22/2015 6：00 AM - 7/22/2015 2：00 PM  <br/> 若要验证令牌，首先尝试新证书，如果新证书无法验证令牌，则尝试旧证书。 此过程在 8 小时（默认令牌生存期和重叠 (8 小时内) 令牌。  <br/> |
|4  <br/> |结束时间：2015/7/22 2：00：01 PM  <br/> 旧证书已过期，新证书已接管。 使用 -Type -Previous 可以安全地Remove-CsCertificate旧 \<certificate usage type\> 证书  <br/> |
   
在 2015 年 7 月 22 (上午 6：00：00) 到达生效时间时，所有新令牌都由新证书颁发。 在验证令牌时，将首先根据新证书验证令牌。 如果验证失败，将尝试旧证书。 尝试新证书并回到旧证书的过程将一直继续下去，直到到达旧证书的到期时间。 旧证书在 (2015 年 7 月 22 日下午 2：00：00) 过期后，令牌将仅由新证书进行验证。 可以使用带 -Previous 参数的 Remove-CsCertificate cmdlet 安全地删除旧证书。
...
在 2015 年 7 (7 月 21 日上午 1：00：00) 到达生效时间时，所有新令牌都由新证书颁发。 在验证令牌时，将首先根据新证书验证令牌。 如果验证失败，将尝试旧证书。 尝试新证书并回到旧证书的过程将一直继续下去，直到到达旧证书的到期时间。 旧证书在 (2015 年 7 月 22 日下午 2：00：00) 过期后，令牌将仅由新证书进行验证。 可以使用带 -Previous 参数的 Remove-CsCertificate cmdlet 安全地删除旧证书。
```PowerShell
...
[管理 OAuth (中的服务器) 身份验证和合作伙伴Skype for Business Server](server-to-server-and-partner-applications.md)
