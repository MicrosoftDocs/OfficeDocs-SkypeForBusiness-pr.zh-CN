...
3. 将新的 AudioVideoAuthentication 证书导入到池中的边缘服务器和所有其他边缘服务器 (如果已部署池) 。
...
|3   <br/> |7/22/2015 6：00 AM - 7/22/2015 2：00 PM  <br/> 若要验证令牌，首先尝试新证书，如果新证书无法验证令牌，则尝试旧证书。 此过程用于默认令牌生存期和重叠 (8 小时内) 令牌。  <br/> |
|4   <br/> |结束时间：2015/7/22 2：00：01 PM  <br/> 旧证书已过期，新证书已接管。 使用 -Type -Previous 可以安全地删除旧Remove-CsCertificate \<certificate usage type\> 证书  <br/> |
   
在 2015 年 7 月 22 (上午 6：00：00) 到达生效时间时，所有新令牌都由新证书颁发。 在验证令牌时，将首先根据新证书验证令牌。 如果验证失败，将尝试旧证书。 尝试新证书并回到旧证书的过程将一直继续下去，直到到达旧证书的到期时间。 旧证书在 2015 年 7 (2 月 22 日下午 2：00：00) 过期后，令牌将仅通过新证书进行验证。 可以使用带 -Previous 参数的 Remove-CsCertificate cmdlet 安全地删除旧证书。
...
3. 如果您已经部署了池，则向池中的前端服务器 (新的 OAuthTokenIssuer) 。 将全局复制 OAuthTokenIssuer 证书且只需在部署中的任何服务器更新和续订这些证书。 前端服务器用作示例。
...
在 2015 年 7 (1：00：00 AM) 到达生效时间时，所有新令牌都由新证书颁发。 在验证令牌时，将首先根据新证书验证令牌。 如果验证失败，将尝试旧证书。 尝试新证书并回到旧证书的过程将一直继续下去，直到到达旧证书的到期时间。 旧证书在 (2015 年 7 月 22 日下午 2：00：00) 过期后，令牌将仅通过新证书进行验证。 可以使用带 -Previous 参数的 Remove-CsCertificate cmdlet 安全地删除旧证书。
```PowerShell
