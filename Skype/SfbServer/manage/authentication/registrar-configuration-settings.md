...
- **证书身份验证**当服务器需要从 Lync Phone Edition 客户端、公共区域电话、Skype for business 和 Lync Windows 应用商店应用获取证书时，这是新的身份验证方法。 在 Lync Phone 版客户端上，在用户通过提供个人识别码（PIN）登录并通过提供个人识别码（PIN）成功进行身份验证后，Skype for business 服务器会将 SIP URI 设置到手机，并为该电话提供一个 Skype for business 服务器签名的证书或标识 Joe （Ex： SN=joe@contoso.com）的用户证书。 此证书用于使用注册器和 Web 服务进行身份验证。
