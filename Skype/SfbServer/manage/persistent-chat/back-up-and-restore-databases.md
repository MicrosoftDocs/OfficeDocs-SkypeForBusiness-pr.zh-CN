...
持久聊天服务器SQL Server软件来存储聊天室数据，如历史记录和内容、配置、用户设置和其他相关元数据。 此外，如果组织规定需要存档持久聊天活动，并且启用了可选的合规性服务，SQL Server 数据库软件将用于存储合规性数据，包括聊天内容和事件，如加入和离开聊天室。 聊天室内容存储在持久聊天数据库中 (mgc) 。 合规性数据存储在合规性数据库中 (mgccomp) 。 这是应定期备份的业务关键数据。 
