...
**SmtpServer** 默认情况下，脚本使用此参数的保留环境 **$PSEmailServer** 变量的值。 如果未 **$PSEmailServer** 变量，则必须指定此参数。
...
下一个示例强制为 Bob 使用值为"383042650"的新 PIN（即使 Bob 已有 PIN，）然后从在 Bob 中向 Bob 发送欢迎电子邮件。 由于已指定 Credential 参数，因此系统会提示运行命令的人员输入密码。 电子邮件使用安全套接字层和 SSL (发送) ：
