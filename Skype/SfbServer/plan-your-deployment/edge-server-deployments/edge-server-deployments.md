...
如果具有在Skype for Business Server运行良好的外部环境，下一步是向该环境中引入边缘服务器或边缘池。 如果希望外部人员使用由 Skype for Business Server 提供的服务，此角色将非常重要。 这些可能包括：
...
- 用于通信的 IM 和状态：授权的外部用户可以加入 IM 对话和会议。 他们可以获取其他用户状态信息， (获取其状态信息的用户) 。 如果使用公共 IM 提供程序，则不能进行多方会议，这严格来说就是对等通信。 但是支持 SIP 和 XMPP 协议。
...
- [Skype for Business Server 2015 年的边缘服务器环境要求](edge-environmental-requirements.md)
