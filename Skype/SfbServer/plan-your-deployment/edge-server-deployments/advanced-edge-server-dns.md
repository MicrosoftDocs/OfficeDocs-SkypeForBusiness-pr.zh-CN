...
对于域名系统 (DNS) 规划Skype for Business Server，你的决策可能会Skype for Business Server许多因素。 如果组织的域结构已就位，这可能就是查看如何继续的问题。 我们将从以下主题开始：
...
Skype for Business客户端在客户端中查找和访问服务的方式与早期版本的 Lync 客户端Skype for Business Server。 本节详细介绍服务器位置过程。
...
> [!NOTE]
> 在创建 SRV 记录时，必须记住，如果使用 IPv6 寻址) （在创建 DNS SRV 记录的同一域中）中，它们需要指向 DNS A (和 AAAA。 例如，如果 SRV 记录在 contoso.com 中，则它指向的 A (和 AAAA) 记录不能为 fabrikam.com。 
...
这带来了一些挑战。 最重要的是，移动功能不支持 **拆分式 DNS** 。 这是因为 LyncDiscover 和 LyncDiscoverInternal DNS 记录 (LyncDiscover 必须定义在外部 DNS 服务器上，而 LyncDiscoverInternal 必须定义在内部 DNS 服务器) 。
  
我们将在此处列出内部和外部区域 DNS 记录，但您可以在边缘服务器环境要求部分找到详细示例。
  
### <a name="internal-dns"></a>内部 DNS

- 包含名为 dns 区域 (例如) contoso.com 其权威的 DNS 区域。
    
- 此内部 contoso.com 包含：
    
  - DNS A 和 AAAA (如果使用 IPv6 寻址前端池、控制器池或控制器池名称以及组织网络中运行 Skype for Business Server 的所有内部服务器的) 记录。
    
  - 如果对外围网络中 (边缘服务器的边缘内部接口使用 IPv6 寻址) 记录，则 DNS A 和 AAAA Skype for Business Server。
    
  - DNS A 和 AAAA (如果对外围网络 (中每台反向代理服务器的内部接口使用 IPv6 寻址) 记录，则 DNS A 和 AAAA) ，这是可选的，用于管理反向代理) 。
...
- 所有运行 Skype for Business Server 的服务器和在企业网络中运行 Skype for Business Server 的客户端指向内部 DNS 服务器以解析对 contoso.com 的查询，或者，如果使用 IPv6 寻址下一跃点服务器 (（特别是控制器）的 IPv6 寻址)  (记录，则它们在每个边缘服务器上使用主机文件，并列出 A 和 AAAA (控制器池 VIP、前端池 VIP 或 Standard Edition 服务器) 。
...
- 此外部 contoso.com 包含：
  ...
  - 如果对外围网络中每台 Skype for Business Server 边缘服务器或硬件负载平衡 (HLB) VIP 的边缘外部接口使用 IPv6 寻址) 和 SRV 记录，则 DNS A 和 AAA (A) 。
...
- _sipinternaltls._tcp.fabrikam.com。 86400 IN SRV 0 0 5061 pool01.fabrikam.com
     ...
     *如果用户以登录 alice@fabrikam.com，此记录将适用于第二个域的自动配置，同样，因为 SIP 域与该域的前端池匹配。* 
    
若要进一步说明示例，此操作将不起作用：
  
- _sipinternaltls._tcp.litwareinc.com。 86400 IN SRV 0 0 5061 pool01.fabrikam.com
    
     *作为用户登录 tim@litwareinc.com 不能用于自动配置，因为用户的 SIP 域 (litwareinc.com) 与池中的域不匹配 (fabrikam.com) 。* 
...
如果在 GeoDNS 提供程序上为 Web 服务的内部和外部) IPv6 记录，定义和配置其他 DNS 主机 A 或 AAAA (AAAA。 以下详细信息假定池已配对，地理位置分散，且您的提供商支持的 GeoDNS 要么具有 **循环 DNS，** 要么配置为使用 Pool1 作为主池，如果有任何通信丢失或电源故障，则出现故障到 Pool2。
