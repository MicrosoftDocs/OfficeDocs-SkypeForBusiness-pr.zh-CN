...
- **匹配内部区域**
    ...
    例如, 如果你有一个用户驻留在 pool01.contoso.net 上, 但登录 Skype for business 的 bob@contoso.com, 请创建名为 contoso.com 的内部 DNS 区域, 并在其中创建一个名为的内部 DNS 区域, 并在其中创建 DNS A (和 AAAA for IPv6 寻址使用的 AAAA) 记录pool01.contoso.com。
