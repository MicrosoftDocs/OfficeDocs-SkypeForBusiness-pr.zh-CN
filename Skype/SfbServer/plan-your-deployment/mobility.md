...
ms.locfileid: "58632716"
---
# <a name="plan-for-mobility-for-skype-for-business-server"></a>规划移动Skype for Business Server
 
规划移动技术Skype for Business Server。
  
通过Skype for Business Server，您可以部署移动功能以在Skype for Business Server提供移动功能。 本文提供有关移动功能的详细信息，并帮助您规划部署。
  
Skype for Business Server的移动功能支持移动客户端Skype for Business 2010 年更新的 Lync 客户端。 部署后，你的用户可以使用受支持的 iOS、Android 和 Windows Phone 移动设备连接到 Skype for Business Server 部署，以利用几种不同的功能，包括 企业语音 功能。 我们在下面包含了部分列表，还可以查看桌面客户端功能比较，Skype for Business[了解详细信息](clients-and-devices/desktop-feature-comparison.md)：
...
所有这些操作都是通过统一通信 Web API 或 UCWA 完成的。 UCWA 在 Lync Server 2013 中首次引入，现在仍用于Skype for Business Server。 还有一项与 Lync 2010 客户端进行通信的附加功能，即 Mobility Service (MCX) 。 这些服务是免费服务，允许 Lync Server 2010 和 2013 客户端以及 Skype for Business 客户端成功Skype for Business Server部署。
...
值得注意的是，虽然这些功能一经实现就可用，但在某些设备上可能有点不同。 我们在移动客户端功能比较上获取了一个网站，该网站讨论了哪些功能在哪些设备上[Skype for Business。](clients-and-devices/mobile-feature-comparison.md) 我们还在 Plan for clients and devices 中提供一 [些出色的设备和操作系统信息](clients-and-devices/clients-and-devices.md)。
  
移动性利用自动发现功能，该功能允许客户端自动找到 Skype for Business Server Web 服务，而无需输入任何 URL (他们甚至无需知道这些 url) 。 如果需要执行一些故障排除，仍支持手动输入 URL。
  
推送通知也受支持，Skype for Business应用未在后台 (中运行时，或者对于不支持在后台运行应用程序的移动设备) 。 当设备或应用处于非活动状态时，会向移动设备发送有关所发生事件的推送通知。 一个很好的示例是在手机不活动时缺少 IM 消息，这可能会导致推送通知发送 (这将显示为 toast 或通知，就像应用在后台) 中运行时一样。 使用推送通知，用户不会错过 IM 或语音呼叫。
...
有四种服务构成移动Skype for Business Server：
  
- **统一通信 Web API (UCWA)**
    
    提供用于与移动客户端和 Web 客户端进行实时通信的服务，Skype for Business Server。 部署Skype for Business Server时，UCWA 虚拟目录在内部和外部 Web 服务中创建。 此虚拟目录中的一个虚拟组件，该虚拟组件接受来自启用 UCWA 的客户端的呼叫。 客户端应用通过表述性状态传输 (REST) 接口进行通信，用于：
...
- **Mobility service (MCX)**
    ...
    支持Skype for Business Server移动设备上的即时消息、状态和联系人等功能。 Mobility Service 安装在每个池中的每台前端服务器上，旨在Skype for Business Server移动设备上的移动设备功能。 在 2015 Skype for Business Server安装时， (Mcx) 在前端服务器的内部和外部网站下创建一个新的虚拟目录。
...
- **推送通知服务**
    ...
    基于云的服务，位于 Skype for Business Online 数据中心。 在未在后台运行 Skype for Business 客户端的手机上，当发生新事件时，会向移动设备发送名为 (推送通知) 未接事件的通知。 Mobility Service 向 MPNS (推送通知服务发送) ，然后发送到移动设备。 然后，用户可以在移动设备上响应通知以激活应用。 此功能需要边缘服务器。
...
Mobility Services Mcx 和 UCWA 使用相同的 DNS。 通过自动发现，移动设备可使用 DNS 查找资源。 在 DNS 查找过程中，会尝试连接到与 lyncdiscoverinternal 中的内部 DNS 记录关联的 FQDN (FQDN。[内部域名]) 。 如果内部 DNS 记录不能用于建立该连接，则尝试进行第二次连接，这次连接到 lyncdiscover (DNS 记录。[sipdomain]) 。 那么，为什么有两个？ 网络内部的移动设备将能够使用内部自动发现 URL。 外部移动设备将使用外部自动发现 URL。 在任一情况下，自动发现服务都会返回用户主池的所有 Web 服务 URL，其中包括 Mobility service (Mcx 和 UCWA) 。
  
预计外部自动发现请求将经过为外部发现配置反向代理Skype for Business Server。 但是，内部 Mobility Service URL 和外部 Mobility Service URL 都与外部 Web 服务 FQDN 关联。 因此，无论移动设备是网络内部设备还是外部设备，设备始终通过反向代理Skype for Business Server外部连接到 Skype for Business Server Mobility Service。
  
> [!NOTE]
> 正如我们刚才指出的，内部和外部 (所有 mobility service) 流量都将通过反向代理。 但有时，当内部流量通过接口离开时，仅尝试并返回到同一接口时，会出问题。 这可能会违反你的欺骗 (，它正式称为 TCP 数据包欺骗) 安全规则。 你需要允许发夹 **具有** 移动功能。
  
> [!NOTE]
> 如果你已准备好这样做，还可以选择使用独立于防火墙 (的反向代理来用于安全目的，欺骗防护应始终在防火墙) 。 这样，发夹可以在反向代理的外部接口（而不是防火墙的外部接口）上发生。 这允许你在防火墙上正确检测欺骗，同时在反向代理上放宽规则，并获取移动功能。 
...
我们已在其他文档中涵盖了大部分内容，但专门针对移动功能，如果你有任何用户位于 Survivable Branch Appliance (SBA 上，你将希望让企业 Wi-Fi 网络上打开以下) 端口：
...
如果对 Skype for Business 移动客户端使用自动发现，则需要修改证书上的 SAN (主题备用名称) 列表，以支持来自移动客户端的安全连接。 如果已有证书就地，则需要请求并分配包含此处所述的 SAN 条目的新证书。 如果环境中运行的是自动发现服务 (前端服务器和控制器) 需要完成此操作。 我们还建议修改反向代理证书上的 SAN 列表，为组织中添加每个 SIP 域的 SAN 条目。
  
如果您要从内部 CA (证书颁发机构) 请求新证书，这应该是一个简单的过程，但公共证书更为复杂，并且重新请求的成本可能要高很多，更不用提将大量 SIP 域添加到新的公共证书的成本。在这种情况下，有一种方法受支持，但不 **推荐使用**。 您可以配置反向代理，以通过端口 80（将使用 HTTP，而不是端口 443，即 HTTPS (）提出初始自动发现服务请求，443 是默认配置) 。 该传入请求将重定向到前端池或控制器上的端口 8080。 通过执行此操作，无需进行任何证书更改，因为此流量不会对请求使用 HTTPS。 但同样，我们不建议这样做，尽管它适合你。
  
### <a name="windows-and-iis-requirements"></a>Windows和 IIS 要求

您应该具有适用于您的 Windows 环境的支持 Skype for Business Server 版本。 因此，您还应具有 IIS 8 或 IIS 8.5 以满足您的移动需求。 将需要对默认策略设置进行 ASP.NET 更改，但 Mobility Service 安装程序会自动执行这些更改。
  
### <a name="hlb-requirements"></a>HLB 要求

如果对 Skype for Business Server 使用包含前端池 (（即包括多个前端服务器) 的任何拓扑）的 HLB 的拓扑，则需要为源配置用于 Web 服务流量的外部 Web 服务虚拟 IP ( (IP) 。 源相关性有助于确保将来自单个客户端的多个连接发送到同一服务器以保持会话状态。
  
如果计划仅Skype for Business内部 Wi-Fi 网络支持移动客户端，应为源配置内部 Web 服务 ISP，如外部 Web 服务 ISP 所述。 在这种情况下，您应对 HLB 上的source_addr (WEB 服务) TCP 或 TCP 关联。
  
有关所有这些内容的详细信息，请查看 Load [balancing requirements for Skype for Business](network-requirements/load-balancing.md) documentation。
...
- 如果您决定对初始自动发现服务请求使用 HTTP，以避免必须更新反向代理证书 (我们不推荐使用) 的 SAN 列表，则需要为端口 HTTP/TCP 80 创建新的 Web 发布规则（如果尚未更新）。 如果该规则存在，请更新该规则以包含 lyncdiscover。\<sipdomain\> 条目。
...
我们提供了一个表，可帮助了解所有用户可用的一些功能，以及默认情况下是否按此方式设置这些功能。 有关完整列表，请查看 [New-CsMobilityPolicy](/powershell/module/skype/new-csmobilitypolicy?view=skype-ps)。
...
|**功能**|**参数名**|**说明**|**默认设置**|
|:-----|:-----|:-----|:-----|
|启用移动性  <br/> |EnableMobility  <br/> |控制给定范围内已安装移动Skype for Business的用户。 如果策略设置为 False，用户将无法使用其客户端登录。  <br/> |正确  <br/> |
|外部语音  <br/> |EnableOutsideVoice  <br/> |使用户能够使用通过工号呼叫功能，这使用户可以使用其工作号码而不是其移动电话号码发送和接收呼叫。 如果设置为 False，用户在使用工作电话号码时将无法通过移动电话拨打或接听电话。  <br/> |正确  <br/> |
|启用 IP 音频和视频  <br/> |EnableIPAudioVideo  <br/> |设置为默认值，允许用户使用 VoIP 在移动设备上拨打或接听电话或视频呼叫。 设置为 False 时，用户将无法使用其移动设备执行上述任一操作。  <br/> |正确  <br/> |
|IP 音频需要 WiFi  <br/> |RequireWiFiForIPAudio  <br/> |定义客户端是否需要通过 WiFi（而不是手机数据网络）通过 VoIP 拨打和接听电话。 如果设置为 True，则用户只有在通过 WiFi 连接时才能拨打和接听 VoIP 呼叫。  <br/> |错误  <br/> |
...
若要使用户可以访问移动功能和通过工位呼叫功能，需要为用户启用企业语音。 但是，即使未启用它们，他们仍然可以通过单击其移动设备上的链接来加入会议，但只有在他们分配了适当的语音策略时才能加入会议。 您可以：
