...
|| (与响应组应用程序和会议助理应用程序) 的应用程序的 Contact 对象。  <br/> |
||为了向后兼容而发布的数据。  <br/> |
||用于中央管理存储 (SCP) 的服务控制点。  <br/> |
||Kerberos 身份验证帐户 () 的可选计算机对象。  <br/> |
   
### <a name="os-for-domain-controllers"></a>适用于域控制器的操作系统

那么，可以使用哪些域控制器操作系统？ 我们有以下列表：

- Windows Server 2019 (您必须具有 Skype for Business Server 2015 累积更新5或更高版本) 
...
此图再次显示了一个林，但它还具有一个或多个子域， (此特定示例中有三个) 。 因此，在其中创建用户的域可能不同于将 Skype for business Server 2015 部署到的域。 为什么要考虑这一点呢？ 请务必记住，在部署 Skype for business Server 前端池时，池中的所有服务器都需要位于单个域中。 你可以通过 Skype for Business Server 对 Windows 通用管理员组的支持进行跨域管理。
...
这是如何工作的？ 嗯，目录同步产品 (（如 Forefront Identity Manager）或 FIM) 管理组织的用户帐户的整个存在。 从林中创建或删除帐户时，该更改将同步到中央林中的相应联系人。
...
若要在此类型的拓扑中部署 Skype for Business Server 2015，您可以在资源林中为用户 (林中的每个用户帐户创建一个禁用的用户对象。如果 Microsoft Exchange Server 已经在环境中，则可以) 执行此操作。 然后，您需要目录同步工具 (如 Forefront Identity Manager 或 FIM) 以通过其生命周期管理用户帐户。
...
- 它允许统一通信 (未登录的 UC) 设备。在发现运行设备更新 web 服务的前端池或 Standard Edition 服务器以获取更新和发送日志的情况。
...
且绝对不要使用 Unicode 字符或下划线。 标准字符 (为 A-z、a-z、0-9 和连字符的) 是外部 DNS 和公共证书颁发机构支持的 (需要将 Fqdn 分配给证书中的 SN，请不要忘记) ，因此，如果考虑到这一点，你将 grief 大量的。
...
在部署之前，您可以执行的最重要的操作之一是确保您的证书按顺序排列。 Skype for Business Server 2015 需要 (PKI) 的公钥基础结构，以获取传输层安全性 (TLS) 和相互传输层安全性 (MTLS) 连接。 通常，若要以标准化方式进行安全通信，Skype for Business Server 将使用证书颁发机构颁发的证书 (CAs) 。
...
- 与 web 应用程序和 Outlook Web Access (OWA) 的对话
...
- 必须使用操作系统支持的签名算法对所有证书进行签名。 Skype for Business Server 2015 支持 SHA-1 和 SHA-1 套件的摘要大小 (224、256、384和512位) ，并满足或超过操作系统要求。
...
你将需要大多数内部服务器的证书，并且很可能会从位于域) 中的内部 CA (获取证书。 如果需要，可以从位于 internet) 上 (的外部 CA 请求这些证书。 如果您想知道应该转到什么公共 CA，可以查看 [统一通信证书合作伙伴](/SkypeForBusiness/certification/services-ssl) 列表。
...
Skype for Business Server 2015 还包括对 (的支持，而无需使用 SHA-256 加密哈希函数签署) 证书。 若要使用 SHA-256 支持外部访问，外部证书需要由使用 SHA-256 的公共 CA 颁发。
  
为尽量简单地进行处理，我们已将 Standard Edition 服务器、前端池和其他角色的证书要求放在下表中，并将使用虚构的 contoso.com 用于您的环境) 的一些示例 (。 这些是包含不可导出的私钥的所有标准 web 服务器证书。 需要注意的一些其他事项：
  
- 使用证书向导请求证书时，将自动配置服务器增强型密钥用法 (EKU) 。
    
- 每个证书友好名称在计算机存储中必须是唯一的。
    
- 根据下面的示例名称，如果您已在 DNS 中配置 sipinternal.contoso.com 或 sipexternal.contoso.com，则需要将它们添加到证书的使用者备用名称 (SAN) 中。
    
Standard Edition 服务器的证书：
  
|**证书**|**使用者名称/常用名称**|**使用者替代名称**|**示例**|**Comments**|
|:-----|:-----|:-----|:-----|:-----|
|默认值  <br/> |池的 FQDN  <br/> |服务器池和 FQDN 的 FQDN  <br/> 如果具有多个 SIP 域并已启用自动客户端配置，则证书向导会检测并添加所有受支持的 SIP 域 FQDN。  <br/> 如果此池是客户端的自动登录服务器，而且组策略要求执行严格的域名系统 (DNS) 匹配，那么还需要 sip.sipdomain 条目（对应于您拥有的每个 SIP 域）。  <br/> |SN = se01;SAN = se01  <br/> 如果此池是客户端的自动登录服务器，而且组策略要求执行严格的 DNS 匹配，则还需要 SAN=sip.contoso.com; SAN=sip.fabrikam.com。  <br/> |对于 Standard Edition Server，服务器 FQDN 与池 FQDN 相同。  <br/> 向导检测您在安装过程中指定的任何 SIP 域，并自动将其添加为使用者备用名称。  <br/> 您还可以使用此证书进行服务器到服务器身份验证。  <br/> |
...
|:-----|:-----|:-----|:-----|
|默认值  <br/> |控制器池  <br/> |控制器的 FQDN、控制器池的 FQDN。  <br/> 如果此池是客户端的自动登录服务器，并且组策略中需要严格的 DNS 匹配，则还需要针对每个已) 的 SIP 域使用 sipdomain (的条目。  <br/> |pool.contoso.com;SAN = dir01  <br/> 如果此控制器池是客户端的自动登录服务器，而且组策略要求执行严格的 DNS 匹配，那么您还需要 SAN=sip.contoso.com; SAN=sip.fabrikam.com。  <br/> |
...
安装持久聊天服务器时，将需要与 Skype for business Server 2015 内部服务器所使用的 CA 颁发的证书。 需要为运行持久聊天 Web 服务的每台服务器执行文件上载/下载。 强烈建议您在启动持久聊天安装之前，您已具有所需的证书 (s) ，如果您的 CA 是外部的，则更多的 (可能需要花费一些时间才能) 发出。
  
### <a name="certificates-for-external-user-access-edge"></a>外部用户访问 (边缘) 的证书

Skype for Business Server 2015 支持使用 **单个公共证书** 访问和 Web 会议边缘外部接口以及 a/V 身份验证服务，这是通过边缘服务器 (s) 提供的。 你的边缘内部接口通常使用由内部 CA 颁发的专用证书，但如果你愿意，也可以将公共证书用于此证书（如果它来自受信任的 CA）。
  
您的反向代理 (RP) 也将使用公共证书，并通过使用 HTTP (或更精确地通过 HTTP) 对内部服务器的通信进行加密，从而将通信从你的 RP 加密到客户端，并将 RP 加密为内部服务器。
...
现在，这是一个很好的预规划，但有时您已部署 Skype for Business Server 2015，而不打算部署移动性，并且在您的环境中已经有证书的情况下。 通过内部 CA 重新颁发它们通常相当容易，但使用公共 CA 颁发的公共证书可能会更昂贵。
  
如果这是您要查看的内容，并且如果您有大量 SIP 域 (这样做会增加 SAN 的成本) ，则可以将反向代理配置为对初始自动发现服务请求使用 HTTP，而不是使用默认配置) 的 HTTPS (。 "规划移动性" 主题包含有关此功能的详细信息。
...
> [!NOTE]
> 你的反向代理侦听器将为你的外部 Web 服务 URL (s) 提供 San。 例如 SAN = skypewebextpool01 和 dirwebexternal.contoso.com，如果您部署了 Director， (这是可选的) 。 
...
- 文件共享必须位于直接附加存储 (DAS) 或存储区域网络 (SAN) 中，这包括分布式文件系统 (DFS) 以及独立磁盘冗余阵列 (RAID) 用于文件存储。 有关适用于 Windows Server 2012 的 DFS 的进一步阅读，请查看 [此 DFS 页面](https://technet.microsoft.com/library/jj127250.aspx)。
...
> [!CAUTION] 
> 您应该知道，使用网络附加存储 (NAS) 作为文件共享不受支持，因此，请使用上面列出的选项之一。 
