...
我们提供了图表和说明，可帮助您确定您的环境中具有的拓扑，或在安装 2015 年 2 月之前可能需要设置Skype for Business Server项。 为简单一点，我们还包括一个密钥：
...
![显示两个 AD 林，一个用户林和一个资源林。 这两个林具有信任关系。 它们使用Microsoft 365 Office 365或Azure AD 连接。 所有用户均可以通过 Skype for Business 或 Microsoft 365 Office 365。](../../media/6d54558d-8786-4ebf-90f6-55ae3fdb5ae7.jpg)
...
请记住，DNS 中任何名称与在 2015 年 2015 年 10 月使用的任何服务器上配置的计算机名称Skype for Business Server相同。 具体来说，环境中不能有任何短名称，并且必须具有用于拓扑生成器的 FQDN。
  
这看起来对于已加入域的任何计算机来说都符合逻辑，但如果边缘服务器未加入域，则其默认名称可能是短名称，没有域后缀。 对于这一点，请确保在 DNS 或边缘服务器，或任何 Skype for Business Server 2015 服务器或池中都并非如此。
  
绝对不要使用 Unicode 字符或下划线。 标准字符 (即 A-Z、a-z、0-9 和连字符) 是外部 DNS 和公共证书颁发机构 (您需要在证书中将 FQDN 分配给 SN 的字符，不要忘记) ，因此，如果命名时牢记这一点，您将免去许多麻烦。
...
为了简单易行，我们将 Standard Edition 服务器、前端池和其他角色的证书要求放在下表中，其中将虚拟 contoso.com 用作示例 (你可能正在为环境) 使用其他内容。 这些证书都是标准 Web 服务器证书，具有不可导出的私钥。 还需要注意其他一些注意事项：
