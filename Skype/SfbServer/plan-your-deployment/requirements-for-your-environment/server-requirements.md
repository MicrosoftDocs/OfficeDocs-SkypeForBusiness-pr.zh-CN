...
正如您预期，在开始部署 2015 年 10 月之前，需要做一Skype for Business Server准备工作。 本文将介绍规划以下内容：
...
|**硬件组件**|**建议**|
|:-----|:-----|
|CPU  <br/> |64 位双处理器、十六核、2.26 GHz (GHz) 或更高。  <br/> Intel Itanium 处理器不受 Skype for Business Server 2015 角色的支持。  <br/> |
|内存  <br/> |32 GB (GB) 。  <br/> |
...
|网络  <br/> |1 个双端口网络适配器，1 Gbps 或更高 (2 个网络适配器，但它们需要与一个 MAC 地址和一个 IP 地址) 。  <br/> 前端服务器、后端服务器、服务器和持久聊天服务器不支持双Standard Edition多主配置。 <br/> 只要它们未向操作系统公开，并且用于监视和管理服务器硬件，就可以拥有带外管理系统，如 DRAC 或 BAND。 此方案不构成多主服务器，并且受支持。  <br/> |
   
边缘服务器、独立中介服务器、视频互操作服务器和控制器的建议硬件：
  
|**硬件组件**|**建议**|
|:-----|:-----|
|CPU  <br/> |64 位双处理器、四核、2.26 GHz (GHz) 或更高。  <br/> Intel Itanium 处理器不受 Skype for Business Server 2015 角色的支持。  <br/> |
|内存  <br/> |16 GB。  <br/> |
...
如果未在此列表上，它将无法正常工作，请不要尝试在 2015 年 2015 Skype for Business Server安装。
...
安装 Skype for Business Server 2015 Standard Edition 时，SQL Server 2014 Express (64 位) 也会自动安装。
  
Skype for Business Server 2015 Enterprise Edition稍微复杂一些，但支持的列表低于 (所有内容都是 64 位版本，你会注意到，请不要使用 32 位版本) ：
  
||||||
|:-----|:-----|:-----|:-----|:-----|
|Microsoft SQL Server 2019 Enterprise (64 位) ，建议运行最新 Service Pack。 <br/> |Microsoft SQL Server 2017 Enterprise (64 位) ，建议运行最新 Service Pack。 <br/> |Microsoft SQL Server 2016 Enterprise (64 位版本) Service Pack 1 或更高版本，并且必须通过 Skype for Business 累积更新 7 或更高版本运行 ([下载 Skype for Business 累积](https://support.microsoft.com/help/3061064)更新) 。  <br/> |Microsoft SQL Server 2014 Enterprise (64 位版本) ，并且必须运行累积更新 6 或更高版本 (下载累积更新[6](https://support.microsoft.com/kb/3031047/)) 。  <br/> |Microsoft SQL Server 2012 Enterprise (64 位) ，建议运行最新 Service Pack。  <br/> |
...
会议内容文件（如 PowerPoint 演示文稿）将作为附件存档。 如果要将 Skype for Business存档数据与 Exchange 合规性数据一起存储，则必须对 Exchange 部署使用 Exchange，并确保最大存储大小支持存储会议内容文件。 在使用 Microsoft Exchange集成选项部署和启用存档之前，Exchange部署存档。 
...
您需要设置存档存储的基础结构。 这包括选择Exchange存档存储或存档SQL Server。   Skype for Business Server存档基础结构要求与部署Skype for Business Server。 有关详细信息，请参阅[Requirements for your Skype for Business environment](../../plan-your-deployment/requirements-for-your-environment/requirements-for-your-environment.md)。 
  
> [!NOTE]
> 若要支持未位于 Exchange 服务器的用户，或者如果您不想使用 Microsoft Exchange 集成选项，则必须使用 64 位数据库部署存档SQL Server存储。 
...
2015 SQL镜像或SQL 2015 Skype for Business Server群集，受支持。 SQL通过拓扑生成器设置Skype for Business Server镜像。 如果你打算设置群集SQL，则使用 SQL Server。
...
|Windows Identity Foundation  <br/> |我们需要使用 Windows Identity Foundation 3.5 来支持 2015 年 3 月Skype for Business Server身份验证方案。  <br/> • 对于 Windows Server 2012 和 Windows Server 2012 R2，无需下载任何内容。 打开 **服务器管理器**，然后转到添加 **角色和功能向导**。 **Windows Identity Foundation 3.5"** 列于"功能 **"** 部分下。 如果已选中，则没有问题。 否则，请选择它并单击"下一步"以到达 **"安装"** 按钮。 <br/> |
