...
---
# <a name="define-the-user-experience-for-manually-acquiring-a-location-in-skype-for-business-server"></a>定义在 Skype for Business Server 中手动获取位置的用户体验
 
在 Skype for Business Server 企业语音 中使用 SIP 中继提供程序规划 E9-1-1 部署中的漫游企业语音。
...
当用户手动输入位置时，该位置将映射到客户端网络的默认网关的 MAC 地址，并存储在位于客户端上的每用户表中。 当用户返回到之前存储的任何位置时，Skype for Business 客户端会自动将自身设置到该位置。 
