...
> [!NOTE]
> Skype for Business Server三个高级 企业语音 功能：呼叫允许控制 (CAC) 、紧急服务 (E9-1-1) 和媒体旁路。 有关这三项功能共同的规划信息的概述，请参阅 企业语音 中高级功能的网络[Skype for Business Server](network-settings-for-advanced-features.md)。
...
未超出组织限制而强制执行呼叫允许Skype for Business Server控制。 CAC 不能应用于通过 Internet 的媒体流量，该流量不由 Skype for Business Server。 如果被叫终结点属于组织，并且边缘服务器已添加到网络配置中，则对通过企业网络的呼叫部分执行 CAC 检查，如呼叫允许控制部署：[Skype for Business Server 的最终检查](../../deploy/deploy-enterprise-voice/final-checklist.md)表中所述。 如果被叫终结点不属于组织，如联盟或 PIC 用户，则不执行带宽策略检查，传出呼叫将忽略任何 CAC 限制。

### <a name="call-admission-control-of-pstn-connections"></a>PSTN 连接的呼叫允许控制

无论呼叫允许控制是连接到 IP/PBX、PSTN 网关还是 SIP 中继，呼叫允许控制都可以在中介服务器上实施。 由于中介服务器是 B2BUA (后端到后端用户) ，因此它将终止媒体。 它具有两个连接端：连接到 Skype for Business Server 端和网关端，连接到 PSTN 网关、IP/PBX 或 SIP 中继。 有关 PSTN 连接的详细信息，请参阅 Plan [for PSTN connectivity in Skype for Business Server](pstn-connectivity-0.md)。
...
以下各节介绍了此示例拓扑的组件。 有关此拓扑的规划方法（包括带宽限制）[的详细信息](example-gathering-requirements.md)，请参阅示例：收集呼叫允许控制在 Skype for Business Server。

### <a name="identify-network-regions"></a>标识网络区域

网络区域代表网络中枢或网络中心。

网络中枢或中心是计算机网络基础结构的一部分，将网络的不同部分相互连接，为在不同 LAN 或子网之间交换信息提供路径。 网络中枢可以将各种网络（从较小位置到广泛的地理区域）连接在一起。 网络中枢的容量通常大于连接到网络的网络的容量。
...
站点链接由将波特兰、里诺和阿尔伯克基连接到北美区域、将马吉拉连接到 APAC 区域以及将波特兰连接到 EMEA 地区的线路指示。 里诺和阿尔伯克基之间的线路显示这两个站点之间的直接网络链接。
