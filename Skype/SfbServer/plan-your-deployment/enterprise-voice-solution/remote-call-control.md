...
ms.locfileid: "58604731"
---
# <a name="plan-for-remote-call-control-in-skype-for-business"></a>规划远程呼叫控制Skype for Business
 
远程呼叫控制是 Lync Server 早期版本中的一项功能，它使用户能够使用 Lync Server 控制其 PBX 电话。 在Skype for Business Server，此功能已替换为通过工位呼叫功能。  *在 Skype for Business Server 2015 和未来版本的客户端版本中，远程呼叫控制不再可在客户端中配置，并且已删除供其使用。* 
  
 组织中位于运行 Lync Server 的前端服务器上运行的远程呼叫控制用户可以继续使用远程呼叫控制，即使他们使用的是 Skype for Business 客户端。 但是，对于在远程呼叫Skype for Business Server，不支持远程呼叫控制。 有关服务器/客户端组合及其是否可以支持远程呼叫控制或通过工位呼叫的信息，请参阅下表。
