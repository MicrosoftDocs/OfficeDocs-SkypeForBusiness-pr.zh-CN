---
title: 收集业务服务器 2015 Skype 中的呼叫允许控制的要求的示例
ms.author: crowe
author: CarolynRowe
manager: serdars
ms.date: 2/16/2018
...
2. 标识每个网络区域相关联的中央站点。 中央站点包含至少一个前端服务器，将通过 WAN 连接的网络区域的所有媒体通信的管理 CAC 的业务服务器部署 Skype。
   ...
   **企业网络示例分为三个网络区域**
    ...
    > [!NOTE]
    > 多协议标签交换 (MPLS) 网络应代表一个网络区域，在该网络区域中，每个物理位置都具有一个相应的网络站点。 有关详细信息，请参阅规划文档中的"[的组件和拓扑的呼叫允许控制中的业务 2015 Skype](components-and-topologies.md)"主题。 
...
4. 对于每个网络区域，标识所有通过受带宽限制的 WAN 链路连接到网络区域的网络站点。
   ...
   **CAC 网络区域北美地区具有三个受带宽 （芝加哥、 纽约和底特律） 的网络站点和三个网络站点都受 WAN 带宽 （波特兰、 里诺和阿尔）**
...
6. 对于网络中的每个子网，指定其关联的网络站点。
    ...
    > [!IMPORTANT]
    > 如果部署音频/视频边缘服务器，则每台边缘服务器的公共 IP 地址都必须与部署边缘服务器的网络站点关联。 必须将 A/V 边缘服务器的每个公共 IP 地址作为子网掩码为 32 的子网添加到网络配置设置中。 例如，如果在芝加哥部署 A/V 边缘服务器，则为这些服务器的每个外部 IP 地址创建一个子网掩码为 32 的子网，并将网络站点芝加哥与这些子网相关联。 有关公用 IP 地址的详细信息，请参阅[规划业务 2015年的 Skype 的网络要求](../../plan-your-deployment/network-requirements/network-requirements.md)。 
...
9. 对于通过单链路 （称为站点间链路） 直接进行连接的网络站点的每个对，确定以下信息：
   ...
   **CAC 网络区域北美显示的带宽容量和里诺和阿尔伯克基之间的站点间链路的带宽限制**

     ![受 WAN 带宽限制的网络站点的示例](../../media/Plan_CS_VoiceCAC_limitsforNAdirectlinksRenoAlbuq.jpg)
  
   **带宽信息 （带宽单位： kbps） 的两个网络站点间的站点间链接**

   |**站点间链路名称**|**第一个网站**|**第二个站点**|**BW 限制**|**音频限制**|**音频会话限制**|**视频限制**|**视频会话限制**|
   |:-----|:-----|:-----|:-----|:-----|:-----|:-----|:-----|
