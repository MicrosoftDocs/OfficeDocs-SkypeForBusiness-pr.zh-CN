...
Skype for Business Server两种方法将紧急呼叫路由到 E9-1-1 服务提供商：

- 到合格的 E9-1-1 服务提供商的会话初始协议 (SIP) 中继连接

- 到基于公用电话交换网 (PSTN) 的 E9-1-1 服务提供商的紧急位置标识号 (ELIN) 网关

使用 SIP 中继 E9-1-1 服务提供商时，需要向位置信息服务数据库添加 ELL，然后根据主街道地址指南 (MSAG) （由 E9-1-1 服务提供商维护）验证位置。 如果 E9-1-1 服务提供商收到没有位置信息或位置尚未根据 MSAG 进行验证的呼叫，则 E9-1-1 服务提供商将呼叫路由到国家/地区紧急呼叫响应中心 (ECRC) ，该中心配备专门培训的人员，这些人员通过口头方式获取呼叫者的位置， ，并手动将呼叫路由至相应的 PSAP。  (某些 SIP 中继 E9-1-1 服务提供商还会为客户提供到 ECRC 的 PSTN 外线直拨 (DID) 号码，从而在 SIP 中继因任何原因失败的情况下，提供路由 9-1-1 呼叫的替代方法。) 
...
- **E9-1-1 服务提供商选项** 您可以在 E9-1-1 服务提供商处注册模拟电话 DID 及其对应的 ELL（如果 E9-1-1 服务提供商支持）。 如果提供商收到来自不包含 PIDF-LO Skype for Business Server的呼叫，则提供商可以看到呼叫方 DID 号码上是否有数据库匹配项。 通过使用从数据库检索到的 ERL，提供商会自动将紧急呼叫路由到正确的 PSAP，PSAP 将接收模拟设备的 DID 和允许调度程序查找呼叫者位置的 ESQK 记录。
...
下图显示了客户端Skype for Business如何获取位置 (第三方客户端 MAC 地址的位置方法除外) ：
...
2. Skype for Business Server (匹配紧急号码，并基于在适用的位置策略) 中定义的 **PSTN** 用法值将呼叫路由至中介服务器，然后通过 SIP 中继路由到 E9-1-1 服务提供商。
...
将 ELIN 网关用于紧急呼叫时，使用与 SIP 中继Skype for Business Server E9-1-1 基础结构相同的 E9-1-1 基础结构。 即，位置信息服务数据库向客户端Skype for Business位置，位置策略将启用该功能并定义路由。 但是，使用 ELIN 网关时，你需要将 ELIN 添加到位置信息服务数据库，并且让 PSTN 运营商将它们上载到 ALI (ALI) 数据库。
...
1. 包含位置、呼叫者的回拨号码和可选的 (通知 URL 和会议) 的 SIP INVITE 将路由到 Skype for Business Server。
