...
description: 了解 Skype for Business Server 企业语音 中的增强型 9-1-) 1 (E9-1-1 Skype for Business Server 企业语音 服务，包括位置获取和呼叫路由。
...
了解 Skype for Business Server 企业语音 中的增强型 9-1-) 1 (E9-1-1 Skype for Business Server 企业语音 服务，包括位置获取和呼叫路由。
...
> [!NOTE]
> Skype for Business Server三个高级企业语音功能：呼叫允许控制、紧急服务 (E9-1-1) 和媒体旁路。 有关这三项功能共同的规划信息的概述，请参阅 Skype for Business Server 中高级网络功能的网络[企业语音](network-settings-for-advanced-features.md)设置。

Skype for Business Server支持增强型 9-1-1 (E9-1-1) 客户端Skype for Business Lync 电话 Edition 设备进行呼叫。 为 E9-1-1 配置 Skype for Business Server 时，从 Skype for Business 或 Lync 电话 Edition 拨打的紧急呼叫包括位置信息服务数据库中的紧急响应位置 (ERL) 信息。 ERL 包含市政（即街道）地址，以及有助于确定在办公楼和其他多租户设施中的更精确位置的其他信息。 当用户进行紧急呼叫时，Skype for Business Server中介服务器将呼叫音频以及位置和回拨信息路由到 E9-1-1 服务提供商。 E9-1-1 服务提供商会使用呼叫者的市政地址，将呼叫路由到为呼叫者的位置提供服务的公共安全应答点 (PSAP)，并沿着 PSAP 用来查找呼叫者的 ERL 的紧急服务查询键 (ESQK) 进行发送。
...
与时区多路复用 (TDM) 和基于 IP 的专用交换机 (PBX) 电话（具有固定位置）不同，Skype for Business 终结点可以非常移动。 部署 E9-1-1 功能时，Skype for Business Server 有助于确保无论呼叫者位于何处，紧急呼叫都可以路由到为呼叫者所在的位置服务 PSAP。 例如，如果用户的总部位于华盛顿州雷德蒙德，但用户从位于华盛顿州威奇托的分支机构的计算机拨打紧急呼叫，则基于 SIP 中继或基于 PSTN 的 E9-1-1 服务提供商将呼叫路由到格林威奇托的 PSAP，而不是雷德蒙德的 PSAP。
...
下图显示了客户端Skype for Business如何获取位置 (除基于第三方客户端 MAC 地址的位置方法) ：
...
从兼容的客户端进行紧急呼叫Skype for Business Server客户端：

1. 包含位置、呼叫者的回拨号码和 (（可选) 通知 URL 和会议回拨号码）的 SIP INVITE 将路由到 Skype for Business Server。
...
1. 包含位置、呼叫者的回拨号码和可选的 () 通知 URL 和会议回拨号码的 SIP INVITE 将路由到 Skype for Business Server。
