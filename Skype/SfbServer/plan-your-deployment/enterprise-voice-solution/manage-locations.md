...
- 一个位置标识符，可帮助用户轻松查看其Skype for Business客户端选取了正确的位置。 客户端Skype for Business连接并在其标头中显示发现的 **Location** 和 **City** 字段。 一个好的做法是将建筑物的街道地址添加到每个位置标识符 (例如，"1st Floor \<street number>") 。 如果没有街道地址，那么通用的位置标识符（例如“1st Floor”）可以适用于城市中的所有建筑。
...
有几个方案需要更新位置数据库，包括添加 WAP、重新布置 (导致不同的交换机分配) 和子网扩展。 是直接更新各个位置，还是使用 CSV 文件执行所有位置的批量更新？
