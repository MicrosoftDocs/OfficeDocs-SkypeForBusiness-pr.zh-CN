...
description: 本文列出了哪些联机和本地拓扑受 Skype for Business 中的新式验证支持，以及适用于每个拓扑的安全功能。
---

# <a name="skype-for-business-topologies-supported-with-modern-authentication"></a>新式验证支持的 Skype for Business 拓扑

本文列出了哪些联机和本地拓扑受 Skype for Business 中的新式验证支持，以及适用于每个拓扑的安全功能。
...
MA 使用的拓扑中可能涉及Microsoft 365两个服务器Office 365或Skype for Business或工作负载。
...
- Active Directory 联合服务器 (ADFS) 内部部署

因此，它看起来有点类似，在云中 EXO 和 SFBO 与 Azure AD、Exchange Server (EXCH) 和 Skype for Business 服务器 (SFB) 一样。
...
|混合 2  <br/> |![S4B 混合拓扑 2 支持 MA，SFBO 加上 MA 与 EXCH on-prem 一起运行。](../../media/247a985d-39cd-4c16-a19e-b8b65207d82e.PNG)           <br/> 用户所定位/邮箱：EXCH 和 SFBO  <br/> |MA 仅适用于 SFBO。 授权服务器Azure AD SFBO 中，而 AD 为本地 EXCH。  <br/> |使用 Intune 的 MFA、CBA、CA/MAM。\*  <br/> |
...
|混合 5  <br/> |![SFB 中支持的 MA、带 MA 的 EXO 和带 MA 的 SFBO，以及本地 EXCH 和 SFB。](../../media/ecc366cf-1a7b-4ad1-bf8e-57111b8ad94f.PNG)           <br/> 用户所在的用户/邮箱：EXO + SFBO、EXO + SFB、EXCH + SFBO 或 EXCH + SFB  <br/> |MA 同时在 EXO 和 SFBO 中打开，因此授权服务器Azure AD SFBO 中用户;EXCH 和 SFB 中的用户使用 AD。  <br/> |MFA、CBA、CA/MAM with Intune 仅适用于联机用户。\*  <br/> |
|混合 6  <br/> |![在混合 6 拓扑中，所有四个假设位置都启用新式验证- 对于新式身份验证，这是理想选择。](../../media/8de21756-9152-466d-a706-58b258e2271c.png)           <br/> 用户所在的用户/邮箱：EXO + SFBO、EXO + SFB、EXCH + SFBO 或 EXCH + SFB  <br/> |MA 位于任何地方，因此授权服务器Azure AD所有用户使用。  (联机和本地部署)   <br/>  请参阅了解 [https://aka.ms/ModernAuthOverview](/microsoft-365/enterprise/hybrid-modern-auth-overview) 部署步骤。 <br/> |MFA、CBA 和 CA/MAM (Intune) 适用于所有用户。  <br/> |
