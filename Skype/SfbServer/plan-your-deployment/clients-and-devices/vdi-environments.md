...
在一些 (和) 问题特别敏感的组织中，会使用虚拟桌面基础结构) VDI 基础结构环境。 他们的用户使用远程桌面服务或类似的远程连接，在所有桌面应用程序和文件的虚拟桌面上工作。 在这样的连接上将 Skype for Business 与完整的音频和视频一同使用需要在虚拟桌面上的客户端上执行大量音频和视频处理。 提供了其他 VDI 插件软件，用于将处理卸载到最终用户的本地计算机，并减少虚拟桌面上的负载。
...
- **Citrix RealTime Optimization Pack** 基于 Lync VDI 插件构建，与虚拟桌面上的 Lync 2013 或 Skype for Business 2016 客户端一起运行。 它由 Citrix 和 Microsoft 共同开发，以改进原始 VDI 插件。 它可以安装在具有 Windows 和非 Windows 操作系统（包括 Windows 10、Mac 和 Linux (）的客户端) 。 它包含两个组件：安装在虚拟桌面) 上的 RealTime 连接器 (和安装在最终用户的本地计算机) 上的 RealTime Media Engine (。 这两个组件允许用户的本地计算机使用在虚拟桌面上运行的 Skype for Business 客户端，同时将 A/V 处理移动到本地计算机。 对于基于 Citrix 的虚拟桌面环境，建议使用 Citrix RealTime Optimization Pack，并且计划进一步提供支持。
    
- 与 VMWare 协作开发的适用于 Skype for Business 的 **VMWare Horizon Virtualization** Pack 允许你在虚拟桌面中交付 Skype for Business，同时提供出色的用户体验。 解决方案的工作方式是利用客户端的媒体引擎来创建优化的解决方案，客户端终结点为音频和视频呼叫提供媒体卸载功能。 此解决方案可以直接在终结点之间传递音频和视频，进行一对一协作，或卸载到中央多点控制单元 (MCU) 进行多方电话会议或会议。
...
Citrix 的 VDI 环境插件 (XenApp 和 XenDesktop) 的一项功能与 Lync 2013 和 Skype for Business 2015 和 2016 (完整客户端兼容，使用任意单击即可运行安装程序，或在安装在虚拟桌面上的 2017 年 1 月 PU) 客户端后发布的 MSI 安装程序。 其整体功能基于 Microsoft Lync VDI 插件，但它适用于更广泛的客户端操作系统，包括 Windows 10、Macintosh 和 Linux。
  
可在 Citrix 网站上找到功能和支持的技术的完整列表，网址为 [Delivering Microsoft Skype for Business to XenApp and XenDesktop Users](https://www.citrix.com/content/dam/citrix/en_us/documents/products-solutions/delivering-microsoft-lync-to-xenapp-and-xendesktop-users.pdf)。
...
Microsoft Lync [VDI 插件位于 Microsoft Lync VDI 2013 插件 (32 位) ](https://www.microsoft.com/download/details.aspx?id=35457)或 Microsoft Lync [VDI 2013 插件 (64 位) 。 ](https://www.microsoft.com/download/details.aspx?id=35454) 此插件受 Skype for Business 2015 客户端支持，尽管名称不一样。
...
- **支持：VDI (个性化虚拟桌面) 。** 在此方案中，每个用户登录到可自定义的虚拟桌面，并能够将文件保存在桌面上跨会话保留。 Microsoft 远程桌面服务和 VMware 水平视图是经过测试以与 Skype for Business 2015 一同使用的示例实现。 正在进行验证的其他实现包括 Citrix XenDesktop。 有关供应商特定的 VDI 环境和经 Microsoft 测试的客户端硬件的信息，请参阅适用于 [Microsoft Lync 的](../../../SfbPartnerCertification/lync-cert/qualified-ip-pbx-gateway.md)基础结构。
    
- **不支持：远程桌面会话。** 在此方案中，每个用户登录到无法自定义的通用虚拟桌面会话。 示例包括与 Citrix Receiver (RDSH) 和 Citrix XenApp 的 Microsoft 远程桌面会话。
    
Lync VDI 插件不支持其他虚拟化技术，例如应用程序虚拟化，它允许使用应用程序，而无需在本地安装完整应用程序。 示例实现包括 Citrix XenApp 和 Microsoft Application Virtualization (App-V) 。 应用程序流、应用程序远程处理和混合虚拟化模式 (例如，不支持在完整桌面远程处理) 应用程序远程处理。
...
- 本地计算机必须运行 [最新版本的远程桌面客户端](/windows-server/remote/remote-desktop-services/clients/remote-desktop-clients)。 从 Microsoft 安装远程桌面服务客户端的最新更新，或安装虚拟化解决方案提供商的最新远程桌面客户端软件。 
...
- 匿名加入 (，即加入由组织托管的 Skype for Business 会议，而该组织未与贵组织联盟) 。
...
Microsoft 365 或 Office 365 环境不支持 Lync VDI 插件。
