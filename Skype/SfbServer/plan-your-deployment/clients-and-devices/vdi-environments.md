...
在一些 (和) 问题特别敏感的组织中，会使用虚拟桌面基础结构) VDI 基础结构环境。 他们的用户使用远程桌面服务或类似的远程连接，在所有桌面应用程序和文件的虚拟桌面上工作。 在Skype for Business连接上使用完整的音频和视频，这需要在虚拟桌面上的客户端上执行大量的音频和视频处理负载。 提供了其他 VDI 插件软件，用于将处理卸载到最终用户的本地计算机，并减少虚拟桌面上的负载。
...
- **支持：VDI (个性化虚拟桌面) 。** 在此方案中，每个用户登录到可自定义的虚拟桌面，并能够将文件保存在桌面上跨会话保留。 Microsoft 远程桌面Services 和 VMware Horizon View 是经过测试以与 Skype for Business 2015 一同使用的示例实现。 正在进行验证的其他实现包括 Citrix XenDesktop。 有关供应商特定的 VDI 环境和经 Microsoft 测试的客户端硬件的信息，请参阅适用于 [Microsoft Lync 的](../../../SfbPartnerCertification/lync-cert/qualified-ip-pbx-gateway.md)基础结构。
    
- **不支持：远程桌面会话。** 在此方案中，每个用户登录到无法自定义的通用虚拟桌面会话。 示例包括Microsoft 远程桌面与 Citrix Receiver (RDSH) 和 Citrix XenApp 之间的会话。
...
- 匿名加入 (，即Skype for Business与您的组织联盟的组织主持的) 。
