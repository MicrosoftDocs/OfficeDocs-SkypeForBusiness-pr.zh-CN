...
- **拓扑规划工具** 可自动执行大部分拓扑设计过程。 可以将结果从规划工具导出到拓扑生成器，拓扑生成器是安装每台运行拓扑Skype for Business Server。
...
在Skype for Business Server中，可以使用 Open Authorization 和 OAuth (协议在服务器) 身份验证。 例如，可以将 Skype for Business Server配置为向运行 2016 Microsoft Exchange Server的服务器进行身份验证。 通过使用 OAuth 协议，Skype for Business Server和Microsoft Exchange Server相互信任。 这提供了以无缝方式集成产品的能力。 有关详细信息，请参阅管理 OAuth (中的服务器) [和合作伙伴Skype for Business Server。](../../manage/authentication/server-to-server-and-partner-applications.md)
  
### <a name="windows-powershell-based-management-and-web-based-management-interface"></a>Windows PowerShell基于管理和基于 Web 的管理界面

Skype for Business Server提供了一个基于命令行界面构建Windows PowerShell功能强大的管理界面。 其中包含用于管理安全性的 cmdlet，并且默认情况下会启用 Windows PowerShell 安全功能，以防止用户轻易或无意中运行脚本。 这意味着软件默认设置为自动帮助实现最大安全性并减少攻击事件。 有关在命令行Windows PowerShell管理支持Skype for Business Server，请参阅Skype for Business Server[命令行管理程序](../../manage/management-shell.md)。 
...
部署向导可以自动将主题名称 (SN) 主题备用名称 (SN) ，从而减少包括不必要的和可能不安全的条目的可能性。
