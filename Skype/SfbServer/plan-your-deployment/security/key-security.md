...
本文提供了安全安全Skype for Business Server概述。 
  
## <a name="key-security-features-in-skype-for-business-server"></a>安全与Skype for Business Server

安全性是一个非常广泛的主题。 安全将跨越整个生态系统Skype for Business Server功能以及数据库、服务和硬件Skype for Business Server。 本文概述了本文中专为Skype for Business Server设计的功能。
...
在Skype for Business Server中，有关服务器和服务的配置数据是中央管理存储的一部分。 中央管理存储为定义、设置、维护、管理、描述和运行部署部署所需的数据提供了可靠的Skype for Business Server存储。 它还会验证数据，以确保配置的一致性。 对此配置数据进行的所有更改都发生在中央管理存储中，消除了"不同步"问题。 
...
在Skype for Business Server中，可以使用 Open Authorization (OAuth 协议在服务器) 身份验证。 例如，您可以将 Skype for Business Server配置为向运行 2016 Microsoft Exchange Server的服务器进行身份验证。 通过使用 OAuth 协议，Skype for Business Server和Microsoft Exchange Server可以相互信任。 这提供了以无缝方式集成产品的能力。 有关详细信息，请参阅在[OAuth](../../manage/authentication/server-to-server-and-partner-applications.md) (和合作伙伴应用程序中) 服务器到Skype for Business Server。
  
### <a name="windows-powershell-based-management-and-web-based-management-interface"></a>Windows PowerShell基于管理和基于 Web 的管理界面

Skype for Business Server提供了一个基于命令行接口构建的功能强大的Windows PowerShell接口。 其中包含用于管理安全性的 cmdlet，并且默认情况下会启用 Windows PowerShell 安全功能，以防止用户轻易或无意中运行脚本。 这意味着软件默认设置为自动帮助实现最大安全性并减少攻击事件。 有关在 Windows PowerShell 中提供管理Skype for Business Server的详细信息，请参阅 Skype for Business Server [Management Shell](../../manage/management-shell.md)。 
...
Skype for Business Server符合 SDL (Microsoft 可信任[计算安全](/previous-versions/ms995349(v=msdn.10))开发生命周期) 。
...
- **默认情况下可信赖** 默认情况下，加密Skype for Business Server网络通信。 由于所有服务器都使用证书和 Kerberos 身份验证、TLS、安全 Real-Time 传输协议 (SRTP) 以及其他行业标准加密技术（包括 128 位高级加密标准 (AES) 加密）因此，几乎所有 Skype for Business Server 数据都受网络保护。 此外，利用基于角色的访问控制，可以部署运行 Skype for Business Server 的服务器，以便每个服务器角色仅运行这些服务，并且仅具有与适用于服务器角色的这些服务相关的权限。
