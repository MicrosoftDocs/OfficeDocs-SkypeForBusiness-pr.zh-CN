...
- **拓扑生成器**。 使用拓扑生成器创建、调整和发布您规划的拓扑，并且它会在您开始安装服务器之前验证拓扑。 当您在Skype for Business Server安装服务器时，服务器会读取发布的拓扑作为安装过程的一部分，并且安装程序将按照拓扑中的指示部署服务器。 安装完成后，配置信息将自动复制到所有服务器。 组件只能使用拓扑生成器添加到部署中。
