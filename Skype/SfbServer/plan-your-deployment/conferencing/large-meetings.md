...
Skype for Business Server支持的会议规模取决于会议是托管在共享池还是专用池中：从共享池中的 250 个参与者到专用池中的 1000 个参与者。 
...
> [!NOTE]
> Skype 会议广播允许用户主持会议，并广播给最多 10，000 名参与者的大型联机受众。 使用广播Skype 会议要求已在Skype for Business Server或组织混合安装中配置Microsoft 365 Office 365。 所有用户都必须建立联机租户作为先决条件。 如果你有兴趣部署可以利用 Skype 会议 广播的混合解决方案，请参阅什么是 [Skype 会议 广播？](https://go.microsoft.com/fwlink/?LinkId=617071)和配置本地部署以Skype 会议[广播](../../deploy/configure-skype-meeting-broadcast.md)。 
...
若要为多达 1000 个用户的会议提供足够的 CPU 和内存资源，宿主前端服务器不应承载任何其他即时消息 (IM) 以及状态或 企业语音 工作负荷。 服务器不应承载任何其他会议，而不管其他会议的大小如何。 若要承载多达 1000 个用户的会议，您需要设置一个专用于Skype for Business Server大型会议单独的会议池。
...
会议审查方可以使用所有客户端的所有演示Skype for Business管理大型会议。 这些功能包括：
...
> [!NOTE]
> 对会议中的大型Skype for Business Server要求 **将 AllowLargeMeetings** 设置设置为 true。 当此设置设置为 true 时，Skype for Business会议体验将针对用户加入会议时的额外会议进行优化。 具体而言，在大型Skype for Business，系统不会显示完整会议参与者列表的初始或更新，这是客户端和会议Skype for Business Server。 相反，Skype for Business将只显示有关用户和会议演示者列表的信息。 Skype for Business仍显示大型会议中可用的参与者总数。
...
此外，您需要配置专用大型会议池，以便Skype for Business Server该池并负责管理会议日程的每个用户都有相应的权限。 为此，请执行以下操作：
