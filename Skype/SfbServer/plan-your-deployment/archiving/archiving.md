...
- **使用哪个存储选项**。 可以通过以下两种方法之一实现存储，或结合使用这两种方法：
  ...
  - **Exchange存储。** 如果您有一个Exchange部署，您可以集成 Skype for Business Server 和 Exchange 存档，以便 Skype for Business Server 和 Exchange 存档数据一起存储在 Exchange 中。 如果启用 Microsoft Exchange 集成选项，则位于 Exchange Server 上的用户邮箱对存档数据使用 Exchange 存储，但只有在邮箱已置于 In-Place 保留状态时。 默认情况下，Microsoft Exchange集成未启用。
...
- **何时部署存档**。 可以在初始部署部署中部署存档Skype for Business Server，也可以将其添加到现有部署中。 若要Skype for Business Server存档存储 (SQL Server数据库) ，请使用拓扑生成器将数据库添加到拓扑，然后再次发布拓扑。 如果您的所有用户都位于 Exchange 且其邮箱都置于 In-Place 保留状态，则不必更新拓扑，而只需启用 Microsoft Exchange 集成以在 Exchange 中存储存档数据。 
...
> [!NOTE]
> 如果同时实现 Skype for Business Server 存档数据库并启用 Microsoft Exchange 集成，Exchange 策略将覆盖 Skype for Business Server 存档策略，但仅针对那些位于 Exchange 且拥有其将邮箱置于In-Place保留状态。 Skype for Business存档取决于 Microsoft Exchange In-Place保留策略。 
...
### <a name="microsoft-exchange-storage"></a>Microsoft Exchange存储
 ...
 如果选择 Microsoft Exchange集成，Exchange策略和配置来控制 Skype for Business Server。 可以在全局级别Exchange、站点级别和池级别配置 Microsoft Exchange 集成选项。 如果您的部署包含多个林，则必须在部署和部署Skype for Business Server Exchange。 你将需要确定：
...
- **全局级别选项**。 这是默认存档配置，适用于整个部署。 在部署外部通信时Skype for Business Server，默认情况下会同时禁用内部和外部通信的存档。 不能删除此选项。 如果选择删除选项，则全局选项将重置为默认设置。
...
若要详细了解如何协同工作Skype for Business Server Exchange，请参阅 Plan to integrate Skype for Business [and Exchange](../../plan-your-deployment/integrate-with-exchange/integrate-with-exchange.md)。
