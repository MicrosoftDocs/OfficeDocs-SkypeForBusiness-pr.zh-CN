...
- **使用哪种存储选项**。 可以通过以下两种方法之一实现存储，或结合使用这两种方法：
...
- **何时部署存档**。 可以在初始部署部署中部署Skype for Business Server存档，也可以将其添加到现有部署中。 若要Skype for Business Server存档存储 (SQL Server数据库) ，请使用拓扑生成器将数据库添加到拓扑中，然后再次发布拓扑。 如果您的所有用户都位于 Exchange 且其邮箱已置于 In-Place 保留状态，则无需更新拓扑，而只需启用 Microsoft Exchange 集成以在 Exchange 中存储存档数据。 
    
- **组织中哪些站点和用户需要存档**。 您可以为整个组织以及（可选）特定站点、池、用户和用户组配置存档设置。
    
- **应存档的内容**。 无论是在全局级别还是为特定网站和用户在每个级别指定存档，您都指定是否启用以下类型的内容： 
...
- **是否实现关键模式**。 如果您的组织需要存档，则配置关键模式将在出现阻止存档的故障时Skype for Business Server IM 和会议会话。 例如： 
...
> [!NOTE]
> 如果同时Skype for Business Server存档数据库并启用 Microsoft Exchange 集成，Exchange策略将覆盖Skype for Business Server存档策略，但仅适用于位于存档Exchange 并且其邮箱已置于In-Place保留状态。 Skype for Business存档取决于 Microsoft Exchange In-Place保留策略。 
...
### <a name="microsoft-exchange-storage"></a>Microsoft Exchange存储
 ...
 如果选择 Microsoft Exchange集成，Exchange策略和配置来控制 Skype for Business Server。 可以在全局级别、站点级别和池级别配置 Microsoft Exchange集成选项。 如果您的部署包含多个林，则必须在部署和部署Skype for Business Server设置Exchange。 你将需要确定：
...
如果选择"Skype for Business Server"，则使用Skype for Business Server策略和配置来控制如何启用和实施存档。 Skype for Business Server存储SQL Server数据库，因此您需要向拓扑SQL Server相应的数据库，然后配置存档策略。 
...
如果将存档数据库与监控数据库、后端数据库或这两个数据库并并，您可以对任意或所有数据库使用单个 SQL 实例，也可以针对每个数据库使用单独的 SQL 实例，但存在以下限制：每个 SQL 实例只能包含一个后端数据库、一个监控数据库。 和单个存档数据库。
...
Skype for Business Server可在以下级别指定存档选项。 
...
有关在部署存档时如何设置初始存档配置的详细信息，请参阅部署存档[Skype for Business Server](../../deploy/deploy-archiving/deploy-archiving.md)。 有关在部署后管理存档的详细信息，请参阅在 Skype for Business Server 中[管理存档](../../manage/archiving/archiving.md)。 
  
## <a name="archiving-configuration-tools"></a>存档配置工具

 通过使用"控制面板"，可以控制Skype for Business Server选项。 但是，只有使用命令行管理程序Skype for Business Server一些选项。 这些选项包括存档重复消息和导出存档数据。 有关使用命令行管理Skype for Business Server命令行管理Skype for Business Server管理程序管理存档策略[Skype for Business Server。](../../manage/archiving/archiving.md)
...
- **Skype for Business Server存档存储**。 如果设置 Skype for Business Server 存档数据库，Skype for Business Server存档数据库将存档内容存储在 Skype for Business Server 存档数据库中，以便存储未位于 Exchange 上且尚未保留其邮箱的任何用户In-Place保留。 虽然此数据是不可搜索的，但可以采用可通过其他工具搜索的格式导出此数据。 有关导出存档数据库中存储的数据的详细信息[，请参阅导出](../../manage/archiving/export-archived-data.md)存档Skype for Business Server。
