...
配置自动发现服务后，您必须修改 Skype for Business Server OAuth 配置设置;这将Skype for Business Server自动发现服务查找位置。 若要在命令行管理程序Skype for Business Server OAuth 配置设置，请从命令行管理程序Skype for Business Server命令。 运行此命令时，请确保为在 Exchange Server 上运行的自动发现服务指定 URI，并使用 **autodiscover.svc** 指向服务位置，而不是 **指向autodiscover.xml** (它指向服务) ：
...
除了配置自动发现服务之外，还必须为指向您的 Exchange Server 的服务创建 DNS 记录。 例如，如果自动发现服务位于 autodiscover.litwareinc.com 则需要为解析为 autodiscover.litwareinc.com 的完全限定域名的 Exchange Server (例如，atl-exchange-001.litwareinc.com) 。
  
如果要将 Skype for Business Server与 Exchange Online 集成，则下一步将位于配置本地[](../../deploy/integrate-with-exchange-server/outlook-web-app.md)Skype for Business Server 与 Outlook Web App 之间的集成中，否则请参阅[集成Skype for Business Server Exchange Server](../../deploy/integrate-with-exchange-server/integrate-with-exchange-server.md)。
...
|通过 IM、PSTN 呼叫、Skype OWA 电子邮件中的呼叫或视频呼叫进行响应   |Y   |Y   |N   |N   |Y   |
...
|将内容 (IM 和会议) 存档Exchange   |Y (2016 Exchange 2013 年 10 月)    |Y   |N   |N   |Y   |
