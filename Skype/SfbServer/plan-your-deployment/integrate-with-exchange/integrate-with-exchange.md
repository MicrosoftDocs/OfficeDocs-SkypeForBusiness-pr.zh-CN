...
在集成 Skype for Business Server 和 Exchange Server 之前，必须确保Exchange Server和Skype for Business Server完全安装并启动并运行。 
  
有关安装Exchange Server的详细信息，请参阅 Exchange Server 版本的规划和部署文档Exchange。 
   
服务器启动并运行后，必须将服务器到服务器身份验证证书分配给 Skype for Business Server 和 Exchange Server;这些证书允许Skype for Business Server Exchange Server 交换信息并相互通信。 安装证书Exchange Server，将创建一个名称为 Microsoft Exchange Server身份验证证书的自签名证书。 此证书（可在本地计算机证书存储中找到）应该用于在 Exchange Server 上进行服务器到服务器身份验证。 有关在邮箱中分配证书Exchange Server，请参阅配置[邮件Flow客户端访问](/exchange/configure-mail-flow-and-client-access-exchange-2013-help)。
...
必须先配置自动发现服务，然后才能集成Skype for Business Server Exchange Server。 您可以通过从 Exchange Server 命令行管理程序运行以下命令并检查 AutoDiscoverServiceInternalUri 属性的值来验证是否已配置自动发现服务：
...
配置自动发现服务后，您必须修改 Skype for Business Server OAuth 配置设置;这将确保 Skype for Business Server知道在哪里可以找到自动发现服务。 若要在命令行管理程序Skype for Business Server OAuth 配置设置，请从命令行管理程序Skype for Business Server以下命令。 运行此命令时，请确保为在 Exchange Server 上运行的自动发现服务指定 URI，并使用 **autodiscover.svc** 指向服务位置，而不是 **指向autodiscover.xml** (它指向服务) ：
...
如果要将 Skype for Business Server与 Exchange Online 集成，则下一步骤位于配置本地 Skype for Business Server 与 Outlook Web App 之间的集成[](../../deploy/integrate-with-exchange-server/outlook-web-app.md)中，否则请参阅[集成Skype for Business Server Exchange Server](../../deploy/integrate-with-exchange-server/integrate-with-exchange-server.md)。
  
## <a name="feature-support"></a>功能支持
<a name="feature_support"> </a>

>[!Important]
> Skype for Business 2021 年 7 月 31 日停用了 Online。 下面Exchange包括该服务的集成不再受支持。
...
|状态Outlook Web App   |Y   |Y   |N   |N   |Y   |
|通过 IM、PSTN 呼叫、Skype OWA 电子邮件进行呼叫或视频呼叫进行响应   |Y   |Y   |N   |N   |Y   |
|通过会议安排和加入联机Outlook Web App   |Y   |Y   |N   |N   |Y   |
|移动客户端中的 IM/状态   |Y   |Y   |Y   |Y   |Y   |
|在移动客户端中加入联机会议   |Y   |Y   |Y   |Y   |Y   |
|根据日历忙/Outlook信息发布状态   |Y   |Y   |Y   |Y   |Y   |
|联系人列表 (统一联系人存储)    |Y (2016 Exchange 2013 年 10 月)    |Y   |N   |N   |Y   |
|高分辨率联系人照片 (至少需要 Lync 2013 Skype for Business客户端。 不支持 LWA、移动应用程序、Lync 2010、Lync for Mac 和其他较旧的客户端)    |Y (2016 Exchange 2013 年 10 月)    |Y   |N   |Y   |Y   |
|会议委派   |Y   |Y   |Y   |Y   |Y   |
|错过的对话历史记录和呼叫日志将写入到用户的 Exchange 邮箱   |Y   |Y   |Y   |Y   |Y   |
|存档 (IM 和会议) 内容Exchange   |Y (2016 Exchange 2013 年 10 月)    |Y   |N   |N   |Y   |
|搜索存档内容   |Y (2016 Exchange 2013 年 10 月)    |Y   |N   |N   |Y   |
|Exchange UM 语音邮件   |Y   |Y   |N   |N   |N   |
|服务器端对话历史记录   |Y   |Y   |N   |Y   |Y   |

> [!NOTE]
> 云语音邮件 Online、Skype for Business Skype for Business Server 2019、Skype for Business Server 2015 和 Lync Server 2013 支持一种服务。
> 

## <a name="see-also"></a>另请参阅
<a name="feature_support"> </a>

[配置本地部署和Skype for Business Server Outlook Web App](../../deploy/integrate-with-exchange-server/outlook-web-app.md)
