...
---
# <a name="user-experience-during-pool-failure-in-skype-for-business-server"></a>Skype for Business Server 中的池故障期间用户体验
 
了解在 Skype for Business Server 的灾难恢复期间前端池出现故障或故障恢复时用户的体验。
  
如果对池进行故障备份，则受影响池中的所有用户将强制注销，然后登录到备份池。 登录到备份池的用户在短时间内可能会处于恢复能力模式。 在恢复能力模式下，用户无法执行将导致 Skype for Business Server 上的永久性更改的任务，例如添加联系人。 故障转移完成后，所有用户均可从备份池获得所有服务。
  
池出现故障时用户拥有的任何呼叫、会议或对话将中断，并且用户必须在故障转移后重新建立这些会话以继续。
  
故障转移或故障回复期间，不能重新连接用户。 位于失败的池上的用户将由备份池临时提供服务。 还原主池后，管理员可以故障回复这些用户，以便由原始池提供服务，这些用户仍位于其原来的池中。
  
请注意，位置信息服务器数据库不会复制到备份池。 作为最佳做法，管理员应定期备份 LIS 数据库，并在故障转移后使用最新备份副本在备份池中还原 LIS 数据库。
  
## <a name="user-experience-during-failover"></a>故障转移期间用户体验

当用户位于出现故障的池中时，该用户将注销。用户参与的任何对等会话都结束，该用户组织的会议也结束。 在注册器恢复能力计时器过期或管理员启动故障转移过程之前（以先发生的为准），该用户将无法重新登录。 当用户重新登录时，将会登录到备份池。 如果他们在故障转移完成前登录，则在故障转移完成前，他们将一直处于恢复能力模式。 只有这样，用户才能建立新会话或重新建立以前的会话。
...
下表显示了有关故障回复期间和故障后用户如何受到影响的更多详细信息，以及其他池中的用户如何查看正在故障回复的池中的用户并与之交互。 
  
术语受影响用户 指从主池进行故障转移并由备份池提供服务的用户。 最初位于备份池上的用户不是受影响的用户。
