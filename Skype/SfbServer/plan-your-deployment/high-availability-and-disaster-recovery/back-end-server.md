...
- SQL Server 2019 年，Enterprise Edition 和 Standard Edition

- SQL Server 2017 年 10 月，Enterprise Edition 和 Standard Edition

- SQL Server 2016 年 10 月，Enterprise Edition 和 Standard Edition

- SQL Server 2014 年 10 月，Enterprise Edition 和 Standard Edition
    
- SQL Server 2012 SP2 和 CU2，包括 Enterprise Edition 和 Standard Edition
...
配置服务器镜像之前，必须先正确设置SQL数据库权限。 有关详细信息，请参阅["为数据库镜像或 AlwaysOn](/sql/database-engine/database-mirroring/set-up-login-accounts-database-mirroring-always-on-availability) 可用性组设置登录 (SQL Server) "。
...
> [!IMPORTANT]
> 只有在主服务器、镜像服务器和见证 (服务器都属于同一个域时) 才支持使用拓扑生成器或 cmdlet 设置和删除 SQL 镜像。 如果您需要在不同域中的服务器之间设置 SQL 镜像，请参阅 SQL Server 文档。 
...
有关部署 AlwaysOn 可用性组的步骤，请参阅在 Skype for Business Server 中的后端[服务器上部署 AlwaysOn 可用性Skype for Business Server](../../deploy/deploy-high-availability-and-disaster-recovery/alwayson-availability-group.md)。
