...
安装新实现（如为 Skype for Business Server 创建[DNS](../../deploy/install/create-dns-records.md)记录并构建 Skype for Business Server 拓扑中所述时，我们意识到可以选择使用 Windows Server 2016 或第三方 DNS 程序包中内置的 DNS 功能，因此我们将ep 本文中讨论内容是常规的，而不是具体的。 我们详细介绍了需要满足哪些需求以及如何满足此需求，这是您的决定。
...
|A/AAAA   | 后端服务器SQL服务器 FQDN <br/> *SQL1。 <span></span>contoso <span></span> .com*   |服务器 IP 地址  <br/> *192.168.11.90*   |地图前端池的后端池SQL服务器的服务器名称更新到其 IP 地址   ||
|A/AAAA   |后端服务器镜像SQL服务器 FQDN  <br/> *SQL2。 <span></span>contoso <span></span> .com*   |服务器 IP 地址  <br/> *192.168.11.91*   |地图前端池的后端池SQL服务器的服务器名称恢复为 IP 地址   ||
...
|A/AAAA   |ucupdates-r2。*\<sipdomain\>* <br/> ucupdates-r2。*<span></span> contoso <span></span> .com*  |HLB FE 池 VIP 或控制器池 HLB VIP 或 标准版/控制器服务器 IP  <br/>  192.168.21.121  |部署此记录是可选的&#x2778;  ||
|SRV   |\_sipinternaltls。 \_tcp.*\<sipdomain\>* <br/>端口 5061 <br/>\_sipinternaltls。 \_tcp.*<span></span> contoso <span></span> .com* <br/>端口 5061  |前端池 FQDN  <br/>*FE-Pool。 <span></span>contoso <span></span> .com*  |启用内部用户自动登录 1 到前端服务器/池标准版验证和重定向客户端登录请求的服务器/池。  |&#x2777; |
...
|SRV   |\_sipfederationtls。 \_tcp.*\<sipdomain\>* <br/>\_sipfederationtls。 \_tcp. <span></span>*contoso <span></span> .com*  | 访问边缘服务 FQDN <br/> EdgePool-int。 <span></span>*contoso <span></span> .com*  |为具有 IOS 或电话移动客户端的每个 SIP 域Windows SRV 记录。   |对于移动客户端支持   |
...
|A/AAAA   |Web 会议边缘服务池 FQDN  <br/>*Webcon1。 <span></span>contoso <span></span> .com*  |Web 会议边缘服务外部 IP 地址  <br/> 131.107.16.90, 131.107.16.91   |Web 会议边缘服务使外部用户能够加入在内部会议环境中Skype for Business Server会议。   |Y   |
...
|SRV   |\_xmpp-server。 \_tcp.*<sipdomain \>* <br/>\_xmpp-server。 \_tcp.*<span></span> contoso <span></span> .com*  |外部访问边缘 FQDN  <br/>*Access1。 <span></span>contoso <span></span> .com*  |XMPP 代理服务接受 XMPP 联盟伙伴 (和发送) XMPP 联盟伙伴的可扩展消息传递和状态协议。   |Y，用于部署联盟，否则为可选  <br/> 在 2019 Skype for Business Server中不可用。|
...
|A/AAAA   |外部前端池 Web 服务 FQDN  <br/>*Web-ext。 <span></span>contoso <span></span> .com*  |反向代理公用 IP 地址，代理到前端池外部 Web 服务 VIP &#x2776; <br/> 131.107.155.1 代理到 192.168.21.120   |前端池使用前端池外部Skype for Business Web应用   |Y   |
...
|A/AAAA   |OfficeWeb Apps Server 池 FQDN  <br/> OWA。 <span></span>contoso <span></span> .com   | 反向代理公用 IP 地址，代理到 Office Web Apps Server 的外部 Web 接口 <br/> 131.107.155.1 代理到 192.168.1.5   | OfficeWeb Apps Server 池 VIP 地址 <br/> 192.168.1.5   |定义 Office Web Apps Server 池 FQDN   |
...
> [!IMPORTANT]
> Skype for Business Server支持使用 IPv6 寻址。 有关详细信息，请参阅 Skype for Business 中的规划[IPv6。](ipv6.md)
...
如果你计划让一些用户联机并某些用户位于本地，请参阅混合连接规划文章[Skype for Business server 2019。](../../../SfbHybrid/hybrid/plan-hybrid-connectivity.md?toc=/SkypeForBusiness/sfbhybridtoc/toc.json) 对于 2015，您需要将 DNS 配置为正常Skype for Business Server并添加其他 DNS 记录。
...
- **管理员** 允许快速访问Skype for Business Server控制面板。 在组织防火墙内的任何计算机上，管理员可通过在浏览器中Skype for Business Server管理简单 URL 打开"管理"控制面板。 管理简单 URL 是组织内部的。 管理简单 URL 的一个示例是 https:// <span></span> 管理员。 <span></span>contoso <span></span> .com。
