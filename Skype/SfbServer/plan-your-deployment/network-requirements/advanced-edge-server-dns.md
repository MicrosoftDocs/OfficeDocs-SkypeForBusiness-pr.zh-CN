...
- 此内部 contoso.com 包含：
  ...
  - DNS A 和 AAAA (如果使用 IPv6 为外围网络中每台 Skype for Business Server 边缘服务器的边缘内部接口寻址) 记录。
    
  - DNS A 和 AAAA (如果你为外围网络 (中每台反向代理服务器的内部接口使用 IPv6 寻址) 记录 (则对于反向代理) 的管理是可选的。
    
  - DNS A 和 AAAA (如果使用 IPv6 寻址) 和 SRV 记录进行内部 Skype for Business Server 客户端自动配置 (是可选的 **)**。
...
- 所有运行 Skype for Business Server 的服务器和在企业网络中运行 Skype for Business Server 的客户端指向内部 DNS 服务器以解析对 contoso.com 的查询，或者，如果使用 IPv6 寻址)  (记录（专门针对控制器）或列出 A 和 AAAA (控制器池 VIP、前端池 VIP 或 Standard Edition 服务器) 。
...
- 此外部 contoso.com 包含：
  ...
  - 如果对外围网络中每台 Skype for Business Server 边缘服务器的边缘外部接口或硬件负载平衡 (HLB) VIP 使用 IPv6 寻址) 和 SRV 记录，则 DNS A 和 AAA (A) 。
...
- _sipinternaltls._tcp.fabrikam.com。 86400 IN SRV 0 0 5061 pool01.fabrikam.com
     ...
     *如果用户以用户 alice@fabrikam.com 登录，则此记录适用于第二个域的自动配置，同样，因为 SIP 域与该域的前端池匹配。* 
...
> [!NOTE]
> 此外，前端池 FQDN 值在示例和 contoso.com fabrikam.com 更改，但 IP 地址保持不变。 这是因为从任一 SIP 域登录的用户将使用相同的前端池进行自动配置。 
...
如果在 GeoDNS 提供程序上为 Web 服务的内部和外部) IPv6 记录，定义和配置其他 DNS 主机 A 或 AAAA (AAAA。 以下详细信息假定池已配对，地理位置分散，且您的提供商支持的 GeoDNS 要么具有 **循环 DNS，** 要么配置为使用 Pool1 作为主池，并且在任何通信丢失或电源故障时将故障转网到 Pool2。
