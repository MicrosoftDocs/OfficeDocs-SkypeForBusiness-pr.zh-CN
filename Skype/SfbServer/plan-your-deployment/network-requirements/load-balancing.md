...
扩展Skype for Business Server边缘拓扑已针对主要使用 Skype for Business Server 或 Lync Server 与其他组织联盟的新部署优化了 DNS 负载平衡。 如果下列任何方案要求高可用性，则必须在边缘服务器池上对以下内容使用硬件负载平衡器： 
  
- 使用 communications Server 2007 R2 或 Office Communications Server 2007 与Office联盟
    
- Exchange 2010 SP1 之前Exchange UM 的远程Exchange UM
...
若要确定硬件负载平衡器是否支持 Skype for Business Server所需的必要功能，请参阅基础结构 [for Skype for Business](../../../SfbPartnerCertification/certification/infra-gateways.md)。 
...
DNS 负载平衡通常在应用程序级别实现。 如果对池完全限定域名 (FQDN) 使用 IPv) 6 寻址，则应用程序 (（例如，运行 Skype for Business) 的客户端）会尝试连接到池中的服务器，并连接到从 DNS A 和 AAAA (返回的 IP 地址之一。 
  
例如，如果名为 pool01.contoso.com 的池中有三个前端服务器，将发生以下情况：
  
- 运行 dns 的Skype for Business查询 DNS pool01.contoso.com。 查询返回三个 IP 地址，并按如下 (，但不一定按以下顺序) ：
...
> [!NOTE]
> 基于 DNS 的负载平衡不同于 DNS 循环 (DNS RR) 它通常是指通过依赖 DNS 来提供与池中服务器对应的不同 IP 地址顺序的负载平衡。 通常，DNS RR 仅启用负载分布，但不启用故障转移。 例如，如果与 DNS A 和 AAAA 返回的一个 IP 地址的连接 (如果使用 IPv6 寻址，则) 失败。 因此，DNS 轮循机制本身不如基于 DNS 的负载平衡可靠。 可以将 DNS 轮循机制与 DNS 负载平衡结合使用。 
...
此外，如果您使用 Exchange UM，则必须至少使用 Exchange 2010 SP1 才能获得对 SKYPE FOR BUSINESS SERVER DNS 负载平衡的支持。 如果使用早期版本的 Exchange，则对于以下 UM 方案，Exchange故障转移功能：
...
- 除了基于 XMPP 的提供程序和服务器（如 Google Talk）外，即时消息交换还与公共即时消息 (IM) 服务 AOL 和 Yahoo！的用户交换即时消息，这是目前唯一受支持的 XMPP 合作伙伴。
...
- 对于 Web 会议边缘服务，池中每台服务器都需要一个条目。 每个条目必须将 Web 会议边缘服务的 FQDN (例如，webconf.contoso.com) 解析为池中某台边缘服务器的 Web 会议边缘服务的 IP 地址。
