...
1. 在运行 Skype for Business Server 或安装了 Skype for Business Server 管理工具的计算机上，使用作为本地 Users 组 (成员的帐户或具有同等用户权限的帐户登录) 。
    ...
    > [!NOTE]
    > 可以使用作为本地 Users 组的成员的帐户来定义拓扑，但若要发布安装 Skype for Business Server 所需的拓扑，必须使用 Domain **Admins** 组和 **RTCUniversalServerAdmins** 的成员帐户。 组，并且对要用于持久聊天服务器文件存储 (的文件存储具有完全控制权限 (可读取、写入和修改) ，以便拓扑生成器可以配置所需的 DACLs) 或具有同等权限的帐户。
  
2. 启动拓扑生成器。
    
3. 在控制台树中，导航到"持久聊天池"节点并展开它以选择Skype for Business Server池，或者右键单击该节点并选择"**新建持久聊天池"**。 必须使用 FQDN 定义池的完全限定域名 (FQDN) ，并指示该池是单服务器池还是多服务器池部署。
...
8. 通过SQL之一为持久聊天 **服务器** 后端 (定义聊天室内容) 存储位置：
   ...
   - 选择“默认实例”以使用默认实例，或指定其他实例，选择“命名实例”，然后指定要使用的实例。
     ...
     > [!NOTE]
     > 若要详细了解如何配置SQL Server数据库进行灾难恢复，请参阅 Configure [high availability and disaster recovery for Persistent Chat Server in Skype for Business Server 2015](../../deploy/deploy-high-availability-and-disaster-recovery/configure-hadr-for-persistent-chat.md)。 
...
1. 在运行 Skype for Business Server 或安装了 Skype for Business Server 管理工具的计算机上，使用 **Domain Admins** 组和 **RTCUniversalServerAdmins** 成员的帐户登录 组，并且具有完全控制权限 (对要用于持久聊天服务器文件存储 (的文件存储执行) 读取、写入和修改操作，以便拓扑生成器可以配置所需的随意访问控制列表 (DACLS) ) 或具有同等用户权限的帐户。
