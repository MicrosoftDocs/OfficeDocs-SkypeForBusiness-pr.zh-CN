...
在已发布拓扑和拓扑中的每台服务器上安装了 Skype for Business 服务器系统组件后，即可验证拓扑是否按预期方式工作。 这包括验证配置是否已传播到所有 Active Directory 服务器，以便整个域知道 Skype for Business 在域中可用。 您可以按照任意顺序完成第 1 步至第 5 步。 但是，第 6、7、8 步必须在第 1 步至第 5 步之后按图表所示顺序执行。 验证拓扑是 8 个步骤中的第 8 步。
