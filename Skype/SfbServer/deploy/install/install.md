...
> [!NOTE]
> 若要了解有关最新累积更新的信息，请参阅 Skype for Business Server[更新](https://support.microsoft.com/kb/3061064)。 安装 CU1 修补程序后，管理员需要执行  `Update-CsAdminRole` cmdlet。 需要此 cmdlet 才能通过远程 PowerShell 访问新的 GCP cmdlet。
...
- [准备 Active Directory Skype for Business Server：Skype for Business Server](prepare-active-directory.md)与 Active Directory 紧密配合。 必须准备 Active Directory 域，以使用Skype for Business Server。 可以通过部署向导完成此操作，并且仅对域执行一次此操作。 这是因为此过程会创建组并修改域，并且只需执行一次。
