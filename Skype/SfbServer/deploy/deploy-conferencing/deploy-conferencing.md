...
|**创建相应的内部拓扑以支持会议** <br/> |您需要运行拓扑生成器才能将会议添加到拓扑，然后发布拓扑。  <br/> |若要定义拓扑，需具有本地 Users 组成员的帐户  <br/> 若要发布拓扑，是域管理员组和 RTCUniversalServerAdmins 组的成员的帐户，并且具有对 Skype for business Server 文件存储使用的文件共享的完全控制权限（读/写/修改），以便拓扑生成器可以配置所需的 Dacl。  <br/> |[在 Skype for Business 服务器中创建和发布新拓扑](../../deploy/install/create-and-publish-new-topology.md) <br/> |
