...
必须先确保已部署Skype for Business Server Web Apps Server Office Web Apps Office Server，然后才能配置 Web Apps Server。 有关 Web Apps Server Office，请参阅文章[部署基础结构：Office Online Server。](/webappsserver/deploy-the-infrastructure-office-web-apps-server) 
...
在Office Web Apps Server 添加到拓扑之后，发布该拓扑后，您应该在事件日志中看到两个新的事件日志Skype for Business Server事件。 首先，应 (LS 数据 MCU 事件) ID 41034;此事件将报告已Office Web Apps Server：
...
如果 Office Web Apps Server 似乎配置正确并且已由发现过程识别，则可以通过在 Skype for Business 客户端对之间共享 PowerPoint 演示文稿来验证 Office Web Apps Server 是否按预期工作。 如果用户 A 可加载并显示 PowerPoint 演示文稿，然后如果用户 B 可以加入会议，并看到该演示文稿，则 Office Web Apps Server 正在工作。
  
即使您Office Web Apps Server 配置正确，当您尝试共享演示文稿时，您也可能会收到错误消息"某些共享功能由于服务器连接问题PowerPoint不可用"。 如果收到该错误消息，您应该重新启动与新 Office Web Apps Server 关联的前面服务器（或服务器）。
