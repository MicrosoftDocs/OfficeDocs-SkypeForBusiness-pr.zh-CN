...
8. 部署池和 AG 后，执行一些最终步骤，以确保 SQL 登录名位于 AlwaysOn 可用性组的每个副本上。 
   ...
   - 重复上述两个步骤 (将组故障转移到辅助副本，然后对) 副本使用  `Install-CsDatabase -Update` 前两个步骤。
...
10. 创建可用性组。
    ...
    - 在"**选择初始** 数据同步"页中，选择"完全"，并指定副本可访问的文件夹，并且这两个副本使用的 SQL Server 服务帐户具有其写入权限。 然后单击 **下一步**。
...
11. 创建一个新存储，指定 AG 侦听器，并指定旧镜像的主体作为 AG 的主节点。
    ...
    - 在"**定义** SQL存储"页中，首先选中"高可用性 **设置"** 复选框，然后确保"SQL AlwaysOn 可用性组"出现在下拉框中。
...
7. 创建指定 AG 侦听器的新存储区。
   ...
   - 在"**定义** SQL应用商店"页中，首先选中"高可用性设置"复选框，然后确保"SQL AlwaysOn 可用性组"出现在下拉框中。
...
10. 执行最后一些步骤，确保SQL登录名位于 AG 的每个副本上。
