...
部署 (部署) 部署中的 Always On 可用性 (AG) 安装Skype for Business Server。
...
3. 使用 WSFC Windows创建 (服务器故障转移) 。
   ...
   - 在" **故障转移群集管理"** 向导中，右键单击"故障转移群集管理 **"**，然后单击"创建 **群集"**。
...
4. 建议您将群集仲裁设置配置为使用文件共享见证。 为此，请使用以下步骤：
   ...
   - 右键单击群集名称，单击"更多 **操作**"，然后单击"配置 **群集仲裁设置**"。
...
5. 在群集中的每台服务器上，在 SQL Server 配置管理器 中启用 AG 功能。
...
6. 使用拓扑生成器创建前端池，如在拓扑结构创建和发布[Skype for Business Server。](../../deploy/install/create-and-publish-new-topology.md) 这样做时，请指定 AG SQL池的存储区。
...
7. 创建可用性组。
   ...
   - 单击 **"添加**"，然后在 **"IPv4** 地址"框中，提供首选虚拟 IP 地址，然后单击"确定 **"**。
    
   - 在"**选择初始数据** 同步"页中，选择"完全"，并指定副本可访问的文件夹，并且这两个副本使用的 SQL Server 服务帐户具有其写入权限。 然后单击 **下一步**。
    
     初始化数据库时，将临时使用此文件共享。 如果要处理大型数据库，建议您手动初始化它们，以防网络带宽无法容纳数据库备份的大小。
    
   - 在"验证"页中，验证所有验证检查是否均成功，然后单击"下一步 **"**。
...
8. 部署池和 AG 后，执行一些最终步骤，以确保 SQL 登录名位于 AlwaysOn 可用性组的每个副本上。 
   ...
   - 重复上述两个步骤 ( `Install-CsDatabase -Update` 将组故障转移到辅助副本，然后对) 中每个副本使用上述步骤。
...
4. 使用SQL Server Management Studio中断镜像。
   ...
   - 打开SQL Server Management Studio，导航到数据库，右键单击 **"任务"**，然后单击"镜像 **"**。 然后单击" **删除镜像"，** 然后单击"确定 **"**。
...
9. 在群集中的每台服务器上，在 SQL Server 配置管理器 中启用 AG 功能。
...
14. 执行一些最后的步骤，以确保SQL登录名位于 AlwaysOn 可用性组的每个副本上。
    ...
    - 展开Skype for Business Server、展开拓扑和 SQL Server **Stores**。 右键单击SQL AG 的应用商店，然后单击"编辑 **属性"**。
    ...
    - 重复上述两个步骤 ( `Install-CsDatabase -Update` 将组故障转移到辅助副本，然后对) 中每个副本使用组。
...
6. 创建可用性组。
   ...
   - 在"**选择初始数据** 同步"页中，选择"完全"，并指定副本可访问的文件夹，以及两个副本使用的 SQL Server 服务帐户具有其写入权限的文件夹。 然后单击 **下一步**。
