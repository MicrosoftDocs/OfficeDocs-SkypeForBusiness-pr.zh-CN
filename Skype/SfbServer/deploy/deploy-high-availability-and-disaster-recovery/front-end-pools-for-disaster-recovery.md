...
---
# <a name="deploy-paired-front-end-pools-for-disaster-recovery-in-skype-for-business-server-2015"></a>在 Skype for Business Server 2015 中针对灾难恢复部署配对的前端池
 
你可以决定使用配对的前端池来提供灾难恢复保护，但这并不是必需满足的要求。
  
您可以轻松地部署成对使用拓扑生成器的前端池的灾难恢复拓扑。 
  
## <a name="to-deploy-a-pair-of-front-end-pools"></a>部署配对前端池

1. 如果池是新的不是尚未定义，则使用拓扑生成器来创建池。
    
2. 拓扑生成器中一个两个池，用鼠标右键单击，然后单击**编辑属性**。
...
9. 从 Skype 的业务服务器管理外壳程序命令提示符处，运行以下命令： 
...
> [!NOTE]
> **自动故障切换和回切的声音**选项和拓扑生成器中的关联的时间间隔仅适用于 Lync Server 中引入语音可恢复性功能。 选择此选项并不意味着会自动执行本文档中讨论的池故障转移。 池故障转移和故障回复始终需要管理员手动且分别调用故障转移和故障回复 cmdlet。
  
## <a name="see-also"></a>另请参阅

#### 

[为业务服务器 2015年前结束池在 Skype 的灾难恢复](../../plan-your-deployment/high-availability-and-disaster-recovery/disaster-recovery.md)
