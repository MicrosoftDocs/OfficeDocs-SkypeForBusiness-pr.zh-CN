...
description: 摘要：在 Exchange Server 2019、Exchange Server 2016、Exchange Server 2013 或 Exchange Online 和 Skype for Business Server 中配置对高分辨率照片的使用。
---
...
高分辨率照片（通过使用 Exchange Web 服务访问）可通过运行 Outlook 2013 Web App 的用户上载;仅允许用户更新自己的照片。 但是，管理员可以使用命令行管理程序以及一系列Exchange命令行管理程序Windows PowerShell更新任何用户的照片：
...
上述示例中的第 `Get-Content` 一个命令使用 cmdlet 读取文件文件C:\Photos\Kenmyer.jpg将数据存储在名为 $photo 的变量中。 第二个命令Exchange cmdlet `Set-UserPhoto` 上载照片并将该照片附加到 Ken Myer 的用户帐户。
...
如果管理员可以使用 Internet Explorer但用户无法在 Skype for Business中查看其照片Exchange Web 服务或 Exchange 自动发现服务可能有连接问题。
  
另请注意，无需进行其他配置，此照片在Skype for Business。 相反，照片在上载并 `Set-UserPhoto` 运行 cmdlet 后会立即可用。