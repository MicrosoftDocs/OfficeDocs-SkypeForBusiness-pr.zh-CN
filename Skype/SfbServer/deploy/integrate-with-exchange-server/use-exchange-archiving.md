...
启用存档 (和 Exchange 存档) 后, 必须修改相应的存档策略, 以确保实际存档用户会话。 请注意, 仅启用存档 (步骤 1) 不会导致 Skype for Business 服务器开始存档即时消息和网络会议脚本。 相反，您必须使用存档策略启用内部和/或外部存档。 安装 Skype for Business 服务器时, 还会安装单个全局存档策略, 其中包含两个属性:
