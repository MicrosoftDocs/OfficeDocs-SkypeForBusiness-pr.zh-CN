...
如果已在 Skype for Business Server 和 Exchange Server 2016 或 Exchange Server 2013 之间配置服务器到服务器身份验证，则已准备好设置统一消息。 为此，必须先在客户端上创建和分配新的统一消息拨号Exchange Server。 例如，这两个 (命令行管理程序Exchange运行) 配置新的 3 位拨号计划以用于Exchange：
...
- AllowedNumberString (，一个指示允许任意数字 \* 字符串的通配符) 
...
若要Exchange UM () UM 企业语音，需要执行以下任务：
...
- 脚本 exchucutil.ps1，它将执行以下任务：
    ...
    - 为每个网关创建一个智能寻线。 每个智能寻线引导标识符指定前端池或与网关关联的Standard Edition使用的 UM SIP URI 拨号计划。
    - 授予Skype for Business Server Active Directory 域服务Exchange UM 对象的权限。



### <a name="configure-unified-messaging-on-microsoft-exchange-with-exchucutilps1"></a>在 Microsoft Exchange 上配置统一ExchUCUtil.ps1 

在将 Microsoft Skype for Business Server与 Exchange 统一消息 (UM) 集成时，您必须在命令行管理ExchUcUtil.ps1运行 ExchUcUtil.ps1 脚本。 ExchUcUtil.ps1 脚本会执行下列操作：
...
在ExchUcUtil.ps1拓扑相同的Exchange服务器中运行 Skype for Business Server 脚本。 您可以使用命令行管理程序从邮箱服务器运行此脚本，也可以使用客户端访问服务器上的远程 Windows PowerShell 运行此脚本。 如果在组织中的客户端访问服务器上运行此脚本，则客户端访问服务器会将远程 Windows PowerShell 会话代理到组织中的邮箱服务器。
> [!IMPORTANT]
...
> [!IMPORTANT]
> 对于内部证书，运行 Skype for Business Server 服务器和运行 Microsoft Exchange必须具有相互信任的受信任的根证书颁发机构证书。 只要 (证书) 证书颁发机构在受信任的根证书颁发机构证书存储中注册证书颁发机构根证书，CA 证书颁发机构就可以是同一个证书颁发机构，也可以不同证书颁发机构。 
