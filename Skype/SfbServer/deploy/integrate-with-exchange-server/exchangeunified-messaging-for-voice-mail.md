...
Skype for Business Server使你可以将语音邮件存储在 Exchange Server 2016 或 Exchange Server 2013 中;这些语音邮件随后将在用户的收件箱中显示为电子邮件。 
...
如果已在 Skype for Business Server 和 Exchange Server 2016 或 Exchange Server 2013 之间配置服务器到服务器身份验证，则已准备好设置统一消息。 为此，必须先在客户端上创建和分配新的统一消息拨号Exchange Server。 例如，这两个命令 (命令行管理程序Exchange内部) 配置新的 3 位拨号计划Exchange：
...
在此示例的第一个命令中，VoIPSecurity 参数和参数值"Secured"指示信号通道是使用传输层安全性 (TLS) 。 URIType“SipName”指示将使用 SIP 协议发送和接收消息，CountryOrRegionCode 为 1 指示拨号计划适用于美国。
  
在第二个命令中，传递给 ConfiguredInCountryOrRegionGroups 参数的参数值指定可在此拨号计划中使用的国家/地区组。 参数值"Anywhere，\*，"\*\*将设置以下内容：
  
- 组名 ("Anywhere")
    
- AllowedNumberString (\*，一个指示允许任意数字字符串的通配符) 
    
- DialNumberString (\*，一个指示允许拨打的任何号码的通配符) 
    
- TextComment (\*，一个指示允许任何文本命令的通配符) 
...
在启用其邮箱后，用户 kenmyer@litwareinc.com 应该能够使用 Exchange 统一消息。 您可以通过从命令行管理程序Exchange [Test-CsExUMConnectivity](/powershell/module/skype/test-csexumconnectivity) cmdlet 来验证用户能否连接到 Skype for Business Server UM：
...
如果已为第二个用户启用了统一消息，则可使用 [Test-CsExUMVoiceMail](/powershell/module/skype/test-csexumvoicemail) cmdlet 验证第二个用户是否可为第一个用户留下语音邮件。
...
> [!IMPORTANT]
> 如果要使用 Exchange 统一消息 (UM) 为 企业语音 用户提供呼叫应答、Outlook Voice Access 或自动助理服务，请阅读 Skype for Business 中的规划 [Exchange](../../plan-your-deployment/integrate-with-exchange/unified-messaging.md) 统一消息集成，然后按照本节中的说明进行操作。 
...
在开始之前，本节中的文档假定你已部署以下Exchange角色：客户端访问和邮箱。 在Microsoft Exchange Server，Exchange UM 作为服务在这些服务器上运行。
...
- 在运行统一消息服务的 Exchange Server 角色和运行统一消息服务的服务器上执行集成Skype for Business Server。 您应先执行Exchange Server统一消息集成步骤，然后再执行 Lync Server 2013 集成步骤。
  > [!NOTE]
  > 若要了解哪些集成步骤在哪些服务器上执行以及由哪些管理员角色执行，请参阅部署过程概述，以集成本地统一消息[和](../../plan-your-deployment/integrate-with-exchange/deployment-overview.md)Skype for Business。 
...
如果已部署 Exchange 统一消息 (UM) ，如规划文档中的规划 Skype for Business Server Exchange 统一消息集成中所述，并且希望向 Exchange 提供 um 企业语音 组织中用户，可以使用以下过程在运行 UM 的服务器上配置Exchange证书。
...
1. 在运行 UM 的Exchange，**\<name of your Issuing CA Server>** 单击"开始"，单击"运行"，http:///certsrv"，然后单击"确定"。
...
1. 在运行 UM 的Exchange，打开 Microsoft 管理控制台 (MMC) ，方法是单击"开始"，单击"运行"，在"打开"框中键入 **mmc**，然后单击"确定 **"**。
