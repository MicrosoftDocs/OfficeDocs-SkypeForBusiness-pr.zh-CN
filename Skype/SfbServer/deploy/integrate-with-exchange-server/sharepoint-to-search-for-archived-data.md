...
在 Exchange Server 而不是 Skype for Business 服务器中存储即时消息和 Web 会议脚本的主要优势之一是将数据存储在同一位置, 使管理员能够使用单个工具搜索已存档的 Exchange 数据和/或存档的 Skype for business 服务器数据。 由于所有数据都存储在同一位置 (Exchange), 因此任何可以搜索已存档 Exchange 数据的工具也可以搜索存档的 Skype for Business 服务器数据。
