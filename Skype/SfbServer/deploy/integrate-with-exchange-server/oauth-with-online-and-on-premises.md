...
配置 OAuth 的在线业务内部部署的 Exchange 和 Skype 之间的身份验证使业务和交换集成功能所述[功能支持](../../plan-your-deployment/integrate-with-exchange/integrate-with-exchange.md#feature_support)Skype。
  
本主题适用于 Exchange Server 2016年和 Exchange Server 2013年。
  
## <a name="what-do-you-need-to-know-before-you-begin"></a>开始之前你需要了解哪些信息？

- 完成此任务的估计时间：15 分钟
    
-  在可以执行此过程或其他过程之前，你需要被分配适当的权限。 您需要哪些权限，请参阅[Exchange 和外壳程序的基础结构权限](https://go.microsoft.com/fwlink/p/?LinkId=746511)主题。
    
- 有关键盘快捷方式适用于本主题中的过程的信息，请参阅[Exchange 管理员中心中的键盘快捷键]( https://go.microsoft.com/fwlink/p/?LinkId=746512)。
...
指定您 Skype 的在线业务的组织已验证的域。 此域应是用作基于云的帐户的主要 SIP 域的相同域。 这个域被称为\<验证域\>在下面的过程。
  
在您在内部 Exchange 管理外壳 (Exchange PowerShell) Exchange 组织运行下面的命令。
...
在您在内部 Exchange PowerShell 在 Exchange 组织运行下面的命令。
...
指定您的 Exchange 组织的验证的域。 此域应该用作主要 SMTP 域内部 Exchange 帐户使用同一域。 这个域被称为\<验证域\>在下面的过程。 此外， \<DomainControllerFQDN\>应该是域控制器的 FQDN。 
...
创建新的合作伙伴应用程序，并且将使用你刚刚创建的帐户。 在您在内部 Exchange PowerShell 在 Exchange 组织运行下面的命令。 
...
运行导出内部授权证书，将导入到您的在线业务组织的下一步的 Skype PowerShell 脚本。
...
在 Exchange PowerShell 内部 Exchange 组织中的运行刚创建的 PowerShell 脚本。 例如：.\ExportAuthCert.ps1
...
接下来，使用 Windows PowerShell 将你在上一步中导出的本地授权证书上载到 Azure Active Directory 访问控制服务 (ACS)。 要执行此操作，必须已经安装用于 Windows PowerShell cmdlet 的 Azure Active Directory 模块。 如果未安装，请转到[https://aka.ms/aadposh](https://aka.ms/aadposh)安装 Azure 活动目录模块用于 Windows PowerShell。 在安装用于 Windows PowerShell 的 Azure Active Directory 模块之后，请完成以下步骤。
...
2. 例如，将下面的文本保存到 PowerShell 脚本文件命名， `UploadAuthCert.ps1`。
...
指定您的 Exchange 组织的验证的域。 此域应该用作主要 SMTP 域内部 Exchange 帐户使用同一域。 这个域被称为\<验证域\>在下面的过程。
  
> [!NOTE]
> 要成功运行以下脚本，需要按照上一节的步骤 4 中的说明将“用于 Azure Active Directory 的 Windows PowerShell”连接到 Microsoft Online Azure AD 租户。 
  
1. 将以下文本保存到一个 PowerShell 脚本文件中，例如将脚本文件命名为 RegisterEndpoints.ps1。 此示例使用通配符注册 contoso.com 所有终结点。替换为 Exchange 组织内部的主机名机构 contoso.com
...
1. 启动 Windows Phone 或 iOS 设备上的移动应用程序的业务 Skype 和以 Skype 与交换 2016年或 Exchange 2013 内部邮箱业务在线用户的身份登录。
    
2. 具有与另一个 Skype 业务在线用户的即时消息对话。
