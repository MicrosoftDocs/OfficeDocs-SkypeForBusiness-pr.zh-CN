...
description: 在本地和 Skype for Business Online Exchange OAuth 身份验证可启用功能Skype for Business Exchange集成功能。
---
...
配置 Exchange Server 和 Skype for Business Online 之间的集成Skype for Business功能Skype for Business Exchange功能[支持中所述](../../plan-your-deployment/integrate-with-exchange/integrate-with-exchange.md#feature_support)的集成功能。

本主题适用于与 2013 Exchange Server 2019 的集成。
...
为组织指定验证Exchange域。 此域应该与用作本地部署帐户的主 SMTP 域Exchange域。 以下过程中引用 \<your Verified Domain\> 了此域。 此外， \<DomainControllerFQDN\> 应为域控制器的 FQDN。
...
### <a name="step-3-create-and-enable-a-partner-application-for-skype-for-business-online"></a>步骤 3：创建和启用适用于 Skype for Business Online 的合作伙伴应用程序 
...
在本地Exchange PowerShell 中Exchange，运行刚创建的 PowerShell 脚本。 例如：.\ExportAuthCert.ps1
...
1. 在 PowerShell 中打开并Azure Active Directory，运行以下代码
```powershell
...
2. 确认移动客户端的对话历史记录显示在"Outlook历史记录"文件夹中。

3. 确认存档的聊天消息存储在用户使用 [EWSEditor](/archive/blogs/webdav_101/where-to-get-ewseditor) 的"清除"文件夹中的用户本地邮箱中。

或者，查看流量。 OAuth 握手中的流量非常独特 (，看起来并不如基本身份验证) ， 尤其是在领域周围，你将开始看到如下所示的颁发者流量：000000004-0000-0ff1-ce00-0000000000000@ (有时在要传递的令牌中 @ 符号) 之前有 / 。 你将看不到用户名或密码，这是 OAuth 的要点。 但是，你将看到"Office"颁发者（本例中为"4"Skype for Business）和订阅的领域。

如果你希望确保成功使用 OAuth，请确保你了解预期结果，并知道流量应该是什么样。 因此[](https://tools.ietf.org/html/draft-ietf-oauth-v2-23#page-34)，下面是 Microsoft 应用程序 (中 [OAuth](https://download.microsoft.com/download/8/5/8/858F2155-D48D-4C68-9205-29460FD7698F/[MS-SPS2SAUTH].pdf) 流量的一个相当标准的标准示例，虽然它不使用刷新令牌) ，但是有 Fiddler 扩展，可让你查看 OAuth JWT (JSON Web 令牌) 。

下面是设置 [一个网络跟踪](/archive/blogs/kaevans/updated-fiddler-oauth-inspector)的示例，但您可以使用任何要执行此过程的网络跟踪工具。
