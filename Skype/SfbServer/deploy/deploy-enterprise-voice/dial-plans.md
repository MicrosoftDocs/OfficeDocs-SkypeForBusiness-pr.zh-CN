...
5. “简单名称”字段会使用与“名称”字段中显示的相同名称预先填充。您可以选择编辑该字段来指定描述性更强的名称，以反映应用该拨号计划的站点、服务或用户。
   ...
   > [!IMPORTANT]
   > 简单 **名称** 在部署的所有拨号计划中必须是唯一的。 它不能超过 256 个 Unicode 字符，每个 Unicode 字符可以是字母或数字字符、连字符 (-) 、周期 (.) 或下划线字符 (_) .> 不支持的字符包括空格和保留字符，如 RFC  3966 () 中的定义。 <http://www.ietf.org/rfc/rfc3966.txt> "简单名称" **中** 不支持的保留字符 **包括** ：>";""/" "?"":" "@" "&amp;" "=" "+" "$" ","
