...
5. 在 **"选择并并服务器角色"中**，选中" **并并中介服务器"选项**。
    
    > [!NOTE]
    > 如果选择的前端池的类型为Enterprise Edition，则中介服务器组件将安装在该前端池的所有前端服务器上。 
...
7. 将 **一** 个或多个对等方关联到前端池后，单击"完成"。
    ...
    > [!NOTE]
    > 在继续执行 企业语音 部署过程中的下一步之前，请确保中介服务器池 (即与) 并排的中介服务器组件的前端池使用指定的 FQDN。 
...
7. 在"**选择下一跃** 点"页上，单击"下一个跃点池"，单击将使用此中介服务器池的前端池的 FQDN，然后单击"下一步 **"**。
