...
创建、 修改或删除 Skype 中为 Business Server 企业语音的通知应用程序的未分配号码范围。 这将影响如何处理打给未分配号码的呼叫。
  
Skype 业务服务器，您可以说到可用于您的组织，但未分配给用户或电话的电话号码的传入呼叫会发生什么情况。 为了处理此类呼叫，请设置未分配号码表。 表用于将呼叫路由到通知应用程序或 Exchange UM 服务器。
...
使用以下过程之一来配置未分配号码范围的通知应用程序。
  
> [!IMPORTANT]
> 在配置未分配号码表之前，您的系统必须已定义通知或 Exchange 统一消息 (UM) 自动助理设置。 
  
> [!TIP]
> 当有人呼叫未分配的号码时，业务服务器 Skype 搜索从上到下未分配号码表，并使用匹配的第一个范围。 因此，要在万不得已时执行的操作应指定给表中最后一个范围。 
...
5. 在第一个“号码范围”**** 字段中，键入号码范围的起始号码，在第二个“号码范围”**** 字段中，键入该范围的结束号码。
   ...
   - 数必须匹配的正则表达式 (tel:) ? (\+) ? [1-9] \d{0,17}(; ext = [1-9] \d{0,9}) ?。 这意味着数可能以字符串 tel 开头: （如果不指定该字符串，它将自动添加为您），一个加号 （+） 和一个数字 1 至 9。 电话号码最长为 17 位数，并可以后跟分机号，格式为 ;ext= 后跟分机号。
...
7. 如果在上一步中单击了“通知”****，则执行下列操作：
    ...
    a. 在**目标服务器的 FQDN**，单击**选择**，单击运行将处理到此未分配号码范围的传入呼叫，然后单击**确定**的通知应用程序的应用程序服务的服务 ID。
...
10. 在“未分配号码”**** 页上，确保未分配号码范围按照所需顺序排列。要更改号码范围在表中的位置，请在范围列表中单击一个或多个连续名称，然后单击向上箭头或向下箭头。
    ...
    > [!TIP]
    > Skype 业务服务器搜索从上到下未分配号码表，并使用第一个匹配未分配的号码的区域。 如果有重叠的范围并且有一个范围指定了最后的操作，请确保将该范围置于列表底部。 
  
11. 按照希望的顺序排列未分配号码范围后，单击“全部提交”****。
    
### <a name="to-use-skype-for-business-server-management-shell-to-configure-unassigned-phone-numbers"></a>使用 Skype 的业务 Server 命令行管理程序配置未分配的电话号码
...
### <a name="to-use-skype-for-business-server-management-shell-to-delete-an-unassigned-number-range"></a>使用 Skype 的业务 Server 命令行管理程序删除未分配号码范围
