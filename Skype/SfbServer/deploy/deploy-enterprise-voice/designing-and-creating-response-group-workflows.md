...
设计和创建响应组的工作流，在 Skype 的业务服务器企业语音。 包括智能寻线工作流和互动工作流。
  
工作流定义了从电话响铃到有人接听电话这段时间内呼叫的行为。 工作流指定的队列，用于存放该调用，并指定要使用查寻组工作流或问答交互式响应组工作流使用的路由方法。 
...
## <a name="creating-or-modifying-a-hunt-group-workflow"></a>创建或修改一个查寻组工作流

### <a name="to-use-response-group-configuration-tool-to-create-or-modify-a-hunt-group-workflow"></a>若要使用响应组配置工具创建或修改一个查寻组工作流
...
5. 在**选择服务**搜索字段中，键入承载您要创建或更改工作流的**应用程序服务器**服务名称的全部或部分。 在服务结果列表中，单击想要的服务，然后单击“确定”****。
    
    > [!NOTE]
    > 将打开响应组的配置工具。 此外可以打开响应组配置工具直接从 web 浏览器中键入以下 URL: https:// _ \<webPoolFqdn\>_/RgsConfig。 
...
8. 要允许联盟用户呼叫组，请选中“启用联盟”****复选框。 您还必须适用于为联盟配置响应组应用程序外部访问策略。
    
    > [!NOTE]
    > 全球外部访问策略适用于响应组应用程序。 通过 Skype 业务服务器控件面板或通过**设置 CsExternalAccessPolicy** cmdlet 将 EnableOutsideAccess 参数设置为 True，您可以配置响应组联盟的全球策略。 请记住，除非为全局策略设置分配站点或用户策略，否则这些设置适用于所有用户。 因此，在针对响应组更改此设置之前，请确保联盟设置满足您的组织的要求。 有关如何将策略应用于用户的详细信息，请参阅[您的组织管理外部访问策略](http://technet.microsoft.com/library/5571811e-34c8-443a-b94c-1ab5d4275581.aspx)。 有关联合身份验证设置的详细信息，请参阅[设置 CsExternalAccessPolicy](https://docs.microsoft.com/powershell/module/skype/set-csexternalaccesspolicy?view=skype-ps)。 
  
    > [!NOTE]
    > 在线业务承载于 Skype 的用户不能在内部部署中放置承载响应组对的调用。 这是在这两种混合部署，在其中将后端部署联合，Skype 的在线业务部署的情况下，则返回 true。 
...
10. 在“输入将接收呼叫的组的地址”****下，键入将应答工作流呼叫的组的主 SIP 统一资源标识符 (URI) 地址。
    ...
    > [!NOTE]
    > 工作流的主 URI 是标识和引用工作流的方式。 输入您的 SIP URI 创建为联系人对象在 Active Directory 域服务。 若要创建 URI，该对象必须是唯一在 Active Directory 中。 
  
11. 在**显示名称**中，键入想要显示的工作流 （例如，销售响应组） 的名称。
...
14. （可选）在**说明**框中，键入希望在业务在 Skype 的联系人卡片上显示工作流说明。
    
15. 如果此工作流将由响应组管理员管理，则在“工作流类型”****中选择“托管”****。 执行下列操作以将响应组管理员分配到工作流：
...
17. 如果要配置欢迎消息，则在“步骤 3 配置欢迎消息”****下选中“播放欢迎消息”****复选框，然后执行以下操作之一：
   ...
   - 要使用 Wave (.wav) 或 Windows Media 音频 (.wma) 文件录音作为欢迎消息，请单击“选择录音”****。如果要上载新的音频文件，请单击“录音”****链接。在新浏览器窗口中，单击“浏览”****，选择要使用的音频文件，然后单击“打开”****。单击“上载”****加载该音频文件。
    ...
    > [!NOTE]
    > 用户提供的所有音频文件都必须满足特定要求。 有关受支持的文件格式的详细信息，请参阅[响应组的技术要求](http://technet.microsoft.com/library/477488bd-124f-437b-9327-732a0d7271ca.aspx)。 
...
19. 通过执行下列操作之一选择要使用的工作时间日程表类型：
   ...
   - 要使用预定义工作时间日程表，请单击“使用预设日程表”****，然后从下拉列表中选择要使用的日程表。
     ...
     > [!NOTE]
     > 之前您必须至少已定义一个预设日程表才能选择该选项。 可使用  **New-CSRgsHoursOfBusiness** cmdlet 来定义预设日程表。 有关详细信息，请参阅[在业务 2015年的 Skype （可选） 定义响应组营业时间](optional-define-response-group-business-hours.md)。 
...
22. 如果要在办公室未开放时播放消息，请选中“响应组在工作时间以外时播放消息”****复选框，然后通过执行以下操作之一指定要播放的消息：
   ...
   - 要使用音频文件录音作为消息，请单击“选择录音”****。如果要上载新的音频文件，请单击“录音”****链接。在新浏览器窗口中，单击“浏览”****，选择要使用的文件，然后单击“打开”****。单击“上载”****，加载该音频文件。
     ...
     > [!NOTE]
     > 用户提供的所有音频文件都必须满足特定要求。 有关受支持的音频文件格式的详细信息，请参阅[响应组的技术要求](http://technet.microsoft.com/library/477488bd-124f-437b-9327-732a0d7271ca.aspx)。 
  
23. 指定播放消息后如何处理呼叫（如果配置了消息）：
    
   - 要断开呼叫，请单击“断开呼叫”****。
    
   - 要将呼叫转接到语音邮件，请单击“转接到语音邮件”****，然后键入语音邮件地址。 语音邮件地址的格式是_\<用户名\>_@ _\<域名\>_(例如，bob@contoso.com)。
    
   - 要将呼叫转接到另一个用户，请单击“转接到 SIP URI”****，然后键入用户地址。 用户地址的格式是_\<用户名\>_@ _\<域名\>_。
    
   - 要将呼叫转接到另一个电话号码，请单击“转接到电话号码”****，然后键入该电话号码。 电话号码的格式是_\<号\>_@ _\<域名\>_(例如，+14255550121@contoso.com)。 域名可用来将呼叫者路由至正确的目标。
    
24. 在“步骤 5 指定您的假日”****下，单击定义响应组停止营业日期的一个或多个假日集对应的复选框。
    
    > [!NOTE]
    > 配置工作流之前，您需要先定义假日和假日集。 使用 **New-CsRgsHoliday** 和 **New-CsRgsHolidaySet** cmdlet 可定义假日和假日集。 有关详细信息，请参阅[（可选） 定义响应组中业务 2015年的 Skype 的假日集](optional-define-response-group-holiday-sets.md)。 
...
### <a name="to-use-skype-for-business-server-management-shell-to-create-or-modify-a-hunt-group-workflow"></a>若要使用 Skype 的业务服务器管理外壳程序创建或修改一个查寻组工作流
...
4. 获取将在其中定向呼叫的队列或问题的标识。在命令行中运行：
   ...
   ```
    ...
    有关创建队列的详细信息，请参阅[新建 CsRgsQueue](https://docs.microsoft.com/powershell/module/skype/new-csrgsqueue?view=skype-ps)。
...
6. 如果要定义工作时间和假日，则需要在创建或修改工作流之前创建工作时间和假日。 有关详细信息，请参阅[在业务 2015年的 Skype （可选） 定义响应组营业时间](optional-define-response-group-business-hours.md)和[（可选） 定义响应组中业务 2015年的 Skype 的假日集](optional-define-response-group-holiday-sets.md)。
    
7. 如果您想要的业务时间以外或节假日上接收到的电话提示，使用**New CsRgsPrompt** cmdlet 来定义提示，并使用**New CsRgsCallAction**定义提示后要采取的操作。 有关详细信息，请参阅[新建 CsRgsPrompt](https://docs.microsoft.com/powershell/module/skype/new-csrgsprompt?view=skype-ps)和[新建 CsRgsCallAction](https://docs.microsoft.com/powershell/module/skype/new-csrgscallaction?view=skype-ps)。
    
8. 检索 Lync 服务器响应组服务的服务名并将其分配给一个变量。 在命令行中运行：
    
   ```
   $serviceId="service:"+(Get-CSService | ?{$_.Applications -like "*RGS*"}).ServiceId;
   ```

9. 创建或修改工作流。 若要创建工作流，请使用**New CsRgsWorkflow**。 若要修改工作流，请使用**组 CsRgsWorkflow**。 在命令行中键入：
   ...
   ```
     ...
     > [!NOTE]
     > 有关其他可选参数的详细信息，请参阅[新建 CsRgsWorkflow](https://docs.microsoft.com/powershell/module/skype/new-csrgsworkflow?view=skype-ps)或[设置 CsRgsWorkflow](https://docs.microsoft.com/powershell/module/skype/set-csrgsworkflow?view=skype-ps)
  
## <a name="designing-an-interactive-workflow"></a>设计互动工作流

使用互动语音响应 (IVR) 可从呼叫者获取信息，并将呼叫引导到相应的队列。 问题/答案对确定要使用哪个队列。 具体取决于调用方的响应，调用方会听到一个跟进的问题，或将被路由到相应的队列。 响应代理人员接受调用中，向代理提供有价值的信息提供了 IVR 问题和调用方的响应。
  
### <a name="overview-of-ivr-features"></a>IVR 功能概述

响应组应用程序提供语音识别和 26 种语言中的文本到语音转换功能。 可以使用文本到语音转换功能或 Wave (.wav) 或 Windows Media 音频 (.wma) 文件输入 IVR 问题。 呼叫者可以使用语音或双音多频 (DTMF) 响应进行响应。
  
互动工作流最多支持两级问题，每个问题最多有四个可能的答案。 IVR 提出问题的调用方，具体取决于调用方的响应，将呼叫者路由到队列或提出第二个问题。 第二个问题也可以有四个可能的答案。 根据呼叫者对第二级问题的回答，将呼叫者路由至相应的队列。
  
> [!NOTE]
> 通过 Skype 业务服务器管理外壳设计调用流时，您可以定义任何数字级别的 IVR 问题和任意数目的答案。 但是，为了方便呼叫者使用，建议不要使用三个级别以上的问题，并且每个问题的答案不要超过五个。 此外，如果您要设计调用流具有以上两个级别的四个以上回答的问题，不能通过 Skype 业务服务器控件面板编辑呼叫流。 
  
给接受呼叫响应代理提供了 IVR 问题和调用方的响应。
...
下例介绍使用单级问题的 IVR。 它使用语音识别来检测调用方的响应。
...
- 在调用方的语言说话。 避免造作的语言。 自然地讲话。
    
- 高效书写和有效提示。 删除任何不必要的选项。 结构信息，以便调用方的预期响应是在句子的末尾。 例如，若要与销售团队沟通，按 1。
...
### <a name="to-use-response-group-configuration-tool-to-create-or-modify-an-interactive-workflow"></a>若要使用响应组配置工具创建或修改一个交互式的工作流
...
5. 在**选择服务**搜索字段中，键入承载您要创建或修改的工作流的**应用程序服务器**服务名称的全部或部分。 在服务结果列表中，单击想要的服务，然后单击“确定”****。
...
8. 要允许联盟用户呼叫组，请选中“启用联盟”****复选框。 您还必须适用于为联盟配置响应组应用程序外部访问策略。
    ...
    > [!NOTE]
    > 全球外部访问策略适用于响应组应用程序。 通过 Skype 业务服务器控件面板或通过**设置 CsExternalAccessPolicy** cmdlet 将 EnableOutsideAccess 参数设置为 True，您可以配置响应组联盟的全球策略。 请记住，除非为全局策略设置分配站点或用户策略，否则这些设置适用于所有用户。 因此，在针对响应组更改此设置之前，请确保联盟设置满足您的组织的要求。 有关如何将策略应用于用户的详细信息，请参阅[您的组织管理外部访问策略](http://technet.microsoft.com/library/5571811e-34c8-443a-b94c-1ab5d4275581.aspx)。 有关联合身份验证设置的详细信息，请参阅文档中的**一组 CsExternalAccessPolicy** .
...
11. 在“显示名称”****中，键入希望显示的工作流名称（例如，销售 IVR 响应组）。
    ...
    > [!NOTE]
    > 不包括"\<"\>"显示名称中的字符。 不要使用以下保留的显示名称：“RGS Presence Watcher”或“Announcement Service”。 
  
12. 在“电话号码”****中，键入响应组的线路 URI（例如，+14255550165）。
    
13. 在“显示号码”****下，键入希望显示的响应组号码（例如，+1 (425) 555-0165）。
    
14. （可选）在**说明**框中，键入想要在业务在 Skype 的联系人卡片上显示的工作流的说明。 
    
15. 如果此工作流将由响应组管理员管理，则在“工作流类型”****中选择“托管”****。 执行下列操作以将响应组管理员分配到工作流：
    
    a. 键入此工作流的管理员的 SIP URI，然后单击“添加”****。
...
19. 通过执行下列操作之一选择要使用的工作时间日程表类型：
   ...
   - 要使用预定义工作时间日程表，请单击“使用预设日程表”****，然后从下拉列表中选择要使用的日程表。
     ...
     > [!NOTE]
     > 之前您必须至少已定义一个预设日程表才能选择该选项。 可使用  **New-CSRgsHoursOfBusiness** cmdlet 来定义预设日程表。 有关详细信息，请参阅[在业务 2015年的 Skype （可选） 定义响应组营业时间](optional-define-response-group-business-hours.md)。 选择预设日程表时，“天”****、“开放”****和“关闭”****中会自动填写响应组可以应答的日期和时间。
...
28. 在“步骤 7 配置互动语音响应”****下的“用户将听到以下文本或录制的消息”****标题中，指定要向呼叫者提出的问题，具体操作如下所示：
   ...
   - 若要使用包含问题是预先录制的音频文件，单击**选择记录**，然后单击上载文件的**录制**链接。 在新浏览器窗口中，单击**浏览**，选择的音频文件，然后单击**打开**。 **上载**要加载该文件，请单击，然后或者键入问题 （这使得问题和调用方的响应，转发给该响应的代理） 的文本框中。
...
### <a name="to-use-skype-for-business-server-management-shell-to-create-or-modify-an-interactive-workflow"></a>若要使用 Skype 的业务服务器管理外壳程序创建或修改一个交互式的工作流
...
[（可选）在业务 2015年的 Skype 的定义响应组假日集](optional-define-response-group-holiday-sets.md)
#### 

[（可选）在业务 2015年的 Skype 的定义响应组营业时间](optional-define-response-group-business-hours.md)
#### 

[新 CsRgsWorkflow](https://docs.microsoft.com/powershell/module/skype/new-csrgsworkflow?view=skype-ps)
  
[一组 CsRgsWorkflow](https://docs.microsoft.com/powershell/module/skype/set-csrgsworkflow?view=skype-ps)
