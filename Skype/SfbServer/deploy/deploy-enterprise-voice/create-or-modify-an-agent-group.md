...
9. 在“路由方法”中，选择将呼叫路由至组中代理的方法，具体如下：
   ...
   - 要首先向空闲时间最长的代理提供新呼叫 (在 Skype for Business 中处于"空闲"或"非活动"状态) ，请单击"最长空闲时间 **"**。 
...
10. 在“代理”中，指定创建代理列表要采用的方式：
    ...
    - 要将 Microsoft Exchange Server 通讯组列表用作代理组，请单击“使用现有的电子邮件通讯组列表”，然后在“通讯组列表地址”中键入该通讯组列表的电子邮件地址（例如 NetworkSupport@contoso.com）。
      ...
      - 如果通讯组列表包含已启用 Lync Server 2010 但没有企业语音用户，则这些用户将被添加为不正常代理。 确保通讯组列表的所有成员都企业语音用户帐户启用。
    ...
    在下列情况中，可以看见隐藏成员身份或隐藏列表：
     ...
     - 如果通讯组列表配置为在 Exchange 全局地址列表中隐藏，响应组管理员可以查看通讯组列表，如果响应组进程具有相应的用户权限，即使该管理员没有相应的用户权限，也可能将其分配给代理列表。
