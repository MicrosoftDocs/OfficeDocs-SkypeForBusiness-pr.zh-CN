...
**摘要：** 了解如何使用控制面板配置呼叫Skype for Business Server ID。

使用 Skype for Business Server，被叫方的电话号码 (即，称为) 的电话号码可以从 E.164 格式转换为中继对等 _方 (（_ 即关联网关、专用交换机 (PBX) 或 SIP 中继) ）所需的本地拨号格式。 为此，必须定义一个或多个转换规则，以便在将请求 URI 路由至中继对等方之前对其执行转换。

Skype for Business Server 还可以选择将呼叫者的电话号码 (即呼叫者从) 呼叫的电话号码从 E.164 格式转换为中继对等方所需的本地拨号格式。 例如，可以编写用于删除拨号串开头的 +44 并将其替换为 0144 的转换规则。
