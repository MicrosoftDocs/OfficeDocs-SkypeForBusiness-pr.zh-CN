---
title: 配置视频互操作服务器Skype for Business Server
ms.reviewer: ''
ms.author: v-cichur
author: cichur
manager: serdars
...
ms.locfileid: "58595976"
---
# <a name="configure-the-video-interop-server-in-skype-for-business-server"></a>配置视频互操作服务器Skype for Business Server
 
**摘要：** 配置视频互操作服务器 (VIS) 角色Skype for Business Server。
  
 使用自定义设置配置 VIS 将与视频中继Windows PowerShell。 安装 VIS 服务后，将创建具有全局范围的视频中继配置。 VIS 将此视频中继配置应用于没有更具体的范围的视频中继配置的所有中继。 请注意，视频中继配置是适用于视频中继的设置集合。
...
每个部署都创建一个全局范围的Skype for Business Server计划。 VIS 将应用此拨号计划给没有更具体的拨号计划的所有中继。 
  
### <a name="configure-the-vis-using-windows-powershell"></a>使用配置策略配置 VIS Windows PowerShell
...
1. 使用以下 Windows PowerShell cmdlet 创建新的视频中继配置 (用于 VIS 和 Cisco 统一通信管理器 (CallManager 或 CUCM) 之间的中继的设置) 集合：
   ...
   ```
    ...
    若要删除特定视频中继配置，请使用以下 Windows PowerShell cmdlet (请注意，如果没有针对特定中继中继配置更具体的视频中继配置，将应用全局范围视频中继) ：
...
前面Windows PowerShell的视频中继配置的 cmdlet 是 2015 Skype for Business Server新增功能。 与视频中继配置关联的设置需要简要说明。
 ...
 **ForwardErrorCorrectionType** 此参数用于确定在视频互操作服务器和视频网关之间的 (是否对视频流应用FEC) 前向错误更正。 将 ForwardErrorCorrectionType 设置为"None"将关闭 VIS 和视频网关/VTC 之间的 FEC。 将 ForwardErrorCorrectionType 设置为"Cisco"可使 FEC 与 Cisco 的视频网关兼容，例如 Cisco 统一通信管理器 (CUCM) 。 默认值为 **None**。
