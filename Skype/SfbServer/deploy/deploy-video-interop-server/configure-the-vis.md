...
使用以下 Windows PowerShell 命令指定要与 VIS 与所有视频网关之间的拓扑文档中定义的新定义的中继 () 关联的视频中继配置和拨号计划。 所有这些设置都可以在全局、站点或服务或视频网关 (设置) 级别。 
  
每个部署将创建一个全局范围的Skype for Business Server计划。 VIS 将此拨号计划应用于没有更具体的拨号计划的所有中继。 
  
### <a name="configure-the-vis-using-windows-powershell"></a>使用"配置"Windows PowerShell
...
2015 Windows PowerShell中新增了用于视频中继配置Skype for Business Server cmdlet。 与视频中继配置关联的设置需要简要说明。
 ...
 **ForwardErrorCorrectionType** 此参数用于确定在视频互操作服务器 (视频网关之间的) 是否对视频流应用FEC 前向错误更正。 将 ForwardErrorCorrectionType 设置为"None"将关闭 VIS 和视频网关/VTC 之间的 FEC。 将 ForwardErrorCorrectionType 设置为"Cisco"可使 FEC 与 Cisco 的视频网关兼容，例如 Cisco 统一通信管理器 (CUCM) 。 默认值为 **None**。
