...
**摘要：** Configure the Video Interop Server (VIS) role in Skype for Business Server.
...
每个 Skype for Business Server 部署创建一个具有全局范围的拨号计划。 VIS 将应用此拨号计划给没有更具体的拨号计划的所有中继。 
  
### <a name="configure-the-vis-using-windows-powershell"></a>使用自定义配置 WINDOWS POWERSHELL

1. 使用以下 Windows PowerShell cmdlet 创建新的视频中继配置 (VIS 和 Cisco 统一通信管理器 (CallManager 或 CUCM) 之间的中继上使用的设置) 集合：
...
前面Windows PowerShell的视频中继配置的 cmdlet 是 Skype for Business Server 2015 的新增功能。 与视频中继配置关联的设置需要简要说明。
 ...
 **ForwardErrorCorrectionType** 此参数用于确定视频互操作服务器 (视频网关) 段之间是否对视频流应用前向错误更正或FEC 错误更正。 将 ForwardErrorCorrectionType 设置为"None"将关闭 VIS 和视频网关/VTC 之间的 FEC。 将 ForwardErrorCorrectionType 设置为"Cisco"可使 FEC 与 Cisco 的视频网关兼容，例如 Cisco 统一通信管理器 (CUCM) 。 默认值为 **None**。
