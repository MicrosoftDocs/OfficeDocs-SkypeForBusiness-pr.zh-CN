...
在一些 (和) 问题特别敏感的组织中，会使用虚拟桌面基础结构) VDI 基础结构环境。 他们的用户位于本地Windows计算机上，并且使用虚拟桌面上的客户端。 在Skype for Business这样的连接上使用 VDI 插件软件。
...
## <a name="sign-in-and-use-skype-for-business-on-the-virtual-desktop"></a>登录并使用虚拟Skype for Business登录
<a name="SfB_signin"> </a>

启用 Lync VDI 插件后，用户在登录虚拟桌面时将Skype for Business以下步骤。
...
3. Skype for Business开始与 Lync VDI 插件配对。 发生这种情况时，客户端会在状态栏中Skype for Business两个图标。 左下角的图标表示没有可用的音频设备，右下角的闪烁图标表示 VDI 配对正在进行：a. VDI 配对成功后，图标将更改为指示将用于呼叫的音频设备和 VDI 配对成功：b. 现在，用户可以在连接到本地Skype for Business的兼容设备上查看其状态，并像往常一样拨打和应答呼叫。
