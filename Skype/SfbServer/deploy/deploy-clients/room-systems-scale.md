...
## <a name="configure-system-center-configuration-manager-for-operating-system-deployment"></a><span data-ttu-id="d44fa-133">配置操作系统部署 System Center Configuration Manager</span><span class="sxs-lookup"><span data-stu-id="d44fa-133">Configure System Center Configuration Manager for operating system deployment</span></span>
...
2.  <span data-ttu-id="d44fa-144">选择分发点服务器，将提供 Skype 会议室系统 v2 部署中，然后选择**属性**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-144">Select the distribution point server that will serve the Skype Room Systems v2 deployment, and then select **Properties**.</span></span>
...
1.  <span data-ttu-id="d44fa-160">在 Configuration Manager 控制台中，转到**软件库** \> **操作系统** \> **启动映像**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-160">In the Configuration Manager console, go to **Software Library** \> **Operating System** \> **Boot Images**.</span></span>
...
## <a name="create-configuration-manager-packages"></a><span data-ttu-id="d44fa-176">创建配置管理器包</span><span class="sxs-lookup"><span data-stu-id="d44fa-176">Create Configuration Manager packages</span></span>
...
| <span data-ttu-id="d44fa-206">Windows 10 Enterprise</span><span class="sxs-lookup"><span data-stu-id="d44fa-206">Windows 10 Enterprise</span></span>                | <span data-ttu-id="d44fa-207">操作系统映像</span><span class="sxs-lookup"><span data-stu-id="d44fa-207">Operating system image</span></span> | <span data-ttu-id="d44fa-208">操作系统安装文件 (install.wim) 包</span><span class="sxs-lookup"><span data-stu-id="d44fa-208">Package for the operating system installation file (install.wim)</span></span>                          |
...
### <a name="create-the-microsoft-operations-management-suite-agent-package"></a><span data-ttu-id="d44fa-232">创建 Microsoft 操作管理套件代理包</span><span class="sxs-lookup"><span data-stu-id="d44fa-232">Create the Microsoft Operations Management Suite agent package</span></span>
...
7.  <span data-ttu-id="d44fa-258">查看**确认的设置**页，然后选择**下一步**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-258">Review the **Confirm the settings** page, and then select **Next**.</span></span>
...
### <a name="create-the-skype-room-systems-v2-deployment-kit-package"></a><span data-ttu-id="d44fa-275">创建 Skype 会议室系统 v2 部署套件包</span><span class="sxs-lookup"><span data-stu-id="d44fa-275">Create the Skype Room Systems v2 deployment kit package</span></span>
...
4.  <span data-ttu-id="d44fa-292">输入以下信息以创建包：</span><span class="sxs-lookup"><span data-stu-id="d44fa-292">Enter the following information to create the package:</span></span>
    ...
    -   <span data-ttu-id="d44fa-294">制造商： **Microsoft Corporation**</span><span class="sxs-lookup"><span data-stu-id="d44fa-294">Manufacturer: **Microsoft Corporation**</span></span>
...
7.  <span data-ttu-id="d44fa-312">选择**关闭**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-312">Select **Close**.</span></span>
...
1.  <span data-ttu-id="d44fa-326">下载最新驱动程序和固件。</span><span class="sxs-lookup"><span data-stu-id="d44fa-326">Download the latest drivers and firmware.</span></span>
...
12. <span data-ttu-id="d44fa-340">将所有已导入的驱动程序移动到新创建的文件夹，以便于导航和操作。</span><span class="sxs-lookup"><span data-stu-id="d44fa-340">Move all the imported drivers to the newly created folder for easier navigation and operation.</span></span>
...
2.  <span data-ttu-id="d44fa-352">分发驱动程序包。</span><span class="sxs-lookup"><span data-stu-id="d44fa-352">Distribute driver packages.</span></span>
    ...
    3.  <span data-ttu-id="d44fa-356">向列表中，添加所有分发点服务器 （或分发点组，具体取决于您的配置管理器层次结构），然后选择**下一步**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-356">Add all the distribution point servers (or distribution point groups, depending on your Configuration Manager hierarchy) to the list, and then select **Next**.</span></span>
...
### <a name="import-the-sample-task-sequence"></a><span data-ttu-id="d44fa-370">导入示例任务序列</span><span class="sxs-lookup"><span data-stu-id="d44fa-370">Import the sample task sequence</span></span>
...
1.  <span data-ttu-id="d44fa-381">演练每个步骤并完成建议的更新：</span><span class="sxs-lookup"><span data-stu-id="d44fa-381">Walk through each step and complete the recommended updates:</span></span>
    ...
    8.  <span data-ttu-id="d44fa-402">**设置 Windows 和配置管理器**： 此步骤部署和配置 Configuration Manager 客户端。</span><span class="sxs-lookup"><span data-stu-id="d44fa-402">**Set up Windows and Configuration Manager**: This step deploys and configures the Configuration Manager client.</span></span> <span data-ttu-id="d44fa-403">更新指定的内置的配置管理器客户端包此步骤。</span><span class="sxs-lookup"><span data-stu-id="d44fa-403">Update this step to specify the built-in Configuration Manager Client Package.</span></span>

    9.  <span data-ttu-id="d44fa-404">**安装根证书**： 此步骤分发非加入域的设备的根证书，因此是可选的默认为禁用。</span><span class="sxs-lookup"><span data-stu-id="d44fa-404">**Install Root Certificate**: This step distributes the root certificate for non–domain-joined devices, and therefore is optional and disabled by default.</span></span>
        -   <span data-ttu-id="d44fa-405">如果您需要部署到的 Skype 会议室系统 v2 单位的根证书，则启用此步骤。</span><span class="sxs-lookup"><span data-stu-id="d44fa-405">Enable this step if you need to deploy a root certificate to the Skype Room Systems v2 units.</span></span>
        -   <span data-ttu-id="d44fa-406">如果需要执行此步骤，请验证选中的**SR v2 – 根证书包**和**禁用 64-bit 文件系统重定向**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-406">If you do need to perform this step, verify that the **SRS v2 – Root Certificate Package** and **Disable 64-bit file system redirection** are selected.</span></span>

    10. <span data-ttu-id="d44fa-407">**安装和配置 OMS 代理**： 此步骤安装 Microsoft 操作管理套件代理的 64 位版本和配置要连接到您的日志分析工作区的代理。</span><span class="sxs-lookup"><span data-stu-id="d44fa-407">**Install and Configure OMS Agent**: This step installs the 64-bit version of the Microsoft Operations Management Suite agent and configures the agent to connect to your Log Analytics workspace.</span></span>
        -   <span data-ttu-id="d44fa-408">默认情况下禁用此步骤。</span><span class="sxs-lookup"><span data-stu-id="d44fa-408">This step is disabled by default.</span></span> <span data-ttu-id="d44fa-409">仅当您要使用 OMS to monitor the health 的您 Skype 会议室系统 v2 单位，则启用此步骤。</span><span class="sxs-lookup"><span data-stu-id="d44fa-409">Enable this step only if you’re going to use OMS to monitor the health of your Skype Room Systems v2 units.</span></span>
        -   <span data-ttu-id="d44fa-410">编辑此步骤和更新的命令行参数指定您的**工作区 ID**和**工作区键**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-410">Edit this step and update the command-line parameters to specify your **Workspace ID** and **Workspace Key**.</span></span>
        -   <span data-ttu-id="d44fa-411">有关获取操作管理套件工作区 ID 和主关键字的详细信息，请参阅[到 Azure 中的日志分析服务的连接的 Windows 计算机](with-oms.md#configure-test-devices-for-operations-management-suite-setup)。</span><span class="sxs-lookup"><span data-stu-id="d44fa-411">See [Connect Windows computers to the Log Analytics service in Azure](with-oms.md#configure-test-devices-for-operations-management-suite-setup) for more information about obtaining the Operations Management Suite Workspace ID and the primary key.</span></span>
        -   <span data-ttu-id="d44fa-412">验证选中的**SR v2 – Microsoft OMS 代理包**和**禁用 64-bit 文件系统重定向**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-412">Verify that the **SRS v2 – Microsoft OMS Agent Package** and **Disable 64-bit file system redirection** are selected.</span></span>
        -   <span data-ttu-id="d44fa-413">有关监控的 Skype 会议室系统 v2 部署运行状况的详细信息，请参阅[使用 OMS 的规划 Skype 会议室系统 v2 管理](../../plan-your-deployment/clients-and-devices/oms-management.md)和[使用 OMS 的部署 Skype 会议室系统 v2 管理](with-oms.md#configure-test-devices-for-operations-management-suite-setup)。</span><span class="sxs-lookup"><span data-stu-id="d44fa-413">For more information about monitoring the health of your Skype Room Systems v2 deployment, see [Plan Skype Room Systems v2 management with OMS](../../plan-your-deployment/clients-and-devices/oms-management.md) and [Deploy Skype Room Systems v2 management with OMS](with-oms.md#configure-test-devices-for-operations-management-suite-setup).</span></span>

    11. <span data-ttu-id="d44fa-414">**复制 SR v2 配置文件**： 此步骤将从 Skype 会议室系统 v2 部署工具包所需的安装和配置文件复制到本地硬盘。</span><span class="sxs-lookup"><span data-stu-id="d44fa-414">**Copy SRS v2 Configuration Files**: This step copies the required setup and configuration files from the Skype Room Systems v2 deployment kit to the local hard drive.</span></span> <span data-ttu-id="d44fa-415">无自定义，则需要此步骤。</span><span class="sxs-lookup"><span data-stu-id="d44fa-415">No customization is required for this step.</span></span>
        -   <span data-ttu-id="d44fa-416">验证选中的**SR v2 – SR 应用程序包**和**禁用 64-bit 文件系统重定向**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-416">Verify that the **SRS v2 – SRS Application Package** and **Disable 64-bit file system redirection** are selected.</span></span>

    12. <span data-ttu-id="d44fa-417">**安装 SRSv2 操作系统更新**： 此步骤将部署与 Skype 会议室系统 v2 部署所需的任何必需的操作系统更新。</span><span class="sxs-lookup"><span data-stu-id="d44fa-417">**Install-SRSv2-OS-Updates**: This step deploys any mandatory operating system updates required with the Skype Room Systems v2 deployment.</span></span> <span data-ttu-id="d44fa-418">请执行下列操作：</span><span class="sxs-lookup"><span data-stu-id="d44fa-418">Do the following:</span></span>
        -   <span data-ttu-id="d44fa-419">检查以查看哪些更新所需的[配置 Skype 会议室系统 v2 控制台](console.md)。</span><span class="sxs-lookup"><span data-stu-id="d44fa-419">Check [Configure a Skype Room Systems v2 console](console.md) to see which updates are required.</span></span>
        -   <span data-ttu-id="d44fa-420">确认您**SR v2 – 操作系统更新程序包**包含所有必需的更新。</span><span class="sxs-lookup"><span data-stu-id="d44fa-420">Verify that your **SRS v2 – OS Updates Package** includes all the required updates.</span></span>
        -   <span data-ttu-id="d44fa-421">确认已选中**SR v2 – 操作系统更新包**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-421">Verify that the **SRS v2 – OS Updates Package** is selected.</span></span>
        -   <span data-ttu-id="d44fa-422">验证 PowerShell 执行策略设置为**绕过**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-422">Verify that the PowerShell execution policy is set to **Bypass**.</span></span>

    13. <span data-ttu-id="d44fa-423">**重新启动计算机**： 在安装必需的操作系统更新后，此步骤重新启动计算机。</span><span class="sxs-lookup"><span data-stu-id="d44fa-423">**Restart Computer**: This step reboots the computer after the mandatory operating system updates are installed.</span></span> <span data-ttu-id="d44fa-424">无自定义，则需要此步骤。</span><span class="sxs-lookup"><span data-stu-id="d44fa-424">No customization is required for this step.</span></span>

    14. <span data-ttu-id="d44fa-425">**配置 Windows 组件**： 此步骤配置所需的 Windows 功能。</span><span class="sxs-lookup"><span data-stu-id="d44fa-425">**Configure Windows Components**: This step configures the required Windows features.</span></span> <span data-ttu-id="d44fa-426">无自定义，则需要此步骤。</span><span class="sxs-lookup"><span data-stu-id="d44fa-426">No customization is required for this step.</span></span>

    15. <span data-ttu-id="d44fa-427">**重新启动计算机**： 配置 Windows 功能后，此步骤重新启动计算机。</span><span class="sxs-lookup"><span data-stu-id="d44fa-427">**Restart Computer**: This step reboots the computer after the Windows features are configured.</span></span> <span data-ttu-id="d44fa-428">无自定义，则需要此步骤。</span><span class="sxs-lookup"><span data-stu-id="d44fa-428">No customization is required for this step.</span></span>

    16. <span data-ttu-id="d44fa-429">**添加本地 Skype 用户**： 此步骤创建用于自动登录到 Windows 和启动 Skype 会议室系统 v2 应用程序的本地 Skype 帐户。</span><span class="sxs-lookup"><span data-stu-id="d44fa-429">**Add Local Skype User**: This step creates the local Skype account used to automatically sign in to Windows and start the Skype Room Systems v2 application.</span></span> <span data-ttu-id="d44fa-430">此步骤没有与其关联的任何软件程序包，它需要无自定义。</span><span class="sxs-lookup"><span data-stu-id="d44fa-430">This step doesn’t have any software package associated with it, and no customization is required for it.</span></span>

    17. <span data-ttu-id="d44fa-431">**设置并配置 SR 应用程序**： 此步骤配置下次启动操作系统的 Skype 会议室系统 v2 应用程序安装。</span><span class="sxs-lookup"><span data-stu-id="d44fa-431">**Set up and configure SRS application**: This step configures the Skype Room Systems v2 application installation for the next boot of the operating system.</span></span>
        -   <span data-ttu-id="d44fa-432">验证选中**SR v2 – 配置 SR 安装程序包**并**禁用 64-bit 文件系统重定向**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-432">Verify that the **SRS v2 – Configure SRS Setup Package** and **Disable 64-bit file system redirection** are selected.</span></span>       

> [!IMPORTANT]
> <span data-ttu-id="d44fa-433">它是非常重要的任务序列步骤必须在提供的顺序。</span><span class="sxs-lookup"><span data-stu-id="d44fa-433">It is very important that the task sequence steps must be in the provided order.</span></span> <span data-ttu-id="d44fa-434">修改顺序的步骤，或配置其他步骤可能会中断部署。</span><span class="sxs-lookup"><span data-stu-id="d44fa-434">Modifying the order of steps, or configuring additional steps might break the deployment.</span></span>
> 
> <span data-ttu-id="d44fa-435">**设置并配置 SR 应用程序**步骤必须在任务序列中的最后一步，否则部署可能会失败。</span><span class="sxs-lookup"><span data-stu-id="d44fa-435">**Set up and configure SRS application** step must be the last step in the task sequence, otherwise the deployment might fail.</span></span>

### <a name="create-deployment-for-the-task-sequence"></a><span data-ttu-id="d44fa-436">创建部署任务序列</span><span class="sxs-lookup"><span data-stu-id="d44fa-436">Create deployment for the task sequence</span></span>

1.  <span data-ttu-id="d44fa-437">选择任务序列，，，然后选择**部署**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-437">Select the task sequence, and then select **Deploy**.</span></span>

2.  <span data-ttu-id="d44fa-438">选择**浏览**选择部署目标集合。</span><span class="sxs-lookup"><span data-stu-id="d44fa-438">Select **Browse** to select target collection for deployment.</span></span>

3.  <span data-ttu-id="d44fa-439">选择**所有未知的计算机**，然后选择**确定**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-439">Select **All Unknown Computers** and then select **OK**.</span></span>

4.  <span data-ttu-id="d44fa-440">选择**下一步**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-440">Select **Next**.</span></span>

5.  <span data-ttu-id="d44fa-441">选择**用途**下拉列表上的**有空**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-441">Select **Available** on the **Purpose** drop down list.</span></span>

6.  <span data-ttu-id="d44fa-442">在**使可用于以下**列表中，选择**仅媒体和 PXE** ，然后选择**下一步**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-442">Select **Only Media and PXE** in the **Make available to the following** list, and then select **Next**.</span></span>
> [!WARNING]
> <span data-ttu-id="d44fa-443">它是非常重要的**目的**设置为**可用**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-443">It is very important that **Purpose** is set to **Available**.</span></span> <span data-ttu-id="d44fa-444">确保**用途**是**** 设置为**所需**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-444">Make sure that the **Purpose** is **NOT** set to **Required**.</span></span> <span data-ttu-id="d44fa-445">另外，请确保您选择中**对以下公开****仅媒体和 PXE** 。</span><span class="sxs-lookup"><span data-stu-id="d44fa-445">Also make sure that you select **Only Media and PXE** in the **Make available to the following**.</span></span>
>
> <span data-ttu-id="d44fa-446">为其他设置这些值可能会导致获取 Skype 会议室系统部署映像时启动的所有计算机。</span><span class="sxs-lookup"><span data-stu-id="d44fa-446">Setting these values to something else might cause all computers to get the Skype Room Systems deployment image when booted.</span></span>
7.  <span data-ttu-id="d44fa-447">不指定任何计划，并选择**下一步**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-447">Do not specify any schedule and select **Next**.</span></span>

8.  <span data-ttu-id="d44fa-448">不更改**用户体验**部分中的任何内容，并选择**下一步**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-448">Do not change anything within the **User Experience** section and select **Next**.</span></span>

9.  <span data-ttu-id="d44fa-449">不更改**通知**部分中的任何内容，并选择**下一步**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-449">Do not change anything within the **Alerts** section and select **Next**.</span></span>

10.  <span data-ttu-id="d44fa-450">不更改该**分发点**内容中的任何内容，并选择**下一步**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-450">Do not change anything within the **Distribution Points** section and select **Next**.</span></span>

11.  <span data-ttu-id="d44fa-451">确认设置，然后选择**下一步**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-451">Confirm the settings and then select **Next**.</span></span>

12.  <span data-ttu-id="d44fa-452">选择**关闭**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-452">Select **Close**.</span></span>

<a name="validate-and-troubleshoot-the-solution"></a><span data-ttu-id="d44fa-453">验证并排查解决方案</span><span class="sxs-lookup"><span data-stu-id="d44fa-453">Validate and troubleshoot the solution</span></span>
--------------------------------------

<span data-ttu-id="d44fa-454">在您完成 System Center Configuration Manager 任务序列后，您需要执行测试运行以验证任务序列可以部署和配置 Skype 会议室系统 v2 单位。</span><span class="sxs-lookup"><span data-stu-id="d44fa-454">After you’ve completed the System Center Configuration Manager task sequences, you’ll need to perform a test run to validate that the task sequence can deploy and configure Skype Room Systems v2 units.</span></span>

1.  <span data-ttu-id="d44fa-455">通过使用受支持的以太网适配器之一或使用曲面停靠到有线网络连接的测试设备。</span><span class="sxs-lookup"><span data-stu-id="d44fa-455">Connect the test device to the wired network by using one of the supported Ethernet adapters or using the Surface dock.</span></span> <span data-ttu-id="d44fa-456">如果尚未为您的环境配置 PXE 启动功能，您可用于启动映像上 USB 闪存驱动器[您先前创建](https://docs.microsoft.com/sccm/osd/deploy-use/create-bootable-media)从 USB 启动并连接到配置管理器。</span><span class="sxs-lookup"><span data-stu-id="d44fa-456">If PXE boot functionality hasn’t been configured for your environment, you can use the boot image on the USB flash drive [that you created earlier](https://docs.microsoft.com/sccm/osd/deploy-use/create-bootable-media) to boot from USB and connect to Configuration Manager.</span></span>

2.  <span data-ttu-id="d44fa-457">访问固件并启动 PXE 启动：</span><span class="sxs-lookup"><span data-stu-id="d44fa-457">Access the firmware and initiate PXE boot:</span></span>

    1.  <span data-ttu-id="d44fa-458">确保曲面设备处于关闭状态。</span><span class="sxs-lookup"><span data-stu-id="d44fa-458">Ensure the Surface device is powered off.</span></span>

    2.  <span data-ttu-id="d44fa-459">按住**批量向上**按钮。</span><span class="sxs-lookup"><span data-stu-id="d44fa-459">Press and hold the **Volume Up** button.</span></span>

    3.  <span data-ttu-id="d44fa-460">按下并释放**高级**按钮。</span><span class="sxs-lookup"><span data-stu-id="d44fa-460">Press and release the **Power** button.</span></span>

    4.  <span data-ttu-id="d44fa-461">设备后开始启动，释放**卷 Up**按钮。</span><span class="sxs-lookup"><span data-stu-id="d44fa-461">After the device begins to boot, release the **Volume Up** button.</span></span>

    5.  <span data-ttu-id="d44fa-462">选择**启动配置**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-462">Select **Boot configuration**.</span></span>

    6.  <span data-ttu-id="d44fa-463">执行下列操作之一：</span><span class="sxs-lookup"><span data-stu-id="d44fa-463">Do one of the following:</span></span>

        -   <span data-ttu-id="d44fa-464">选择**PXE 启动**，并将其拖动至列表顶部。</span><span class="sxs-lookup"><span data-stu-id="d44fa-464">Select **PXE boot**, and drag it to the top of the list.</span></span> <span data-ttu-id="d44fa-465">此外，您可以扫过网络适配器立即启动到设备上的左侧。</span><span class="sxs-lookup"><span data-stu-id="d44fa-465">Alternatively, you can swipe left on the network adapter to boot to the device immediately.</span></span> <span data-ttu-id="d44fa-466">这不会影响启动顺序。</span><span class="sxs-lookup"><span data-stu-id="d44fa-466">This won’t affect the boot order.</span></span>
        -   <span data-ttu-id="d44fa-467">选择包含启动媒体的 USB 闪存驱动器。</span><span class="sxs-lookup"><span data-stu-id="d44fa-467">Select the USB flash drive that holds the boot media.</span></span>

3.  <span data-ttu-id="d44fa-468">选择**退出**，，，然后选择**立即重新启动**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-468">Select **Exit**, and then select **Restart Now**.</span></span>

4.  <span data-ttu-id="d44fa-469">出现提示时，选择网络启动服务**Enter** 。</span><span class="sxs-lookup"><span data-stu-id="d44fa-469">When prompted, select **Enter** for network boot service.</span></span>

5.  <span data-ttu-id="d44fa-470">Windows PE 将加载到内存中，并将启动任务序列向导。</span><span class="sxs-lookup"><span data-stu-id="d44fa-470">Windows PE will load into memory, and the Task Sequence Wizard will start.</span></span> <span data-ttu-id="d44fa-471">选择**下一步**以继续。</span><span class="sxs-lookup"><span data-stu-id="d44fa-471">Select **Next** to continue.</span></span>

6.  <span data-ttu-id="d44fa-472">选择之前，导入任务序列，然后选择**下一步**。</span><span class="sxs-lookup"><span data-stu-id="d44fa-472">Select the task sequence that you imported earlier, and then select **Next**.</span></span>

7.  <span data-ttu-id="d44fa-473">应用磁盘配置后，将提示您指定设备的计算机名称。</span><span class="sxs-lookup"><span data-stu-id="d44fa-473">After the disk configuration is applied, you’ll be prompted to specify a computer name for the device.</span></span> <span data-ttu-id="d44fa-474">用户界面会显示基于 Surface Pro 设备序列号的推荐的计算机名称。</span><span class="sxs-lookup"><span data-stu-id="d44fa-474">The user interface will display a recommended computer name based on the serial number of the Surface Pro device.</span></span> <span data-ttu-id="d44fa-475">您可以接受建议的名称，或指定一个新。</span><span class="sxs-lookup"><span data-stu-id="d44fa-475">You can either accept the proposed name or specify a new one.</span></span> <span data-ttu-id="d44fa-476">按照计算机名称分配屏幕上的说明。</span><span class="sxs-lookup"><span data-stu-id="d44fa-476">Follow the instructions on the computer name assignment screen.</span></span> <span data-ttu-id="d44fa-477">当选择**接受**时，在开始部署。</span><span class="sxs-lookup"><span data-stu-id="d44fa-477">When you select **Accept**, the deployment begins.</span></span>

8.  <span data-ttu-id="d44fa-478">部署过程的其余部分是自动并不要求输入任何多个用户。</span><span class="sxs-lookup"><span data-stu-id="d44fa-478">The rest of the deployment process is automatic and doesn’t ask for any more user input.</span></span>

9.  <span data-ttu-id="d44fa-479">部署任务序列完成配置设备后，您将看到要求您配置 Skype 会议室系统 v2 应用程序设置下配置屏幕。</span><span class="sxs-lookup"><span data-stu-id="d44fa-479">After the deployment task sequence finishes configuring the device, you’ll see the following configuration screen that asks you to configure the Skype Room Systems v2 application settings.</span></span>

    ![Skype 会议室系统 v2 应用程序的初始安装屏幕](../../media/room-systems-scale-image2.png)

10.  <span data-ttu-id="d44fa-481">Surface Pro 插入 Skype 会议室系统 v2 控制台中，并配置的应用程序设置。</span><span class="sxs-lookup"><span data-stu-id="d44fa-481">Plug the Surface Pro into the Skype Room Systems v2 console, and configure the application settings.</span></span>

11.  <span data-ttu-id="d44fa-482">验证已部署的设备上的[Skype 会议室系统 v2 帮助](https://support.office.com/article/Skype-Room-Systems-version-2-help-e667f40e-5aab-40c1-bd68-611fe0002ba2)中列出的功能都正常工作。</span><span class="sxs-lookup"><span data-stu-id="d44fa-482">Validate that the capabilities listed in [Skype Room Systems v2 help](https://support.office.com/article/Skype-Room-Systems-version-2-help-e667f40e-5aab-40c1-bd68-611fe0002ba2) are working on the deployed device.</span></span>


<span data-ttu-id="d44fa-483">要解决安装失败，请检查日志在 Configuration Manager 任务序列中执行的所有步骤的**SMSTS.log**文件。</span><span class="sxs-lookup"><span data-stu-id="d44fa-483">To troubleshoot a failed installation, check the **SMSTS.log** file, which logs all the steps executed in a Configuration Manager task sequence.</span></span>

<span data-ttu-id="d44fa-484">SMSTS.log 文件存储在多个路径，具体取决于生成过程的阶段之一。</span><span class="sxs-lookup"><span data-stu-id="d44fa-484">The SMSTS.log file is stored on one of a number of paths, depending on the stage of the build process.</span></span> <span data-ttu-id="d44fa-485">检查下表标识 SMSTS.log 文件的路径。</span><span class="sxs-lookup"><span data-stu-id="d44fa-485">Check the following table to identify the path to the SMSTS.log file.</span></span>


| <span data-ttu-id="d44fa-486">**部署阶段**</span><span class="sxs-lookup"><span data-stu-id="d44fa-486">**Deployment phase**</span></span>                                                            | <span data-ttu-id="d44fa-487">**任务序列日志路径**</span><span class="sxs-lookup"><span data-stu-id="d44fa-487">**Task sequence log path**</span></span>                         |
|---------------------------------------------------------------------------------|----------------------------------------------------|
| <span data-ttu-id="d44fa-488">WinPE 之前 HDD 格式</span><span class="sxs-lookup"><span data-stu-id="d44fa-488">WinPE, before HDD format</span></span>                                                        | <span data-ttu-id="d44fa-489">X:\\Windows\\Temp\\smstslog\\smsts.log</span><span class="sxs-lookup"><span data-stu-id="d44fa-489">X:\\Windows\\Temp\\smstslog\\smsts.log</span></span>             |
| <span data-ttu-id="d44fa-490">WinPE 之后 HDD 格式</span><span class="sxs-lookup"><span data-stu-id="d44fa-490">WinPE, after HDD format</span></span>                                                         | <span data-ttu-id="d44fa-491">C:\\_SMSTaskSequence\\日志\\Smstslog\\smsts.log</span><span class="sxs-lookup"><span data-stu-id="d44fa-491">C:\\_SMSTaskSequence\\Logs\\Smstslog\\smsts.log</span></span>    |
| <span data-ttu-id="d44fa-492">部署之前的 Configuration Manager 代理已安装, 的操作系统</span><span class="sxs-lookup"><span data-stu-id="d44fa-492">Operating system deployed, before the Configuration Manager agent was installed</span></span> | <span data-ttu-id="d44fa-493">c:\\_SMSTaskSequence\\日志\\Smstslog\\smsts.log</span><span class="sxs-lookup"><span data-stu-id="d44fa-493">c:\\_SMSTaskSequence\\Logs\\Smstslog\\smsts.log</span></span>    |
| <span data-ttu-id="d44fa-494">操作系统和部署 Configuration Manager 代理</span><span class="sxs-lookup"><span data-stu-id="d44fa-494">Operating system and the Configuration Manager agent deployed</span></span>                   | <span data-ttu-id="d44fa-495">%windir%\\System32\\ccm\\日志\\Smstslog\\smsts.log</span><span class="sxs-lookup"><span data-stu-id="d44fa-495">%windir%\\System32\\ccm\\logs\\Smstslog\\smsts.log</span></span> |
| <span data-ttu-id="d44fa-496">完成执行任务序列</span><span class="sxs-lookup"><span data-stu-id="d44fa-496">Task sequence execution complete</span></span>                                                | <span data-ttu-id="d44fa-497">%windir%\\System32\\ccm\\日志\\smsts.log</span><span class="sxs-lookup"><span data-stu-id="d44fa-497">%windir%\\System32\\ccm\\logs\\smsts.log</span></span>           |

> [!TIP]
> <span data-ttu-id="d44fa-498">可以随时期间任务序列以打开命令控制台中，选择**F8** ，然后获取对 SMSTS.log 文件的访问。</span><span class="sxs-lookup"><span data-stu-id="d44fa-498">You can select **F8** at any time during the task sequence to open a command console, and then get access to the SMSTS.log file.</span></span>

<span data-ttu-id="d44fa-499">要解决 PXE 启动问题，请检查两个日志文件的配置管理器服务器上的特定于 PXE 操作：</span><span class="sxs-lookup"><span data-stu-id="d44fa-499">To troubleshoot PXE boot issues, check the two log files on the Configuration Manager server that are specific to PXE actions:</span></span>

-   <span data-ttu-id="d44fa-500">**Pxecontrol.log**，位于配置管理器安装日志目录</span><span class="sxs-lookup"><span data-stu-id="d44fa-500">**Pxecontrol.log**, located in the Configuration Manager installation logs directory</span></span>

-   <span data-ttu-id="d44fa-501">**Smspxe.log**，位于配置管理器管理点 (MP) 日志目录中</span><span class="sxs-lookup"><span data-stu-id="d44fa-501">**Smspxe.log**, located in Configuration Manager Management Point (MP) logs directory</span></span>

<span data-ttu-id="d44fa-502">您可以使用进一步解决配置管理器安装的日志文件的完整列表，请参阅[System Center Configuration Manager 中的日志文件](https://docs.microsoft.com/sccm/core/plan-design/hierarchy/log-files)。</span><span class="sxs-lookup"><span data-stu-id="d44fa-502">For a complete list of the log files that you can use to further troubleshoot your Configuration Manager installation, see [Log files in System Center Configuration Manager](https://docs.microsoft.com/sccm/core/plan-design/hierarchy/log-files).</span></span>
