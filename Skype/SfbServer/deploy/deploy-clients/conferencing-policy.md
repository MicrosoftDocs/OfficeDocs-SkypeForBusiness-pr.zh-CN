...
分配给 Skype 房间系统帐户的会议策略必须具有特定的特征。 大多数情况下, Skype 会议室系统客户加入一个计划会议, 因此会议组织者的会议策略将影响会议。 但是, 在 Skype for Business 服务器中, 某些功能取决于参与者的配置。 例如, 如果参与者的策略允许最高视频分辨率为 1080p, 参与者将在会议中体验此更高分辨率视频功能, 即使组织者的策略不允许。 下表介绍了在为组织中的 Skype 会议室系统帐户设置会议策略时应注意的几个设置。 
