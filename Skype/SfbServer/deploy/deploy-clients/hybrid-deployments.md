...
如果您的拓扑具有 Skype for Business Server 和 Exchange Online，并且您希望在 Skype 上托管会议室系统资源邮箱，请按照Exchange Online。 本节还介绍了混合方案，其中你已部署Exchange Online和Exchange Server部署。
  
为便于说明，我们将 LyncSample.com 用于本地域，LyncSample.ccstp.net 用于联机域。
  
1. 通过连接到 Exchange 命令行管理 (LyncSample.ccsctp.net) ，在管理中心 Exchange Online 创建资源邮箱，如 Exchange Online Provisioning 中所述。
    
   ```powershell
   New-Mailbox -room -name "LRS Test 5" -RoomMailboxPassword (ConvertTo-SecureString <password> -AsPlainText -Force) -EnableRoomMailboxAccount $true 
   ```

    您可以验证 OWA 连接是否 lrstest5@LyncSample.ccsctp.net 登录。
...
5. 验证 OWA 连接是否 lrstest5@LyncSample.com。  (之前，您验证了使用联机域的 OWA ) 
...
> [!NOTE]
> 如果已Skype for Business Online 而不是 Skype for Business Server，则设置 Exchange 资源邮箱后，设置 Skype for Business 帐户，如 Skype for Business Online Provisioning 中所述。 
