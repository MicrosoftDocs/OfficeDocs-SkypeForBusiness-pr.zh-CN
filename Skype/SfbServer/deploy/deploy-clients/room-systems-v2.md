...
- 为业务和 Exchange 帐户创建 Skype 并将其分配给 （请参阅[配置帐户的 Microsoft 团队会议室](room-systems-v2-configure-accounts.md)） 的控制台设备
...
- （可选）设置您的系统 （请参阅[部署 Microsoft 团队聊天室管理使用 Azure 监视器](azure-monitor.md)） Microsoft 操作管理套件
...
-   [设置电源和本地帐户设置](../../manage/skype-room-systems-v2/room-systems-v2-operations.md#configuring-group-policy-for-microsoft-teams-rooms)。
-   启用 Windows Update。
...
每个 Microsoft 团队聊天室设备需要必须启用业务和 Exchange 这两个 Skype 的专用和唯一资源帐户。 此帐户必须具有会议室邮箱位于 Exchange，并且将启用为会议室中 Skype 业务部署。 在 Exchange 一侧，以便设备可以自动接受传入会议请求，则必须配置日历处理。 有关创建这些帐户的详细信息，请参阅[配置帐户的 Microsoft 团队会议室](room-systems-v2-configure-accounts.md)。 
...
每个 Microsoft 团队聊天室后系统已从物理上隔离部署和支持的外围设备连接，您需要配置要分配的 Microsoft 团队会议室资源帐户和密码之前创建的 Microsoft 团队聊天室应用程序以使 Microsoft 团队聊天室系统的业务和 Exchange 登录到 Skype。 它的键来利用 Skype 业务认证 USB 音频和视频的外围设备链接文档中的其他位置。 不执行此操作会导致不可预测的行为。 

您可以手动配置每个聊天室 Microsoft 团队系统。 或者，您可以使用集中存储 – Microsoft 团队聊天室管理应用程序设置并利用启动 GPO 脚本重新应用您希望，每次 Microsoft 团队聊天室系统启动配置的 XML 配置文件。 

有关如何使用 XML 配置文件的详细信息，请参阅[管理 Microsoft 团队聊天室控制台设置远程使用 XML 配置文件](../../manage/skype-room-systems-v2/xml-config-file.md)。 

您可以使用[远程 PowerShell](../../manage/skype-room-systems-v2/room-systems-v2-operations.md#remote-management-using-powershell)提取报告需求的 Microsoft 团队聊天室配置。 
