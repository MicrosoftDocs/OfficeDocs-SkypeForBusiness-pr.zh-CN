...
description: 摘要：了解如何将Skype for Business Server与Skype连接。 也称为连接Skype连接。
ms.openlocfilehash: 467d87968e7e5b0357e8d23b578a511adb281e27
ms.sourcegitcommit: 15e90083c47eb5bcb03ca80c2e83feffe67646f2
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 08/30/2021
ms.locfileid: "58729881"
---
...
- **通过电子邮件进行搜索，例如"johndoe@outlook.com"-** 在大多数情况下，这应该返回一个结果;与指定电子邮件完全匹配的电子邮件。 但是，如果同一电子邮件与多个帐户关联，可能会返回多个结果。
...
|:-----|:-----|:-----|:-----|
|Skype for Business ServerEdge  <br/> |支持  <br/> |不支持  <br/> |Skype for Business Server和边缘是进行Skype目录搜索的先决条件  <br/> |
|Skype for Business Server并行部署的边缘 + Lync Server 2013 边缘  <br/> |支持  <br/> |不支持  <br/> |Skype目录搜索流量流经Skype for Business Server边缘服务器。 联盟流量通过管理员配置的边缘。 例如，管理员可以选择继续通过 Lync Server 2013 边缘服务器发送联盟通信，这些边缘服务器不支持Skype搜索。  <br/> |
...
> [!NOTE]
> 如果客户在本地部署中有多个站点，并且他们仅部署了一个 Skype for Business Server 边缘服务器/池，则来自所有网站的搜索流量将经过单个可用的边缘服务器。 管理员需要确保所有站点中的池都可以访问边缘服务器/池中Skype for Business Server部署的内容。 
  
> [!NOTE]
> Skype如果请求速率超过 15 个请求/秒，Microsoft 365或 Office 365 客户限制来自任何本地或 Office 365 的搜索请求。 
  
> [!NOTE]
> 对于大型企业本地客户，需要将域添加到具有 Skype 搜索服务的允许列表中，以允许更高的请求速率。
  
> [!NOTE]
> Skype for Business Server队列中的待处理请求过多，则限制传入请求。 
...
> [!NOTE]
> 如果Skype for Business Server使用 PUBLIC Instant Messaging Connectivity (PIC) 与 Windows Messenger 连接，则您的部署已配置为 Skype 连接。 您可能要考虑的唯一更改是，将现有 Messenger PIC 条目重命名为Skype。 
...
**为什么要关闭设置网站？**
2006 年 (公共 IM) PIC (pic.lync.com) 设置机制不再可用，将在 2019 年 8 月 15 日关闭。 相反，公共 IM 联盟将假定 Skype for Business Online 使用的相同联盟模型（称为"合作伙伴发现"，其中本地部署可以公开发现其联盟 DNS SRV 记录 (s) ）。

**此更改是否意味着公用 IM 联盟被弃用？**
否。 公共 IM 联盟将继续受支持很多年，可能Skype for Business本地产品生命周期结束。
...
现在重点介绍配置 Skype for Business Server 连接所需的环境Skype任务。 在此部分中，我们假定管理员已Skype for Business Server和配置外部访问（也称为边缘服务器）。 
...
需要联盟，Skype才能与Skype for Business用户进行通信。 公共即时消息连接 (PIC) 是一类联盟，必须配置为允许 Skype for Business 用户与 Skype 通信。 联盟和 PIC 使用"控制面板"Skype for Business Server配置。
...
需要联盟，Skype才能与Skype for Business用户进行通信。 公共即时消息连接 (PIC) 是一类联盟，必须配置为允许 Skype for Business Server 用户与 Skype 通信。 联盟和 PIC 使用"Skype for Business Server"的边缘配置对话框进行配置，如图所示。
...
通过使用Skype for Business Server控制面板，管理员必须配置一个或多个外部用户访问策略，以控制 Skype用户是否可以与内部用户Skype for Business Server协作。
...
对于新安装，Skype使用"Skype控制面板"启用 Skype for Business Server 公共提供商来配置连接，如图所示。
  
![SIP 联盟提供程序。](../media/8fc7b566-72b5-4c43-961c-9249fdf7e575.png)
  
> [!NOTE]
> 若要在Skype时配置 Skype for Business Server Connectivity，必须删除现有连接并Skype公共提供商。 
  
此外，Skype PowerShell 配置连接。 若要使用 powerShell Skype连接，请运行：
...
2. 运行以下两个命令：
   ...
   - ProxyFqdn：Microsoft Skype拥有/ (/维护的联盟边缘) 
   ...
   - NameDecorationRoutingDomain 和 NameDecorationExcludedDomainList：通过设置这些设置，用户可以输入 Skype 用户的 MSA，而无需知道"修饰"具有"msn.com"的非 Microsoft 域。 这样就无需为不在 ExcludedDomainList 中的所有域键入"user (contoso.com) @msn.com"。 如果域不在"已排除"列表中，则 SfB 客户端将自动设置 MSA 的格式。 我们已将最常见的 Microsoft 帐户域添加到排除列表中。
     ...
     > [!NOTE]
     > &amp;NameDecorationRoutingDomain 和 NameDecorationExcludedDomainList 在 Office 2013 SP1 的 Lync Server 2013 CU5 Lync 桌面客户端中添加，可改善 Lync 用户添加 Skype 联系人以"修饰"非 Microsoft 域以标识和路由到 Skype (（格式为：user (contoso.com) @msn.com) ）的情况。 这些新设置将允许在 NameDecorationRoutingDomain (的"添加 Skype 联系人"对话框中自动设置用户输入的地址格式，如果该对话框不包含 NameDecorationExcludedDomainList (我们当前可以支持 msn.com、live.com、Hotmail.com、outlook.com) 中的域，该对话框应设置为 msn.com) 。 
  
3. 现在，Skype for Business客户端用户就可以搜索并添加Skype用户。
    
## <a name="clients-and-interoperability-matrix"></a>客户端和互操作性矩阵

下表概述了最新版本的 Skype 与最新版本的 Skype for Business 之间的互操作状态。
  

|**Skype客户端**|**添加联系人、IM、状态、音频和视频呼叫**|**Comment**|
|:-----|:-----|:-----|
|Skype Windows桌面  <br/> |7.6 或更高版本，Windows XP 及更高版本  <br/> |**新增**：增加了对在 Windows XP 上运行的 Windows Skype 客户端的支持，Windows Vista (需要最新的客户端版本 **7.26 或)** <br/> |
|Skype移动 - Android 电话 和平板电脑  <br/> |6.19 或更高版本，运行 Android OS 版本 4.0.3 或更高版本  <br/> |低规格设备可能不支持视频呼叫  <br/> |
|Skype移动 - iOS  <br/> |IOS 7 或更高版本上的 6.11 或更高版本  <br/> |第 4 代iPhone、iPod 第 4 代和更早版本（第 1 代iPad版）不支持  <br/> |
|SkypeMac  <br/> |7.19 或更高版本，在 Mac OS X 10.9 (Mave进行) 或更高版本  <br/> |需要 Mac OSX 10.9 或更高版本  <br/> |
|Skype通用Windows桌面 (Windows 10) 移动应用  <br/> |Windows 10 (Redstone 1 更新或更高版本)   <br/> |Windows通用应用将在 2016 年秋季收到更新，添加互操作支持  <br/> |
