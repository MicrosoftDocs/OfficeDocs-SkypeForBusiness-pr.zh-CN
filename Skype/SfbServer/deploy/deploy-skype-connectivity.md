...
Skype连接Skype for Business用户能够搜索和添加Skype用户。 Skype连接是一项Skype for Business，允许您启用与用户之间的联盟和Skype搜索。 启用 Skype 连接后Skype for Business用户将能够搜索并添加Skype用户。
...
- **按显示名称位置进行搜索，例如"John Doe in 则 In 则使用"-** 这将大大减少搜索结果。
...
|:-----|:-----|:-----|:-----|
|Skype for Business ServerEdge   |支持   |不支持   |Skype for Business Server和边缘是执行Skype搜索的先决条件   |
...
> [!NOTE]
> Skype如果请求速率超过 15 个请求/秒Microsoft 365，Office 365图形服务将限制来自任何本地或 Office 365 客户的搜索请求。 
...
- [在"联系人"Skype for Business](https://support.office.com/article/Add-a-contact-in-Skype-for-Business-89338023-2adf-4f5c-90b6-f8b6f72fadd1)
...
通过现有公共 IM 基础结构Skype for Business本地边缘服务器Skype用户之间的通信现在要求本地边缘服务器配置与 Skype for Business Online 兼容。
...
**为什么要关闭设置网站？**
2006 (公共 IM) PIC (pic.lync.com) 设置机制不再可用，将在 2019 年 8 月 15 日关闭。 相反，公共 IM 联盟将假定 Skype for Business Online 使用的联盟模型（称为"合作伙伴发现"，其中本地部署通过联盟 DNS SRV 记录公开 (发现) ）。

**此更改是否意味着公用 IM 联盟被弃用？**
不正确。 公共 IM 联盟将继续受支持很多年，可能Skype for Business本地产品生命周期结束。

**我们公司与 (Online 共享) 空间Skype for Business，是否受到影响？**
...
2. 运行以下两个命令：
   ...
   - NameDecorationRoutingDomain 和 NameDecorationExcludedDomainList：通过设置这些设置，用户可以输入 Skype 用户的 MSA，而无需知道"修饰"具有"msn.com"的非 Microsoft 域。 这样就无需为不在 ExcludedDomainList 中的所有域键入"user (contoso.com) @msn.com"。 如果域不在"已排除"列表中，则 SfB 客户端将自动设置 MSA 的格式。 我们已将最常见的 Microsoft 帐户域添加到排除列表中。
     ...
     > [!NOTE]
     > &amp;NameDecorationRoutingDomain 和 NameDecorationExcludedDomainList 在 Office 2013 SP1 的 Lync Server 2013 CU5 Lync 桌面客户端中新增了一些情况，即，添加 Skype 联系人的 Lync 用户需要"修饰"非 Microsoft 域，以标识这些联系人Skype (格式为：user (contoso.com) @msn.com) 。 这些新设置将允许自动设置用户在"添加 Skype 联系人"对话框中输入的地址格式（如果 NameDecorationRoutingDomain (如果该对话框不包含 NameDecorationExcludedDomainList (我们当前可以支持 msn.com、live.com、Hotmail.com、outlook.com) 中的域，该对话框应设置为 msn.com) 。 
...
|:-----|:-----|:-----|
|Skype Windows 桌面   |7.6 或更高版本，Windows XP 和更高版本   |**新增**：增加了对在 Windows XP 上运行的 Windows Skype 客户端的支持，Windows Vista (需要最新的客户端版本 **7.26 或)**  |
|Skype移动版 - Android 电话 和平板电脑   |6.19 或更高版本，运行 Android OS 版本 4.0.3 或更高版本   |低规格设备可能不支持视频呼叫   |
|Skype移动 - iOS   |IOS 7 或更高版本上的 6.11 或更高版本   |不支持第 4 代iPhone、iPod 第 4 代及更早版本（第 1 代iPad版）   |
