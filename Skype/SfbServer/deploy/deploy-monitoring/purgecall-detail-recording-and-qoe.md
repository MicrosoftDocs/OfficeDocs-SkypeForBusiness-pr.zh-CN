...
除了自动清除，两个新 cmdlet 和 #x 2014;Invoke-cscdrdatabasepurge 和调用 CsQoEDatbasePurge 和 #x 2014;已添加到 Skype 业务服务器;这些 cmdlet，管理员可以随时从 CDR 和 QoE 数据库手动清除记录。 例如，若要从 CDR 数据库中手动清除所有期限长于 10 天的记录，则可以使用类似于下面的命令：
