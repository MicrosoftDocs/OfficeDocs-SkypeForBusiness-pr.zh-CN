...
6. 在 "管理组配置" 页面上, 在 "**管理组名称**" 框中键入 Operations Manager 管理组的名称, 然后在管理服务器中键入 operations manager 服务器的主机名 (例如, atl-scom-001) **** 框。 如果更改了 Operations Manager 使用的端口号, 请在 "**管理服务器端口**" 框中输入新的端口号。 否则, 将该端口保留为默认值 5723, 然后单击 "**下一步**"。
