...
在理想中，你绝不会遇到 Skype for Business Server 2015 问题。 但是，Skype for Business Server 可能会受外部因素（例如，网络崩溃和硬件故障）的影响。 通过使用 Skype for Business Server 2015 管理包，可以主动识别和解决潜在问题。 这样，Skype for Business Server 2015 管理包将扩展 System Center Operations Manager 的功能。
  
此信息是基于 Skype for Business Server 2015 通信软件监视包 9319.0 版编写的。
  
## <a name="configuration-overview"></a>配置概述

 若要配置 Skype for Business Server 2015 基础结构以使用 System Center Operations Manager，必须执行三项操作：
  
标识和  [配置主管理服务器](configure-the-primary.md)。 为此，必须安装 System Center Operations Manager 2012 SP1 或 R2。 
  
 标识 [和配置将监视](configure-computers-to-monitor.md)的 Skype for Business Server 计算机。 若要使用 System Center Operations Manager 监视 Skype for Business Server 计算机，必须安装 System Center Operations Manager 代理文件，并配置每个服务器以充当代理。 
  
 标识并 [安装和配置观察程序节点](watcher-nodes.md)。 观察程序节点是定期运行 Skype for Business Server 综合事务的计算机 -Windows PowerShell cmdlet，用于验证关键 Skype for Business Server 组件（如登录到系统的能力或交换即时消息的能力）是否正常工作。 
...
管理包可以与 System Center Operations Manager 2007 R2 (64 位)  (一起使用，仅支持迁移) 或 System Center Operations Manager 2012 SP1 R2 (64 位) 或 &amp; System Center Operations Manager 2016 (64 位) 。 下表显示了 Skype for Business Server 2015 管理包支持的配置： 
  
|配置|是否支持？|
|:-----|:-----|
|Windows Server 2008 R2 操作系统  <br/> Windows Server 2012 R2 操作系统  <br/> |能。 在 Skype for Business Server 2015 服务器和综合事务观察程序节点上。  <br/> |
|群集服务器  <br/> |不支持。  <br/> |
|无代理监视  <br/> |不支持。  <br/> |
|虚拟环境  <br/> |能。  <br/> |
...
Skype for Business Server 2015 监视包包括以下文件：
...
Skype for Business Server 2015 管理包新增了以下功能。

- **[2019 年 9 月更新中的更改](https://www.microsoft.com/en-in/download/details.aspx?id=47364)** 某些警报已删除特殊字符。 在某些情况下，特殊字符会干扰 SCOM 命令通道通知功能。

- **客户端登录的自动发现** 登录到 Skype for Business Server 2015 的客户端应用程序通常会自动发现要登录的服务器。 综合事务现在支持验证自动发现是否正确配置。
...
- **视频互操作服务综合事务** 从其他供应商解决方案迁移到 Skype for Business Server 2015 的客户通常希望继续使用来自其他供应商的视频电话会议设备 (VTC) 。 视频互操作服务器是一种新的 Skype for Business Server 2015 服务器角色，使客户可以通过视频 SIP 中继连接到 Cisco CUCM 来继续使用其会议室中的 Cisco VTC。 此功能还添加综合事务以帮助验证视频互操作服务器是否启动，并可以处理通过视频 SIP 中继的传入连接。
    
- **应用程序共享会议综合事务** 现在支持应用程序共享会议的端到端方案验证。
    
## <a name="monitoring-scenarios"></a>监视方案

Skype for Business Server 2015 管理包利用各种功能来帮助检测和诊断问题。 这些功能可实时查看 Skype for Business Server 2015 环境的运行状况。
...
|呼叫可靠性警报  <br/> |Skype for Business Server 2015 服务器 (CD) R 的呼叫详细信息记录反映了用户能否连接到呼叫或呼叫终止的原因。 呼叫可靠性警报查询 CDR 数据库以生成警报，以指示大量用户何时遇到对等呼叫或基本会议功能的连接问题。  <br/> 方案覆盖包括音频呼叫、对等即时消息 (IM) 和其他会议功能。  <br/> |
|媒体质量警报  <br/> |查看每个呼叫结束时由 Skype for Business Server 2015 (发布的 QoE) 报告用户体验质量的数据库查询。 这些查询会生成警报，以准确指出用户最有可能在通话和会议期间遇到媒体质量受损的情况。 数据基于关键指标（如数据包延迟和丢失）构建，这直接影响用户体验的质量。  <br/> |
 ...
 **高优先级警报：** 这些警报指示导致大型用户组的服务中断的条件，需要立即采取措施。 综合事务和脱机服务（如 Skype for Business Server 音频/视频会议 (检测到的中断) 高优先级警报。 相比之下，单台计算机中的组件故障不是高优先级警报。 Skype for Business Server 2015 具有针对这些情况的内置高可用性功能，例如，负载平衡器后面的多个前端服务器。
...
Skype for Business Server 2015 管理包通过综合事务增加了警报的覆盖范围。 综合事务Windows PowerShell集成到 Operations Manager 管理包中以测试端到端用户方案的 cmdlet。 当您指定服务器来执行综合事务时，管理包会定期触发这些 cmdlet。 综合事务导致的失败将生成有状态警报。 以下是 Skype for Business Server 2015 支持的综合事务：
...
|8   <br/> |MCX 对等即时消息 (移动设备)   <br/> |在 Lync Server 2010 到 Skype for Business 2015 的 2011 年 9 月版本中可用  <br/> |
...
|14   <br/> |电话拨入式会议  <br/> |Skype for Business Server 2015 中的新增功能  <br/> |
|15   <br/> |应用程序共享会议  <br/> |Skype for Business Server 2015 中的新增功能  <br/> |
|16   <br/> |UCWA 会议 (Web 会议加入)   <br/> |Skype for Business Server 2015 中的新增功能  <br/> |
||||
...
| 21  <br/> |视频互操作服务器  <br/> |Skype for Business Server 2015 中的新增功能  <br/> |
||||
...
|管理包对象|说明|
|:-----|:-----|
|Skype for Business Server 部署  <br/> |表示在组织中部署 Skype for Business Server 2015。  <br/> |
...
创建用于存储密封管理包的自定义设置的管理包时，我们建议适当地命名新的管理包，例如"Skype for Business Server 2015 自定义"。 
