...
级别收集致命错误、错误消息、警告消息和信息消息。 所使用的标志均为 Lyss 提供程序定义的所有标志, 包括 TF_Connection、TF_Diag 和 TF_Protocol。定义 $LyssProvider 变量后, 可以将其与**新的 CsClsScenario** cmdlet 一起使用, 以便从 Lyss 提供程序中收集跟踪。 若要创建提供程序并将其分配给新的方案，请键入：
