...
- 按如下方式配置日志记录会话：
  ...
  - 使用现有提供程序或创建新提供程序。 Aprovider 定义日志记录会话收集的内容、何种级别的详细信息、要跟踪的组件以及应用的标志。
    ...
    > [!TIP]
    >  如果你熟悉 OCSLogger，termproviders 指的是**组件**（如 S4、SIPStack）、**日志记录类型**（例如，WPP、EventLog 或 IIS 日志文件）、**跟踪级别**（例如，"所有"、"详细"、"调试"）和 "**标志**" （例如，TF_COMPONENT、TF_DIAG）。 这些项在提供程序（Windows PowerShell 变量）中定义并传递到集中式日志记录服务命令。
...
与 OCSLogger 一样，集中式日志记录服务有多个要跟踪的组件，并提供选择标志的选项，例如 TF_COMPONENT 和 TF_DIAG。 集中式日志记录服务还保留 OCSLogger 的日志记录级别选项。
