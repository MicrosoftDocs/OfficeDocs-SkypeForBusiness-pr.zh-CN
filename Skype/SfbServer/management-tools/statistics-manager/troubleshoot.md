...
**摘要：** 阅读本主题，解决 Skype for Business Server 统计信息管理器的部署问题。
  
本主题介绍如何通过描述您可能在应用程序事件日志中看到的事件以及可能采取的相应操作来排查统计信息管理器部署问题。 本主题包含以下各部分：
...
- **1000** — 无法在作业对象 (设置) 限制程序 — 未知原因
    
- **1001** — 在进程上不允许进程限制 (可能已经在作业对象对象) 
    
    代理在 Windows 作业对象内部运行，以自动限制其内存占用。 如果代理不会启动，并且这些事件条目存在于事件日志中，则作业对象无法在服务器上实例化。 若要处理此情况，可以通过更改以下项中的值来删除上限配置文件：
...
- **2001**— 无法连接到任何源 IP 上的服务
    ...
    2. 使用 telnet 等连接工具验证从代理计算机到正确端口上的侦听器的连接。
        ...
        如果没有，请确保在侦听器计算机上为侦听器计算机连接到专用/公共/域 (类型的网络类型启用传入防火墙) 。 如果侦听器计算机未加入域，则网络可能列为公用网络，在这种情况下，随统计信息管理器一起安装的防火墙规则在默认情况下将不适用。
...
- **4000** — 无法从侦听器下载服务器信息 (未知原因) 
  ...
  - 如果代理位于拓扑 (中未列出的服务器上，例如 SQL AlwaysOn 群集) 中的节点，则需要按照导入拓扑中的说明手动添加[代理。](deploy.md#BKMK_ImportTopology)
    
  - **4002** — 侦听器密码无效
    
    代理尝试使用的加密密码与侦听器本身的服务密码不匹配。 卸载代理，然后使用正确的服务密码重新安装它。
...
- **10000** — 启动失败 未知 (无法恢复，服务将停止/崩溃，) 
  ...
  - **10001** — 配置问题
    ...
    通常，当 [listener_install_location]\PerfAgentListener.exe.config 文件已手动修改并且应用程序无法读取时，将会出现此情况。
    
  - **10002** — HTTP 侦听器初始化错误
    
    在安装期间未正确设置 URL ACL 或 SSL 证书无效时，通常会记录此事件。 确保配置中的证书有效。 如果是，请根据部署统计信息管理器 [中的说明重新安装侦听器](deploy.md#BKMK_Deploy)。
...
- **10100** — Redis PING 中断
  ...
  - **10101** — Redis PING (每隔 60 秒中断一次) 
    
  - **30100** — 已还原 Redis PING 中断
    
    当侦听器无法连接到 Redis 时，将记录这些记录。 确保 Redis 已启动，并且侦听器和 Redis 之间的网络连接可用。
    
- **10200** — Redis 写入中断
    
  - **10201** — Redis Write 中断 (每隔 60 秒) 
...
- **23000** — EventLogQueryManager 初始化 (或失败后成功) 
    
- **24000** — 首次初始化 serverinfo (失败或失败后) 
