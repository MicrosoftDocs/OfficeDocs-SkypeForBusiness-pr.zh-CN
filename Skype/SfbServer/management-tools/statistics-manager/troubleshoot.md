...
**摘要：**阅读本主题解决 Skype 您部署的统计信息管理的业务服务器 2015年。
  
本主题介绍如何解决通过描述您可能会看到在应用程序事件日志中的事件和适当的措施，以纠正该事件，则可以采用统计管理器部署。 本主题包含以下部分：
...
- **1001** — 过程限制不允许进程 (可能已在作业对象)
  ...
  ```
    ...
    搜索"MaxProcessMemoryMB"并将该值更改为"0"，如下所示：
  ...
  ```
    ...
    > [!NOTE]
    > 如果进行此更改后，代理将通常仍会占用\<100 MB 的内存，但是它不会强行限制为 300 MB 是默认值。 如果进行此更改，我们建议密切监视内存使用情况，以确保代理不会占用大量的内存在其宿主计算机上。 
...
1. 确保侦听器服务正在侦听器计算机上运行。 否则，请确保 Redis 正在服务器上运行，然后重新启动侦听器服务。
    ...
    检查统计管理器事件日志侦听器计算机，以确保统计数据管理器侦听器服务本身没有问题。
    
2. 使用连接工具（例如 telnet）验证正确端口上从代理计算机到侦听器的连接。
    
    否则，请确保根据侦听器计算机连接到的网络类型（专用/公共/域）在侦听器计算机上启用传入防火墙规则。 如果侦听器计算机未加入到域中，网络可能被列为公共和在这种情况下使用统计数据管理器安装的防火墙规则不会默认应用。
    
- **4000** - 无法从侦听器下载服务器信息（未知原因）
    
  - **4001** - 在侦听器拓扑未找到服务器
    
    如果服务器成功地连接到该侦听器，但服务器未添加到拓扑结构中该侦听器的缓存中，则将发生此错误。 解决办法选项：
  ...
  - 检查侦听程序计算机的 StatsMan 日志中为其配置的问题。
...
- 重复的登录提示中铬-这是 1.1 版本中已解决的 bug。 请确保如果看到重复的登录提示，在 Chrome 浏览器升级到最新版本的统计数据管理器。 要验证你正在运行的网站的版本，请执行以下操作：
