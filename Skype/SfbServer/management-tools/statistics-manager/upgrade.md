...
**摘要：**阅读本主题，了解如何升级统计管理器为 Skype 业务服务器 2015年。
  
本主题介绍如何升级现有安装的统计信息管理器为 Skype 业务服务器--允许您查看 Skype 实时业务服务器运行状况和性能数据的强大工具。 可以跨数百个服务器每隔几秒钟轮询性能数据和统计数据管理器网站上可立即查看结果。 
  
有关统计数据管理器和版本 1.1 版中的新增功能的详细信息，请参阅[规划的业务服务器 2015年的 Skype 统计信息经理](plan.md)和[部署业务服务器 2015年的 Skype 统计管理](deploy.md)。 有关 1.1 版中解决的已知问题的信息，请参阅 [1.1 版中解决的已知问题](upgrade.md#BKMK_Fixed)。
  
升级方法有两种：
  
- **自动升级**。 此方法使用一个自动化的脚本。 它是最简单的方法，并应适用于所有的升级方案。
    
- **手动升级**。 提供此方法是在不寻常的情况下，自动的升级失败的备份计划。
...
1. 将 MSI 文件的新版本代理、 侦听器和更新 StatsMan.ps1 脚本的网站放到侦听程序计算机上的单个文件夹。
...
> [!NOTE]
> 统计管理器服务密码将以明文形式在命令行上显示，因为它将传递到安装程序。 必要时请务必遮蔽显示器。 
  
1. 在运行脚本时，系统将提示您卸载产品的旧版本。 请单击“是”。
    
2. 如果侦听器服务正在运行，系统将提示你关闭该应用程序，然后再继续。 允许应用程序关闭 （统计数据管理器侦听器服务将被停止）。
    
3. 继续执行安装过程。 您应该会注意到服务密码和证书指纹已预先填充。 如果不是，请添加您保存的值，然后继续。
    
3. 打开 PowerShell 管理窗口。 升级网站组件：
...
2. 如果代理服务正在运行，系统将提示你关闭该应用程序，然后再继续。 允许该应用程序关闭（StatsMan 代理服务将停止）。
    
3. 继续执行安装过程。 您应该会注意到服务密码和证书指纹已预先填充。 如果不是，请添加您保存的值，然后继续。
    
4. 打开 PowerShell 管理窗口。 升级代理组件：
...
2. 继续执行安装过程。 您应该会注意到网站端口已预先填充。 如果不是，请添加您保存的值，然后继续。
    
3. 验证是否能够使用浏览器正常浏览该网站。
...
- 用户界面/服务器/代理的许多非常重要的可靠性和性能的改进
    
- 用户界面的主筛选器控件现在排序正确与不同的用例 （已领先人觉得某些服务器系统中时它们是不是）
    
- 服务器 - 服务器组件现在将安装在非英语服务器上。
    
- 服务器/代理 - 在某些情况下，代理和服务器组件无法安装并显示 .NET 错误，错误的原因是特定版本的 Net 4.0。 此问题已得到解决。
    
- 代理-扩展 StatsMan 代理程序添加事件日志记录。 安装的服务器不在拓扑中时，代理不再崩溃，现在会在事件日志中记录此错误，同时记录许多其他可能的错误条件。
    
- UI-当不使用客户端计算机连接到相同的工作组或域与统计信息管理器 Web 服务器时，使用 Chrome 浏览器的 Web 客户端将看到多个登录提示。 现在，每个会话只需要登录一次。
