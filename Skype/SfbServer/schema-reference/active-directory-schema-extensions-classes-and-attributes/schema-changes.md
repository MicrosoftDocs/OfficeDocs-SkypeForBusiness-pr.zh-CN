...
Skype for Business Server需要几个新的类和属性，并修改一些现有的类和属性。 此外，与以前版本Skype for Business Server一样，这些配置信息存储在中央管理存储中，而不是 AD DS 中。 以下信息仍存储在 AD DS 中的 Skype for Business Server：
...
- **数据存储** (扩展架构Skype for Business Server现有架构类中存储) ：
...
**由属性添加Skype for Business Server**
...
|msRTCSIP-MirrorBackEndServer  <br/> |此属性用于存储前端池SQL Server的镜像后端。  <br/> |
