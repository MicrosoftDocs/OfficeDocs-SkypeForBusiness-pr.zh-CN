...
---
# <a name="schema-classes-and-descriptions-in-skype-for-business-server"></a>Skype for Business Server 中的架构类和说明
...
|**类**|**说明**|**Comments**|
|:-----|:-----|:-----|
|Mail-Recipient  <br/> |Exchange 统一消息 (UM) 电子邮件收件人。  <br/> |此辅助类与 Exchange UM 共享。  <br/> |
...
|msRTCSIP-ApplicationServerSettings  <br/> |msRTCSIP-ApplicationServer 的辅助类包含表示应用程序服务实例设置的属性。  <br/> |Communications Server 2007 R2 中的新增功能。  <br/> |
...
|msRTCSIP-ConnectionPoint  <br/> |通用服务控制点 (SCP) 将计算机指定为运行 Skype for Business Server 的服务器。  <br/> |Lync 2010 中的新增功能。  <br/> |
|msRTCSIP-DefaultCWABank  <br/> |此辅助类包含 Skype for Business Web App Bank 的设置。  <br/> |Communications Server 2007 R2 中的新增功能。  <br/> |
|msRTCSIP-Domain  <br/> |此类包含定义 SIP 注册器的已配置域的属性。  <br/> |-  <br/> |
|msRTCSIP-EdgeProxy  <br/> |此类容器表示单个访问边缘服务。 由于访问边缘服务部署在外围网络中，并且客户通常不允许从外围网络访问 Active Directory 域服务，因此访问边缘服务的实例未加入 Intranet 的 Active Directory 网络。 这样，访问代理就不会在 AD DS 中自动注册。 管理员必须手动配置 AD DS 中每个访问边缘服务实例是否存在。  <br/> |-  <br/> |
|msRTCSIP-EnterpriseMCUSettings  <br/> |这是 msRTCSIP-MCU 的辅助类，其中包含表示会议服务器的设置的属性。  <br/> |Communications Server 2007 Microsoft Office新增功能。  <br/> |
...
|msRTCSIP 联合身份验证  <br/> |这是 msRTCSIP-GlobalContainer 的辅助类，其中包含与联盟相关的所有设置。  <br/> |-  <br/> |
|msRTCSIP-GlobalContainer  <br/> |此类包含适用于整个 Skype for Business Server 部署的所有设置。  <br/> |-  <br/> |
...
|msRTCSIP-LocationContactMapping  <br/> |此类由会议助理应用程序创建，并保留用于按区域分类会议电话号码的属性。  <br/> |Communications Server 2007 R2 中的新增功能。  <br/> |
...
|msRTCSIP-Pool  <br/> |此类表示单个 Skype for Business Server 池。  <br/> |-  <br/> |
|msRTCSIP-Pools  <br/> |此类包含多个 Skype for Business Server 池，并且自身没有任何属性。  <br/> |-  <br/> |
...
|msRTCSIP-Server  <br/> |此类表示运行 Skype for Business Server 的单个服务器。  <br/> |-  <br/> |
...
|msRTCSIP-TrustedService  <br/> |此类是表示可使用全局可路由用户代理 URI (GRUU) 地址进行路由的受信任服务的容器。 当激活受 Skype for Business Server 信任的新服务器时，将创建此类的实例。 此受信任服务器必须加入 Active Directory 域。  <br/> |Communications Server 2007 中的新增功能。  <br/> |
