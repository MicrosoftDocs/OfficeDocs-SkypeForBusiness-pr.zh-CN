...
如果将全局设置存储在配置容器中 (对于所有新的 Skype for business 服务器部署而言), 林准备将使用现有服务容器, 并在 Configuration\Services 下添加**RTC 服务**对象。目标. 在 RTC 服务对象下, 林准备添加了类型 msRTCSIP-GlobalContainer 的**全局设置**对象。 全局设置对象包含适用于 Skype for Business 服务器部署的所有设置。 如果在系统容器中存储全局设置, 林准备将使用根域系统容器下的 Microsoft 容器和 System\Microsoft 对象下的 RTC 服务对象。
