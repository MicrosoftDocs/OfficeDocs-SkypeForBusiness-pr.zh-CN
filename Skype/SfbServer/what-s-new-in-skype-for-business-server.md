...
客户端Skype for Business客户端外观与客户端的使用者版本Skype并使用相同的按钮和图标。 更少的菜单和扁平的任务层次结构使用户能够轻松快速找到所需的控件和命令。 
...
Skype for Business Server 2015 增加了对 AlwaysOn 可用性SQL Server和 AlwaysOn 故障转移SQL Server实例的支持。 除了这些功能之外，Skype for Business Server像以前版本的 Lync Server 一样，继续支持数据库镜像和SQL Server群集。
...
SQL Server故障转移群集实例利用 Windows Server 故障转移群集 (WSFC) 功能通过服务器实例级别的冗余提供本地高可用性，即故障转移群集实例 (FCI) 。 FCI 是一个 SQL Server 实例，它安装在 Windows Server 故障转移群集 (WSFC) 节点上，并且可能跨多个子网安装。
...
- 推荐根据客户本地资产的状态（由 OnRamp for Office 365自动协助工具确定）进行混合部署。
