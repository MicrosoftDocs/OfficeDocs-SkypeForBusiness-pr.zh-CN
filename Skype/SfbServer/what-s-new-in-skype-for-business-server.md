...
Lync 现已Skype for Business，它是一个通信和协作平台，它提供由 Skype 启发的体验与 Lync 的企业级安全性、合规性和控制。 Skype for Business提供状态、IM、语音和视频呼叫以及联机会议等功能。 Skype for Business提供了新的客户端体验、新的服务器版本，以及 Microsoft 365 或 Office 365 中的服务更新。 如果贵组织的用户已经熟悉Skype，他们将会非常体验 Skype for Business 在轻松查找同事并联系同事时，Skype for Business这种功能和简单性。 如果贵组织的用户即将从 Lync Skype for Business，他们将识别他们已使用的所有功能，但在全新的界面中，使用简化的控件和新增功能。 除了新的客户端体验，Skype for Business Server 2015 还提供了一些新功能，以提高本地服务器和混合解决方案的可管理性。
...
Skype for Business包括上述新用户体验和之前发布的 Lync 2013 用户体验。 通过包含这两种体验，企业可以控制新客户端推出的过程和时间，来管理用户的更改。默认用户体验取决于您使用的服务器的哪个版本。 管理员使用 **Set-CsClientPolicy** cmdlet 和 EnableSkypeUI 参数选择首选体验。 有关配置客户端体验的信息，[请参阅使用 Skype for Business](deploy/deploy-clients/configure-the-client-experience.md) 配置客户端体验和桌面客户端[功能比较Skype for Business](plan-your-deployment/clients-and-devices/desktop-feature-comparison.md)。
...
视频互操作服务器 (VIS) 充当 Skype for Business Server 和 Cisco 视频电话会议 (VTC) 中间。 加入会议时，用户现在可以选择 Cisco VTC 系统。 视频互操作服务器 (VIS) 作为本地部署的独立服务器角色实现。 有关详细信息，请参阅 [Plan for Video Interop Server in Skype for Business Server 2015](plan-your-deployment/video-interop-server.md)。
  
### <a name="call-via-work"></a>通过工作电话呼叫

单位电话呼叫功能允许企业用户从 Skype for Business 进行语音呼叫。 当用户发出语音呼叫时，该呼叫Skype for Business到发起方 PBX 或 PSTN 电话。 一旦发起方接听电话，呼叫将定向到目标号码。 呼叫接收者应答，呼叫建立时Skype for Business控制面板。 发起方可以从用户管理其状态和呼叫Skype for Business。 服务器管理员为企业启用和配置单位电话呼叫。 有关详细信息，请参阅 [Plan for Call Via Work in Skype for Business Server 2015](plan-your-deployment/enterprise-voice-solution/call-via-work.md)。 
  
## <a name="mobile-device-support-improvements"></a>移动设备支持改进

移动设备支持的改进包括为 Enterprise Edition 用户改进移动体验的功能，例如访问对话历史记录和日志数据、增强的移动会议体验以及跨 Office 的单一登录支持。 此外，还改进了 Android 支持、性能改进和功能，以通过 Common App Framework 简化集成。 
...
为了允许移动访问对话历史记录、错过的 IM 和呼叫日志数据，此信息的存档现在通过服务器处理所有移动客户端。 IM 的自动接受功能在移动用户没有立即响应 IM 时阻止服务器出现错误消息，从而提高了可靠性。 若要利用基于服务器的 Exchange/Outlook文件夹集成，Exchange Server 2013 或更高版本和更新的移动客户端是必需的。 
...
Skype for Business Server引入了两个新的 cmdlet，这两个 cmdlet 可帮助使升级或修补前端服务器比在以前版本的 Lync Server 中容易得多。
  
当您需要向前端服务器应用修补程序或执行任何其他维护时，只需键入 **Invoke-CsComputerFailOver** 并指定该服务器的名称。 Skype for Business Server将该服务器的工作负荷临时移动到池中的其他服务器。 然后，您可以执行维护，然后使用 **Invoke-CsComputerFailback** cmdlet 使该服务器重新进入服务。 如果需要修补池中的每台服务器，只需针对每台服务器执行此过程，一次修补一台服务器。 这些新 cmdlet 使您能够比早期版本更快地修补服务器，并且具有更高的可靠性和更简单的工作流。
...
Skype for Business Server 2015 增加了对 AlwaysOn 可用性SQL Server和 AlwaysOn 故障转移SQL Server的支持。 除了这些功能之外，Skype for Business Server像以前版本的 Lync Server 一样SQL Server对数据库镜像和数据库群集的支持。
  
SQL Server AlwaysOn 可用性组是 SQL Server 2012 和 SQL Server 2014 中的高可用性和灾难恢复解决方案，可提供数据库镜像的替代方法。 可用性组支持一组离散数据库的故障转移 (称为可用性数据库) 一起故障转移。 可用性组支持一组读写主数据库以及一到四组相应的辅助数据库。 （可选）辅助数据库可用于只读访问和某些备份操作。
  
SQL Server故障转移群集实例利用 Windows Server 故障转移群集 (WSFC) 功能通过服务器实例级别的冗余提供本地高可用性，即故障转移群集实例 (FCI) 。 FCI 是一个 SQL Server 实例，它安装在 Windows 服务器故障转移群集 (WSFC) 节点上，并且可能跨多个子网。
...
集中日志记录服务是 Skype for Business Server 2015 的首选日志记录环境。 此版本没有新功能，但服务和集中日志记录服务代理 (ClsAgent.exe) --与控制器通信并接收管理员发出的命令的服务可执行文件的可靠性和性能得到了改进。
  
Skype for Business Server 2015 Windows PowerShell cmdlet 管理日志记录服务代理、启动跟踪并生成报告。  (Lync Server 2013 ClsController.exe执行这些任务。) 
  
集中日志记录服务可在任何 Skype for Business Server 2015 运行。 预定义跟踪 (的内置) 与创建自定义方案的能力相同。 有一种称作 AlwaysOn 的特殊方案始终在运行，并允许管理员以近实时状态查找常见问题。
  
Snooper 调试工具也进行了更新，以允许调试移动日志，并用于连接到 Lync 2013 或 Skype for Business Server 2015 的设备。 该工具可从调试工具下载 [Web](https://go.microsoft.com/fwlink/?LinkId=285257)。
...
- 控制面板增强功能，使管理员能够登录到 Microsoft 365 或 Office 365 租户，并设置与 Skype for Business Online 的混合使用混合配置向导。
...
- 使用控制面板管理从本地可管理的属性和管理中心管理从 Skype for Business Online 的属性的能力。
...
多重身份验证是一种身份验证方法，要求使用多种验证方法，并将关键的第二层安全性添加到用户登录和事务中。 例如，需要用户名和密码以及证书。 Skype for Business Server 2015 继续基于 Lync Server 2013 累积更新中提供的多重身份验证功能构建。 多重身份验证的显著变化包括：
  
- 使用 Office 2013 SP1 Active Directory 身份验证库与 Exchange 和 SharePoint
- 支持客户端中的多重Skype for Business Web应用功能
    
使用Skype for Business多重身份验证，现在可以根据地理位置提供不同的身份验证选项。 例如，客户可以配置其环境，以便内部身份验证依赖于集成身份验证Windows身份验证，而从组织外部进行身份验证的员工使用多重身份验证。 
  
无论Skype for Business，多因素身份验证体验都是无缝的：
  
- 地理位置 - 用户是从组织内部还是外部登录 
- 客户端/设备类型 - Skype for Business客户端以及客户端在 (电脑、移动设备、iPad等设备上) 
- 帐户位置 - 用户是托管在本地 Active Directory 中还是 Azure Active Directory Online 中。
