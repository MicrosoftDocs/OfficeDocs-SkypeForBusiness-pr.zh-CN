...
每个活动电话号码必须有紧急地址 （当您在 Office 365 或电话号码的转移时获得电话号码相关联） 当电话号码分配给用户。 时数与紧急通讯，还可以提供更精确的物理位置位于紧急位置。 紧急的位置可以是平面布置、 建筑机翼或办公室号码用户所在的位置。 可以有无限的多个位置为给定的紧急地址，并可以更改紧急位置，如果用户移动到不同的办公室或建筑 — — 例如，如果用户将从地面到地板 35 34。
