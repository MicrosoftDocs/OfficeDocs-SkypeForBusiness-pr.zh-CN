---
title: "将电话号码转网后需要执行哪些操作？"
ms.author: tonysmit
author: tonysmit
manager: scotv
ms.date: 11/17/2017
ms.audience: Admin
ms.topic: get-started-article
f1_keywords:
- ms.lync.lac.PortOrderFinish
ms.prod: office-online-server
localization_priority: Normal
ms.collection: Adm_Skype4B_Online
ms.custom: Adm_O365_FullSet
ms.assetid: 825a6cf0-3d22-419a-8358-a8678f14b61d
description: "See what you need to do next after you've ported your phone numbers to Skype for Business."
---

# 将电话号码转网后需要执行哪些操作？

> [!IMPORTANT]
> 本文是由机器翻译的，请参阅[免责声明](825a6cf0-3d22-419a-8358-a8678f14b61d.md#MT_Footer)。请在 [此处](https://support.office.com/en-us/article/825a6cf0-3d22-419a-8358-a8678f14b61d) 中查找本文的英文版本以便参考。
  
在提交端口订单之后，我们将运营商联系，并与它们的细节。如果已设置的最终传输日期，我们将通过电子邮件通知您。但是，等待运营商以接受顺序，请继续，创建所需的紧急地址将使用这些数字。
  
- 了解有关[什么是紧急位置、地址和呼叫路由？](what-are-emergency-locations-addresses-and-call-routing.md)。
    
- 如果需要，你还可以[为你的组织添加或删除紧急地址](add-or-remove-an-emergency-address-for-your-organization.md)。
    
- 如果您的电话号码已经有紧急的地址，[添加、 更改或删除您的组织紧急地点](add-change-or-remove-an-emergency-location-for-your-organization.md)。
    
- [为用户分配、更改或删除电话号码](assign-change-or-remove-a-phone-number-for-a-user.md) 。
    
## 
<a name="MT_Footer"> </a>

> [!NOTE]
> **机器翻译免责声明**：本文是由无人工介入的计算机系统翻译的。Microsoft 提供机器翻译是为了帮助非英语国家/地区用户方便阅读有关 Microsoft 产品、服务和技术的内容。由于机器翻译的原因，本文可能包含词汇、语法或文法方面的错误。 
  
## 另请参阅
<a name="MT_Footer"> </a>

#### 其他资源

[紧急呼叫条款和条件](emergency-calling-terms-and-conditions.md)
  
[音频会议免费拨出时间段](../accessibility-and-regulatory/audio-conferencing-complimentary-dial-out-period.md)

