---
title: "紧急呼叫条款和条件"
ms.author: tonysmit
author: tonysmit
manager: scotv
ms.date: 9/22/2017
ms.audience: Admin
ms.topic: reference
ms.prod: office-online-server
localization_priority: Normal
ms.collection: Adm_Skype4B_Online
ms.custom: Adm_O365_FullSet
ms.assetid: ca2c751b-53ab-42c7-aed9-cfe27e662940
description: "Learn how Emergency Services  (911) differ from traditional telephone services and how to make a call. "
---

# 紧急呼叫条款和条件

> [!重要信息]
> 本文是由机器翻译的，请参阅[免责声明](ca2c751b-53ab-42c7-aed9-cfe27e662940.md#MT_Footer)。请在 [此处](https://support.office.com/en-us/article/ca2c751b-53ab-42c7-aed9-cfe27e662940) 中查找本文的英文版本以便参考。
  
## 重要信息

紧急服务调用以不同方式运行调用比 Office 365 中计划与传统电话服务。值得您了解这些差异，并传递到具有调用计划在 Office 365 中的所有用户。承认和同意您已阅读和理解调用我们紧急服务之间的差异，并将提供此通知每个用户致电 Office 365 中计划。
  
呼叫功能我们紧急服务之间的差异包括以下内容: (i) Skype for Business 可能不知道实际位置的呼叫者进行紧急服务呼叫，这会导致呼叫路由到错误的紧急服务呼叫中心和/或位置错误; 调度紧急服务（（ii） 如果用户的设备具有未 power、 遇到停电或者由于任何原因而无法访问 Internet，用户不能通过致电计划紧急服务呼叫在 Office 365; 和 (iii) 虽然可以致电 Office 365 中计划在哪里可连接到 Internet 世界的任何位置使用，用户不应从其家庭国家/地区之外的位置紧急服务呼叫因为呼叫可能不会将路由到相应呼叫中心所在国家/地区。
  
## 
<a name="MT_Footer"> </a>

> [!注释]
> **机器翻译免责声明**：本文是由无人工介入的计算机系统翻译的。Microsoft 提供机器翻译是为了帮助非英语国家/地区用户方便阅读有关 Microsoft 产品、服务和技术的内容。由于机器翻译的原因，本文可能包含词汇、语法或文法方面的错误。 
  
## 另请参阅
<a name="MT_Footer"> </a>

#### 其他资源

[Skype for Business Online： 紧急呼叫免责声明标签](https://go.microsoft.com/fwlink/?LinkID=692099)
  
[紧急呼叫条款和条件](ca2c751b-53ab-42c7-aed9-cfe27e662940.md)
  
[音频会议免费拨出时间段](../accessibility-and-regulatory/audio-conferencing-complimentary-dial-out-period.md)

