...
在攻击者获取对网络中数据路径的访问权并能够监控和读取流量内容时，会发生窃听。 窃听也称为嗅探或窥探。 如果流量内容采用纯文本形式，那么攻击者在获取路径的访问权之后即可读取流量内容。 例如，通过控制数据路径上的路由器进行攻击。 
...
- **增强状态数据**&nbsp;&nbsp;&nbsp;增强状态数据是一些信息，用户可以选择是否通过指向联盟伙伴的链接共享此信息，或选择是否与组织内的联系人共享此信息。 不与公共即时消息络上的用户共享此数据。 客户端策略和其他客户端配置可能会为系统管理员提供一些控制能力。 在 SfBO 服务中，可为单个用户配置增强的状态隐私模式，以防止不在用户的联系人列表中的 SfBO 用户看到用户的状态信息。 
...
本节概述构成 Microsoft SfBO 安全框架的基本要素。这些要素如下：
- Azure Active Directory (AAD) 为用户帐户提供单个受信任的后端存储库。 
- 公钥基础架构 (PKI) 使用受信任的证书颁发机构 (CA) 颁发的证书来对服务器进行身份验证，从而确保数据的完整性。
- 传输层安全性 (TLS)、HTTPS over SSL (HTTPS) 和相互 TLS (MTLS) 可实现端点身份验证和即时消息密。 点对点音频、视频和应用程序共享流使用安全实时传输协议 (SRTP) 进行加密和完整性检查。
...
- 想要参加电话拨入式会议的匿名用户（即未经身份验证的用户）拨打一个会议接入号码后，系统会提示他们输入会议 ID。 还会提示未经身份验证的匿名用户记录其姓名。 记录的名称用于在会议中标识未经身份验证的用户。 除非至少一个主持人或经过身份验证的用户已参加会议，否则不允许匿名用户参加会议，且无法向其分配预定义角色。
