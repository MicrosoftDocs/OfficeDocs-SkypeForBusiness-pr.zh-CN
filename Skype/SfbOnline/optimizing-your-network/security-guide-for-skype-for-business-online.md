...
Skype for BusinessOnline 使用 Windows Server 操作系统中的 PKI 功能来保护用于传输层安全加密的密钥数据， (TLS) 连接。 用于媒体加密的密钥是通过 TLS 连接进行交换。 
...
SfBO 使用相互 TLS (MTLS) 在 Microsoft 365 或 Office 365 内进行服务器通信，并使用 TLS 从客户端到服务，使得在给定的聊天可能受到攻击的时段内，很难实现此攻击。 TLS 可对各方执行身份验证，并对所有流量内容进行加密。 这样不能阻止窃听，但攻击者不能读取流量内容，除非破坏加密。
...
此操作对于传输层安全性 (TLS) 连接而言不成问题，因为 TLS 对所有方进行验证并加密所有通信。 使用 TLS 可防止攻击者对特定连接（例如，相互 TLS 连接）执行 IP 地址欺骗攻击。 但攻击者仍可能欺骗 SfBO 使用的 DNS 服务器地址。 但是，由于 SfBO 中的身份验证是使用证书执行的，因此攻击者在通信中没有欺骗其中一方所需的有效证书。
...
SfBO 服务依赖证书进行服务器身份验证，在客户端和服务器之间以及不同的服务器角色之间建立信任链。 Windows Server 公钥基础架构 (PKI) 为建立和验证此信任链提供了基础架构。
证书是数字 ID。 它们通过名称来标识某台服务器并指定其属性。 为确保证书上的信息有效，证书必须由证书颁发机构 (CA) 颁发，该证书受连接到服务器的客户端或其他服务器的信任。 如果服务器仅与专用网络中的其他客户端和服务器建立连接，则 CA 可以是企业 CA。 如果服务器与专用网络外部的实体进行交互，则可能需要公共 CA。
...
- **我的组织中的任何人** &nbsp; &nbsp;租户或租户Microsoft 365 Office 365 SfBO 用户可以加入会议，而无需在大厅中等待，即使这些用户不在通讯组列表中。 所有其他人（包括所有外部和匿名用户）必须在大厅等候，直到被允许参加会议。
