...
ms.locfileid: "44164341"
---
# <a name="expressroute-and-qos-in-skype-for-business-online"></a>Skype for Business Online 中的 ExpressRoute 和 QoS

通过使用 Azure ExpressRoute for Microsoft 365 或 Office 365 和 Skype for Business Online 的专用网络连接，连接到 Microsoft 365 或 Office 365。您的 Skype for business 应用的专用连接将为您提供可靠且可预测的性能，以及从公共 internet 断开的隐私。现在，你可以为 Microsoft 365 或 Office 365 和 Skype for business Online 购买更好的网络连接，从而增加可预测性、商业级的可靠性并随附正常运行时间 SLA。
...
使用 Microsoft 的 ExpressRoute 合作伙伴，您可以通过专用连接连接到云中的各种 Microsoft 365 和 Office 365 应用程序，包括云中的 Skype for Business Online。 但是，Skype for business 的实时语音和视频通信功能需要网络服务，这些服务专门配置为支持这些 Microsoft 365 或 Office 365 实时工作负荷。 这包括具有足够带宽承载所需的流量并且能够支持服务质量 (QoS) 来为用户提供企业级体验的网络。
...
MPLS 提供商提供许多服务类别级别，但是每个服务类别都使用不同的术语进行标识。 你必须与你的提供商密切协作，以确保他们了解你已在[Lync 2010 和2013带宽计算器](https://go.microsoft.com/fwlink/?LinkID=690282)中输入的数据，以及适用于不同 Microsoft 365 和 Office 365 实时工作负荷应用程序的选项。
...
- 将每个网站连接到 ExpressRoute 和 Microsoft 365 或 Office 365 所需的总带宽量。
...
在指定每种角色的设置后，你需要在"站点"工作表中输入每个站点每种角色的用户数。 所有站点的用户总数会自动更新。 由于 Microsoft 365 或 Office 365 位置没有用户，它们应全部输入到工作表的 "分支" 行中。 然后，Lync 2010 和 2013 带宽计算器会填充"尽力而为类别"、"数据流量类别"和"实时流量类别"列。 这显示在下表的数据中。
...
既然您已拥有所需的带宽和服务级别（CoS）的带宽号码，您就可以将订单与所选网络服务提供商进行联系。 不要忘记包括对其他应用程序和服务的流量的估计。 我们提供了其他 Microsoft 365 和 Office 365 服务的网络规划指南，包括 Exchange 和 OneDrive 的带宽计算器。 网络服务提供商的带宽套餐将更高，因为站内流量将需要重新添加。 Lync 2010 和2013带宽计算器仅提供预期流量的估计，因此建议确认网络能够支持执行压力测试的流量量。 
...
Microsoft 的 ExpressRoute 服务提供到 Azure 云的专用连接，但是 Office 365 实时工作负荷的通信服务需要具有足够带宽的网络服务才能携带流量，并支持服务质量（QoS）以提供业务级用户体验。 支持 QoS 的连接必须配置为端到端（电脑、网络交换机和云的路由器），因为无法支持 QoS 的路径中的任何部分可能会降低整个通话的质量。
  
本部分的目的是帮助你了解在 IP 网络中支持实时流量以及使用 Microsoft 的 ExpressRoute Exchange 提供程序或网络服务提供商合作伙伴成功配置和支持 Microsoft 365 或 Office 365 实时工作负荷的成功的 ExpressRoute 部署的挑战。
...
- 在数据包丢失时使语音编码/解码更有弹性。 这可以通过使用正向错误更正（FEC）来更正遇到的错误的百分比，这些错误是 Microsoft 365 或 Office 365 实时传输中的一种功能，或通过设计语音解码系统来尝试掩盖丢失了 Microsoft 编解码器特征的数据包的影响。 
...
这提供了高可用性（99.9% 的正常运行时间 SLA）和可靠的路由（它是安全的（无 internet 传输），不受 Internet 通信变体的影响，并尊重通信的服务质量标记（如下所述）。 ExpressRoute 和精心规划的 WAN 可为你提供商业级语音网络。
  
对于连接到电路的办公室或数据中心（如果是混合拓扑），您可以使用 ExpressRoute 进行数据传输。 非现场用户（例如，家庭办公室或旅行等）的数据不会利用 ExpressRoute 线路，除非用户已连接 VPN 且不需要包含在用于调整 ExpressRoute 线路大小的带宽评估中。 如果你是多国客户，则可以在每个区域中购买 ExpressRoute 线路，并使用 BGP 社区标记来通知路由规则，以便流量定向到首选的 ExpressRoute 线路（通常是每个站点最接近的），而其他线路则在影响单个线路的停电时提供冗余。 
...
在 Wi-fi 网络中，所有与接入点相关联的客户共享一个半双工频道（即，一次只能发送一个客户端工作站或接入点）。 为了最大程度地减少无线电频道上发生冲突的可能性，在发送帧以使频道在指定时间段内被称为 "帧间距" 的情况下，如果频道占线，则会在随机时间段内退出。 发送帧后，如果发件人未收到来自接收方的确认消息，则它会假定发生冲突或其他失败，并返回随机间隔，然后再尝试访问要重新发送的收音机频道。 后退间隔是随机的，以降低同一两个工作站再次发生冲突的可能性。
...
多协议标签交换（MPLS）是为网络服务提供商提供的一种方法，为网络服务提供商提供了一种可保证延迟、抖动和数据包丢失性能的 IP 服务。 为提供这些性能保证，MPLS 将从传统 IP 中提取一些 unpredictability。 第一，而不是让每个数据包找到其目标的路由器到路由器的方式（这可能是每个数据包都采用不同于源的路由的方式），MPLS 通过一个称为标签交换路径（LSP）的固定路由来路由 "虚拟线路" 连接上的所有数据包。 如果该路径中的其中一个链接失败，则使用该链接的所有 Lsp 都将快速重新路由。
...
MPLS 提供程序提供多种服务渐变类，这些渐变类很遗憾使用不同术语来标识它们。 你必须与你的提供商密切合作，确保他们理解[Lync 2010 和2013带宽计算器](https://go.microsoft.com/fwlink/?LinkID=690282)中的输出，以及不同 Microsoft 365 或 Office 365 实时工作负荷应用程序的推荐选项。
  
## <a name="conclusion"></a>结论

Skype for Business 增强了业务通信的开展方式。 Skype for business 可将所有这些功能放在单个用户界面中，而不是将电话连接到 PBX、独立的视频会议系统、单独的电子邮件平台、音频会议的外部服务以及 IM 和联机状态的一些车辆。
...
有效的商业工具必须以稳定、一致的方式执行，并提供可鼓励用户采纳的用户体验。 从网络角度来看，让网络基础结构（包括本地和广域区域、固定和移动）能够允许发生这种情况。 规划、设计、实施和维护基础结构不一定是一种简单的特征。 目前可以完成的硬件、工具和网络服务，但 it 专业人员有责任查看他们是否已设计、实施和维护，从而确保用户获得一组通信和协作服务，以确保用户能够高效高效地工作，并且组织可以获得此技术提供的全部好处。 
