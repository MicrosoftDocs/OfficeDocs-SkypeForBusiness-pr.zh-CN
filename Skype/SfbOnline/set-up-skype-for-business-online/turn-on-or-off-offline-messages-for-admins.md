...
您可以向您的联系人的业务 IMs 为发送 Skype 即使没有登录。 此功能可让你的联系人知道你已尝试联系他们。 你不必等待直到有人在线才发送消息给他们。 
...
要启用或禁用用户脱机邮件发送脱机消息，请将_EnableIMAutoArchiving_设置为`True`或`False`。 默认情况下设置为  `True`。 您可以使用现有策略或创建一个类似于下面的示例。
