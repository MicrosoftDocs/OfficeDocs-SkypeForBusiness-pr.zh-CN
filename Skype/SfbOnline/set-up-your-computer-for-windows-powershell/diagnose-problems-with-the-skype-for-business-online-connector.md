...
最新版本的 Skype for Business Online 连接器模块使用新式身份验证，但基础 Windows 远程管理 (WinRM) 客户端必须配置为允许基本身份验证。  新式身份验证使用通常在 *Authorization： Bearer* 标头中传递的 bearer 令牌。 Windows PowerShell Skype for Business PowerShell 的构建方式不允许操作此标头。  相反，Skype for Business PowerShell 使用 *Authorization： Basic 标头* 来传递 bearer 令牌。

有关如何 [启用](./download-and-install-windows-powershell-5-1.md) WinRM Windows PowerShell身份验证的说明，请参阅下载并安装。
...
使用 PowerShell 管理 Skype for Business Online 的先决条件之一是安装 Microsoft Online Services 登录助手。 如果未安装登录助手，当您尝试与 Skype for Business Online 建立远程会话时，您将收到以下错误消息：
...
无法与Skype for Business Online 建立远程 PowerShell 连接，除非你是租户组管理员组。 如果不是，连接尝试会失败，并且将收到以下错误消息：

- 错误 *：New-PSSession： [admin.vdomain.com] 处理来自远程服务器 admin.vdomain.com 的数据失败并出现以下错误消息：用户"user@foo.com"无权管理此租户。可以通过将用户分配到相应的 RBAC 角色来授予权限。有关详细信息，请参阅 [远程故障排除](/powershell/module/microsoft.powershell.core/about/about_remote_troubleshooting )。*

- **解决方法**：如果您认为自己是或应该是租户组的成员管理员组，则需要联系 Microsoft 支持部门。
...
尽管允许每个管理员同时与 Skype for Business Online 租户建立三个连接，但不允许单个租户同时具有 20 多个连接。 例如，六个管理员可能各自有三个打开的会话。 如果第四个管理员尝试建立两 (连接，导致总共 21 个) 连接，则此尝试会失败，并出现以下错误消息：
