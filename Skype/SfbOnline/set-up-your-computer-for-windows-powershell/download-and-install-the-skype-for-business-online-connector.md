...
安装程序会将 Skype for Business Online 连接器模块 (以及**CsOnlineSession** cmdlet) 复制到你的计算机。 若要访问该模块, 请在 "管理员凭据" 下启动 Windows PowerShell 会话, 然后运行以下命令:
...
```
 ...
 出现 "记事本" 时, 将以下行添加到配置文件中已存在的命令底部 (如果有):
