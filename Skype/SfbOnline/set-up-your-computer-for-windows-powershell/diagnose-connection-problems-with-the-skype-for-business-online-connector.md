...
PowerShell 执行策略可帮助确定哪些配置文件可以加载到 PowerShell 控制台中，以及用户可以从该控制台运行哪些脚本。 除非执行策略已设置为 RemoteSigned，否则Skype for Business连接线连接器模块。 如果尚未导入，在尝试导入模块时，将收到以下错误消息：
...
- **解决方法**：此错误通常意味着Microsoft Online Services助手未运行。 可以通过从 PowerShell 提示符运行以下命令来验证此服务的状态： 
    ...
    如果服务正在运行，则可能会遇到计算机与 Microsoft Live ID 身份验证服务器之间的网络连接问题。 若要检查此状态，请Internet Explorer并导航到[ https://login.microsoftonline.com/ 。](https://login.microsoftonline.com/.) 尝试登录以Microsoft 365或Office 365登录。 如果无法连接，可能遇到网络连接问题。
...
尽管每个管理员可以同时与一个 Skype for Business Online 租户建立三个连接，但单个租户不能同时具有 20 多个连接。 例如，六个管理员可能各自有三个打开的会话。 如果第七个管理员尝试打开两 (连接，导致总共 21 个同时连接) ，则此尝试将失败，并出现以下错误消息：
