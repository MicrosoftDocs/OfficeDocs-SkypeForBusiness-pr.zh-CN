...
> [!NOTE]
> 你不能使用 **Skype for Business 管理中心**分配的许可证。 你必须使用 Office 365 管理中心。 请参阅[分配 Skype for Business 许可证](../skype-for-business-and-microsoft-teams-add-on-licensing/assign-skype-for-business-and-microsoft-teams-licenses.md)。
...
4. 单击 **保存**。
    ...
    你还可以通过进入用户的音频会议属性并单击**通过电子邮件发送会议信息**向拥有音频会议设置的用户发送电子邮件。 会议 ID 和默认音频会议电话号码包括在会议邀请上而不是 PIN 上。

    请参阅[使用其音频会议信息向用户发送电子邮件](send-an-email-to-a-user-with-their-dial-in-information.md)。
...
你可以对自动发送给用户的电子邮件进行更改，包括实际的电子邮件地址和发件人联系信息的显示名称。 默认情况下，此电子邮件将显示为发自 Office 365，但你可以使用 Windows PowerShell 和 [Set-CsOnlineDialInConferencingTenantSettings](https://go.microsoft.com/fwlink/?LinkId=627285) cmdlet 更改电子邮件地址和显示名称。 若要更改向用户发送电子邮件的电子邮件地址，你必须执行以下操作：
...
请参阅 [在音频会议设置更改时自动向用户发送电子邮件](emails-sent-to-users-when-their-settings-change.md)。
...
用户安排的每次会议将分配到一个唯一的会议 ID。 虽然会议 ID 是自动创建并分配给用户的，但有时用户不希望使用此标识，你又希望将它设置为特定数字，或者你的用户无法记住或者已丢失了其会议 ID。 可以使用 Skype for Business 管理中心和 Windows PowerShell 来查看、更改和重置其会议 ID。
...
3. 单击 **用户**，然后选择你要重置 PIN 的用户。

4. 在操作窗格中，在**PIN**下单击 **重置**。

当用户启用音频会议或者重置 PIN 时，用户将收到包含 PIN 的电子邮件。 但如果你已禁用自动发送电子邮件，则不会向用户发送 PIN 重置电子邮件，并且你必须手动将 PIN 发送给用户。 PIN 将仅在重置后显示一次。 PIN 在重置后即会显示，PIN 不会在用户属性上显示，而是会显示 *****。
...
3. 在 **Skype for Business 管理中心**中的左侧导航中，转到 **音频会议** > **Microsoft 桥接设置**。

4. 在**加入会议体验**下，选择下列操作：

  - **启用会议进入和退出通知**此选项默认情况下选中。 如果清除此复选框，默认情况下已加入会议的用户将不会在某人进入或离开会议时收到通知。

    当用户使用 Skype for Business 应用程序加入会议时，可以在会议的基础上设置这一功能，并且他们可以在 Skype 会议的**选项**菜单中修改**当人员进入或离开时发出通知**设置。

  - **要求呼叫者在加入会议之前记录其姓名**此选项默认情况下选中。 如果清除此复选框，呼叫者在加入会议之前不需要记录其姓名。

5. 完成更改后，单击 **保存**。
    
请参阅[更改音频会议桥的设置](/MicrosoftTeams/change-the-settings-for-an-audio-conferencing-bridge)。
...
4. 在**安全**下，在 **PIN 长度**列表中输入所需的 PIN 的位数，然后单击 **保存**。
...
4. 单击 **保存**。
    ...
    您还可以通过进入用户的音频会议属性并单击**通过电子邮件发送会议信息**向拥有音频会议设置的用户发送电子邮件。
...
4. 从列表中选择一个电话号码，单击操作窗格中的 **设置语言**，然后在 **设置语言**页上，单击使用 **主要语言**列表，以查看受支持的语言的完整列表。

    您还可以设置选择 Microsoft 作为音频会议提供程序时所支持的主语言和辅助语。 您在列表中选择的顺序与将语言呈现给调用方的顺序相同。
...
3. 在**Skype for Business 管理中心**中的左侧导航中，转到 **音频会议** > **Microsoft 桥接**。 您可以在这里：

  - 查看由 Office 365 设置以用于音频会议的电话号码。

  - 查看将由音频会议自动助理使用的位置和主要和辅助语言。

  - 选择在启用音频会议时将提供给用户的默认电话号码。 但是，如果电话拨入式会议网桥的默认电话号码发生更改，现有用户的默认电话号码不会改变。

您可以转到 **音频会议** > **用户**，选择用户的属性，通过从组织中可用的数字列表中选择一个新号码来更改用户的默认编号。

请参阅 [查看音频会议号码列表](see-a-list-of-audio-conferencing-numbers.md)。

## <a name="see-a-list-of-users-that-are-enabled"></a>查看启用的用户列表

1. 使用你的工作或学校帐户登录 Office 365。

2. 转到**Office 365 管理中心** > **Skype for Business**。

3. 在 **Skype for Business 管理中心**，在左侧导航窗格中，转到 **音频会议**，然后转到**用户**。

请参阅[查看启用了音频会议的用户列表](see-a-list-of-users-that-are-enabled-for-audio-conferencing.md)。

## <a name="want-to-know-how-to-manage-with-windows-powershell"></a>想知道如何使用 Windows PowerShell 进行管理吗？

可以使用 Windows PowerShell 在组织级别管理多个设置。 这便于将设置应用于所有用户。

要获得有关每个 cmdlet 的更多帮助，请参阅 [Skype for Business Online cmdlets](https://go.microsoft.com/fwlink/?LinkId=627324)。
