...
ms.custom:
- Audio Conferencing
description: 会议迁移服务 (MMS) 是在后台运行的服务，可Skype for Business Microsoft Teams会议。
...
默认情况下，MMS 会在每种情况下自动触发。 此外，管理员可以使用 PowerShell cmdlet 为给定用户手动触发会议迁移。
...
2. 根据在用户的邮箱中发现的信息，它会在 Teams 中更新或安排新会议，具体取决于具体方案。
3. 在电子邮件中，它将替换会议详细信息中的联机会议块。
4. 它代表会议组织者将会议的更新版本发送给所有会议收件人。 会议被邀请者将收到其电子邮件中更新的会议坐标的会议更新。 

    ![MMS 更新的会议块。](../images/210a03ee-30c1-46f3-808f-4c2ebdaa3ea1.png)
...
- 仅Skype for Business Microsoft Teams Web 上单击 Outlook 中的"添加 Skype 会议"按钮或者使用 Outlook 的 Skype 会议 加载项安排的 Skype 会议 或 Outlook 会议。 如果用户将联机会议信息从一Skype复制并粘贴到新会议，该新会议将不会更新，因为原始服务中没有任何会议。
...
这是 MMS 可帮助为用户创建更平滑过渡的最常见方案。 如果不进行会议迁移，则由本地用户Skype for Business Server的现有会议在用户联机移动后将不再有效。 因此，使用本地管理工具 (或管理控制面板) 将用户移动到云时，现有会议会自动移动到云并转换为 `Move-CsUser` TeamsOnly。 

如果在将用户移动到云之前为用户分配了音频会议许可证，将创建具有拨入坐标的会议。 如果将用户从本地移动到云，并且希望该用户使用音频会议，我们建议在移动用户之前先分配音频会议，以便仅触发 1 个会议迁移。
...
默认情况下，当向用户授予 具有 或 的实例时，会自动 `TeamsUpgradePolicy` 触发 `mode=TeamsOnly` 会议迁移 `mode= SfBWithTeamsCollabAndMeetings` 。 如果在授予上述任一模式时不想迁移会议，则使用 PowerShell) 时，在 (中指定 ，或者取消选中设置用户共存模式 (（如果使用 Teams 管理门户 `MigrateMeetingsToTeams $false` `Grant-CsTeamsUpgradePolicy`) ）时迁移会议的框。
...
- 授予 TeamsOnly 或 SfBWithTeamsCollabAndMeetings Teams不会将现有会议转换为Skype for Business会议。
...
- 使用 `TargetMeetingType Current` 指定会议Skype for Business保留Skype for Business，Teams保留Teams会议。 但是，音频会议坐标可能会更改，任何本地会议Skype for Business迁移到 Skype for Business Online。 这是 TargetMeetingType 的默认值。
- 使用 指定必须将任何现有会议迁移到 Teams，无论会议是托管在 Skype for Business 在线还是本地，以及是否需要任何音频会议更新。 `TargetMeetingType Teams` 
...
1. 确定受影响的用户。运行以下命令获取受影响的用户列表，以及报告的具体错误：
    ...
    ```
2. 对于每个受影响的用户，请查看 LastMessage 属性的值，以确定会议迁移失败的原因以及要采取的纠正措施。 采取纠正措施后，使用 PowerShell cmldet 为受影响的用户重新触发会议 `Start-CsExMeetingMigration` 迁移，如上所述。 

3. 如果迁移仍不起作用，可以使用两个选项：
