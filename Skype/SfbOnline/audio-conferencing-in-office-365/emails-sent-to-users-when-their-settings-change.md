...
默认情况下，有四种类型的电子邮件将发送给您的用户启用了音频会议。 但是，如果要限制向用户发送的电子邮件数，你可以将其关闭。 Office 365 中的音频会议会将电子邮件发送到用户的电子邮件时：
...
- **会议 ID 或用户默认会议电话号码更改。**
    ...
    此电子邮件包括会议 ID、默认会议电话号码以及使用 Skype for Business Online 会议更新工具为用户更新现有会议的说明和链接。 但这封电子邮件中不包含用户的音频会议针。 请参阅[重置用户的会议 ID](reset-a-conference-id-for-a-user.md)。
