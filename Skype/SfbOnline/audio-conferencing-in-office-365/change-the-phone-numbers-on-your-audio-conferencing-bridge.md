...
4. 在" **分配**"页面上，单击" **保存**"。
    ...
    > [!NOTE]
    > [!注释] 分配了新电话号码后，即使该号码已经是新默认号码，现有用户的默认号码也不会更改。 若要设置默认收费或免费电话号码添加到组织者的会议邀请，请参阅[设置数字包括在邀请电话](set-the-phone-numbers-included-on-invites.md)。 
...
用户的默认电话是他们安排会议时在会议邀请中包含的号码。 有关详细信息，请参阅[设置数字包括在邀请电话](set-the-phone-numbers-included-on-invites.md)。
...
使用会议迁移服务，您可以选择更新之前它们默认的电话号码已更改已发送到您组织中的用户的会议邀请。 有关更多信息，请参阅[设置会议迁移服务 (MMS)](setting-up-the-meeting-migration-service-mms.md)。
  
- 运行已在第 2 步中更改其默认电话号码的用户会议迁移服务 (MMS)。 要执行此操作，请运行以下命令：
...
取消为会议网桥分配的服务电话号码时，用户将不能再使用该电话号码加入任何会议。 因为更改电话号码，务必来更新所有可能有的电话号码作为其默认编号 （如果有的话） 的用户，并更新现有电话号码的音频会议桥取消分配之前会议的邀请。 
  
如果删除电话号码时没有通知用户及更新其会议，那么他们的现有会议邀请可能包含无法再用于加入其会议的电话号码。
  
对于前三步，需要启动 Windows PowerShell。 若要了解如何执行此操作，请单击[想要了解如何使用 Windows PowerShell 管理？](change-the-phone-numbers-on-your-audio-conferencing-bridge.md#bkPowerShell)
