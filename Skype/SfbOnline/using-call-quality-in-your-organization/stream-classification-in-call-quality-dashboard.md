...
|**指标**|**条件**|**解释**|
|:-----|:-----|:-----|
|音频降级平均值|> 1.0|关于流降级的网络平均意见得分的平均值。 表示网络损耗和抖动对所收到的音频质量的影响程度。|
|往返|> 500|平均网络传播往返时间根据 RFC3550 中的规定计算，以毫秒为单位。|
|丟包率|> 0.1|流的平均丢包率。|
|抖动|> 30|流的平均抖动值，以毫秒为单位。|
|隐藏样本平均比率|> 0.07|含有丢包修复生成隐藏样本的音频帧数与总音频帧数的平均比率。|
...
|:-----|:-----|:-----|:-----|:-----|:-----|:-----|
|1|视频本地帧丢失百分比平均值|> 50% |差|好|继续前往步驟 2|向用户显示的视频帧丢失平均百分比。 其中包括从网络丢失恢复的帧。|
|2|视频帧平均速率|< 7|差|好|继续前往步驟 3|在会话持续期间计算到的视频流每秒接收的平均帧数。|
|3|视频文章 FECPLR|> 0.15|差|好|未经分类|应用 FEC 后，跨所有视频流和编解码器汇总的丢包率。|
...
|:-----|:-----|:-----|:-----|:-----|:-----|:-----|
|1|视频本地帧丢失百分比平均值|> 50% |差|好|继续前往步驟 2|向用户显示的视频帧丢失平均百分比。 其中包括从网络丢失恢复的帧。|
|2|视频帧平均速率|<2|差|好|继续前往步驟 3|在会话持续期间计算到的视频流每秒接收的平均帧数。|
|3|视频文章 FECPLR|> 0.15|差|好|未经分类|应用 FEC 后，跨所有视频流和编解码器汇总的丢包率。|
...
**指标**|**条件**|**解释**|
|:-----|:-----|:-----|
|损坏的图块的总百分比|> 36|没有被发送到远程对等点（例如，从 MCU 到查看器）而被丢弃的图块的百分比。 丢弃（或损坏）图块可能是由于客户端与服务器之间的带宽限制所致。|
|AppSharing RDP 图块处理平均延迟|> 400|在会议服务器上处理 RDP 堆栈图块的平均延迟，以毫秒为单位。|
|AppSharing 相对单向平均值|> 1.75|端点之间的应用程序共享流的相对单向平均延迟，以秒为单位。|
