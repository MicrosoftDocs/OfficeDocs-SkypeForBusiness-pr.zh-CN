---
title: "未启用此 Skype for Business Online 功能"
ms.author: TONYSMIT
author: tonysmit
manager: scotv
ms.date: 11/21/2017
ms.audience: Admin
ms.topic: troubleshooting
f1_keywords:
- ms.lync.lac.FeatureNotEnabled
ms.prod: office-online-server
localization_priority: Normal
ms.collection: Adm_Skype4B_Online
ms.custom: Adm_O365_FullSet
ms.assetid: e95a7d09-0c45-4b95-956e-f099b7b0e659
description: "Find possible causes and action suggestions when you get a Skype for Buisness Online feature isn't enabled error. "
---

# 未启用此 Skype for Business Online 功能

> [!重要信息]
> 本文是由机器翻译的，请参阅[免责声明](e95a7d09-0c45-4b95-956e-f099b7b0e659.md#MT_Footer)。请在 [此处](https://support.office.com/en-us/article/e95a7d09-0c45-4b95-956e-f099b7b0e659) 中查找本文的英文版本以便参考。
  
非常抱歉，该用户可能未获授权，无法使用此功能。
  
|**可能的原因**|**建议的操作**|
|:-----|:-----|
|您有可用的许可证，但尚未启用此用户的功能。  <br/> |转到 **Office 365 管理中心**> **活动用户**许可证此用户。  <br/> |
|用户所在的位置不可使用此功能。  <br/> |如果可能，请转到 **Office 365 管理中心**> **活动用户**并将该用户分配给该功能可用的位置。  <br/> |
|尚未为您的组织启用该功能。  <br/> |转到" **Office 365 管理中心**">" **Skype for Business**"并为整个组织启用该功能。  <br/> |
   
## 
<a name="MT_Footer"> </a>

> [!注释]
> **机器翻译免责声明**：本文是由无人工介入的计算机系统翻译的。Microsoft 提供机器翻译是为了帮助非英语国家/地区用户方便阅读有关 Microsoft 产品、服务和技术的内容。由于机器翻译的原因，本文可能包含词汇、语法或文法方面的错误。 
  

