---
title: "致电 office 365 计划"
ms.author: tonysmit
author: tonysmit
manager: scotv
ms.date: 11/22/2017
ms.audience: Admin
ms.topic: overview
f1_keywords:
- O365E_E5Voiceplandetails
- O365M_E5Voiceplandetails
- O365P_E5Voiceplandetails
ms.prod: office-online-server
localization_priority: Normal
ms.collection:
- Adm_Skype4B_Online
- Adm_UI_Elements
ms.custom:
- Adm_O365_FullSet
- DianeF_Adm_Simplified
- LIL_Placement
- Strat_SB_PSTN
ms.assetid: f47c6a97-bc8b-42e6-b5d4-ce6b41ed1918

description: "Learn about Calling Plan (PSTN Calling plan) for Skype for business options and how to get licenses for your organization. "
---

# 致电 office 365 计划

> [!IMPORTANT]
> 本文是由机器翻译的，请参阅[免责声明](f47c6a97-bc8b-42e6-b5d4-ce6b41ed1918.md#MT_Footer)。请在 [此处](https://support.office.com/en-us/article/f47c6a97-bc8b-42e6-b5d4-ce6b41ed1918) 中查找本文的英文版本以便参考。
  
呼叫转接到其他Skype for Business和 Microsoft 团队用户是免费的但如果您希望用户能够呼叫常规电话，并且您还没有服务提供商进行语音呼叫，您需要购买呼叫计划。[什么是致电 Office 365 中计划？](../what-are-calling-plans-in-office-365/what-are-calling-plans-in-office-365.md)
  
下面是调用计划选项：
  
- **国内拨规划** ： 许可的用户可以拨国家/地区的数字在哪里分配 Office 365 中。
    
- **国内和国际呼叫计划** ： 许可的用户可以拨数字国家/地区的位置，其 Office 365 许可证分配给用户基于用户的位置和[Users can make outbound calls to these countries and regions](../countries-and-region-availability-for-audio-conferencing-and-calling-plans/users-can-make-outbound-calls-to-these-countries-and-regions.md).
    
对于每个组织和每个国家/地区可用的每月分钟位于[国家和地区和的可用性音频会议呼叫计划](../countries-and-region-availability-for-audio-conferencing-and-calling-plans/countries-and-region-availability-for-audio-conferencing-and-calling-plans.md)。
  
> [!IMPORTANT]
> 国家/地区基于用户的许可证，在 **Office 365 管理中心中**的位置 > **活动用户**和 **未** 列出在 **Office 365 管理中心**中的 **组织配置文件**下的帐单地址。 > ![User's license location](../images/cc1e16d1-8a5e-43e0-99a3-dc991efdfbab.png)
  
有关使用限制和使用条款的详细信息，请参阅[音频会议免费拨出时间段](../accessibility-and-regulatory/audio-conferencing-complimentary-dial-out-period.md)。
  
## 如何购买调用计划

1. **您必须先购买 **电话系统**加载项许可证** 。若要执行此操作，[登录到 Office 365 管理中心中](https://portal.office.com/adminportal/home?add=sub&amp;adminportal=1#/catalog) ，然后选择 **帐单**> **购买服务**> **加载项订阅**> **立即购买**。
    
    ![When you buy your Cloud PBX licenses, you'll see an option to buy a voice calling plan.](../images/5893fca0-292c-4cdf-9b43-c507a8b44b74.png)
  
    > [!NOTE]
    > 根据您的计划，您可能需要购买更多的加载项，您可以购买电话系统许可证之前。若要了解详细信息，请参阅[我需要购买哪些附加许可证才能获得更多功能？](skype-for-business-and-microsoft-teams-add-on-licensing.md#bkmk_whichaddons)
  
2. 购买电话系统许可证后，您可以通过登录到 **Office 365 管理中心**购买调用规划、 选择 **帐单**> **购买服务**> **加载项订阅**> 单击 **立即购买**，您会看到那里呼叫计划。
    
    ![After you buy a Cloud PBX license, you can buy voice calling plans.](../images/ab2d6dce-56eb-4bbc-ac1a-430b0c065d18.png)
  
您可以购买并分配给其他用户，具体取决于您的组织的需求的不同呼叫计划。调用规划您需要选择后，请继续执行签出。在 Office 365 管理中心中每个用户分配一个计划。若要了解如何，请参阅[分配 Skype for Business 和 Microsoft 团队合作的用户许可证](assign-skype-for-business-and-microsoft-teams-licenses.md)。
  
## 您是否已为混合用户提供本地 PSTN 连接的服务提供商？

如果是这样，您不需要购买呼叫计划。Office 365 企业版 E5 包含 **电话系统**加载项，以便您可以继续来到签出。
  
然后，为在 Office 365 管理中心中的用户分配企业 E5 或 **电话系统**的加载项许可证。若要了解如何，请参阅[分配 Skype for Business 和 Microsoft 团队合作的用户许可证](assign-skype-for-business-and-microsoft-teams-licenses.md)。
  
## 定价信息

- [用于呼叫计划定价](https://go.microsoft.com/fwlink/?LinkId=799761)
    
- [电话系统定价](https://go.microsoft.com/fwlink/?linkid=799763)
    
- [定价的音频会议](https://go.microsoft.com/fwlink/?linkid=799762)
    
## 有关详细信息

下面是解释如何设置呼叫计划的更多文章：
  
- [设置呼叫计划](../what-are-calling-plans-in-office-365/set-up-calling-plans.md)
    
- [Skype for Business 和 Microsoft 团队许可加载项](skype-for-business-and-microsoft-teams-add-on-licensing.md)
    
- [规划 Skype for Business 云连接线版本](https://technet.microsoft.com/EN-US/library/Mt605227.aspx)： 这篇文章介绍如何设置 Office 365 电话系统与 PSTN 连接。
    
- [规划 Office 365 (云 PBX) 解决方案中电话系统](https://go.microsoft.com/fwlink/p/?LinkId=717926)
    
## 

||
|:-----|
|![领英学习快捷图标。](../images/7e5cb7c8-dc66-4c9a-a16d-a30f10a970bd.png) **刚开始使用 Office 365？**         探索由 LinkedIn Learning 提供的适用于 **Office 365 管理员和 IT 专业人士**的免费视频课程。 |
   
## 相关主题

[设置 Skype for Business Online](../set-up-skype-for-business-online/set-up-skype-for-business-online.md)
  
## 
<a name="MT_Footer"> </a>

> [!NOTE]
> **机器翻译免责声明**：本文是由无人工介入的计算机系统翻译的。Microsoft 提供机器翻译是为了帮助非英语国家/地区用户方便阅读有关 Microsoft 产品、服务和技术的内容。由于机器翻译的原因，本文可能包含词汇、语法或文法方面的错误。 
  

