...
4. 清除 "**为此 Edge 池启用联盟（端口5061）** " 复选框，然后选择 **"确定"** 关闭页面。 
...
4. 选中 "为**此 Edge 池启用联盟（端口5061）**" 复选框，然后单击 **"确定"** 关闭页面。 
  
5. 从 "**操作**" 菜单中，选择 "**发布拓扑**"，然后单击 "**下一步**"。
    
6. **发布向导**完成后，单击 "**完成**" 关闭向导。 
    
7. 验证在拓扑结构生成器中是否已将 "**联盟" （端口5061）** 设置为 "**已启用**"。
...
3. 在 "**常规**" 页面上，验证已选中 "**为此 Edge 池启用联盟（端口5061）** " 复选框。 
...
2. 更新外部防火墙路由规则或硬件负载平衡器设置，以将外部访问（通常为端口443）和联盟（通常为端口5061）的 SIP 流量发送到 Skype for business Server 2019 Edge 服务器，而不是旧版边缘服务器。
