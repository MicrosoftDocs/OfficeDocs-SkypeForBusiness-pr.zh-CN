...
使用以下过程为单站点部署将联盟路由和媒体流量路由从以前版本的边缘服务器和控制器转换到 Skype for Business Server 2019 边缘服务器。
...
> [!IMPORTANT]
> 如果将旧边缘服务器配置为对访问边缘服务、Web 会议边缘服务和 A/V 边缘服务使用相同的 FQDN，则本节中的过程不受支持。 如果旧边缘服务配置为使用相同的 FQDN，则必须先迁移所有用户，然后在 Skype for Business Server 2019 边缘服务器上启用联盟之前停用以前版本的边缘服务器。 
...
## <a name="to-remove-the-legacy-federation-association-from-skype-for-business-server-2019-sites"></a>从 2019 网站中删除旧Skype for Business Server关联
...
3. 将访问代理外部证书分配给Skype for Business Server 2019 外部接口。
...
3. 在"**常规"** 页面的"下一 **个** 跃点选择"下，从下拉列表中选择"Skype for Business Server 2019 池"。
...
3. 稍待片刻以便在部署中的所有池间执行 Active Directory 复制。
    ...
    > [!NOTE]
    > 您可能会看到以下消息："**警告： 拓扑包含多个联盟边缘服务器"。在迁移到产品的更新版本期间可能发生此情况。在这种情况下，只有一台边缘服务器会主动用于联盟。验证外部 DNS SRV 记录是否指向正确的边缘服务器。如果要部署多个联盟边缘服务器以同时 (（而不是迁移方案) ，请验证所有联盟伙伴是否都Skype for Business Server。验证外部 DNS SRV 记录是否列出了所有启用联盟的边缘服务器。** 该警告是预期警告，可以放心地忽略。 
...
2. 更新外部防火墙路由规则或硬件负载平衡器设置以发送外部访问的 SIP 流量 (通常端口 443) 和联盟 (通常将 5061) 移植到 Skype for Business Server 2019 边缘服务器，而不是旧边缘服务器。
    ...
    > [!NOTE]
    > 如果您没有硬件负载平衡器，则需要更新联盟的 DNS A 记录，以解析为新的 Skype for Business Server访问边缘服务器。 若要以最少的中断完成此操作，请减小外部 Skype for Business Server 访问边缘 FQDN 的 TLL 值，以便当 DNS 更新为指向新的 Skype for Business Server 访问边缘时，联盟和远程访问将快速更新。 
