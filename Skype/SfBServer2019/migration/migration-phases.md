...
在 Skype for Business Server 2019 中，您在网络上定义包含 Skype for Business Server 2019 组件的网站。 站点是一组通过高速度、低延迟网络（例如单个局域网 (LAN) 或由高速光纤网络连接的两个网络）正确连接的计算机。 
  
前端池 是一组配置相同的前端服务器，这些服务器协同工作来为公用组用户提供服务。 池为用户提供可伸缩性和故障转移功能。 池中的每台服务器必须运行一个或多个相同的服务器角色。 适用于小型组织的 Standard Edition 服务器还定义了一个池，并在一台服务器上运行。 这使您可以将 Skype for Business Server 2019 功能设置为较低的成本，但不提供真正的高可用性解决方案。 
  
以下阶段介绍了迁移到 Skype for business Server 2019 的池的过程。 对于包含多个池的多个站点，每个池都应遵循此分阶段方法。
  
1. [第 1 阶段：规划迁移](phase-1-plan-your-migration.md)
    
2. [第 2 阶段：准备迁移](phase-2-prepare-for-migration.md)
    
3. [第3阶段：部署 Skype for Business Server 2019 试点池](phase-3-deploy-pilot-pool.md)
    
4. [第4阶段：将测试用户移动到引导池](phase-4-move-test-users-to-the-pilot-pool.md)
    
5. [第5阶段：将 Skype for Business Server 2019 边缘服务器添加到引导池](phase-5-add-edge-server-to-pilot-pool.md)
