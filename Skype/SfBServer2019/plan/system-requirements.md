...
## <a name="hardware-for-skype-for-business-server-2019"></a>2019 Skype for Business Server的硬件
<a name="Hardware"> </a>
...
|网络  <br/> |1 个双端口网络适配器，1 Gbps 或更高 (2 个网络适配器，但它们需要与一个 MAC 地址和一个 IP 地址) 。  <br/> 视频互操作服务器 **和控制器不支持** 双主或多主配置。 <br/> 边缘服务器将需要两个网络接口，即两个双端口网络适配器（1 Gbps 或更高 (）或两个成对网络适配器，总共 4 个，每个对与一个 MAC 地址和一个 IP 地址组合在一起，总共两对) 。  <br/> 在独立中介服务器上，支持安装其他网络接口 (NIC) 允许配置特定的 PSTN IP 地址。  <br/> |
...
您需要为运行 2019 年 2019 的任何服务器安装或配置Skype for Business Server一些操作。 下面列出了这些要求，后跟特定服务器角色的其他要求。
...
|Windows Media Format Runtime  <br/> | 对于Windows Server 2016，你需要在服务器管理器中安装 **媒体****基础功能**。 实际上，无需此Skype for Business Server即可启动 Skype for Business Server 2019 安装，但系统会提示您安装它，然后在继续安装 Skype for Business Server 2019 之前重新启动服务器。 最好提前完成。 <br/> |
...
Skype for Business Server 2019 Enterprise Edition需要完整SQL Server，如下所示， (64 位版本;请勿使用 32 位版本) ：
  
||||
|:-----|:-----|:-----|
|Microsoft SQL Server 2019 (64 位) 版本，必须运行最新更新。  <br/> |Microsoft SQL Server 2017 (64 位) 版本，并且必须使用最新更新运行。  <br/> |
Microsoft SQL Server 2016 (64 位) ，并且必须使用最新更新运行。|
 |
...
尽管服务器和服务大部分配置数据存储在 Skype for Business Server 2019 中央管理存储中，但 Active Directory 中仍存储有一些内容：
...
我们提供了图表和说明，可帮助您确定您的环境中具有的拓扑，或在安装 2019 年 2 月之前可能需要Skype for Business Server项。 为简单一点，我们还包括一个密钥：
...
此图同样显示一个林，但它也有一个或多个子域 (此特定示例中有三个子) 。 因此，在 中创建用户的域可能不同于部署 2019 Skype for Business Server域。 为什么担心这一点？ 请记住，在部署前端Skype for Business Server时，该池中的所有服务器都需要位于单个域中。 可以通过通用管理员组Skype for Business Server跨Windows管理。
...
这如何工作？ 目录同步产品 (Forefront Identity Manager 或 FIM) 管理您组织的用户帐户的一直存在。 在林中创建或删除帐户时，该更改将同步到中央林中的相应联系人。
...
若要在此类型的拓扑中部署 Skype for Business Server 2019，可以在资源林中为用户林中的每个用户帐户创建一个已禁用的用户对象 (如果环境中已有 Microsoft Exchange Server，则此操作可能针对) 。 然后，你需要目录同步工具 (Forefront Identity Manager 或 FIM) 来管理用户帐户的生命周期。
...
绝对不要使用 Unicode 字符或下划线。 外部 DNS 和公共证书颁发机构支持标准字符 (包括 A-Z、a-z、0-9 和连字符)  (您需要将 FQDN 分配给证书中的 SN，记住) 很重要，因此，如果您从一开始就牢记这一点，您将可以省去许多麻烦。
...
- 2019 年 10 月运行的内部服务器Skype for Business Server自动注册。
...
当 2019 年 2019 Skype for Business Server其他应用程序和服务器（如服务器）通信时，还需要证书Microsoft Exchange Server。 显然，这应该是其他这些应用和服务器可以支持的方式使用的证书。 Skype for Business Server 2019 和其他 Microsoft 产品支持 Open Authorization (OAuth) 协议，用于服务器到服务器身份验证和授权。 如果您对此感兴趣，我们提供了另一篇有关 OAuth 和 Skype for Business Server 2019 的规划文章。
  
Skype for Business Server 2019 还包括对 (的支持，而无需) SHA-256 加密哈希函数签名的证书。 若要支持使用 SHA-256 进行外部访问，外部证书需要由公共 CA 使用 SHA-256 颁发。
  
为了简单明了，我们将 Standard Edition 服务器、前端池和其他角色的证书要求放在下表中，其中将虚拟 contoso.com 用于示例 (你可能正在对环境) 使用其他内容。 这些证书都是标准 Web 服务器证书，具有不可导出的私钥。 还需要注意其他一些注意事项：
...
|:-----|:-----|:-----|:-----|
|默认值  <br/> |控制器池  <br/> |控制器的 FQDN，控制器池的 FQDN。  <br/> 如果此池是客户端的自动登录服务器，并且组策略要求执行严格的 DNS 匹配，则还需要为已注册的每个 SIP 域 (sip.sipdomain) 。  <br/> |pool.contoso.com;SAN=dir01.contoso.com  <br/> 如果此控制器池是客户端的自动登录服务器，而且组策略要求执行严格的 DNS 匹配，那么您还需要 SAN=sip.contoso.com; SAN=sip.fabrikam.com。  <br/> |
...
Survivable Branch Appliance 的证书 (201) 9 年 10 月 Survivable Branch Appliance 2015 Skype for Business Server：
...
如果这是您正在寻找的内容，并且您有很多 SIP 域 (这会使添加 SAN 的成本更高) ，您可以将反向代理配置为将 HTTP 用于初始自动发现服务请求，而不是使用 HTTPS (这是默认配置) 。 规划 [移动功能](../../SfbServer/plan-your-deployment/mobility.md) 一文对此有详细信息。
...
> [!Note]
> **为什么使用最新Windows？** 旧版本可能没有启用所有功能所需的正确权限。 可以使用群集管理员创建文件共享。 有关详细信息，请参阅此支持文章 [如何在群集上](https://support.microsoft.com/help/224967) 创建文件共享。
