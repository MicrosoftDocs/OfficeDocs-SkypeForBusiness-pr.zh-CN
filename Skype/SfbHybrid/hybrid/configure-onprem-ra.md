...
ms.locfileid: "58602027"
---
# <a name="configure-resource-accounts"></a>配置资源帐户

[!INCLUDE [sfbo-retirement](../../Hub/includes/sfbo-retirement.md)]

Skype for Business Server 2019 混合实现仅将 电话系统 提供的云服务用于统一消息，不与 Exchange Online。 在 Skype for Business Server 2019 中，你现在可以使用云呼叫队列和自动助理，如下面介绍的在[](/MicrosoftTeams/here-s-what-you-get-with-phone-system)电话系统 或 Microsoft 365 Office 365。

若要将 电话系统 自动助理或呼叫队列与 Skype for Business Server 2019 一同使用，需要创建充当应用程序终结点的资源帐户，并可以分配电话号码，然后使用联机 Teams 管理中心配置呼叫队列或自动助理。 此资源帐户可以联机 (请参阅管理[Microsoft Teams](/MicrosoftTeams/manage-resource-accounts)中的资源帐户以创建联机或本地) 资源帐户，如本文所述。 通常，你将有多个电话系统自动助理或呼叫队列节点，每个节点都映射到一个资源帐户，可以联机或 Skype for Business Server 2019。

如果您具有现有的 Exchange UM 自动助理和呼叫队列系统，在切换到 Exchange Server 2019 或 Exchange online 之前，您需要手动记录详细信息，如下所述，然后使用 Teams 管理中心实现全新的系统。
...
1. 获取服务号码。
2. 获取免费电话系统 - 虚拟[用户许可证](/MicrosoftTeams/teams-add-on-licensing/virtual-user)或电话系统许可证以用于资源帐户。
...
如果自动助理或呼叫队列嵌套在顶级自动助理下，则如果您希望自动助理和呼叫队列的结构中有多个入口点，则关联的资源帐户只需要一个电话号码。

若要将呼叫重定向到组织中在线用户，他们必须具有 **电话系统** 许可证，并且必须为 企业语音 启用，或者拥有 Microsoft 365 或 Office 365 通话套餐。 请参阅[分配Microsoft Teams许可证](/MicrosoftTeams/assign-teams-licenses)。 若要为用户启用企业语音，可以使用Windows PowerShell。 例如，运行：  `Set-CsUser -identity "Amos Marble" -EnterpriseVoiceEnabled $true`
...
1. 移植或获取收费或免费服务号码。 无法将号码分配给任何其他语音服务或资源帐户。
   ...
   在将电话号码分配给资源帐户之前，你需要获取或移植现有的收费或免费服务号码。 获取收费或免费服务电话号码后，这些号码会显示在 **Microsoft Teams** 管理中心语音 电话 号码中，列出的号码类型将列为服务  >    >  -**免费**。 若要获取服务号码，[请参阅获取服务](/MicrosoftTeams/getting-service-phone-numbers)电话号码，或者如果要转移现有服务号码，请参阅将电话号码转移到[Teams。](/MicrosoftTeams/phone-number-calling-plans/transfer-phone-numbers-to-teams)

   如果你在美国以外，则你无法通过Microsoft Teams中心获取服务号码。 转到 ["管理你的组织的电话号码"，](/MicrosoftTeams/manage-phone-numbers-for-your-organization/manage-phone-numbers-for-your-organization) 以查看如何从美国以外地区进行管理。
...
3. 通过为每个自动助理或呼叫队列电话系统 cmdlet 创建本地资源帐户，并为每个帐户指定名称 `New-CsHybridApplicationEndpoint` 、sip 地址等。
...
4.  (可选) 创建资源帐户后，可以等待 AD 联机和本地同步，也可以强制同步并继续联机配置 电话系统 自动助理或呼叫队列。 若要强制同步，需要在运行 AAD 连接 (的计算机上运行以下命令（如果尚未执行）需要加载以运行命令 `import-module adsync`) ：
...
5. 将电话系统 - 虚拟用户电话系统许可证分配给资源帐户。 请参阅[Microsoft Teams加载项许可证"和"](/MicrosoftTeams/teams-add-on-licensing/assign-teams-add-on-licenses)[将许可证分配给用户"。](/microsoft-365/admin/manage/assign-licenses-to-users)
...
1. 通过为每个自动助理或呼叫队列电话系统 cmdlet 创建本地资源帐户，并为每个帐户指定名称 `New-CsHybridApplicationEndpoint` 、sip 地址等。
...
2.  (可选) 创建资源帐户后，可以等待 AD 联机和本地同步，也可以强制同步并继续联机配置 电话系统 自动助理或呼叫队列。 若要强制同步，需要在运行 AAD 连接 (的计算机上运行以下命令（如果尚未执行）需要加载以运行命令 `import-module adsync`) ：
...
测试实现的最佳方法就是呼叫为自动助理或电话系统队列配置的号码，并连接到其中一个代理或菜单。 您还可以使用管理中心"操作"窗格中的 **"测试** "按钮快速进行测试呼叫。 如果要对自动助理或呼叫队列电话系统，请选择它，然后在"操作"窗格中单击"编辑 **"。** 
...
4. 配置电话系统终结点的自动助理或呼叫队列。

5. 测试电话系统自动助理或呼叫队列。

6. 将链接到 UM 呼叫队列或自动助理Exchange的电话号码重新分配给相应的电话系统自动助理或呼叫队列。  
