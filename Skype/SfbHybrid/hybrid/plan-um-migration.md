...
description: 本主题介绍决定将现有 Skype for Business Server 或 Exchange Server 部署迁移到最新版本或迁移到 Skype for Business Online 或 Exchange Online 时需要考虑的内容。
ms.openlocfilehash: 1972da1afbfb4d7b3236a5b6f03b904181cc1701
ms.sourcegitcommit: 9879bc587382755d9a5cd63a75b0e7dc4e15574c
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 07/21/2021
ms.locfileid: "53509813"
---
...
本主题介绍决定将现有部署或部署迁移到 Skype for Business Server Exchange Server时需要考虑Exchange Online。 您可以迁移哪些内容以及迁移时间很大程度上取决于您已在组织中设置哪些内容。
...
云语音邮件为专用交换机和 PBX 交换机 (语音邮件) 。 如果对 PBX 使用 Exchange Server 统一消息，并且要升级到 Exchange Server 2019，则需要采用博客文章[Exchange](https://blogs.technet.microsoft.com/exchange/2018/04/24/new-date-for-discontinuation-of-support-for-session-border-controllers-in-exchange-online-unified-messaging/)团队博客上不再支持[Exchange Online](https://blogs.technet.microsoft.com/exchange/)统一消息中的会话边界控制器的新日期中列出的三个选项之一。
...
在 Skype for Business Server 2019 中，我们将从 um Exchange Online 移动到 云语音邮件。 将用户移至 Skype for Business 2019 服务器时，当为托管语音邮件云语音邮件用户将自动开始使用该邮箱。 如果您当前使用的是 Exchange Online UM，则无需执行任何其他操作，只需将用户移至 Skype for Business Server 2019，云语音邮件。 但是，您需要注意对功能进行一些更改：
...
以下方案要求当前部署中不存在 PBX 或 SBC 配置，并假定已在内部部署服务器上配置了 UM Exchange配置。 其中每个解决方案还假定你已决定在本地部署服务器和本地服务器Skype for Business或Microsoft 365或Office 365。 有关混合部署Skype for Business，请参阅规划[混合连接](plan-hybrid-connectivity.md)。

### <a name="exchange-2013exchange-2016-and-skype-for-business-2015-to-exchange-2019-and-skype-for-business-2019"></a>Exchange 2013/Exchange 2016 和 Skype for Business 2015 Exchange 2019 和 Skype for Business 2019

在此方案中，您希望将现有 Exchange 2013、Exchange 2016 和 Skype for Business 2015 服务器迁移到 Exchange 2019 和 Skype for Business 2019。

如本主题前面所述，Exchange 2019 不再包括 UM 服务。 这意味着，对于要移动到 Exchange 2019 的任何邮箱，您需要使用 云语音邮件 来替换 UM 服务提供的功能。 设置 2019 Skype for Business Server 2019 及其与 Microsoft 365 或 Office 365 之间的混合部署时，云语音邮件 UM Exchange服务。

将用户移至 2019 Exchange 2019 Skype for Business Server的顺序对于确保语音邮件功能对所有用户保持可用至关重要。 语音邮件的处理位置也由邮箱和Exchange Skype for Business和用户所在的位置决定。 查看下表，了解支持哪些Exchange和Skype for Business Server以及在何处处理语音邮件。

| 邮箱位于：            | 位于 2015 Skype for Business Server的用户 | 位于 2019 Skype for Business Server上的用户  |
|--------------------------------|-----------------------------------------|------------------------------------------|
...
- 云语音邮件在 GA 上自动助理组织功能。 如果希望将邮箱移动到 云语音邮件 以继续通过自动助理使用，则需要至少保留一台运行 UM 角色或服务的 Exchange 2013 或 Exchange 2016 服务器。
- 在将用户的邮箱移动到 2019 Skype for Business 2019 之前，至少需要设置一个 2019 Exchange服务器。  如果不这样做，将导致这些邮箱无法接收语音邮件。
- 发送到语音邮件的呼叫将转接到云语音邮件将记录这些呼叫。 呼叫结束后，语音邮件将发送到 2019 年本地服务器中收件人Exchange邮箱。 在确定 Internet 连接是否足以支持语音连接时，您需要考虑云语音邮件。
...
5. 为移动到 Skype for Business Server 2019 的每个用户设置托管语音邮件策略，以云语音邮件。
6. 将邮箱从 Exchange 2013 或 Exchange 2016 服务器移动到 Exchange 2019 服务器。
7. 在上Skype for Business个用户离开服务器后，停用 2015 服务器。
8. 在上Exchange个邮箱Exchange停用 2013 或 2016 服务器后，停止使用这些服务器。

    > [!IMPORTANT]
    > 如果您依赖于自动助理，请至少保留一个 Exchange 2013 或 Exchange 2016 运行且可用。
...
无论用户Exchange UM 云语音邮件处理语音邮件，邮件都将存储在用户的邮箱Exchange邮箱中。
...
4. 为移动到 Skype for Business Server 2019 的每个用户设置托管语音邮件策略，以云语音邮件。
5. 在上Skype for Business个用户离开服务器后，停用 2015 服务器。
