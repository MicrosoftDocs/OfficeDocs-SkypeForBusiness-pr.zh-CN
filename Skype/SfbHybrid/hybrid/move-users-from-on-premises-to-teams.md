...
- 来自其他用户的所有传入呼叫 (无论是从 Skype for Business 还是从 Teams) 发送，都将登录用户的 Teams 客户端。
- 用户将能够与联机或本地部署Skype for Business (其他用户进行互) 。
- 用户将能够与联盟组织的用户通信。
- 该用户安排的新会议将Teams会议。
- 用户仍可加入任何Skype for Business会议。
- 为将来安排的用户预先存在的会议将从本地迁移到Teams。
- 本地存在的联系人在用户首次登录Teams在用户首次登录后立即可用。
- 用户无法从会议发起Skype for Business聊天，也无法在 Skype for Business 中安排新Skype for Business。 如果他们尝试打开 Skype for Business 客户端，他们将被重定向以使用Teams如下所示。 如果未Teams客户端，则它们将被定向到 web 版本的 Teams使用其浏览器。<br><br>
    ![将用户重定向到邮件Teams。](../media/go-to-teams-page.png)
...
> [!IMPORTANT]
>使用 Move-CsUser 将用户从本地迁移到云时，现在会自动为用户分配 TeamsOnly 模式，其从本地会议自动转换为 Teams 会议，而不管是否实际指定了切换。 `-MoveToTeams`  (这包括从 Lync Server 2013 迁移，Lync Server 2013 从未具有 switch.) 如果未指定此开关，则用户从本地 Skype for Business Server 迁移到 Skype for Business Online，其模式保持不变。 `-MoveToTeams` 最近进行了更改，以准备停用 Skype for Business Online。


## <a name="move-a-user-directly-from-skype-for-business-on-premises-to-teams-only"></a>将用户直接从本地Skype for Business移动到仅Teams

利用 Skype for Business Server 和 Lync Server 2013 中的本地管理工具，您可以使用 PowerShell 中的 Move-CsUser cmdlet 或 Skype for Business Server 控制面板，在单个步骤中将用户从本地移动到 TeamsOnly 模式，如下所述。 不再需要指定开关以及直接从内部部署移动到 Teams 无论使用哪个版本的 Skype for Business Server 或 Lync Server，现在仅自动 `-MoveToTeams` 执行。 

你必须在本地环境和云服务中拥有足够的权限 (Microsoft 365或Office 365) ，如所需的管理[凭据中所述](move-users-between-on-premises-and-cloud.md#required-administrative-credentials)。 您可以在这两个环境中使用具有特权的单个帐户，或者可以使用本地凭据启动本地 Skype for Business Server 命令行管理程序窗口，并使用 参数指定具有所需 管理角色 Microsoft 365 `-Credential` 的凭据。
...
- 如果您没有一个同时在本地和云服务服务中具有足够权限的帐户 (Microsoft 365) ，请使用 参数在 Microsoft 365 中为帐户 `-credential` 提供足够Microsoft 365。
- 如果具有权限的帐户Microsoft 365以"onmicrosoft"结尾。 <span>com"，必须指定 参数，并指定正确的值 `-HostedMigrationOverrideUrl` ，如所需的管理[凭据 中所述](move-users-between-on-premises-and-cloud.md#required-administrative-credentials)。
...
### <a name="move-to-teams-using-skype-for-business-server-control-panel"></a>使用"Teams"控制面板Skype for Business Server移动到"
...
Skype for Business Server 2015 CU8 和 Skype for Business Server 2019 中的本地管理工具让你能够通知本地 Skype for Business 用户他们即将移动到 Teams。 启用这些通知后，用户将在其 Skype for Business 客户端 (Win32、Mac、Web 和移动) 通知，如下所示。 如果用户单击"**试用**"按钮，Teams客户端安装后将启动;否则，用户将在浏览器中导航到 Teams Web 版本。 默认情况下，当启用通知时，Win32 Skype for Business客户端会以静默方式下载 Teams 客户端，以便丰富客户端在将用户移动到 TeamsOnly 模式之前可用;但是，您也可以禁用此行为。  通知使用 本地版本进行配置，Win32 客户端的无提示下载通过本地 `TeamsUpgradePolicy` `TeamsUpgradeConfiguration` cmdlet 控制。
