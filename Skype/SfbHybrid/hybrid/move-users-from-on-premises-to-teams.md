...
- 来自其他用户的所有传入呼叫和聊天 (是否从 Skype for Business 或团队) 发送到用户的团队客户端。
- 用户将能够与使用 Skype for Business (的其他用户进行互操作，无论是在线还是在本地) 。
- 用户将能够与联合组织中的用户进行通信。
- 由该用户计划的新会议是团队会议。
- 用户仍可以加入任何 Skype for Business 会议。
- 为未来安排的用户的预先存在的会议将从本地迁移到团队。
- 在用户首次登录后，会立即在团队中提供本地的联系人。
...
- 如果使用早于 Skype for Business Server 2015 CU8 的版本，则该移动需要两个步骤 (如果需要，可以通过脚本将其作为一个步骤一起执行) ：
  - [将用户从 skype For Business Server (本地) 迁移到 skype for Business Online](move-users-from-on-premises-to-skype-for-business-online.md)。
...
使用 CU8 中的 Skype for Business Server 2015 中的本地管理工具和 Skype for Business Server 2019，可以使用 PowerShell 或 Skype for Business Server 控制面板中的 Move-CsUser cmdlet，在一个步骤中将用户从 "仅本地" 模式移动到 "仅工作组" 模式（如下所述）。

### <a name="move-to-teams-using-move-csuser"></a>移动到使用 Move-CsUser 的团队

Move-CsUser 可从本地 Skype for Business 命令行管理程序 PowerShell PowerShell PowerShell 窗口中获取。 下面的步骤和所需的权限与将用户移动到 Skype for business Online 相同，不同之处在于，您还必须指定 MoveToTeams 开关，并且还必须为用户授予 (除 Skype for Business Online) 之外的团队许可证。

您必须在本地环境和云服务 (Microsoft 365 或 Office 365) 中具有足够的权限，如 [必需的管理凭据](move-users-between-on-premises-and-cloud.md#required-administrative-credentials)中所述。 您可以使用在两个环境中具有权限的单个帐户，也可以使用本地凭据启动本地 Skype for Business Server Management Shell 窗口，并使用 `-Credential` 参数指定具有必要管理角色的 Microsoft 365 或 Office 365 帐户的凭据。
...
- 指定 `MoveToTeams` 开关。
- 如果您没有一个帐户在本地和云服务 (Microsoft 365 或 Office 365) 中具有足够的权限，请使用 `-credential` 参数在 Office 365 中提供具有足够权限的帐户。
...
可以使用以下 cmdlet 序列将用户移动到 TeamsOnly，并假设 Microsoft 365 或 Office 365 凭据是一个单独的帐户并作为 Get-Credential 提示符的输入提供。
...
2. 在左侧导航栏中，选择 " **用户**"。
3. 使用 " **查找** " 查找用户 (s) 您希望迁移到团队。
4. 选择 "用户" (s) ，然后从列表上方的 " **操作** " 下拉列表中选择 " **将所选用户移动到团队**"。
5. 在向导中，单击“下一步”。
