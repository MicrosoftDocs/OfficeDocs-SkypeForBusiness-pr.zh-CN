...
description: 此附录包括有关更新 AAD 连接以将多个林作为云合并的一部分以用于Teams Skype for Business。
...
Azure AD 连接[支持从多个林同步](/azure/active-directory/connect/active-directory-aadconnect-topologies)。 但是，它仅支持一个 Azure AD 连接同步到 AAD。 因此，如果 Azure AD 已安装在一个林中，则必须更新现有 AAD 连接，以从其他林同步。
 ...
 ![目录连接页](../media/cloud-consolidation-connect-your-directories.png)
 - 但是，如果用户可以存在于多个目录中，并且你将合并数据 (例如，如果联系人对象存在于与另一个林) 中的用户对应的林中，则需要卸载 Azure AD 连接并重新安装它。  这是因为跨林加入规则条件只能在首次安装过程中配置。 此操作在下面的页面上完成：<br><br>
