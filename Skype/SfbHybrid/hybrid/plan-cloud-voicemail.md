...
ms.locfileid: "61563731"
---
# <a name="plan-cloud-voicemail-service-for-on-premises-users"></a>为云语音邮件用户规划服务
...
云语音邮件统一消息 Exchange (UM) ，为在 Exchange Server 2019 或 Exchange Online 上拥有邮箱的 Skype for Business 2019 语音用户提供语音邮件功能。 云语音邮件为本地用户和联机用户提供以下好处：
...
- 如果要在本地Exchange UM，或者混合使用 Exchange UM 联机和内部部署，则需要同时修改托管语音邮件策略和托管提供商。  有关详细信息，请参阅配置 [云语音邮件 服务](configure-cloud-voicemail.md)。
...
- 必须为本地用户启用 企业语音 和托管语音邮件Skype for Business Server。

- 必须Exchange EWS (EWS) 自动发现的外部 Web 服务，否则某些 云语音邮件功能将受到限制。

- 如果您有内部部署邮箱Exchange，云语音邮件为邮箱用户设置云语音邮件[设置Exchange Server邮箱。](/microsoftteams/set-up-phone-system-voicemail#set-up-cloud-voicemail-for-exchange-server-mailbox-users)

## <a name="migration-and-interoperability"></a>迁移和互操作性

如果您计划部署 Skype for Business Server 2019 和/或 Exchange Server 2019，则必须仔细规划迁移以确保为语音邮件持续提供服务。 请注意以下几点：
...
- 如果要升级到 Skype for Business Server 2019，并且为具有 Exchange Online UM 的语音邮件配置了 Skype for Business Server 2015，则用户的语音邮件将自动从 Exchange Online UM 迁移到 云语音邮件 当其帐户移至 Skype for Business Server 2019 时。 

有关规划迁移的信息，请参阅规划迁移Skype for Business Server [Exchange Server迁移](plan-um-migration.md)。