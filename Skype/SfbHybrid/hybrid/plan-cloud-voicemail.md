...
云语音邮件取代 Exchange 统一消息 (UM) 为在 Exchange Server 2019 或 Exchange Online 上拥有邮箱的 Skype for Business 2019 语音用户提供语音邮件功能。 云语音邮件为您的内部部署用户和联机用户提供以下好处：
...
> [!Important]
> Skype for Business Online 将于2021年7月31日停用，此后用户将无法再通过 Skype for Business Online 客户端访问其 Exchange 邮箱中的语音邮件。
...
- 无需配置 Exchange UM 角色。
- 云语音邮件的设置任务更简单。
- 对语音邮件功能的更新是直接在云中传递的，因此，您的用户始终可以访问最新的功能和更新，而不依赖 (Cu) 的累积更新。
...
- 必须设置 (EWS) URL 和自动发现的外部 Exchange Web 服务，否则一些云语音邮件功能将受到限制。
