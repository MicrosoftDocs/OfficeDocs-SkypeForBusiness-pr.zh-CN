...
> [!Important]
> Skype for BusinessOnline 将于 2021 年 7 月 31 日停用，此后服务将不再可用。  此外，本地环境（无论是通过 Skype for Business Server 还是云连接器版本与 Skype for Business Online）之间的 PSTN 连接将不再受支持。  了解如何使用直接路由将本地电话网络Teams[到呼叫。](/MicrosoftTeams/direct-routing-landing-page)
...
此类配置依赖于共享 SIP 地址空间功能，有时称为"拆分域"，这意味着域的用户（如 contoso.com）在本地使用 Skype for Business Server 和 Teams 之间拆分，如下图所示：
...
必须先为用户分配一个 Teams 许可证以及 Skype for Business Online (计划 2，然后才能) 。 **即使停用 Skype for Business Online，也要求分配 Skype for Business Online 许可证。** 如果用户想要利用其他联机功能（如音频会议或 电话系统，您需要在 Microsoft 365 中为其分配适当的许可证。

## <a name="hybrid-connectivity-infrastructure-requirements"></a>混合连接基础结构要求

<a name="BKMK_Infrastructure"> </a>

若要实现本地环境和 Microsoft 365 通信服务之间的混合连接，您需要满足以下基础结构要求：
...
- Azure Active Directory 连接本地目录与本地目录同步Microsoft 365。 有关详细信息，请参阅[Azure AD 连接：帐户和权限](/azure/active-directory/connect/active-directory-aadconnect-accounts-permissions)。
...
|边缘 Web 会议 (FQDN) DNS A 记录，例如 webcon.contoso.com 解析为 Web 会议边缘外部 IP (服务器)   <br/> |内部企业网络连接用户的计算机  <br/> |允许联机用户在本地托管会议中呈现或查看内容。 内容包括PowerPoint、白板、投票和共享笔记。  <br/> |

根据组织中 DNS 的配置方式，您可能需要将这些记录添加到相应的 SIP 域 (的内部托管 DNS 区域) 以提供这些记录的内部 DNS 解析。
...
根据您的 Microsoft Online Services 数据中心的位置，您还必须配置网络防火墙设备以接受基于通配符域名的连接 (例如来自 \* .outlook.com) 的所有流量。 如果组织的防火墙不支持通配符名称配置，您必须手动确定要允许的 IP 地址范围和指定的端口。
