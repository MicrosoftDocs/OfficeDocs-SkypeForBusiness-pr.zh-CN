...
4. 使用 Microsoft 团队和电话系统实现新的云自动助理服务。 有关实现的详细信息，请参阅 [配置资源帐户](configure-onprem-ra.md) 。 在执行此操作时，请上载与每个 UM 自动助理节点关联的文本到语音的脚本或记录的邮件。
5. 测试云自动助理的功能。
...
标题为 " [小型企业" 的教程示例-设置自动助理](/microsoftteams/tutorial-org-aa) ，以收集有关用户需求的信息，规划自动助理和用户 (和可能的呼叫队列) 的结构、编写菜单提示以及在团队管理中心实施计划的过程。 查看教程并使用此处的练习来创建您的计划。
