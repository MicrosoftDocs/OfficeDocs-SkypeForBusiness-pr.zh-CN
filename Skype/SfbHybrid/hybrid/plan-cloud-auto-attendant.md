...
ms.locfileid: "60011506"
---
# <a name="plan-cloud-auto-attendants"></a>规划云自动助理

[!INCLUDE [sfbo-retirement](../../Hub/includes/sfbo-retirement.md)]

 (Exchange Server 2019) 2019 或 Exchange Server Exchange Online Exchange Server 中不再提供用于 Exchange Unified Messaging (Exchange Server 2016) 的自动助理。 如果 Skype for Business Server 2019 的实现与这些 Exchange 版本之一集成，则需要使用与 电话系统 关联的联机云语音功能。 有关[将](plan-um-migration.md)Exchange server 2013 和 Exchange Server 上托管的 Exchange UM 服务移动到云的信息，请参阅 Plan for Skype for Business Server and Exchange Server migration。
...
自动助理是一种云服务，它接受客户呼叫并播放问候语，为用户提供菜单选项，并且使用语音或拨号盘与呼叫者交互，以将呼叫路由到正确的目标。 每个自动助理都分配有一个资源 *帐户 (请参阅* 在 [](configure-onprem-ra.md)Skype for Business Server 2019 系统上配置资源帐户) ，这些帐户将直接链接到 Microsoft Teams 管理中心中的自动助理。 有关 [什么是自动助理](/microsoftteams/create-a-phone-system-auto-attendant) 以及自动助理存在哪些选项和功能的详细信息，请参阅设置自动助理。
...
2. 通过会话[边界控制器和](/MicrosoftTeams/direct-routing-border-controllers)[直接路由](/microsoftteams/direct-routing-plan-media-bypass)。
...
- 混合连接。 如果已Skype for Business Server，并且希望为本地用户启用云自动助理，则必须确保在本地环境和联机环境之间设置了混合连接。 这有时称为拆分域配置。

   有关详细信息，请参阅在[](plan-hybrid-connectivity.md)Skype for Business Server 或 Microsoft 365 或 Office 365 之间规划混合[Skype for Business Server Microsoft 365或Office 365](configure-hybrid-connectivity.md)。
...
- 为每个自动[助理创建](/MicrosoftTeams/manage-resource-accounts)联机资源帐户或本地[](configure-onprem-ra.md)资源帐户，并分配电话号码和许可证。 
...
- 如果要升级到 Skype for Business Server 2019，可以在 Exchange Server 2013 或 Exchange Server 2016 中使用 Exchange UM，但如果使用的是 Exchange Server 2019，则必须升级到云自动助理。
...
1. 使用Exchange PowerShell 命令查看旧自动助理系统的结构，包括任何嵌套的自动助理和呼叫队列。  
2. 创建与每个 UM 自动助理节点关联的文本到语音脚本或录制的邮件的副本。
3. 为每个自动助理节点创建本地终结点，包括为对象分配测试电话号码和许可证。 请注意，你现在能够分配联机服务（如 电话系统）使用本地电话号码电话系统。
4. 实现新的云自动助理服务，Microsoft Teams电话系统。 有关 [实现的详细信息，请参阅配置](configure-onprem-ra.md) 资源帐户。 在此操作时，上载与每个 UM 自动助理节点关联的文本到语音脚本或录制的邮件。
5. 测试云自动助理的功能。
...
[配置 Skype for Business Server 或 Microsoft 365 之间的混合Office 365](configure-hybrid-connectivity.md)

[KB4480742：将联系对象移动到 Skype for Business Server 2019 后，订阅者访问或自动助理的呼叫失败，出现快速忙碌和"500 服务器内部"错误](https://support.microsoft.com/help/4480742/call-failures-and-500-server-internal-error-after-migration-to-2019)
