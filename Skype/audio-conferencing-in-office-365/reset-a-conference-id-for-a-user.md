...
4. 在**会议 ID 重置？**窗口中，单击**是**。 会议 ID 将自动创建和电子邮件发送给用户提供最新的会议 id。 默认情况下，电子邮件将发送给用户，但这可以被关闭。
...
- 创建一个新的会议 ID 后，调用方不能使用旧的会议 ID。 应通知用户重新安排现有会议，以确保新的会议 ID 添加到邀请的邀请。 用户可以使用 Skype 业务会议工具，以更新其现有的会议。 若要了解如何下载、 安装和运行 Skype 业务会议更新工具，请参阅：
