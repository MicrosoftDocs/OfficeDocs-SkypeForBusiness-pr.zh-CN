...
会议迁移服务 (MMS) 是为业务服务，在后台运行并自动更新用户的业务和 Microsoft 小组会议 Skype Skype。 MMS 旨在消除用户运行会议迁移工具需要更新其 Skype 业务和 Microsoft 小组会议。
...
- 当用户被迁移从内部 Skype 业务服务器到 Skype 的在线业务。
    
- 当管理员将需要更新该用户的会议中的音频会议信息的用户的音频会议设置进行了更改。
...
- 到内部部署 Skype 服务器迁移用户 Skype 的在线业务
...
这是最常见的情况，MMS 可以帮助创建为用户平滑过渡。 当迁移用户从内部 Skype 业务服务器到 Skype 的在线业务时，MMS 将检测到新的用户，并且将扫描该用户的日历 Skype 业务和 Microsoft 小组会议。 任何将来的会议将与该用户的新信息更新。
  
### <a name="if-youre-currently-using-skype-server-2015-for-audio-conferencing"></a>如果您当前使用的音频会议的 Skype 服务器 2015
...
- **音频会议**许可证分配给用户，在运行之前`Move-CSUser`cmdlet 迁移用户。 这是因为 MMS 还更新会议时音频会议设置的更改的用户。 如果不首先分配许可证，MMS 将更新所有会议重新分配许可证时。
    
### <a name="if-youre-currently-using-a-third-party-audio-conferencing-provider-acp"></a>如果你当前使用的是第三方音频会议提供商 (ACP)

与第三方 ACP，MMS 的运行取决于组织的音频会议设置。 您可以选择当您为用户分配**音频会议**许可自动替换从您 ACP 的拨入号码。 另一方面，您可能需要防止问题的发生，并保留从您 ACP 的拨入号码。 要查看您的组织的设置，请运行下面的 Windows PowerShell 命令并检查参数的值`AutomaticallyReplaceAcpProvider`。 如果您需要使用 PowerShell 的帮助，请参阅本文末尾处的[使用 PowerShell 来管理您的业务组织的 Skype](setting-up-the-meeting-migration-service-mms.md#WPSInfo)节。
  
```
Get-CsOnlineDialInConferencingTenantSettings
```

- 如果此参数的值是 $true，然后将运行时为用户分配**音频会议**许可 MMS 并更新他们的会议。 从您 ACP 的拨入号码保留直到**音频会议**许可分配。
    
- 如果此参数的值是 $false，MMS 不会更新会议，如果为用户分派的**音频会议**许可。 从您 ACP 的拨入号码将保留，直到用户手动设置为在 Skype 业务管理中心或使用 Windows PowerShell 的音频会议。
    
## <a name="updating-meetings-when-a-users-audio-conferencing-settings-change"></a>当用户的音频会议设置更改时，更新会议

MMS 将更新现有的 Skype 业务和 Microsoft 小组会议在下列情况下：
...
- 一个电话号码时从音频会议桥未分配。 这是复杂的方案，这需要额外的步骤。 有关详细信息，请参阅[更改收费或免费电话号码在您的音频会议桥上](change-the-phone-numbers-on-your-audio-conferencing-bridge.md)。
    
> [!IMPORTANT]
> 当您使用 Microsoft 桥，MMS 只更新会议。 如果您正在使用第三方音频会议提供商，用户将需要更新其会议手动。 在这种情况下，您可以使用[会议迁移工具](https://go.microsoft.com/fwlink/p/?linkid=626047)。 
  
不是所有用户的音频会议设置更改都触发 MMS。 具体来说，MMS 更新会议不会产生以下两项更改：
...
1. 确定所有的 Skype 用户已计划在将来的业务和 Microsoft 小组会议
    
  - 任何 Skype 业务或 Microsoft 小组的会议发生之前，MMS 在运行时将被跳过。
...
MMS 迁移会议时间的时间量取决于多少用户受到影响，和 Skype 在他们的日历上的每个用户拥有的业务或 Microsoft 小组会议的总次数。 至少，它将需要运行 10 分钟。 虽然一些大型迁移可以长达 12 小时，大多数迁移应完成在 1 小时内。
  
 **限制和潜在问题**
  
- 只通过单击在 Web 上的 Outlook 中的**添加 Skype 会议**按钮或通过 Skype 会议外接使用 Outlook 的计划的业务或 Microsoft 小组会议 Skype 被迁移。 换句话说，如果用户复制和粘贴 Skype 在线会议的信息从一个会议到新的会议，不会更新该新的会议。
...
会议迁移工具，就像 MMS 发送会议更新，代表用户。 因此，您的用户将看到的唯一事情就是另一轮会议接受通知，为他们的会议。 这可能是令人困惑的用户，所以我们建议您事先不只当您迁移从内部到 Skype 的在线业务，但当您更改音频会议，还将触发 MMS 时通知用户。
...
州的 UserCount--<br/> 挂起 21<br/>正在进行 6<br/> 失败的 2 <br/> 成功的 131
> [!IMPORTANT]
...
MMS 默认启用的所有组织，但可以根据需要禁用它。 例如，如果您想手动迁移所有会议或使用第三方音频会议提供商，可能不需要运行的 MMS。 您还可以选择暂时禁用 MMS。 例如，您可能会进行大量更改的音频会议设置为您的组织并不希望运行，直到完成所有更改的 MMS。
...
### <a name="enabling-and-disabling-mms-only-for-audio-conferencing-changes"></a>启用和禁用多媒体短消息仅用于音频会议更改
<a name="Troubleshooting"> </a>

您还可以仅对音频会议更改禁用 MMS。 它仍将运行时迁移用户从 Skype 的业务部署到 Skype 的在线业务。 若要检查音频会议更新当前的 MMS 状态，运行以下命令，检查的值`AutomaticallyMigrateUserMeetings`参数。 如果此参数设置为 $true，MMS 设置音频会议设置发生更改时更新用户会议。
  
```
Get-CsOnlineDialInConferencingTenantSettings
```

要禁用 MMS 的音频会议，请运行以下命令：
...
除了自动会议迁移中，还可以通过运行该 cmdlet**开始 CsExMeetingMigration**运行用户手动将会议迁移。 此 cmdlet 会议迁移队列中添加用户。 会议迁移服务将读取用户请求和迁移他们的会议。 您可以检查会议通过 cmdlet **Get CsMeetingMigrationStatus**迁移的状态。
