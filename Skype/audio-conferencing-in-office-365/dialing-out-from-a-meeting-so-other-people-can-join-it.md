---
title: "从会议拨出，以便其他人可以加入会议"
ms.author: tonysmit
author: tonysmit
manager: scotv
ms.date: 11/10/2017
ms.audience: Admin
ms.topic: article
ms.prod: office-online-server
localization_priority: Normal
ms.collection: Adm_Skype4B_Online
ms.custom:
- Adm_O365_FullSet
- Strat_SB_PSTN
ms.assetid: 3c2db9a5-3a19-4e19-b59e-8e5587f25d31
description: "Learn what you need and how meeting organizers can call out to other people using Skype for Business."
---

# 从会议拨出，以便其他人可以加入会议

> [!IMPORTANT]
> 本文是由机器翻译的，请参阅[免责声明]。
  
作为会议组织者，您可以拨出使用 Skype for Business 或 Microsoft 团队应用以让其他人加入相同的会议使用其电话。 虽然它具有不要求，给人正在拨出时，建议您拨全部号码，包括国家/地区代码。若要获取拨出工作：
  
- 您可以拨出，只有当您加入会议使用 Skype for Business 或 Microsoft 团队应用。
    
- 您作为会议组织者已启用的音频会议。
    
 **步骤 1:** 在会议邀请中，使用 **邀请更多人**选项来拨出的电话号码。
  
![Dial-out with Skype for Business.](../images/9896abec-7a6f-4148-ad09-76a1cf4b56e1.png)
  
 **步骤 2:** 输入完整的电话号码，在框中包括的国家/地区代码。
  
![Dial-out phone number in Skype for Business](../images/084b4fb5-21eb-4f10-9a1a-c92b919084b9.png)
  
## 支持的国家和地区

拨出是仅适用于某些国家/地区。完整列表，请参阅[国家和地区和的可用性音频会议呼叫计划](../countries-and-region-availability-for-audio-conferencing-and-calling-plans/countries-and-region-availability-for-audio-conferencing-and-calling-plans.md)。
  
## 想要了解关于音频会议的详细信息？

- [设置音频会议 for Skype Business 和 Microsoft 团队](set-up-audio-conferencing-for-skype-for-business-and-microsoft-teams.md)
    
- [Skype for Business 和 Microsoft 团队许可加载项](../skype-for-business-and-microsoft-teams-add-on-licensing/skype-for-business-and-microsoft-teams-add-on-licensing.md)
    
## 
<a name="MT_Footer"> </a>

> [!NOTE]
> **机器翻译免责声明**：本文是由无人工介入的计算机系统翻译的。Microsoft 提供机器翻译是为了帮助非英语国家/地区用户方便阅读有关 Microsoft 产品、服务和技术的内容。由于机器翻译的原因，本文可能包含词汇、语法或文法方面的错误。 
  
## 另请参阅
<a name="MT_Footer"> </a>

#### 其他资源

[安装 Skype for Business](https://support.office.com/article/8a0d4da8-9d58-44f9-9759-5c8f340cb3fb)

