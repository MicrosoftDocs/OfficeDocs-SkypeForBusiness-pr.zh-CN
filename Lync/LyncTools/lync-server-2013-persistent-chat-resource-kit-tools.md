...
ms:mtpsurl: https://technet.microsoft.com/zh-cn/library/JJ945599(v=OCS.15)
ms:contentKeyID: 52061191
ms.date: 06/25/2014
...
The Lync Server 2013 持久聊天 Resource Kit tools help to make routine tasks easier for IT administrators who deploy and manage Lync Server 2013 持久聊天服务器. In addition to installation instructions, this topic describes the purpose of each tool, and examples of its use.
...
For optimal performance, the Lync Server 2013 资源工具包工具 should be installed in the same environment and with the same specifications that are required for Lync Server 2013.

## Resource Kit Tools Overview

Here are the tools that are provided in the Lync Server 2013 持久聊天 Resource Kit. The following section provides a description of each tool, including requirements and example usage.
...
The AffCheck tool confirms that the 持久聊天 back-end database user and group affiliation records match that of Active Directory 域服务.

## Requirements

The tool is installed with the PersistentChatResKit installer on a domain joined machine.

The user account under which the tool is run must have Read access to the 持久聊天 back-end database and Active Directory Domain Services.
...
The PersistentChatMonitoringSummary tool moves 持久聊天 monitoring information from the monitoring database into a specified CSV log file.

The CSV file will contain a breakdown of 持久聊天 sessions by number of total sessions, successful sessions, unexpected failures, expected failures, and a breakdown of the unexpected failures by diagnostic ID, number of failures, and failure description.

## Requirements

Install the 持久聊天 Resource Kit tools on a domain-joined machine that has access to the Monitoring database.
...
The 持久聊天 Stress tool provides an easy way to simulate usage of 持久聊天 to test real-world performance, including varied user models to better fit your expected usage scenarios.

## Requirements

Install the 持久聊天 Resource Kit tools onto a domain-joined machine that has access to the 持久聊天 back-end database.
...
Create users and channels for use in the stress run:
  ...
  - Create as many users as your user model calls for, enable them for Lync, and set their 持久聊天 policy to Enabled.
...
6.  Change “ConnectionString” to a connection string for your 持久聊天 back-end database.
...
The ChatUsageReport tool generates an HTML report of 持久聊天 service usage.

## Requirements

Install the 持久聊天 Resource Kit tools on a domain-joined machine that has access to the 持久聊天 back-end database.

The user account under which the tool is run must have Read access to the 持久聊天 back-end database.
...
ScheduleADSyncForPrincipal is a Microsoft SQL Server 2012 script that must be run directly from within SQL Server Management Studio when connected to the 持久聊天 back-end database. This script enables you to force 持久聊天 to synchronize its records of a user with those of Active Directory Domain Services, rather than waiting for the scheduled synchronization time.

## Requirements

The user account under which the script is run must have owner access to the 持久聊天 back-end database.
