...
10. 在 "**端口**" 中，单击或键入希望用户用于 SIP 的端口（默认值为5061）。
...
1.  在包含对等和会议的**即时消息**中，为负载级别指定适当的值。
    ...
    > [!NOTE]  
    > 所有字段（位置信息服务除外）的负载级别值已<STRONG>禁用</STRONG>、<STRONG>低</STRONG>、<STRONG>中</STRONG>、<STRONG>高</STRONG>和<STRONG>自定义</STRONG>。 选中 "低"、"中"、"高" 或 "自定义" 时，将为每个模态和客户端生成配置。 "高" 将导致为服务器生成的最大支持负载，"中" 对应于60% 的负载，"低" 对应于负载的30%。
