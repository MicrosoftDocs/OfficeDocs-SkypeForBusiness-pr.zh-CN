...
ms:mtpsurl: https://technet.microsoft.com/zh-cn/library/JJ945587(v=OCS.15)
ms:contentKeyID: 52061184
ms.date: 08/03/2014
...
Here is a list of terms and definitions that you might find useful as you read through this topic.
  ...
  - Organizational Unit – The Active Directory 域服务 organizational unit (OU).
...
8.  Select the **Voice Enabled** check box if you want to enable the test users for 企业语音.
...
When you click the Create Distribution Lists button, the tool queries Active Directory 域服务 to see if distribution lists matching the prefix and numbers already exist. The tool will create only the ones that do not already exist. When adding members to these newly created Distribution Lists, it will pick the users from the range specified on the User Creation tab.
