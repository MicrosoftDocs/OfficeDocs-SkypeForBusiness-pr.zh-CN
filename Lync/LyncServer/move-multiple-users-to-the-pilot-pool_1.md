...
2.  在命令行中, 键入以下内容并将**User1**和**** 用户2与要移动的特定用户名进行替换, 并将**池\_FQDN**替换为目标池的名称。 在此示例中, 我们将在 Hao Chen's 和 Katie 约旦之间移动用户。
...
4.  **注册机构池**标识现在应指向您在上一步中指定为**\_池 FQDN**的池。 此标识的存在可确认用户已成功移动。 重复步骤以验证**** 服务2是否已移动。
