...
如果你不想向正在部署 Lync Server 2013 的用户或组授予域管理员组的成员身份, 你可以启用 RTCUniversalServerAdmins 组的成员在 CsTopology 上运行**enable-** Windows PowerShell cmdlet运行 Lync Server 2013 的服务器。 默认情况下, RTCUniversalServerAdmins 组的成员不具备运行此 cmdlet 的能力。 在运行 Lync Server 的服务器上, 通过使用**CsSetupPermission** cmdlet 并指定运行服务器的计算机对象所在的组织单位 (OU), 授予管理员权限, 以运行**CsTopology**Lync Server 2013 位于。
