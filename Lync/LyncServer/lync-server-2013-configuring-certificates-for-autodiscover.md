...
12. 如果自动发现服务（即 lyncdiscover.）。 \>域名 \> 和 lyncdiscoverinternal.。\<domain name\>  (基于是否为外部或内部证书) 使用者备用名称缺失，并且您对默认的 WebServicesInternal 和 WebServiceExternal 类型使用了单个默认证书，请执行以下操作：
