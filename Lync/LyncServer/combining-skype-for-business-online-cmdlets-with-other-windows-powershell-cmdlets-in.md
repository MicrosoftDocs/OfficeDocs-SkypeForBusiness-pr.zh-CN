...
由于在您的计算机上有数百个 cmdlet 可供使用，因此您可能很难确定哪些 cmdlet 是 Skype for Business Online cmdlet，哪些是不是哪些。 若要返回 Skype for Business Online cmdlet 列表 (且仅) Skype for Business Online cmdlet，必须首先确定包含所有 Skype for Business Online cmdlet 的临时 Windows PowerShell 模块的名称。 为此，请在 Windows PowerShell 提示符处运行以下命令：
