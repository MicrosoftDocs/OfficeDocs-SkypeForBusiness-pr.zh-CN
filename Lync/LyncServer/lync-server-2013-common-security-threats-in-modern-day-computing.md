...
密钥是用于加密、解密或验证机密信息的机密代码或数字。 必须考虑的 (PKI) 在公钥基础结构中使用两个敏感密钥：。
...
使用中继 NAT 的遍历 (打开) 协议不会要求加密通信，并且要发送的信息受消息完整性保护。 尽管它是开放的，但它所发送的信息 (也就是说，可以直接提取 IP 地址和端口) ，只需查看数据包的源地址和目标地址即可。 A/V 边缘服务可确保数据有效，方法是使用从少数几个项目派生的密钥检查邮件的消息完整性，包括密码（从不以明文形式发送）。 如果使用 (SRTP) 的安全实时协议，则媒体流量也会加密。
