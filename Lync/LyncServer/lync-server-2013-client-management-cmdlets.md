...
客户端管理主要包括确定哪些客户端应用程序 (如 Microsoft Lync 2013) 允许登录到 Lync Server 2013，并确定这些客户端应用程序在登录后可使用的功能。
...
大多数适用于客户端管理的管理任务可以从 Lync Server 2013 控制面板中执行。 可以使用 Lync Server 命令行管理程序中的 cmdlet 或在脚本中执行这些相同的任务。 通过使用脚本，可以自动完成某些任务。 以下是与客户端管理直接相关的 cmdlet 列表：
