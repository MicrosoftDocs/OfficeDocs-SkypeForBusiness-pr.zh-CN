...
如果你的组织有用户在外出时需要出席 Lync Server 2013 本地会议或无法访问计算机，则可以部署电话拨入式会议，以便他们可以使用公共交换电话加入会议网络（PSTN）电话。
...
必须为会议策略中的拨入访问启用会议。 默认情况下，允许拨入访问的会议会在会议邀请中包括以下信息：
  ...
  - 指向 "电话拨入式会议设置" 页面的链接，其中包含具有相关语言的访问号码的完整列表;用于创建、重置或取消阻止个人识别码（Pin）的位置;和其他信息，如双音调多频率（DTMF）控件。
...
拨入式参与者（即，无论是从 PSTN 拨号），在会议期间收听个人公告，例如他们是否已被静音或已取消静音、正在录制会议还是正在会议厅中等待其他人。
