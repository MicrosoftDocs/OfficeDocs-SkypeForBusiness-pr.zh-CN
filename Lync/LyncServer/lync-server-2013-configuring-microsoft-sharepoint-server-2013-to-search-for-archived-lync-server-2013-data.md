...
在 Microsoft Exchange Server 2013 而不是 Microsoft Lync Server 2013 中存储即时消息和 Web 会议脚本的主要优势之一是, 将数据存储在同一位置使管理员能够使用单个工具进行搜索对于已存档的 Exchange 数据和/或已存档的 Lync 服务器数据。 由于所有数据都存储在同一位置 (Exchange), 因此任何可以搜索已存档 Exchange 数据的工具也可以搜索已存档的 Lync 服务器数据。
