...
> [!IMPORTANT]  
> 您指定的名称必须与服务器上配置的计算机名相同。 默认情况下，未加入域的计算机的计算机名是一个短名称，而不是 FQDN。 拓扑生成器使用 FQDN，而不是短名称。 在分配运行 Lync Server、Edge 服务器和池的服务器的 Fqdn 时，<STRONG>只使用标准字符</STRONG>（包括 a-z、a-z、0-9 和连字符）。 不要使用 Unicode 字符或下划线。 外部 DNS 和公共证书颁发机构（Ca）通常不支持 FQDN 中的非标准字符（当 FQDN 必须分配给证书中的 SN 时）。
