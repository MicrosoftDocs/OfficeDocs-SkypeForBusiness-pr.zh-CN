...
Microsoft Lync Server 2013 监控报告为您提供了大量有关在组织中进行的通信会话质量和数量的信息。 但是，在安装 Lync Server 2013 时不会自动安装监控报告;相反，您必须单独安装监控报告，并且仅在计算机上安装了 Lync Server 之后。
...
> [!NOTE]  
> Lync Server 2013 中包含另一个新报告-QoE 呼叫详细信息子报告。 但此报告主要供内部使用，且无法直接访问。



</div>

有两种安装 Lync Server 监视报告的方法：您可以使用 Lync Server 部署向导，也可以使用 Lync Server 2013 安装文件中附带的 Windows PowerShell 脚本。 无论您使用哪种方法安装此报告，都必须先确保：
...
安装监控报告之后，您必须使用 Set-CsReportingConfiguration cmdlet 配置用于访问这些报告的 URL。 可以通过运行以下 Windows PowerShell 命令，从 Lync Server 命令行管理程序执行此任务。 请注意，建议（但不要求）您在配置报告 URL 时使用 HTTPS 协议：
