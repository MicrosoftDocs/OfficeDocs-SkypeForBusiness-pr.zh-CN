...
若要将自动助理（AA）和订阅者访问（SA）的联系人对象迁移到新的 Lync Server 2013 部署，首先需要使用**CsExUmContact**和**CsExUmContact** cmdlet 将对象从旧版 Office 通信服务器 2007 R2 部署移动到新的 lync server 2013 部署。 在 Exchange 服务器上，然后运行**ExchUCUtil** Windows PowerShell 脚本，为新部署的 Lync pool 执行以下操作：
