...
*语音策略作用域*确定可以应用策略的层次级别。 在 Lync Server 中, 你可以配置具有以下范围级别的语音策略 (按从最具体到最常规的顺序列出)。
...
可以为每个语音策略启用或禁用以下呼叫功能：
  ...
  - **呼叫转接和同时拨打 PSTN 使用**情况使管理员能够指定与呼叫转接和同时拨打的语音政策相同的 PSTN 使用情况, 将呼叫转接和同时拨打的电话限制为仅限内部 Lync 用户。或指定不同于语音政策的 PSTN 使用的自定义 PSTN 使用情况。 默认情况下会使用与用于呼叫转接和同时响铃的语音策略相同的 PSTN 用法。
