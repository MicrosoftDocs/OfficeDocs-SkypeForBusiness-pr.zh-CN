...
Lync PreCall 诊断工具 (PCD) 是一种基于客户端的应用程序，它允许您查看网络的当前状态在即将进行的企业语音呼叫中可能会影响音频质量。

PCD 在网络的最后一个跃点可能是最弱 (的情况下非常有用，例如，在公共 WiFi 网络或家庭用户) 上的便携式电脑。 PCD 创建一个可遍历此网络最终网段的小型数据包流。 然后，该工具将分析数据包流，以估计沿此段流量的抖动和丢失可能影响呼叫质量的方式，然后提供报告。 可以在客户端上连续运行 PCD，甚至在发出呼叫时也可以。 数据包流对带宽的影响不大。
...
感谢你的反馈。 请在上将所有支持问题或问题发送到 [PCD 反馈](mailto:pcdfb@microsoft.com) 别名 <pcdfb@microsoft.com> 。
...
本主题介绍了此工具的以下版本，可免费下载：
  ...
  - Windows 桌面应用程序 ([https://go.microsoft.com/fwlink/?LinkId=327914](https://go.microsoft.com/fwlink/p/?linkid=327914)) 
...
> [!NOTE]  
> PCD 要求将统一通信 Web API () UCWA 安装和配置为支持 Lync Server 部署中的移动客户端。 UCWA 与 Lync Server 一起安装。
...
Lync PCD 包括以下功能：
  ...
  - 在捕捉视图) 模式下，在 (始终保持在24小时内运行

  - 测试运行的历史视图

  - 仅 (Lync PCD for Windows 8 中诊断登录失败) 
