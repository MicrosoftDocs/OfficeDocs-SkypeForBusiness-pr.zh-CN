...
在本主题中，示例主数据库将具有 BE1.contoso.com 的完全限定域名（FQDN），并且镜像数据库将具有 FQDN "BE2.contoso.com"。
...
4.  完成故障转移后，请执行以下操作：
      ...
      - 选择 "仍在运行后端（BE2.contoso.com）" 作为新的 SQL 存储。 为此，请右键单击 " **Enterprise Edition 前端池**" 下的 "池"，然后选择 "**编辑属性**"。 在 "**常规**" 选项卡上的 "**关联**" 下，在 " **SQL Server 存储**" 字段中键入功能后端的 FQDN （在我们的示例中为 BE2.contoso.com）。
...
6.  创建具有相同 FQDN （在此示例中为 DB1.contoso.com）的干净或新服务器作为故障计算机，安装操作系统，然后还原或重新注册证书。 此服务器将作为新镜像运行。
...
10. 使用拓扑生成器安装镜像数据库。 执行以下步骤：
      ...
      - 在后端服务器上启用镜像。 为此，请右键单击 " **Enterprise Edition 前端池**" 下的 "池"，然后选择 "**编辑属性**"。 在 "**常规**" 选项卡上的 "**关联**" 下，选中 "**启用 SQL Server 存储镜像**" 复选框。 如果需要，还可以执行存档和监控。
        ...
        然后，在 "**镜像 SQL Server 存储**" 字段中，键入新服务器的 FQDN （n 本示例，BE1.contoso.com）。 单击" **确定**"。
