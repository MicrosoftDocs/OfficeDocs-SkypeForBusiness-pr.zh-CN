...
您可以通过使用户能够与外部用户进行通信，从而大大提高您的投资在 Lync Server 中的优势。 外部用户可能包括：
  ...
  - **远程用户**    在防火墙外工作并使用其便携式计算机或其他 Lync Server 设备时，您的组织自己的用户。

  - **联合用户**    公司的用户也可以与其他人一起运行 Lync Server。 若要使您的用户能够轻松地与这些用户联系，您可以创建与这些公司的联盟关系。

  - **公共用户**    公共 IM 服务（如 Internet 服务、Yahoo 和 AOL 的 Windows Live 网络提供的 IM 服务）的用户 \! ，以及使用可扩展消息和状态协议的提供程序和服务器的用户 (XMPP) （如 Google 谈话）。
