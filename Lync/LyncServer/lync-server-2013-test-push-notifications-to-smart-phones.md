...
<p>使用 Windows PowerShell 的远程实例运行时，必须为用户分配具有运行 CsMcxPushNotification cmdlet 的权限的 RBAC 角色。 若要查看可使用此 cmdlet 的所有 RBAC 角色的列表，请从 Windows PowerShell 提示符处运行以下命令：</p>
...
若要测试推送通知服务，请调用 CsMcxPushNotification cmdlet。 请确保指定边缘服务器的完全限定域名：
...
如果 CsMcxPushNotification 成功，则 cmdlet 将返回测试结果 "成功"：
...
如果 CsMcxPushNotification 无法连接到推送通知交换所，则 cmdlet 通常不会返回失败的测试结果。 相反，命令通常会完全失败。 例如：

CsMcxPushNotification：从网络接收到504（服务器超时）响应，并且操作失败。 有关详细信息，请参阅异常详细信息。

位于行：1个字符：27

\+\< \< CsMcxPushNotification \< - \< AccessEdgeFqdn lyncedge.mydomain.com

\+CategoryInfo： OperationStopped：（:)\[CsMcxPushNotification\]、FailureResponseException
...
如果推送通知服务失败，通常表明与边缘服务器通信时出现问题，或者与推送通知交换所进行通信时出现问题。 如果您在运行 CsMcxPushNotification 时遇到问题，您应执行的第一件事是验证您的边缘服务器是否正常工作。 执行此操作的一种方法是使用 CsAVEdgeConnectivity cmdlet：
...
如果将 PushNotificationProxyUri 属性设置为 sip:push@push.lync.com 以外的任何其他属性，则可以使用 McxConfiguration cmdlet 纠正该问题。 例如，以下命令将在整个组织中正确设置 URI：
...
从网络接收到一个504（服务器超时）响应，并且操作失败。 有关详细信息，请参阅异常详细信息。

测试 CsMcxConfiguration 还可能会失败，并出现以下错误消息：

CsMcxPushNotification：推送通知请求被拒绝。

位于行：1个字符：27

\+Test-CsMcxPushNotification\<\<\<\<

\+CategoryInfo： OperationStopped：（:)\[CsMcxPushNotification\]、SyntheticTransactionException
