...
---
# <a name="single-consolidated-edge-with-public-ip-addresses-in-lync-server-2013"></a>Lync Server 2013 中具有公用 IP 地址的单一合并边缘
...
如果您的组织需要支持低于15000的访问边缘服务客户端连接、1000主动 Lync Server Web 会议服务客户端连接和500并发 A/V 边缘会话，并且边缘服务器的高可用性并不重要，则此拓扑可提供降低硬件成本和简化部署的优势。 如果你需要更大的容量或需要高可用性，应部署一个扩展的合并边缘服务器拓扑。
...
> [!IMPORTANT]  
> 在边缘服务器上使用公用 IP 地址时，边缘服务器上的默认网关不再是你的防火墙或路由器，而是公共外围边缘的路由器或防火墙，它将是公用地址。 反向代理将继续使用与最外面的外围网络关联的路由器或防火墙。 反向代理和边缘服务器与公用 IP 地址之间的区别在于反向代理仍在使用 NAT，而边缘服务器使用的是路由关系。
...
## <a name="in-this-section"></a>本部分内容
  ...
  - [Lync Server 2013 中的 DNS 摘要-使用公用 IP 地址的单一合并边缘](lync-server-2013-dns-summary-single-consolidated-edge-with-public-ip-addresses.md)
