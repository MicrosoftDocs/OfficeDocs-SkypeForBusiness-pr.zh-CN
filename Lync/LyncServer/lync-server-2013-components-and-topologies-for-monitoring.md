...
监视数据可以与其他类型的数据共享 SQL Server 实例。 通常情况下，呼叫详细信息记录数据库（LcsCdr）和体验质量数据库（QoEMetrics）共享相同的 SQL 实例;这两个监视数据库也通常位于与存档数据库相同的 SQL 实例（LcsLog）中。 关于 SQL Server 实例的唯一真正要求是，SQL Server 的任何一个实例仅限于以下内容：
