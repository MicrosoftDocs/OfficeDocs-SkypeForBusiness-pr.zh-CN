...
在 Windows Server 操作系统上运行 Lync Server 的服务器记录以下四种类型的日志中的事件：
  ...
  - **应用程序日志**    应用程序日志包含由应用程序或程序记录的事件。 开发人员确定要记录的事件。 例如，数据库程序可能会在应用程序日志中记录文件错误。 大多数 Lync Server 2013 相关事件都显示在应用程序日志中。

  - **安全日志**    除了与资源使用相关的事件（如创建、打开或删除文件或其他对象）之外，安全日志还记录诸如有效和无效的登录尝试等事件。 例如，如果启用了登录审核，则登录系统的尝试将记录在安全日志中。
...
在每个事件查看器日志中，Lync Server 2013 记录信息、警告和错误事件。 请密切监视这些日志，以跟踪在 Lync Server 2013 服务器上正在执行的事务类型。 应定期存档日志或使用自动滚动更新以避免空间不足。 由于日志文件可能占用有限的空间量，因此将日志大小增加 (例如，50 MB) 并将其设置为覆盖，以便 Lync Server 2013 服务器可以继续写入新事件。
...
您还可以通过使用以下工具和技术来自动执行事件日志管理：
  ...
  - 适用于 Operations Manager 的 Lync Server 2013 管理包监视 Lync Server 2013 的标准版和企业版。 此版本还在 QoE) 管理包中引入了以前是一个单独的管理包的体验质量 (。
...
**事件触发器**    在 Windows Server 2012 中，可以在 Windows 事件查看器中 "将任务附加到此事件" —管理员可以运行某个程序、发送一封电子邮件或显示屏幕上的邮件。 有关此功能的详细信息，请参阅 Windows Server 2008 R2 主题 [运行任务以响应给定事件](https://technet.microsoft.com/library/cc748900.aspx)。 您还可以使用命令行工具（如 "Eventtrigger.exe"）来创建和查询事件日志，并将程序与特定的已记录事件相关联。 通过使用 Eventtriggers.exe，可以创建在特定事件发生时运行程序的事件触发器。
