...
如果你的组织需要支持少于15000的 Access Edge 服务客户端连接, 1000 活动 Lync Server Web 会议服务客户端连接和500并发的 A/V 边缘会话, 并且不支持边缘服务器的高可用性重要信息: 此拓扑提供降低硬件成本和简化部署的优势。 如果需要更大的容量或需要高可用性, 则需要部署经过缩放的合并边缘服务器拓扑。 有关详细信息, 请参阅以下内容之一:
