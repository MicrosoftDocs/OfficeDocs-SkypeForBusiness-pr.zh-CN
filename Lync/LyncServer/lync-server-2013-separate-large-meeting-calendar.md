...
对于每个大型会议池，您应在该池上维护一个单独的已计划大型会议的日历。 例如，您可以在大型会议池上使用单个用户帐户，并将 Outlook 与 Lync 2013 的 Exchange 和联机会议外接程序一起使用来维护单独的日历。 如果使用多个用户帐户来允许支持人员创建大型会议，则可设置一个单独的日历，以聚合支持人员的成员创建的所有大型会议。
