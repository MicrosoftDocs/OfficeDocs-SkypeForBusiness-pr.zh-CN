...
用于 Outlook Web App 的证书必须是由 Lync Server 信任的证书。 确保证书受 Lync Server 和 Exchange 信任的一种方法是使用内部证书颁发机构在邮箱服务器上创建证书，确保服务器 FQDN 用于使用者名称，并且该 FQDN 显示在 t 中。"证书备用名称" 字段。 在创建证书之后，可以将证书导入到后端服务器。 最终结果是，相同的证书用于两个目的：1） Exchange 统一消息和 Lync 服务器之间的通信;和2） Outlook Web App 与 Lync Server 之间的集成。
