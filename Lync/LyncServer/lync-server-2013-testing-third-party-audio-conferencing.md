...
<p>使用 Windows PowerShell 的远程实例运行时，必须为用户分配具有运行 CsAudioConferencingProvider cmdlet 的权限的 RBAC 角色。 若要查看可使用此 cmdlet 的所有 RBAC 角色的列表，请从 Windows PowerShell 提示符处运行以下命令：</p>
...
**CsAudioConferencingProvider** cmdlet 用于验证用户是否能够建立与他/她的音频会议提供商的连接。 请注意，此 cmdlet 可以采用两种方式之一运行。 许多管理员会使用 CsHealthMonitoringConfiguration cmdlet 为其每个注册器池设置测试用户。 这些测试用户表示预先配置的与综合事务配合使用的一对用户帐户。 （通常，这些是测试帐户，而不是属于实际用户的帐户。）如果为某个池配置了测试用户，则管理员可以对该池运行**CsAudioConferencingProvider** cmdlet，而无需为测试中涉及的用户帐户指定标识（并提供凭据）。
...
示例2中显示的命令测试特定用户（litwareinc\\kenmyer）连接到其音频会议提供商的能力。 为执行此操作，示例中的第一个命令使用 Get-Credential cmdlet 创建 Windows PowerShell 命令行接口凭据对象，其中包含用户 Ken Myer 的名称和密码。 （因为登录名 litwareinc\\kenmyer 已作为参数包括在内，所以 "Windows PowerShell 凭据请求" 对话框仅要求管理员输入 Ken Myer 帐户的密码。）生成的凭据对象存储在名为 $credential 的变量中。

然后第二个命令检查此用户能否连接到其音频会议提供商。 若要执行此任务，请调用 CsAudioConferencingProvider cmdlet 以及以下三个参数： TargetFqdn （注册器池的 FQDN）;UserCredential （包含 Ken Myer 的用户凭据的 Windows PowerShell 对象）;和 UserSipAddress （与提供的用户凭据对应的 SIP 地址）。
