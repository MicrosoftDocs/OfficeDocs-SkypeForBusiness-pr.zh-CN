...
在此 cmdlet 中， \<Pool FQDN\> 是要在其中启用基于位置的路由会议应用程序的池。
...
例如，如果 "UdcAgent" 应用程序的优先级值为 "2"，则 "DefaultRouting" 应用程序的优先级值为 "8"，"ExumRouting" 应用程序的优先级值为 "9"，"OutboundRouting" 应用程序的优先级值为 "10"，则应为基于位置的路由会议应用程序分配优先级值 "3"。 这样做会按以下顺序放置应用程序的优先级：其他应用程序（优先级：0到1）、"UdcAgent" （优先级：2）、基于位置的路由会议应用程序（优先级：3）、其他应用程序（优先级：4到8）、"DefaultRouting" （优先级：9）、"ExumRouting" （优先级：10）和 "OutboundRouting" （优先级：11）。
