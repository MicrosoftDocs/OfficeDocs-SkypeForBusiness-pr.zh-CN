...
> [!NOTE]
> 本主题假定你已将来自中介服务器的任何主干连接的所有主干连接配置为 "Internet 电话服务" (公共交换电话网络 (PSTN) 网关、IP PBX 或会话边界控制器 (SBC)) 的媒体旁路您希望媒体绕过中介服务器的特定网站或服务的提供商 (ITSP)。<BR>本主题还假定你已在拓扑生成器中定义了所有中心网站和分支网站, 使其与你根据在中<A href="lync-server-2013-create-or-modify-a-network-region.md">创建或修改网络区域中的步骤所执行的步骤相匹配的网络区域、网络网站和子网配置相匹配。Lync Server 2013</A>,<A href="lync-server-2013-create-or-modify-a-network-site.md">在 lync server 2013 中创建或修改网络网站</A>, 并<A href="lync-server-2013-associate-a-subnet-with-a-network-site.md">将子网与 lync server 2013 中的网络站点相关联</A>。 如果它们不匹配, 则 "媒体绕过" 将不会成功。
