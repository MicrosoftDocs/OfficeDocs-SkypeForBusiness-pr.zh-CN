...
> [!IMPORTANT]  
> 在一个策略级别应用的 Lync Server 策略设置可以覆盖在另一个策略级别应用的设置。 Lync Server 策略优先级为：用户策略（最具影响力）替代网站策略，然后网站策略将覆盖全局策略（最不影响）。 这意味着，策略设置越接近策略所影响的对象，它对对象的影响越大。
...
这些选项包括以下类型的外部访问：
  ...
  - ****   如果要支持用户对联盟伙伴域的访问权限，请启用与联合用户的通信。 此设置配置用户与其他 SIP 联盟域进行通信的能力，以及 Microsoft Office 365 等托管提供程序。 通过选择此设置，可以选择允许与 XMPP 联盟域进行通信的选项。
  ...
  - **启用与远程用户**   的通信如果您希望在您的组织中的用户（如正在旅行的远程办公和用户）能够通过 Internet 连接到 Lync Server，请启用此选项。

  - **启用与公共用户**   的通信如果希望内部用户能够与公共 IM 提供商联系人通信，例如 Windows Live、Yahoo\!和美洲 Online （AOL）提供的联系人，则启用此选项。
