...
若要确定是否已将服务器到服务器身份验证证书分配给 Microsoft Lync Server 2013，请从 Lync Server 2013 命令行管理程序中运行以下命令：
...
在将证书导入到所有前端服务器之后，可以使用 Lync Server 部署向导而不是 Windows PowerShell 来分配该证书。 要使用部署向导分配证书，请在安装了部署向导的计算机上完成以下步骤：
