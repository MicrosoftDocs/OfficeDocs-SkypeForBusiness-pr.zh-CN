...
Lync 2013 客户端支持虚拟桌面基础结构（VDI）环境中的音频和视频。 用户可以将音频或视频设备（例如，耳机或相机）连接到本地计算机（例如，瘦客户端或重新使用用途的计算机）。 用户可以连接到虚拟机，登录到在虚拟机上运行的 Lync 2013 客户端，并参与实时音频和视频通信，就像客户端在本地运行一样。
