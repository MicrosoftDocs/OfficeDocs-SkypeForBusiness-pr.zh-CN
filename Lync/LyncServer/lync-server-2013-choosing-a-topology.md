...
拓扑选择的主要决策点有高可用性和负载平衡。 高可用性的要求可能会影响负载平衡决策。
  ...
  - **高可用性**  如果需要高可用性, 请在池中至少部署两台边缘服务器。 单个边缘池最多支持十二台边缘服务器。 如果需要更多容量, 可以部署多个边缘池。 一般规则是给定用户群的 10% 需要外部访问。
    
    <div>
    

    > [!IMPORTANT]
    > 拓扑生成器将允许你在一个 Edge 池中最多配置二十台边缘服务器。 在池中经过测试和支持的最大边缘服务器数为 12, 如果拓扑生成器允许大于12的数字, 则不应将单个 Edge 池中的12台以上服务器的暗示支持视为暗示支持。
