...
CsLocationPolicy cmdlet 验证是否已向用户分配了位置策略。 位置策略用于应用与 E9-1-1 功能和客户端位置相关的设置。 位置策略确定用户是否已启用 E9-1, 如果答案是 "是", 则是紧急呼叫的行为。 例如, 您可以使用位置策略来定义紧急呼叫的号码 (美国 911)、是否应自动通知企业安全以及如何路由通话等。
