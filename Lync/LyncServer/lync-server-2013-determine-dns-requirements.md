...
> [!IMPORTANT]  
> Microsoft Lync Server 2013 支持使用 IPv6 寻址。 若要使用 IPv6 地址，还必须为 IPv6 DNS 提供支持，并将 DNS 主机 AAAA (称为 "quad A" ) 记录。 在同时使用 IPv4 和 IPv6 的部署中，最好同时配置和维护 IPv4 的主机 A 记录和 IPv6 的主机 AAAA。 即使您的部署已完全转换为 IPv6，当外部用户仍在使用 IPv4 时，仍可能需要 IPv4 DNS 主机记录。
...
1.  lyncdiscoverinternal.。 \<domain\>    内部 Web 服务上自动发现服务的 (主机) 记录

2.  lyncdiscover.。 \<domain\>    外部 Web 服务上自动发现服务的 (主机) 记录

3.  \_sipinternaltls。 \_tcp。 \<domain\>    用于内部 TLS 连接的 SRV (服务定位器) 记录

4.  \_sipinternal。 \_tcp。 \<domain\>    SRV (服务定位器) 内部 TCP 连接的记录 (仅在允许 TCP 时才执行) 

5.  \_sip。 \_tls。 \<domain\>    用于外部 TLS 连接的 SRV (服务定位器) 记录

6.  sipinternal。 \<domain\>    前端池或控制器的 (主机) 记录，仅在内部网络上可解析

7.  sip。 \<domain\>    用于内部网络上的前端池或控制器的 (主机) 记录，或者客户端为外部客户端时的访问边缘服务

8.  sipexternal。 \<domain\>    当客户端为外部客户端时，访问边缘服务的 (主机) 记录
...
当连接成功时，自动发现服务将返回用户的主池的所有 Web 服务 Url，包括移动服务 (称为 Mcx 由为 IIS) 中的服务创建的虚拟目录、Microsoft Lync Web App 和 Web 计划程序 Url。 但是，内部移动服务 URL 和外部移动服务 URL 均与外部 Web 服务 FQDN 相关联。 因此，无论移动设备是在网络内部还是外部，设备都会通过反向代理在外部连接到移动服务。
...
> [!NOTE]  
> 创建 SRV 记录时，请务必记住，如果在创建 DNS SRV 记录的同一个域中使用 IPv6 寻址) 记录，它们必须指向 DNS A 和 AAAA (。 例如，如果 SRV 记录在 contoso.com 中，则 A 和 AAAA (如果您使用的是 IPv6 寻址) 记录它指向的 fabrikam.com 不能位于中。
...
移动设备支持手动发现服务。 在这种情况下，每个用户都必须使用完整的内部和外部自动发现服务 Uri （包括协议和路径）配置移动设备设置，如下所示：
  ...
  - \<IntPoolFQDN\>用于内部访问的 Https:///AutoDiscover/AutoDiscover.svc/Root
...
## <a name="configuring-split-brain-dns-with-lync-server"></a>使用 Lync Server 配置 Split-Brain DNS

拆分的 DNS 由多个名称（例如，拆分 DNS 或水平拆分 DNS）所熟知。 简单地说，它描述了具有相同命名空间的两个 DNS 区域的 DNS 配置，但一个 DNS 区域服务仅限内部请求，另一个 DNS 区域服务仅限外部的请求。 但是，内部 DNS 中包含的许多 DNS SRV 和 A 记录将不包含在外部 DNS 中，反之亦然也是如此。 在内部和外部 DNS 中都存在相同的 DNS 记录的情况下 (例如，www.contoso.com) 时，返回的 IP 地址将根据启动查询 (内部或外部) 的位置不同而有所不同。

<div>


> [!IMPORTANT]  
> 目前，Lyncdiscover. 和 Lyncdiscoverinternal. DNS 记录不支持移动性或更具体的 Split-Brain DNS。 必须在外部 DNS 服务器上定义 Lyncdiscover.，并且必须在内部 DNS 服务器上定义 Lyncdiscoverinternal.。
...
**内部 DNS：**
  ...
  - 内部 contoso.com 区域包含：
      ...
      - 如果使用 IPv6 寻址) 和用于内部 Lync Server 2013 客户端自动配置的 SRV 记录，则 DNS A 和 AAAA ( (可选的) 
    
      - 如果使用 IPv6 寻址) 或 CNAME 记录来自动发现 Lync Server 2013 Web 服务，则 DNS A 和 AAAA ( (可选) 
    
      - 如果使用 IPv6 寻址) 前端池名称、控制器或控制器池名称以及在企业网络中运行 Lync Server 2013 的所有内部服务器，则 DNS A 和 AAAA (
    
      - DNS A 和 AAAA (如果您对外围网络中每个 Lync Server 2013、边缘服务器的边缘内部接口使用 IPv6 寻址) 记录
    
      - 如果在外围网络中对每个反向代理服务器的内部接口使用 IPv6 寻址) 记录，则 DNS A 和 AAAA ( (对反向代理管理的可选) 
    
      - 外围网络中的所有 Lync Server 2013 边缘服务器内部边缘接口使用内部 DNS 区域将查询解析为 contoso.com
    
      - 在企业网络中运行 Lync Server 2013 和运行 Lync 2013 的客户端的所有服务器指向内部 DNS 服务器，以便将查询解析为 contoso.com，或使用每台边缘服务器上的 HOSTS 文件，并列出 A 和 AAAA (如果您使用 IPv6 寻址) 下一个跃点服务器的记录，特别是控制器或控制器 VIP、前端池 VIP 或 Standard Edition server
...
**外部 DNS：**
  ...
  - 外部 contoso.com 区域包含：
      ...
      - DNS A 和 AAAA (如果您使用的是适用于 Lync Server 2013 客户端自动配置的 IPv6 寻址) 和 SRV 记录 (可选的) 
    
      - 如果使用 IPv6 寻址) 或 CNAME 记录自动发现 Lync Server 2013 Web 服务以便与移动功能配合使用，则 DNS A 和 AAAA (。
    
      - DNS A 和 AAAA (如果您在外围网络中使用每个 Lync Server 2013、边缘服务器或硬件负载平衡器虚拟 IP (VIP) 的边缘外部接口的 IPv6 寻址) 和 SRV 记录
    
      - DNS A 和 AAAA (如果您对外围网络中的反向代理服务器池的反向代理服务器或 VIP 的外部接口使用 IPv6 寻址) 记录

</div>

<div>

## <a name="automatic-configuration-without-split-brain-dns"></a>不 Split-Brain DNS 的自动配置

如果内部 DNS 区域包含 sipinternaltls，则使用裂脑 DNS，在内部登录的 Lync Server 2013 用户可以利用自动配置 \_ 。 \_正在使用的每个 SIP 域的 tcp SRV 记录。 但是，如果不使用拆分的 DNS，则运行 Lync 的客户端的内部自动配置将不起作用，除非实现本部分后面所述的解决方法之一。 这是因为 Lync Server 2013 要求用户的 SIP URI 与为自动配置指定的前端池的域相匹配。 这也是早期版本的 Communicator 的情况。

例如，如果您有两个 SIP 域在使用中，则需要以下 DNS 服务 (SRV) 记录：

  - 如果用户登录为 bob@contoso.com，以下 SRV 记录将适用于自动配置，因为用户的 SIP 域 (contoso.com) 与自动配置前端池) 的域相匹配：
...
为了进行比较，如果用户登录为 tim@litwareinc.com，以下 DNS SRV 记录将不能用于自动配置，因为客户端的 SIP 域 (litwareinc.com) 与池位于 (fabrikam.com) 中的域不匹配：

 \_sipinternaltls。 \_tcp.litwareinc.com。 SRV 0 0 5061 pool01.fabrikam.com 中的86400

如果运行 Lync 的客户端需要 "自动配置"，请选择下列选项之一：

  - **组策略对象**    使用组策略对象 (Gpo) 来填充正确的服务器值。
  ...
  - **匹配内部区域**    如果要使用 IPv6 寻址) 与用于自动配置的 Lync Server 2013 池对应的 contoso.com，请在内部 DNS 中创建与外部 DNS 区域相匹配的区域 (例如，) 并创建 DNS A 和 AAAA (。 例如，如果用户驻留在 pool01.contoso.net 上，但以 bob@contoso.com 的形式登录到 Lync，请创建一个名为 contoso.com 的内部 DNS 区域并将其包含在其中，创建一个 DNS A 和 AAAA (如果) 记录对 pool01.contoso.com 使用 IPv6 寻址。

  - **针点内部区域**    如果您在内部 DNS 中创建整个区域不是一个选项，则可以创建 pin 点 (，即专用) 区域与自动配置所需的 SRV 记录相对应，并使用 dnscmd.exe 填充这些区域。 Dnscmd.exe 是必需的，因为 DNS 用户界面不支持创建 pin 点区域。 例如，如果 SIP 域为 contoso.com，并且您有一个名为 pool01 的前端池，该池包含两台前端服务器，则您需要内部 DNS 中的以下 pin 点区域和记录：
    ...
    如果您的环境中包含第二个 SIP 域 (例如，fabrikam.com) ，您需要内部 DNS 中的以下 pin 点区域和记录：
...
有关详细信息，请参阅 DMTF 博客文章 "Communicator 自动配置和 Split-Brain DNS"，网址为 [https://go.microsoft.com/fwlink/p/?linkId=200707](https://go.microsoft.com/fwlink/p/?linkid=200707) 。
...
## <a name="configuring-the-domain-name-system-dns-for-disaster-recovery"></a>配置域名系统 (DNS) 以进行灾难恢复

若要将 DNS 配置为将 Lync Server 2013 Web 流量重定向到灾难恢复和故障转移站点，必须使用支持 GeoDNS 的 DNS 提供程序。 您可以设置 Web DNS 记录以支持灾难恢复，以便即使一个完整的前端池停止运行时，使用 Web 服务的功能仍将继续进行。 此灾难恢复功能支持自动发现 (Lyncdiscover. URL) 、"满足" 和 "拨入" 简单 Url。

如果在 GeoDNS 提供程序中使用适用于内部和外部 Web 服务解析的 IPv6) 记录，则可以定义和配置其他 DNS 主机 (and AAAA。 以下详细信息假设您的提供程序对成对的池、地理位置和 GeoDNS 所支持的循环 DNS，或配置为使用 Pool1 作为主池，并在发生通信丢失或硬件故障时故障转移到 Pool2。
...
DNS 负载平衡通常在应用程序级别实现。 应用程序 (例如，运行 Lync) 的客户端尝试连接到池中的某个 IP (地址（如果使用 IPv6 寻址) 池完全限定域名 (FQDN) 的记录查询），从而尝试连接到池中的某个服务器。

例如，如果名为 pool01.contoso.com 的池中有三台前端服务器，则会发生以下情况：

  - 为 pool01.contoso.com 运行 Lync 查询 DNS 的客户端。 查询将返回三个 IP 地址并将其缓存，如下所示 (不一定按此顺序) ：
...
> [!NOTE]  
> 基于 DNS 的负载平衡不同于 dns 轮循机制 (DNS RR) 这通常指的是负载平衡，具体取决于 DNS 以提供与池中的服务器对应的 IP 地址的不同顺序。 通常，DNS RR 只启用负载分配，但不启用故障转移。 例如，如果在使用 IPv6 寻址) 查询失败时，与 DNS A 和 AAAA (返回的一个 IP 地址的连接，则该连接将失败。 因此，DNS 轮循机制本身不如基于 DNS 的负载平衡可靠。 您可以将 DNS 循环与 DNS 负载平衡结合使用。
...
如果 DNS SRV 查询返回了多个 DNS 记录，则访问边缘服务将始终使用最小的数值优先级和最高的数值权重来选取 DNS SRV 记录。 Internet 工程任务组文档 "用于指定服务位置 (DNS SRV) " 的 DNS RR <http://www.ietf.org/rfc/rfc2782.txt> 指定如果定义了多个 DNS srv 记录，则先使用优先级，再使用 "权重"。 例如，DNS SRV 记录 A 的权重为20，优先级为40，DNS SRV 记录 B 的权重为10，优先级为50。 将选择优先级为40的 DNS SRV 记录 A。 以下规则适用于 DNS SRV 记录选择：
