---
title: Lync Server 2013：确定您的系统要求
description: Lync Server 2013：确定您的系统要求。
ms.reviewer: ''
ms.author: v-lanac
author: lanachin
f1.keywords:
- NOCSH
TOCTitle: Determining your system requirements
ms:assetid: 620e81e2-42df-4eda-8498-bd56a14aa0e1
ms:mtpsurl: https://technet.microsoft.com/en-us/library/Gg398438(v=OCS.15)
ms:contentKeyID: 48184286
ms.date: 07/23/2014
manager: serdars
mtps_version: v=OCS.15
ms.openlocfilehash: 5ceca8eabb234ae7fd483372ff5e611664ecc4fa
ms.sourcegitcommit: d42a21b194f4a45e828188e04b25c1ce28a5d1ae
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 10/17/2020
ms.locfileid: "48569488"
---
# <a name="determining-your-system-requirements-for-lync-server-2013"></a>确定 Lync Server 2013 的系统要求

<div data-xmlns="http://www.w3.org/1999/xhtml">

<div class="topic" data-xmlns="http://www.w3.org/1999/xhtml" data-msxsl="urn:schemas-microsoft-com:xslt" data-cs="https://msdn.microsoft.com/">

<div data-asp="https://msdn2.microsoft.com/asp">



</div>

<div id="mainSection">

<div id="mainBody">

<span> </span>

_**上次修改的主题：** 2014-01-02_

运行 Lync Server 的所有服务器都必须满足特定的最低系统要求。 Lync Server 的系统要求包括服务器硬件、要在每台服务器上安装的操作系统以及相关的软件要求（如 Windows 更新和必须安装在服务器上的其他软件）。

<div>


> [!IMPORTANT]  
> Lync Server 仅可用于64位版本，这需要64位硬件和一个64位版本的 Windows Server。 例外是 Microsoft Lync Server 2013，即规划工具，可在32位版本中使用。



</div>

<div>


> [!NOTE]  
> 有关 Active Directory 支持、支持的拓扑、服务器并置和其他可支持性问题的详细信息，请参阅 <A href="lync-server-2013-supportability.md">Lync server 2013 的可支持</A>性。



</div>

<div>

## <a name="in-this-section"></a>本部分内容

  - [Lync Server 2013 的服务器硬件平台](lync-server-2013-server-hardware-platforms.md)

  - [Lync Server 2013 中的服务器和工具操作系统支持](lync-server-2013-server-and-tools-operating-system-support.md)

  - [Lync Server 2013 中的数据库软件支持](lync-server-2013-database-software-support.md)

  - [Lync Server 2013 的其他软件要求](lync-server-2013-additional-software-requirements.md)

</div>

<div>

## <a name="see-also"></a>另请参阅


[Lync Server 2013 中的客户端和设备硬件支持](lync-server-2013-client-and-device-hardware-support.md)  
[Lync Server 2013 的可支持性](lync-server-2013-supportability.md)  
  

</div>

</div>

<span> </span>

</div>

</div>

</div>

