...
可以使用注册器配置代理服务器身份验证方法。指定的身份验证协议确定池中的服务器向客户端发出的质询类型。可以选择以下协议：
  ...
  - **证书身份验证**   当服务器需要从 Lync Phone Edition 客户端、公共区域手机、lync 2013 和 lync Windows 应用商店应用获取证书时，这是新的身份验证方法。 在 Lync Phone 版客户端上，用户登录并通过提供个人识别码（PIN）成功进行身份验证后，Lync Server 2013 将 SIP URI 设置到手机，并为该电话设置一个 Lync Server 签名证书或标识 Joe （如： SN=joe@contoso.com）的用户证书。 此证书用于使用注册器和 Web 服务进行身份验证。
