...
1.  在相应的 DNS 服务器上，依次单击 "**开始**"、"**控制面板**"、"**管理工具**" 和 " **DNS**"。
    ...
    > [!IMPORTANT]  
    > 您需要为远程用户和联盟伙伴配置 DNS，以便有：1个外部 dns 查找的外部 DNS 条目;2）由外围网络（也称为 DMZ、隔离区域和屏蔽子网）中的边缘服务器使用的条目，包括运行 Lync Server 2013 的内部服务器的记录;和3）运行 Lync Server 2013 的内部客户端和服务器查找的内部 DNS 条目。
