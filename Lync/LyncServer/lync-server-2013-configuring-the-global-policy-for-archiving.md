...
当您部署前端服务器时，Lync Server 将创建一个用于存档的全局策略。 默认情况下，将在全局策略中禁用存档。 全局策略控制是否为您的整个部署启用存档以进行内部和外部通信，除非您设置网站策略或用户策略，后者将覆盖全局策略，或者将 Microsoft Exchange 集成用于部分或所有你的用户。 如果使用 Microsoft Exchange 集成，则全局策略不会应用于托管在 Exchange 2013 上并将邮箱置于就地保留状态的任何用户。
