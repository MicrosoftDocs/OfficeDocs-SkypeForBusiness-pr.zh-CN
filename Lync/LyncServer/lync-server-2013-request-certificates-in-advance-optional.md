...
此部署文档提供使用证书向导作为安装过程的一部分请求证书的过程, 如在[Lync Server 2013 中的 "配置服务器的证书](lync-server-2013-configure-certificates-for-servers.md)" 中所述[。配置证书Lync Server 2013 中的 Director](lync-server-2013-configure-certificates-for-the-director.md), 并在此部署文档的[Lync server 2013 部分中安装中介服务器的文件](lync-server-2013-install-the-files-for-mediation-server.md)。 如果提前申请证书, 则必须根据需要在这些分区中修改证书部署过程, 以便导入和分配证书, 而不是在部署时请求证书。
