...
SLA 是一个文档, 用于定义客户期望的服务。 此文档的复杂性和内容很大程度上取决于客户是内部 (在你的环境中) 还是外部客户。
...
Sla 包括定义最低级别可用性、支持和容量的条件的部分。
  ...
  - **可用性**   定义了网站和其他 Lync 服务将在其上可用的时间和操作系统。 应定义影响服务可用性的任何例行维护。 定义影响服务的外部因素, 例如, 失去互联网连接。

  - **支持**   定义系统支持将在何时可用的时间。 指定用于客户联系支持人员的方法、如何对事件进行分组以及确定响应和解决事件的目标时间。 定义反馈给客户的频率和内容。

  - **容量**   定义了 Lync 网站的最大启用大小以及超过限制时要采取的步骤。 定义执行标准任务 (如从文档库检索文档的时间) 的最大启用时间。 定义每个 Lync 池的最大用户数, 并同意在添加更多用户时增加容量的进程。
