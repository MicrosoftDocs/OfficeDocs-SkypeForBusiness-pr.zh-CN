---
title: Lync Server 2013：集中日志记录 cmdlet
ms.reviewer: ''
ms.author: v-lanac
author: lanachin
f1.keywords:
- NOCSH
TOCTitle: Centralized Logging cmdlets
ms:assetid: 8ba5bcae-8b99-489c-9355-6e77d4ad9100
ms:mtpsurl: https://technet.microsoft.com/en-us/library/JJ205064(v=OCS.15)
ms:contentKeyID: 48184743
ms.date: 07/23/2014
manager: serdars
mtps_version: v=OCS.15
ms.openlocfilehash: 653e259b2e76753363a4a9d5562e93f57897f9a3
ms.sourcegitcommit: 831d141dfc5a49dd764cb296b73b63e5a9f8e599
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 02/21/2020
ms.locfileid: "42191067"
---
<div data-xmlns="http://www.w3.org/1999/xhtml">

<div class="topic" data-xmlns="http://www.w3.org/1999/xhtml" data-msxsl="urn:schemas-microsoft-com:xslt" data-cs="https://msdn.microsoft.com/">

<div data-asp="https://msdn2.microsoft.com/asp">

# <a name="centralized-logging-cmdlets-in-lync-server-2013"></a>Lync Server 2013 中的集中日志记录 cmdlet

</div>

<div id="mainSection">

<div id="mainBody">

<span> </span>

_**上次修改的主题：** 2012-10-08_

集中日志记录 cmdlet 为管理员提供了一种管理和配置 Microsoft Lync Server 2013 中引入的集中日志记录功能的方法。 集中日志记录允许管理员同时在多台计算机上启用或禁用事件跟踪。

<div>

## <a name="centralized-logging-cmdlets"></a>集中日志记录 Cmdlet

集中日志记录 cmdlet 使您能够管理 Lync Server 2013 中引入的集中日志记录服务：

**集中日志记录 Cmdlet**

  - [New-csclsconfiguration](https://technet.microsoft.com/library/JJ619179(v=OCS.15))

  - [新 New-csclsconfiguration](https://technet.microsoft.com/library/JJ619177(v=OCS.15))

  - [New-csclsconfiguration](https://technet.microsoft.com/library/JJ619191(v=OCS.15))

  - [New-csclsconfiguration](https://technet.microsoft.com/library/JJ619182(v=OCS.15))

<!-- end list -->

  - [搜索-Search-csclslogging](https://technet.microsoft.com/library/JJ619189(v=OCS.15))

  - [显示-Search-csclslogging](https://technet.microsoft.com/library/JJ619173(v=OCS.15))

  - [启动-Search-csclslogging](https://technet.microsoft.com/library/JJ619190(v=OCS.15))

  - [Search-csclslogging](https://technet.microsoft.com/library/JJ619180(v=OCS.15))

  - [Sync-Search-csclslogging](https://technet.microsoft.com/library/JJ619169(v=OCS.15))

  - [更新-Search-csclslogging](https://technet.microsoft.com/library/JJ619170(v=OCS.15))

<!-- end list -->

  - [新 CsClsProvider](https://technet.microsoft.com/library/JJ619187(v=OCS.15))

<!-- end list -->

  - [CsClsRegion](https://technet.microsoft.com/library/JJ204879(v=OCS.15))

  - [新 CsClsRegion](https://technet.microsoft.com/library/JJ204658(v=OCS.15))

  - [CsClsRegion](https://technet.microsoft.com/library/JJ204971(v=OCS.15))

  - [CsClsRegion](https://technet.microsoft.com/library/JJ204746(v=OCS.15))

<!-- end list -->

  - [New-csclsscenario](https://technet.microsoft.com/library/JJ205091(v=OCS.15))

  - [新 New-csclsscenario](https://technet.microsoft.com/library/JJ205022(v=OCS.15))

  - [New-csclsscenario](https://technet.microsoft.com/library/JJ205010(v=OCS.15))

  - [New-csclsscenario](https://technet.microsoft.com/library/JJ204622(v=OCS.15))

<!-- end list -->

  - [CsClsSearchTerm](https://technet.microsoft.com/library/JJ205061(v=OCS.15))

  - [CsClsSearchTerm](https://technet.microsoft.com/library/JJ204911(v=OCS.15))

<!-- end list -->

  - [CsClsSecurityGroup](https://technet.microsoft.com/library/JJ205285(v=OCS.15))

  - [新 CsClsSecurityGroup](https://technet.microsoft.com/library/JJ205359(v=OCS.15))

  - [CsClsSecurityGroup](https://technet.microsoft.com/library/JJ204958(v=OCS.15))

  - [CsClsSecurityGroup](https://technet.microsoft.com/library/JJ204700(v=OCS.15))

</div>

</div>

<span> </span>

</div>

</div>

</div>

