...
Lync Server 2013 的管理包现在使用计算机级发现，而不是在 Lync Server 2010 中使用的中央发现机制。 这意味着每个 System Center 代理实质上都会发现自己，并报告它是否存在于 System Center Operations Manager 中。 使用计算机级别发现可简化对 System Center 基础结构的管理，还会启用不同版本的 Lync Server 管理包 (例如，lync server 2010 的管理包和 Lync Server 2013 的管理包) 可更轻松地共存。
...
若要提名非中央管理存储计算机 (例如，Lync Server 前端) 服务器处理中央发现，您需要在非中心管理存储服务器上创建以下注册表项：
...
3.  在注册表编辑器中，展开 " **HKEY \_ 本地 \_ 计算机**"，展开 " **软件**"，再展开 " **Microsoft**"，然后展开 " **实时通信**"。
