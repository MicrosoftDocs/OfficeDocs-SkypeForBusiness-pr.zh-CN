...
你可以通过为用户创建和配置存档策略，然后将策略应用于特定用户或用户组，为特定用户启用或禁用存档。 用户策略会覆盖任何全局策略或站点策略。 仅当你不使用 Microsoft Exchange 集成时，或者，如果你使用的是 Microsoft Exchange 集成，但某些用户没有托管在 Exchange 2013 并将其邮箱放在原地保留中，则仅适用于存档策略。
