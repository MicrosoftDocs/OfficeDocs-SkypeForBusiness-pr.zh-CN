...
8.  可选如果要为此网站提供位置设置, 请从 "**位置策略**" 下拉列表中选择一个位置策略。
    ...
    > [!NOTE]  
    > 位置策略将特定的增强 9-1-1 (E9-1) 和客户端位置设置分配给该网站。 你可以从 "<STRONG>网络配置</STRONG>" 组的 "<STRONG>位置策略</STRONG>" 页查看可用位置策略的详细信息, 或者创建新的位置策略。 有关详细信息, 请参阅<A href="lync-server-2013-viewing-location-policy-information.md">在 Lync Server 2013 中查看位置策略信息</A>。
