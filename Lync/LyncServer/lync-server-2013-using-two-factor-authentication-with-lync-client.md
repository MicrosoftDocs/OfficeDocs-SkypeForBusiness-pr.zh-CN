...
2.  选择 "**启动** \> **所有程序** \> **Microsoft \> Lync lync 2013**"。
    ...
    您应该会看到 Lync 登录屏幕。
      ...
      - 如果不正确，或者如果该框为空，请输入你的 Lync 登录地址（这通常与你的电子邮件地址相同）。
...
<li><p>单击窗口右上角的 "<strong>关闭</strong>" 按钮（X）。</p></li>
</ol></td>
</tr>
<tr class="even">
<td><p>注销</p></td>
...
有些组织现在使用一个称为双重身份验证的多步骤登录过程来提高 Lync 2013 用户的安全性。 如果您希望使用此选项，则需要 "智能卡" 才能登录 Lync。 智能卡分为两种种类，物理和虚拟：
  ...
  - ****   有关信用卡大小的物理信息。 您在登录时将其插入到智能卡读取器中。
...
3.  当系统提示您登录到证书页时，请使用您的域帐户（而不是计算机的管理员）登录。
...
10. 选择此选项可强制用户（你）在首次使用智能卡时更改 PIN。

11. 现在，将空白智能卡插入到连接到注册站的读卡器中，然后单击 "**注册**"。

12. 出现提示时，输入您的个人识别码（PIN），然后单击 **"确定"**。
...
13. 选择此选项可强制用户（你）在首次使用智能卡时更改 PIN。
