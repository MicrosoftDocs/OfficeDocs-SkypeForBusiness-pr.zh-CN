...
呼叫应答组号码范围必须符合以下规则：
  ...
  - 有效值: 必须与正则表达式字符串 (\[\\\*|\#\]？\[1-9\]\\d{0,7}) |(\[1-9\]\\d{0,8})。 这意味着该值必须是以字符\*或\#数字1到9开头的字符串 (第一个字符不能为零)。 如果第一个字符是\*或\#, 则以下字符必须是1到9的数字 (不能为零)。 后续字符可以是0到9的任何数字, 最多可有7个附加字符\#(例如 "6000\*"、"92000\*"、"95551212" 和 "915551212")。 如果第一个字符不\*是或\#, 则第一个字符必须是数字1到 9 (不能为零), 后跟八个字符, 每个字符都是0到9的数字 (例如, "915551212"、"41212"、"300")。
