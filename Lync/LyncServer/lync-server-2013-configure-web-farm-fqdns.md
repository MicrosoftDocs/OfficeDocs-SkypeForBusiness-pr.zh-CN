...
在拓扑生成器中定义 Standard Edition server、前端池、控制器或控制器池的配置时，请将外部 web 服务完全限定的域名配置 (FQDN) 。 在托管在 Standard Edition server 或前端池中的客户端登录过程中，配置的 web 服务的 Fqdn 是通过带内设置的方式发送的。 如果需要添加或更改外部 web 服务 URL，请使用拓扑生成器配置或重新配置 web 服务配置（使用本主题中的过程）。
