...
Microsoft SIP 处理语言（MSPL）服务器应用程序是仅脚本的应用程序，使用的是 MSPL 脚本编写语言，而不是 Microsoft Lync 2010 API。 某些 MSPL 服务器应用程序被指定为关键。 如果脚本非常关键，则必须在系统启动过程中启动该脚本，才能启动 Lync Server 2013。 如果运行 Lync Server 时脚本失败，则服务器不会关闭，但会停止向脚本发送流量，并会在事件日志中写入错误。
