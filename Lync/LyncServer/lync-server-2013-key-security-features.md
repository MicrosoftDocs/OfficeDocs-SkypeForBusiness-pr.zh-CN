...
Lync Server 2013 不支持在边缘服务器的内部接口上使用网络地址转换（NAT），但它支持将访问边缘服务、Web 会议边缘服务和 A/V 边缘服务的外部接口放置在为单一和扩展的合并边缘服务器拓扑执行网络地址转换（NAT）的路由器或防火墙后面。 硬件负载平衡器后面的多台边缘服务器不能使用 NAT。 如果多台边缘服务器在其外部接口上使用 NAT，则需要域名系统（DNS）负载平衡。 反过来，使用 DNS 负载平衡可以减少边缘服务器池中每个边缘服务器的公用 IP 地址的数量。 有关详细信息，请参阅[在 Lync Server 2013 中规划外部用户访问](lync-server-2013-planning-for-external-user-access.md)。
...
部署向导可以自动填充使用者名称（SNs）和使用者备用名称（San），从而降低了包含不必要的和可能不安全的条目的可能性。
...
Lync Server 2013 的设计和开发符合 Microsoft 可信赖计算安全开发生命周期（SDL），这在中进行了<https://go.microsoft.com/fwlink/?linkid=68761>介绍。
  ...
  - **默认情况下受信任**   默认情况下，将对 Lync Server 2013 中的网络通信进行加密。 由于所有服务器都使用证书和 Kerberos 身份验证、TLS、安全实时传输协议（SRTP）和其他行业标准加密技术（包括128位高级加密标准（AES）加密），因此几乎所有 Lync网络上的服务器数据受到保护。 此外，基于角色的访问控制可以部署运行 Lync Server 2013 的服务器，以便每个服务器角色仅运行服务，并且仅拥有与这些服务相关的权限，这些服务适用于服务器角色。

  - **已通过部署**   信任所有 Lync Server 2013 文档包括最佳实践和建议，可帮助您确定和配置最佳安全级别以实现部署，并评估激活非默认选项的安全风险。
