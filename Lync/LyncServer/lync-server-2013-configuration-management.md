...
有许多工具可用于发现、审核和报告资源。 本部分将讨论其中的一些工具。
  ...
  - **自动化工具**   根据您的企业规模和组织需求, 您可能需要考虑使用自动工具。 诸如 System Center Configuration Manager 之类的工具合并标准报表模板 (如服务包级别), 还允许你创建自定义报表, 例如自定义应用程序。 System Center Configuration Manager 还可以用于报告硬件和软件配置。
