...
服务器到服务器连接依靠 MTLS 进行相互身份验证。 在 MTLS 连接上，发出消息的服务器和接收消息的服务器交换来自相互信任的 CA 的证书。 证书可向一台服务器证明另一台服务器的身份。 在 Lync Server 2013 部署中，由于 Active Directory 域的所有成员，所有内部客户端和服务器都会自动认为在其有效期内由企业 CA 颁发且未被颁发 CA 吊销的证书。信任该域中的企业 CA。 在联盟方案中，发证 CA 必须得到联盟伙伴双方的信任。 如果需要，任一伙伴均可以使用不同的 CA，只要该 CA 同时获得另一个伙伴的信任即可。 通过在边缘服务器的受信任的根 CA 中包含伙伴的根 CA 证书，或通过使用双方信任的第三方 CA，可以很轻松地建立此信任。

TLS 和 MTLS 有助于防止窃听攻击和中间人攻击。 在中间人攻击中，攻击者通过其计算机重新路由两个网络实体之间的通信，而这两个网络实体对此毫不知情。 TLS 和 Lync Server 2013 规范的受信任服务器（仅在拓扑生成器中指定的服务器）通过使用公钥加密进行的端到端加密来降低中间人攻击在应用程序层中部分发生的风险在两个终结点之间，攻击者必须拥有具有相应私钥的有效且受信任的证书，并颁发给客户端要与之通信的服务的名称，以解密通信。 不过，最终，您还是必须遵循有关您的网络基础结构（在此示例中为企业 DNS）的最佳安全做法。 Lync Server 2013 假定 DNS 服务器的信任方式与域控制器和全局编录的信任方式相同，但 DNS 确实提供了针对 DNS 劫持攻击的保护级别，阻止攻击者的服务器成功响应对哄骗名称的请求。
