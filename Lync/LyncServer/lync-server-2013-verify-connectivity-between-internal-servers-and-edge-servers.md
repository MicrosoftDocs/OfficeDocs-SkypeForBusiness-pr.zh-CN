...
你可以通过在中央管理存储所在的内部计算机（或已加入的任何加入了 Lync Server 2013 核心组件（OcsCore）的计算机上）运行 Windows PowerShell **CsManagementStoreReplicationStatus** cmdlet 来验证对边缘的配置信息的复制。 初始结果可能表明状态为 "False"，而不是 "True" 用于复制。 如果是这样，请运行**CsManagementStoreReplication** cmdlet 并等待复制完成，然后再再次运行**Get CsManagementStoreReplicationStatus** 。
