...
值得注意的是，此报告在用作真正的清单报告时存在几点限制。 一件事是，IP 电话报告只列出在指定时间段内登录到 Lync Server 的所有电话，按上次登录时间进行排序。 如果某个电话在指定的时间段内未登录，那么就不会在清单报告中列出。 这包括在该时间段开始前已登录并且在指定的时间间隔内仍保持登录的电话。 例如，假设您想要查看 2012 年 7 月的所有电话清单。 同时假设几个电话登录到 Lync Server 的6月30日，2012，在7月1日之前仍在登录。 这些电话将不会在 7 月 1 日的清单报告上出现。
