...
验证企业语音的软件和环境先决条件后, 您可以使用以下内容之一:
  ...
  - 按照在[Lync server 2013 中安装中介服务器的文件](lync-server-2013-install-the-files-for-mediation-server.md)中所述, 安装中介服务器, 但仅当你希望部署独立的中介服务器或池时, 才需要部署独立的中介服务器或池, 因为已将中介服务器作为前端池或标准的一部分进行安装Collocated 版服务器部署过程。
