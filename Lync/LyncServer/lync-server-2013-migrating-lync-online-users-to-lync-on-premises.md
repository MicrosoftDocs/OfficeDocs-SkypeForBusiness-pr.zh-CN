...
2.  确认在本地边缘服务器上，您具有允许连接到 Lync Online 的证书链，如下表所示。 你可以在此处下载此链： https://support.office.com/article/office-365-certificate-chains-0c03e6b3-e73f-4316-9e2b-bf4091ae96bb
    ...
    <tr class="even">
    <td><p>Microsoft Internet 颁发机构 (新的 CA 证书) </p></td>
    <td><p>中间 CA</p></td>
    </tr>
    <tr class="odd">
    <td><p>MSIT Machine Auth CA2 (新的颁发 CA2) </p></td>
    <td><p>中间 CA</p></td>
    </tr>
    </tbody>
    </table>
...
3.  在本地 Active Directory 中，为本地 Lync 启用受影响的用户帐户。 您可以通过键入以下 cmdlet 为单个用户执行此操作：
    ...
    或者，您可以创建一个脚本，从文件中读取用户名并将其作为 Enable-CsUser cmdlet 的输入提供：
...
5.  更新一些 DNS 记录以将所有 SIP 流量定向到本地 Lync：
      ...
      - 如果您的组织使用的是拆分 DNS (有时称为 "拆分的 DNS" ) ，请确保将通过内部 DNS 区域解析名称的用户定向到前端池。
