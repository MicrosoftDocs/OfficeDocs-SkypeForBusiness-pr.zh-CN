...
**A：** 如果您有以下两种情况，您将获得 Skype 连接许可：
  ...
  - Lync Server （2010或2013） + Lync Server 标准客户端访问许可证（"Cal"; 2010 或2013），用于组织中将连接到 Skype 的用户和/或设备。 
...
**A：** 如果你要使用具有非 Windows Live 域的帐户或 ID 添加 Windows live Messenger 用户，请按以下格式输入该\<地址：用户名\>（\<域名\>） @msn .com，其中\<的域名\>是用户的电子邮件地址中的域名。 例如，如果你想要添加 ted@contoso.com，你可以使用以下格式：李小明（contoso） @msn .com。 有关由 Windows Live 管理的域的列表，请参阅 "受支持的域" 部分（在 "安装实时通信服务器服务包1的公共即时消息后出现的已知问题http://support.microsoft.com/?kbid=897567）" 部分中。
