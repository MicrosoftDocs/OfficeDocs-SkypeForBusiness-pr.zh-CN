...
与可用性相关的度量标准如下所示:
  ...
  - **可用性**   通常表示为系统或服务可以访问的时间, 与它处于关闭状态的时间相比较。 它通常表示为百分比。 (你可能会看到 "三个 9" 或 "五个 9" 的引用。 这是 99.9% 或 99.999% 的可用容量。)
...
**可用性 = (MTBF-MTTR)/MTBF**   例如, 如果服务器在六个月的时间段内失败两次, 并且平均不超过20分钟, 则 MTBF 为3个月或90天, 并且 MTTR 为20分钟。 因此, 可用性 = (90 天-20 分钟)/90 days = 99.985%。
