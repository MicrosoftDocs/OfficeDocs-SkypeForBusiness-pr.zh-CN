...
在某些情况下, 你可能会发现你的合成事务因服务器超时错误而失败 (错误代码 504)。 这些错误通常由防火墙问题引起。 执行合成事务时, 该事务将在 MonitoringHost 进程下运行;然后, MonitoringHost 将启动一个 PowerShell .exe 进程的一个实例。 如果你的防火墙阻止了 MonitoringHost 或 cluster.exe, 则合成事务将失败, 并将生成504错误。

若要解决此问题, 你应该在本地计算机上手动创建 MonitoringHost 和 PowerShell 的入站防火墙规则。 这可以通过 Windows 防火墙或第三方本地防火墙软件完成, 具体取决于你的服务器的预先配置。

如果在综合事务主机和要监视的 Lync 服务器之间使用网络防火墙设备, 应将主机视为客户端计算机, 并观察来自[端口和协议的所有防火墙端口要求对于 Lync Server 2013 中的内部服务器](lync-server-2013-ports-and-protocols-for-internal-servers.md)。
