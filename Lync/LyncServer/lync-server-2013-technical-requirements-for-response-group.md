...
如果使用 Windows Media Audio)  ( 响应组音乐和公告的文件，则所有前端服务器或运行响应组应用程序的标准版服务器必须为运行 windows server 2008 R2 的服务器安装 windows Media Format Runtime，或为运行 Windows Server 2012 或 Windows Server 2012 R2 的服务器安装 Microsoft Media Foundation。 对于 Windows Server 2008 R2，Windows Media Format Runtime 将作为 Windows 桌面体验的一部分安装。
...
响应组应用程序支持 wave ( .wav) 文件格式和 Windows Media ( 音频) 响应组邮件的文件格式、保留音乐或交互语音响应 (IVR) 问题。
