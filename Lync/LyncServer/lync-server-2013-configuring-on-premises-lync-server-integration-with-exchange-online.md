...
使用本地 Lync Server 2013 部署的客户可以在混合部署模式下在 Microsoft Exchange Online 中配置与 Microsoft Outlook Web App 的互操作性。 互操作性功能包括单一登录和即时消息 (IM) 和状态与 Outlook Web App 界面的集成。 若要启用此集成，您必须完成以下任务，在本地 Lync Server 部署中配置边缘服务器：
...
> [!NOTE]
> 如果使用由世纪互联运营的 Office 365，请将 ProxyFqdn 参数的值替换为以下 ( 示例中的<STRONG>ProxyFqdn</STRONG>参数的值： "exap.um.outlook.com" ) 和由世纪互联运营的服务的 FQDN： "exap.um.partner.outlook.cn"。



</div>

  - **Identity** 为您要创建的宿主提供程序指定一个唯一的字符串值标识符 (例如，"Exchange Online" ) 。 包含空格的值必须用双引号引起来。
