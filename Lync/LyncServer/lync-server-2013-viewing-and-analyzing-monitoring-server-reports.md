...
3.  可选通过选择 "导出" 选项和 "所需输出" 格式导出报表。

</div>

<div>

## <a name="configure-call-detail-recording-cdr"></a>配置呼叫详细记录 (CDR)

1.  从属于 RTCUniversalServerAdmins 组成员的用户帐户 (或具有同等权限) 或分配给 CsServerAdministrator 或 CsAdministrator 角色的用户帐户登录到你在其中部署 Lync Server 2013 的网络中的任何计算机。
...
6.  在 "**将呼叫详细信息记录保留最长持续时间 (天):** " 中, 选择应保留的通话详细记录的最大天数。
...
6.  在 "**将通话详细信息记录保留最长持续时间 (天):** " 中, 选择应保留 QoE 数据的最大天数。
