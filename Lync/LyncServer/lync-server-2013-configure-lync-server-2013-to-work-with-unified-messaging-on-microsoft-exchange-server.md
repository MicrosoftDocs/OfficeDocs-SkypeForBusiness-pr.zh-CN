...
> <ul><li>如果你使用的是早于 Microsoft Exchange Server 2010 SP1 的 Exchange 版本，则必须在 Lync Server 2013 拨号计划 "<STRONG>简单名称</STRONG>" 字段中输入对应 Exchange 统一消息（UM） SIP 拨号计划的完全限定的域名（FQDN）。 如果您使用的是 Microsoft Exchange Server 2010 SP1 或最新的 service pack，则不需要此拨号计划名称匹配。</li></ul>
...
5.  在 "**联系人**" 框中，接受默认组织单位，或单击 "**浏览**" 以启动**OU 选取器**。 在 " **OU 选取器**" 框中，可以选择一个 OU 并单击 **"确定**"，也可以单击 "创建**新 ou** " 以在域的根或任何其他 OU 下（例如，"OU = RTC 特殊帐户，dc = fourthcoffee，dc = com"）中创建新的组织单元，然后单击 **"确定"**。
