...
将 Standard Edition 服务器部署到现有的企业版部署类似于部署其他服务器角色。 可以将 Standard Edition 服务器部署到另一个网站，以允许该网站中的用户驻留在标准版服务器（而不是跨广域网的前端池） (WAN) 中。 在 [部署 Lync Server 2013](lync-server-2013-deploying-lync-server.md) 文档的其他部分中已定义了在该网站中安装新网站和服务器的过程。
