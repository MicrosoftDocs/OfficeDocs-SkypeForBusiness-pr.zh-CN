...
![7238ca40-340c-491f-b497-ddc2665dadb6](images/Gg398797.7238ca40-340c-491f-b497-ddc2665dadb6(OCS.15).jpg "7238ca40-340c-491f-b497-ddc2665dadb6")
  ...
  - **对后端服务器进行镜像**   以提供基本用户功能的更高可用性，组织已为每个前端池部署了镜像对后端服务器。 这是一个可选拓扑，可以选择部署一台后端服务器。

  - **在分支站点中使用 Standard Edition server。**   此组织将站点 C 视为分支站点，因为它只有600名员工。 但是，该站点上的用户之间有许多 A/V 会议。 如果它是作为分支站点在 Lync Server 中部署的，则这些会议的媒体将跨广域网络（WAN）在部署了前端服务器的中央站点上运行。 为了避免这种潜在的带宽负载，已在此站点上安装了一对 Standard Edition 服务器，这将主持这些会议。 由于安装了 Standard Edition 服务器，因此 Lync Server 按定义将其视为一个中央站点，并在拓扑生成器和规划工具中将其视为这样的情况。
  ...
  - **监视和存档**   此组织已部署监视和存档。 在部署监控或存档时，它将在每台前端服务器上运行。 这些功能的数据库可以与后端数据库并置，也可以位于单独的服务器上。 此组织已将服务器上的这些数据库与后端服务器分开，位于中央站点 B 中。此处的数据库接收来自所有站点中的前端服务器的监视和存档数据。

  - **分支站点部署选项。**   此组织实际上拥有超过50个分支站点，详细信息图中仅显示了其中三个。 分支站点1和3不具有到中央站点的弹性 WAN 链接，因此他们已部署 Survivable 分支机构以提供电话服务，以确保指向中央站点的 WAN 链接断开。 分支站点2但是有一个弹性 WAN 链路，因此您只需要一个公用电话交换网（PSTN）网关。 部署的 PSTN 网关支持媒体旁路，因此分支站点 B 不需要中介服务器。有关决定在分支站点上安装哪些内容的详细信息，请参阅规划文档中的在[Lync Server 2013 中规划企业语音恢复](lync-server-2013-planning-for-enterprise-voice-resiliency.md)。
  ...
  - **可添加控制器。**  如果此组织希望提高抵御拒绝服务攻击的安全性，它还可以部署一个控制器池。 Director 是 Lync Server 中的一个单独的可选服务器角色，它不会家庭用户帐户，也不提供状态或会议服务。 它充当内部下一个跃点服务器，边缘服务器将入站 SIP 流量路由到内部服务器。 控制器对入站请求进行预身份验证，并将其重定向到用户的主池或服务器。 控制器进行的预先身份验证允许放弃来自部署中未知的用户帐户的请求。 控制器可帮助将前端服务器与恶意流量（如拒绝服务（DoS）攻击）隔离。 如果网络在此类攻击中遇到无效的外部通信，则流量将在 Director 处结束。
