...
对于私人聊天 (1:1 聊天) 或群组聊天，最终用户将看到早于保留策略配置的聊天已删除，并显示一条控制消息，显示 "我们已删除由于组织的保留策略而导致的旧邮件"，并显示删除的邮件。
...
- 保留处理作业每天运行，但在某些情况下，时间延迟已运行最长7天。 因此，如果你创建了 Teams 保留策略来删除 60 天之前的项目，则这些项目最长可保留 67 天。 这不是新情况 - 它遵循 Exchange 模型。 当然，在大多数情况下，不会出现延迟。
