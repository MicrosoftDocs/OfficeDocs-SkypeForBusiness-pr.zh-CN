...
- 自定义 Teams 以突出显示对用户最为重要的应用。 选择要固定的应用并设置它们显示的顺序。 固定应用允许你展示组织中用户所需的应用，包括由第三方或你组织中开发人员构建的应用。
- 控制用户是否可以将应用固定到 Teams。
- 代表预览版中的 (**安装) 。** 用户启动 Teams 时，可以选择默认安装哪些应用。 请记住，如果分配给用户的应用权限策略允许，用户仍[](teams-app-permission-policies.md)可以自行安装应用。

应用固定到应用栏，应用栏位于 Teams 桌面客户端的一侧，位于 iOS 和 Android (Teams 移动客户端) 。
...
可以使用 Microsoft Teams 管理中心编辑策略，包括全局 (组织范围的默认) 策略和创建的自定义策略。
...
"呼叫"应用在 Teams 教育中不可用。 创建新的自定义应用设置策略时，"呼叫"应用会显示在应用列表中。 但是，该应用未固定到 Teams 客户端，Teams 教育用户不会在 Teams 中看到"通话"应用。
...
#### <a name="how-can-users-see-all-their-pinned-apps-in-teams"></a>用户如何在 Teams 中查看其所有已固定的应用

若要查看为用户固定的所有应用，用户可能需要执行以下操作，具体取决于已安装的应用数量及其 Teams 客户端窗口的大小。
...
#### <a name="my-organization-built-a-custom-teams-app-and-published-it-either-to-appsource-or-the-tenant-app-catalog-but-the-app-icon-isnt-displayed-as-expected-when-the-app-is-pinned-to-the-app-bar-in-teams-how-do-i-fix-it"></a>我的组织构建了自定义 Teams 应用，并发布到 AppSource 或租户应用目录，但在应用固定到 Teams 中的应用栏时，应用图标不会按预期显示。 如何修复它

在提交应用之前，请确保遵循徽标指南。 若要了解有关详细信息，请参阅["卖家仪表板提交清单"。](/microsoftteams/platform/concepts/deploy-and-publish/appsource/prepare/overview)
