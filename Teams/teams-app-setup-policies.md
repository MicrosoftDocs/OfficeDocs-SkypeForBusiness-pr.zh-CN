...
- **安装应用：** 应用设置策略让你在用户启动应用时和Teams安装应用。 有关详细信息，请参阅 [安装应用](#install-apps)。
...
4. 打开或关闭 **用户固定**，具体取决于是否要让用户通过将应用固定到应用栏来个性化其应用栏。
   ...
   > [!NOTE]
   > "**用户固定"** 设置在 Teams Microsoft 365 政府社区云 (GCC) 环境（ (GCC、GCC High 和 DoD) ）中的 Teams 管理中心中可用，但目前不起作用。
...
你可以选择默认为用户安装在其个人游戏环境中Teams应用，将应用安装为消息传递扩展，以及指定要安装在[](/microsoftteams/platform/messaging-extensions/what-are-messaging-extensions)会议中的应用。
...
#### <a name="what-built-in-app-setup-policies-are-included-in-the-microsoft-teams-admin-center"></a>应用管理中心中包含哪些内置应用Microsoft Teams策略
...
iOS Teams Android Teams移动客户端 (支持具有静态选项卡) 个人应用。 固定到 Teams 桌面客户端的应用将显示在Teams客户端中。 个人机器人将显示在移动客户端上的聊天中。
