...
固定应用可突出显示组织中用户最需要的应用。 固定工作适用于应用 Teams 中的所有应用类型—核心应用、Microsoft 提供的应用、第三方应用和组织内自定义开发的应用。 如果用户允许应用，则通过应用设置策略固定应用也会安装它。 使用应用设置策略可执行以下任务：
...
如果 [应用权限策略](teams-app-permission-policies.md) 允许，并且应用本身允许在其租户上安装，最终用户可以自行安装应用。

要创建应用设置策略以安装应用，请执行以下步骤：
...
:::image type="content" source="media/install-apps-in-meeting.png" alt-text="安装应用策略。":::
...
可以使用 Microsoft Teams 管理中心来编辑策略，包括已创建的全局（组织范围内的默认）策略和自定义策略。
...
使用 Teams 移动客户端，用户将看到核心 Teams 应用，如“活动”、“聊天”和 Teams，可固定 Microsoft 提供的应用。

#### <a name="can-users-change-the-order-of-apps-pinned-through-a-policy"></a>用户能否更改通过策略固定的应用顺序
...
管理员固定始终优先。 如果启用了“**用户固定**”选项，则用户将在管理员固定的应用下保留其固定的应用。 如果禁用了“**用户固定**”选项，则用户将丢失其预先存在的固定应用，并且应用栏中将仅显示管理员固定的应用。
