...
[![管理中心内呼叫计划 PSTN 使用情况报告的屏幕截图] (。/media/teams-reports-pstn-usage-calling-plans-with-callouts.png"管理中心内 PSTN 使用情况报告的屏幕截图Microsoft Teams带编号标注"。](../media/teams-reports-pstn-usage-calling-plans-with-callouts.png#lightbox)
...
[![管理中心中的直接路由 PSTN 使用情况报告屏幕截图] (。/media/teams-reports-pstn-usage-direct-routing-with-callouts.png"直接路由 PSTN 使用情况报告的屏幕截图，Microsoft Teams管理中心带编号标注"。](../media/teams-reports-pstn-usage-direct-routing-with-callouts.png#lightbox)
...
3.    PSTN 终结点上的 PSTN 用户 1，Teams客户端上名为 Teams 2 的用户， 调用类型 Dr_In f45e9a25-9f94-46e7-a457-84f5940efde9，共享关联 ID f45e9a25-9f94-46e7-a457-84f5940efde9。
4.    关联 ID 为"f45e9a25-9f94-46e7-a457-84f5940efde9"的现有调用 3。 PSTN 用户 1 与 Teams 2 通话。 Teams用户 2 已 (失明或) 失明或Teams PSTN 用户的呼叫， 调用类型 Dr_Out_User_Transfer 45a1da7c-9e97-481a-8a05-3fe19a9a77e0，共享关联 ID f45e9a25-9f94-46e7-a457-84f5940efde9。
...
如果同时具有"呼叫计划"和"直接路由"，则导出的文件可能包含这两个产品的数据。 PSTN 使用情况报告文件将包含文件名 **"PSTN.calls"。 `[UTC date]`.csv"** 和"直接路由 **"DirectRouting.calls。 `[UTC date]`.csv"。**

 除了 PSTN 和直接路由文件外，存档还包含文件 **"parameters.json**"，以及选定的导出时间范围和功能。
...
> | 4 | 邀请时间 | `datetimeoffset` | 当初始邀请从 Teams 用户或机器人在出站时调用 SBC，或者从 SBC 直接路由的 SIP 代理组件在入站到 Teams 或机器人调用时收到 |
> | 5 | 开始时间 | `datetimeoffset` | SIP 代理从出站 (Teams/机器人上的 SBC 收到最终答案 (SIP 消息"200 正常") 给 PSTN 用户) 的时间，或者 SIP 代理在入站呼叫 (PSTN 用户上将邀请发送到 Teams Teams/Bot) 后端内的下一跃点之后的时间。<br/>对于失败和未接听的调用，这相当于邀请或失败时间 |
...
> | 12 | 被呼叫者号码 | `nvarchar(32)` | 收到呼叫的用户或机器人的号码。 在团队用户的入站呼叫中，它将是Teams用户，在用户呼叫Teams出站时，它将是 PSTN 用户 |
