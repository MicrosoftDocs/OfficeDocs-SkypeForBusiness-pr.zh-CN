...
2. 在 "**分析**" 选项卡上，选择一个日期范围以显示您是其成员或所有者的所有团队的使用数据。
    ...
    |**活动用户**   |在指定时间段内团队活动的 "团队" 和 "趋势" 行中的活动用户数。 我们定义 
...
> [!NOTE]
> 我们将活动用户定义为用户在桌面客户端、移动客户端和 web 客户端中执行蓄意操作。 有意行动的示例包括开始聊天、进行呼叫、共享文件、编辑团队中的文档、参与会议等。我们去除了诸如自动启动、最小化屏幕或关闭应用之类的被动操作。 我们还跨单个用户 ID 消除了所有操作。
