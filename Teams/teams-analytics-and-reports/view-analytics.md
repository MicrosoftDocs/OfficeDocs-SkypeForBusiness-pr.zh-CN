...
4. 选择一个日期范围以显示团队的使用情况数据。  
    ...
    |项目 |说明  |
    |--------|-------------|
    |**摘要**   |团队活动摘要，包括以下内容：<ul><li>**用户**：指定时间段的用户总数。 这包括团队所有者、团队成员和来宾。</li> <li>**帖子**：在指定的时段内在团队聊天中发布的新消息数。</li><li>**回复**：在指定的时段内，团队聊天中的回复数。</li> <li>**应用** 数：添加到团队的应用数。</li><li>**会议**：Teams级组织的会议数。</li> </ul> |
    |**活动用户**   |活动用户数和非活动用户数。|
...
> [!TIP]
> 还可通过单击跨团队分析视图中的列表中某个团队来 [查看每个团队的分析](#view-cross-team-analytics)。
...
3. 选择一个日期范围以显示频道的使用情况数据。  
    ...
    |项目 |说明  |
    |--------|-------------|
    |**摘要**   |包括以下内容的频道活动摘要：<ul><li>**用户**：指定时间段的用户总数。 这包括团队所有者、团队成员和来宾。</li> <li>**帖子**：指定时段内发布至频道的新消息数。</li><li>**答复** 数：通道中指定时间段的答复数。</li> <li>**应用** 数：添加到通道的应用数。</li> </ul> |
    |**消息** 图表  |按日期向频道聊天发布的消息总数。 将鼠标悬停在给定日期的点上，以查看该日期发布的新帖子和答复数。|

> [!TIP]
> 还可通过在"每团队分析"视图的下拉列表框中选择一个频道来查看每个 [通道的分析](#view-per-team-analytics)。
