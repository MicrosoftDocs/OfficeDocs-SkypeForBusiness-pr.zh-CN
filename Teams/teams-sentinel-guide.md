...
## <a name="sentinel-and-microsoft-teams-activity-logs"></a><span data-ttu-id="611da-109">Sentinel 和 Microsoft Teams 活动日志</span><span class="sxs-lookup"><span data-stu-id="611da-109">Sentinel and Microsoft Teams Activity Logs</span></span>
...
### <a name="register-an-app-in-microsoft-azure-for-log-collection"></a><span data-ttu-id="611da-125">在 Microsoft Azure 中注册应用以进行日志收集</span><span class="sxs-lookup"><span data-stu-id="611da-125">Register an App in Microsoft Azure for log collection</span></span>
...
![显示逻辑应用类的图形。](media/tracyp-teams-sentinel-logic-app.png#thumbnail)
 ...
 <p><details><summary> <span data-ttu-id="611da-159">图形操作说明：</span><span class="sxs-lookup"><span data-stu-id="611da-159">Explanation of actions in the graphic:</span></span> 
  ...
  <span data-ttu-id="611da-160">•“定期”是一种逻辑应用触发器，它指示工作流每小时运行一次。</span><span class="sxs-lookup"><span data-stu-id="611da-160">• Recurrence is the Logic App Trigger that tells the workflow to run every hour.</span></span>
  <p>
<span data-ttu-id="611da-161">•“当前时间”操作是一种非常基本的操作，仅获取当前时间。</span><span class="sxs-lookup"><span data-stu-id="611da-161">• The Current Time action is very basic and just gets the current time.</span></span>
  <p>
<span data-ttu-id="611da-162">•“初始化变量”将创建名为 NextPage 的变量，并将其设置为 1。</span><span class="sxs-lookup"><span data-stu-id="611da-162">• Initialize Variable creates a variable called NextPage and sets it to 1.</span></span> <span data-ttu-id="611da-163">稍后将使用它来处理 O365 API 中的分页。</span><span class="sxs-lookup"><span data-stu-id="611da-163">This will be used later in order to handle pagination from the O365 API.</span></span>
  <p>
<span data-ttu-id="611da-164">•“初始化变量 2”会创建一个名为“开始时间”的空变量。</span><span class="sxs-lookup"><span data-stu-id="611da-164">• Initialize Variable 2 creates an empty variable called Start Time.</span></span>
  <p>
<span data-ttu-id="611da-165">• 运行查询和列表结果是一个 Azure Monitor 操作，它将查询从逻辑应用中输入的最后一个 O365 日志的工作区。</span><span class="sxs-lookup"><span data-stu-id="611da-165">• Run Query and list results is an Azure Monitor action that will query the workspace for the last O365 log that was entered from the Logic App.</span></span>
  <p>
<span data-ttu-id="611da-166">•“条件 4”用于检查运行查询和列表结果查询是否返回了任何数据。</span><span class="sxs-lookup"><span data-stu-id="611da-166">• Condition 4 is used to check whether the Run Query and list results query returned any data.</span></span> <span data-ttu-id="611da-167">执行此操作可检查这是否是首次使用逻辑应用。</span><span class="sxs-lookup"><span data-stu-id="611da-167">This is done to check if this is the very first time the Logic App has been used.</span></span> <span data-ttu-id="611da-168">如果未返回任何数据，StartTime 变量将设置为“现在 – 24 小时”。</span><span class="sxs-lookup"><span data-stu-id="611da-168">If no data is returned, StartTime variable is set to Now – 24 hours.</span></span> <span data-ttu-id="611da-169">如果返回了数据，StartTime 将设置为最后一条记录 TimeGenerated。</span><span class="sxs-lookup"><span data-stu-id="611da-169">If data is returned, StartTime is set to the last record TimeGenerated.</span></span> <span data-ttu-id="611da-170">这样做是为了使查询可以获取针对 O365 API 的轮询中从最后一个条目到现在的数据；或者如果是第一次运行，则可以获取最近 24 小时内的数据。</span><span class="sxs-lookup"><span data-stu-id="611da-170">This is done so that the query can get data from the last entry till now in the poll against the O365 API, or in the last 24 hours if this is the first run.</span></span>
  <p>
<span data-ttu-id="611da-171">•“初始化变量 3”会创建一个名为 AvailableUri 的变量。</span><span class="sxs-lookup"><span data-stu-id="611da-171">• Initialize Variable 3 creates a variable called AvailableUri.</span></span> <span data-ttu-id="611da-172">这是一个字符串，使用 StartTime 和 CurrentTime 分别作为开始时间和结束时间来构建 URL。</span><span class="sxs-lookup"><span data-stu-id="611da-172">This is a string with the URL built using the StartTime and CurrentTime as start and end times, respectively.</span></span>
  <p>
<span data-ttu-id="611da-173">• Until 条件是一个循环，允许逻辑应用保持轮询 API 以查看是否还有更多数据（分页）。</span><span class="sxs-lookup"><span data-stu-id="611da-173">• The Until condition is a loop that allows the logic app to keep polling the API to see if there is more data (pagination).</span></span> <span data-ttu-id="611da-174">只要 NextPage 变量为 1，循环就将继续。</span><span class="sxs-lookup"><span data-stu-id="611da-174">As long as NextPage variable is 1 the loop will continue.</span></span> <span data-ttu-id="611da-175">之后如果没有更多页面可供检索，此变量将更新。</span><span class="sxs-lookup"><span data-stu-id="611da-175">Later this variable will be updated if there are no more pages left to retrieve.</span></span>
  <p>
<span data-ttu-id="611da-176">• 在 Until 循环中，第一个 HTTP 步骤连接到 AvailableURI。</span><span class="sxs-lookup"><span data-stu-id="611da-176">• Inside the Until loop, the first HTTP step Connects to the AvailableURI.</span></span> <span data-ttu-id="611da-177">此 URI 将返回可用内容和每个内容 URI 的列表。</span><span class="sxs-lookup"><span data-stu-id="611da-177">This URI returns a list of available content and each contents URI.</span></span> <span data-ttu-id="611da-178">有关其工作原理的更多信息，请访问此 URL：https://docs.microsoft.com/office/office-365-management-api/office-365-management-activity-api-reference#list-available-content。</span><span class="sxs-lookup"><span data-stu-id="611da-178">There's more on how this works at this URL: https://docs.microsoft.com/office/office-365-management-api/office-365-management-activity-api-reference#list-available-content.</span></span>
  <p>
<span data-ttu-id="611da-179">• 接下来，运行检查以确保返回数据。</span><span class="sxs-lookup"><span data-stu-id="611da-179">• Next a check is run to make sure data is returned.</span></span> <span data-ttu-id="611da-180">该条件将检查正文的长度是否为 0。</span><span class="sxs-lookup"><span data-stu-id="611da-180">The Condition checks if the length of the body is 0.</span></span> <span data-ttu-id="611da-181">如果是，则表示没有数据可写入日志分析。</span><span class="sxs-lookup"><span data-stu-id="611da-181">If so there is no data to write to Log Analytics.</span></span> <span data-ttu-id="611da-182">如果值大于 0，则表示有要处理的数据。</span><span class="sxs-lookup"><span data-stu-id="611da-182">If the value is greater than 0, there is data to process.</span></span>
  <p>
<span data-ttu-id="611da-183">• 如果检测到数据，必须随后对其进行处理。</span><span class="sxs-lookup"><span data-stu-id="611da-183">• If data is detected, it must next be processed.</span></span> <span data-ttu-id="611da-184">分析 JSON 定义返回数据的架构。</span><span class="sxs-lookup"><span data-stu-id="611da-184">Parse JSON defines a schema of the returned data.</span></span> <span data-ttu-id="611da-185">这使逻辑应用能够在后续步骤中将分析数据用作动态内容。</span><span class="sxs-lookup"><span data-stu-id="611da-185">This allows logic apps to use the parsed data as Dynamic content in later steps.</span></span>
  <p>
<span data-ttu-id="611da-186">• 由于返回的可用数据列表是一个数组，因此，“全部应用”操作用于循环读取可用内容列表。</span><span class="sxs-lookup"><span data-stu-id="611da-186">• Since the returned list of available data is an Array, a For Each action is used to loop through the list of available content.</span></span>
  <p>
<span data-ttu-id="611da-187">• 下一步是抓取内容。</span><span class="sxs-lookup"><span data-stu-id="611da-187">• Next is grabbing the content.</span></span>  <span data-ttu-id="611da-188">再次使用 HTTP 获取 contentUri（从分析 JSON 创建的动态属性），即要检索的数据的 URL。</span><span class="sxs-lookup"><span data-stu-id="611da-188">HTTP is used again to get the contentUri (a dynamic property created from Parse JSON), which is the URL of the data to retrieve.</span></span>
  <p>
<span data-ttu-id="611da-189">• 分析 JSON 还用于分析返回的数据。</span><span class="sxs-lookup"><span data-stu-id="611da-189">• Parse JSON is also used to parse the returned data.</span></span> <span data-ttu-id="611da-190">可在此 URL 中找到一些示例内容：https://docs.microsoft.com/office/office-365-management-api/office-365-management-activity-api-reference#list-available-content。</span><span class="sxs-lookup"><span data-stu-id="611da-190">You can find some sample content at this URL: https://docs.microsoft.com/office/office-365-management-api/office-365-management-activity-api-reference#list-available-content.</span></span>
  <p>
<span data-ttu-id="611da-191">• 返回的数据也是一个数组。</span><span class="sxs-lookup"><span data-stu-id="611da-191">• The data returned is also an array.</span></span> <span data-ttu-id="611da-192">也可在此处使用“全部应用”循环。</span><span class="sxs-lookup"><span data-stu-id="611da-192">A For Each loop can be used here as well.</span></span> <span data-ttu-id="611da-193">在此循环中，工作流将提取当前数据项，并使用“发送数据”操作将数据写入日志分析。</span><span class="sxs-lookup"><span data-stu-id="611da-193">In this loop, the workflow takes the current item of data and uses the Send Data action to write the data to Log Analytics.</span></span>
  <p>
<span data-ttu-id="611da-194">• 由于可能存在多个页面的可用内容，因此一个条件将检查 NextPageUri 是否不为 NULL。</span><span class="sxs-lookup"><span data-stu-id="611da-194">• Since there may be multiple pages of available content, a condition checks if the NextPageUri is not NULL.</span></span> <span data-ttu-id="611da-195">如果为 NULL 或为空，NextPage 将设置为 0，这将结束 Until 循环。</span><span class="sxs-lookup"><span data-stu-id="611da-195">If it is NULL, or empty, NextPage is set to 0, which ends the Until loop.</span></span> <span data-ttu-id="611da-196">如果它包含 URL，AvaibleUri 变量将更新为该 URL。</span><span class="sxs-lookup"><span data-stu-id="611da-196">If it contains a URL, the AvaibleUri variable is updated to that URL.</span></span> <span data-ttu-id="611da-197">这样，下次运行 Until 循环时将使用下个可用的 URL，而不是起始 URL。</span><span class="sxs-lookup"><span data-stu-id="611da-197">This way, the next run of the Until loop uses a next available URL, and not the starting URL.</span></span>
...
## <a name="helfpul-hunting-kql-queries"></a><span data-ttu-id="611da-217">有用的搜寻 KQL 查询</span><span class="sxs-lookup"><span data-stu-id="611da-217">Helfpul hunting KQL queries</span></span>
...
#### <a name="external-users-who-were-added-and-then-removed"></a><span data-ttu-id="611da-233">添加后又删除的外部用户</span><span class="sxs-lookup"><span data-stu-id="611da-233">External users who were added and then removed</span></span>
...
#### <a name="many-team-deletions-by-a-single-user"></a><span data-ttu-id="611da-245">单个用户删除多个团队</span><span class="sxs-lookup"><span data-stu-id="611da-245">Many Team deletions by a single user</span></span>
