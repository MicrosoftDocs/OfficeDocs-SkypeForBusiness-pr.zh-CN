...
Q&A 允许演示者接受与会者的提问并实时回答这些问题。 此功能最适合大型结构化会议，例如市政厅、网络研讨会、全手会议和培训。
...
- 如果会议组织者启用匿名发布，则会将与会者发布的问题引入组织者的电子数据展示邮箱中。
...
**答：** 我们简化了通过会议选项启用 Q&A。从 2022 年 8 月开始，Teams 应用商店中的 Q&A 应用将不再受支持，因此必须仅通过会议选项启用 Q&A。 如果你是会议的组织者，其中 Q&A 是通过 Teams 应用商店启用的，请删除 Q&A 应用，而只能通过会议选项启用。

**问：为什么我在会议中看到两个 Q&A 图标？**

**答：** 你在会议中看到两个 Q&A 图标，因为 Q&A 也是通过会议选项启用的。 请使用以下说明删除通过 Teams App Store添加的 Q&应用。 请针对之前通过 Teams 应用商店添加 Q&A 的所有会议执行此操作。

**如何删除从 Teams 应用商店添加的 Q&应用。**

1. 在 Teams 桌面上，加入之前在其中添加了 Q&A 的会议。

2. 在顶部面板中，在 Teams 会议窗口中选择 Q&A 图标的第二个匹配项 - 这是通过 Teams App Store 添加的 Q&A 体验。

3. 打开所选 Q&选项卡后，单击省略号，然后单击“删除”。 这将删除通过 Teams 应用商店添加的 Q&体验。

4. 单击“删除”以永久删除通过 Teams 应用商店添加的 Q&体验。

> [!NOTE]
> 只有通过 Teams 应用商店添加的 Q&A 应用将具有删除 Q&A 应用的省略号。 通过会议选项启用的 Q&体验将没有省略号，也无法从此处删除。

就是这样！ 现在只有一个 Q&A 体验 - 由会议选项提供支持。 删除通过 Teams 应用商店添加的 Q&应用。
