...
<tr><td><img src="media/audio_conferencing_image9.png" alt="An icon depicting the next steps"/><br/>后续步骤</td><td><ul><li>完整查看操作指南。</li><li>实施与组织的目标相一致的运营策略，以提供工作负荷Teams和可靠性。</li><li>查看体验质量评审指南。</li><li> 实施运营策略，定期执行体验质量评审，确保Teams部署在其最高峰功能下运行。</li></ul></td></tr>
...
组织还可以利用本地基础结构的现有投资。 例如，可以使用现有的本地 Active Directory 帐户通过利用 Azure AD 连接。 某些版本的 Exchange Server 可用于使用 Exchange Online。
...
必须了解服务的总体运行状况Microsoft Teams，以便可以主动提醒组织中其他人任何影响该服务的事件。 如上所述，Teams依赖于其他 Microsoft 365 Office 365 服务，例如 Azure Active Directory、Exchange Online、SharePoint Online 和 OneDrive for Business。 因此，监视依赖服务的运行状况也同样重要。
...
可以通过利用服务通信 API 以编程方式访问服务运行状况和更改来监视服务运行状况和即将进行的更改。 使用此 API 创建自己的监视工具，或将现有的监视工具连接到 Microsoft 365或Office 365服务通信，从而可能简化监视环境的方法。 有关详细信息，请参阅Microsoft 365[开发人员Office 365或Enterprise应用](/office/developer-program/microsoft-365-developer-program-faq)。
...
系统Teams向用户推出更新。 你的用户将始终拥有最新的客户端和功能，这些功能在 Teams服务中。 无法管理向用户推出Teams更新，因此，通过有效的通信、培训和采用计划管理更改至关重要。 如果你的用户了解变化、了解好处并能够利用新功能，他们将能够更快适应并 &mdash; 欢迎更改。
...
在[Microsoft Teams 博客](https://techcommunity.microsoft.com/t5/Microsoft-Teams-Blog/bg-p/MicrosoftTeamsBlog)中，您可以了解有关产品更新的最佳实践、趋势Teams新闻。 希望在此处找到要Teams的主要功能更新。 您也可以通过 RSS 源订阅博客。 然后，您可以将[RSS](https://techcommunity.microsoft.com/gxcuf89792/rss/board?board.id=MicrosoftTeamsBlog)源直接添加到Teams频道，以便所有重要新闻都直接在 Teams。
...
还可使用行业范围的方法评估用户情绪，称为 NPS (net promotor 评分) ，如下一部分所述。
...
虽然基本 NPS 编号很有用，但从分析用户评论中可以获取最大的价值。 它们将帮助您了解用户为何会 (推荐) 向Teams用户。 这些评论可以提供有价值的反馈，帮助项目或服务管理团队了解提供高质量服务所需的调整。
...
许多核心规划元素都进入优化、调整大小以及修正网络基础结构，以确保获得高质量、Microsoft Teams路径。 我们的网络准备指南中介绍了规划 [任务和](prepare-network.md) 要求。 由于升级、扩展或其他业务要求，网络通常会随着时间的推移而发展。 在网络规划活动中，Teams要求非常重要。
...
| 执行体验质量评审 (QR)  | 执行 QER 以确定质量和可靠性的趋势，针对定义的目标进行评审，并报告给组织中的关键利益干系人。 | 在 (部署期间每周)  | |
...
2. **验证客户端连接：** 连接问题会导致服务中的功能或登录Teams。 我们建议 (新站点或) 验证服务连接的位置。 确保每个[Office 365遵循](/microsoft-365/enterprise/urls-and-ip-address-ranges)以下 URL 和 IP 地址范围指南。 可以利用 Microsoft[网络评估工具](https://www.microsoft.com/download/details.aspx?id=53885)执行连接测试，验证媒体端口是否正确打开，Teams功能。 网络就绪性指南中提供了有关如何运行连接测试 [的详细](prepare-network.md) 步骤。
...
对租户设置所做的更改会影响已启用租户Teams。 有关这些设置的详细列表，请参阅[管理Microsoft Teams设置](enable-features-office-365.md)。

### <a name="user-administration"></a>用户管理

为了支持用户，组织可能需要执行任意数目的相关任务，具体任务因组织而异。 最终，这些任务需要由分配有这些运营职责的支持团队管理。 通常需要执行以下任务才能支持用户Teams。
