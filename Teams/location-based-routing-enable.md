...
在按照本文中的步骤操作之前，请确保已阅读规划直接路由的 [](location-based-routing-plan.md) Location-Based 路由，并已完成为直接路由配置Location-Based[设置中的步骤](location-based-routing-configure-network-settings.md)。
...
可以使用 Microsoft Teams [管理中心或](#using-the-microsoft-teams-admin-center) [PowerShell](#using-powershell) 来启用Location-Based路由。
...
为Location-Based实施路由限制的站点启用路由。 为此，请在管理中心的左侧导航Microsoft Teams"位置""网络拓扑"，选择一个网络站点，单击"编辑" > ，然后打开"基于位置的 **路由"**。  
...
### <a name="enable-location-based-routing-for-calling-policies"></a>启用Location-Based策略的路由
...
2. 使用 [Set-CSOnlinePSTNGateway](/powershell/module/skype/set-csonlinepstngateway?view=skype-ps) cmdlet 为Location-Based网关启用路由路由。 
    ...
    ```
    不要为不会将Location-Based路由到 PSTN 的网关启用路由。 但是，仍必须将网关关联到系统所在的网络站点。 这是因为对于Location-Based通过此网关连接的终结点的 PSTN 呼叫，需要强制实施路由限制。 此示例中，Location-Based和海得拉巴站点中与 PBX 系统关联的每个网关未启用路由。
...
若要强制Location-Based用户进行语音路由，请设置用户的语音策略以防止 PTSN 收费绕过。 
