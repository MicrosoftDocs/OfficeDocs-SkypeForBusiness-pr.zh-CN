...
当应用准备好在生产环境中使用时，开发人员应生成一个应用包。 他们可以为此使用 [App Studio](/microsoftteams/platform/concepts/build-and-test/app-studio-overview) 。 他们将以.zip格式向你发送文件。

Microsoft 使用[这些准则](/microsoftteams/platform/concepts/deploy-and-publish/appsource/prepare/teams-store-validation-guidelines)来确保应用符合全球Teams应用商店的质量和安全标准。
...
2. 在全局应用设置策略中关闭 **Upload自定义应用** 设置。 要执行此操作：
...
若要使应用可供组织应用商店中的用户使用，请上传该应用。 可以在Microsoft Teams管理中心的“[管理应用](manage-apps.md)”页上执行此操作。

1. 在 Microsoft Teams 管理中心的左侧导航中，转到“**Teams 应用**” > “**管理应用**”。
2. 选择 **Upload**，单击 **Upload**，选择从开发人员处收到的应用包，然后选择 **“打开**”。
...
![显示已发布的应用的“应用”页的屏幕截图。](media/custom-app-lifecycle-discovery.png)

如果创建并分配了应用设置策略，则应用将固定到Teams中的应用栏，以便为分配策略的用户轻松访问。

## <a name="update"></a>更新

若要更新应用，开发人员应继续执行 [“开发](#develop) 和 [验证](#validate) ”部分中的步骤。
...
在大多数情况下，完成应用更新后，会自动为最终用户显示新版本。 但是，需要用户接受才能完成[Microsoft Teams清单](/microsoftteams/platform/resources/schema/manifest-schema)的一些更新：
...
![应用列表的屏幕截图，其中显示了可用的新版本的应用。](media/manage-your-custom-apps-update1.png)
