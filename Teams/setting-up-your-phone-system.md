...
- seo-marvel-apr2020
- intro-get-started
description: 分步指南，详细说明如何在 Microsoft 365 中为组织设置Teams 电话系统。
ms.openlocfilehash: 12e1b4ff8c63b06c108921d8d28e1c1c6b262fb0
ms.sourcegitcommit: 5e9b50cd1b513f06734be6c024ac06d293b27089
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 02/10/2022
ms.locfileid: "62518634"
---
# <a name="set-up-phone-system-in-your-organization"></a>在组织中设置电话系统

下面是有关在 Microsoft 365 中设置 电话系统 的分步Microsoft 365。 在每个步骤的末尾都提供指向其他详细信息的链接。
...
若要将电话系统和呼叫计划许可证分配给单个用户，步骤与分配用户许可证Microsoft 365相同。  也可以批量向多个用户分配许可证。 有关详细信息，请参阅[分配Microsoft Teams许可证](teams-add-on-licensing/assign-teams-add-on-licenses.md)。

如果呼叫计划不适用于你的国家/地区，则有其他选项可用于连接到 PSTN (电话) 。  有关详细信息，请参阅 [PSTN 连接选项](pstn-connectivity.md)。
...
- 使用管理中心Teams号码。
- 获取在管理中心内不可用Teams号码。
- 将现有号码从当前服务提供商或电话运营商转Microsoft 365。
...
1. 登录到Microsoft 365或学校帐户登录。

2. 转到 **Microsoft Teams 管理中心**。
    
3. 在左侧导航中 **，转到"** > 语音 **电话** 号码 **，单击"** 添加"，然后按照提示操作。
...
- 如果需要移植超过 999 个电话号码，请参阅管理组织的电话号码以提交[](manage-phone-numbers-for-your-organization/manage-phone-numbers-for-your-organization.md)移植订单服务请求或订单。 
...
3. 在左侧导航窗格中，转到 >  > "语音电话 **号码""添加新号码**"，然后单击"新建 **服务号码"**。

    > [!IMPORTANT]
    > 若要在 Teams 管理中心的左侧导航窗格中查看"语音"选项，必须先购买至少一个 **Enterprise E5** 许可证、一个 **电话系统** 附加许可证或一个音频会议附加许可证。
...
如果要转移来自当前服务提供商或运营商的服务号码，需要向 Microsoft 手动提交转网订单。 你需要为每种服务编号（收费 (和免费）提交单独的转站订单) 使用授权书和 LOA (转移) 。 在 LOA (授权书) ，必须选择正确的服务编号类型。 联系 Microsoft 支持人员时，请指定要转移服务号码 (而不是用户或订阅者 *号码) ，* 否则并发呼叫容量可能不足以处理呼叫量。 如果您想要转接电话号码，或执行其他操作与您的电话号码，请参阅 [管理您的组织的电话号码](manage-phone-numbers-for-your-organization/manage-phone-numbers-for-your-organization.md) 。
...
有时，组织中的人员需要使用电话拨入会议。 Microsoft Teams音频会议功能仅针对这种情况。 用户可以使用电话Teams会议，而不是使用移动设备Teams或电脑上的"会议"应用。
若要了解如何设置音频会议，请参阅为会议设置[音频](set-up-audio-conferencing-in-teams.md)Teams。
...
- 对于音频会议，你可以向会议网桥分配专用号码，Teams管理中心 **MeetingsConference** > 网桥 **并按照** > 提示。  有关详细信息，请参阅更改音频会议网桥上的收费或  [免费号码](change-the-phone-numbers-on-your-audio-conferencing-bridge.md)。

- 对于自动助理，你可以向自动助理分配专用号码，Teams **管理中心** > **VoiceAuto** >  **attendants** 并按照提示。  有关详细信息，请参阅 [设置云自动助理](create-a-phone-system-auto-attendant.md)。

- 对于呼叫队列，可以通过访问管理中心 **VoiceCall** >  队列并按照提示Teams **呼叫** > 队列分配专用号码。 有关详细信息，请参阅 [创建云呼叫队列](create-a-phone-system-call-queue.md)。
...
如果未设置通信信用额度计费并将通信信用额度许可证分配给用户，并且你的组织 (根据你的呼叫计划或音频会议计划在你的国家/地区) 中用完了分钟数，则这些用户将无法进行呼叫或从音频会议会议拨出。 有关详细信息，包括建议的金额，请参阅 [什么是通信信用额度？](what-are-communications-credits.md) 和为组织设置通信 [信用额度](set-up-communications-credits-for-your-organization.md)。
