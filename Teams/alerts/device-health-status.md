...
|字段 |说明  |
|--------|-------------|
|**规则类型**   |设备状态规则可帮助你进行有效的管理。 Teams设备，并归类为设备管理类型。 将来，更多设备管理类型的规则可用于监视其他相关功能 (例如：不正常的设备和设备登录状态) 。|
|**条件**   |如果设备脱机，可以监视其运行状况。 [在管理](../devices/device-management.md)中心内详细了解Teams管理。 |
...
|**操作**  > **通道警报**   |在"操作"部分中，可以指定要获取其警报的团队频道。 目前，将创建名为 **"管理员** 警报和通知"的默认团队以及名为 **MonitoringAlerts** 的频道，通知将传递到其中。 <BR/> <BR/> 租户中的Teams管理员和管理员将自动添加到此默认团队。|
...
1. 可以通过选择Teams设备的用户来选择要监视的设备。 从 **"设备** 用户" **部分选择"添加** "。
