...
ms.locfileid: "69763683"
---
# <a name="create-a-microsoft-teams-call-queue"></a>创建 Microsoft Teams 呼叫队列 
...
本文中概述的创建呼叫队列的步骤使用 Teams 管理中心。 有关使用 PowerShell 创建呼叫队列的说明，请参阅 [使用 PowerShell cmdlet 创建呼叫队列](create-a-phone-system-call-queue-via-cmdlets.md)。
...
1. 选择要 (仅支持标准频道或专用频道) 使用的频道，然后选择“ **应用**”。
...
必须将代理的 Teams 帐户设置为 `TeamsOnly` 模式。 不符合要求的代理不包括在呼叫路由列表中。 如果代理使用兼容的客户端，我们建议为呼叫队列启用会议模式。

> [!TIP]
> 建议将 **“会议模式** ”设置为“ **打开** ”。

> [!NOTE]
> 从启用了Location-Based路由的直接路由网关路由到队列的呼叫不支持会议模式。
>
...
3. 为代理路由方法选择“最长空闲时间”将自动启用Presence-Based路由。
4. 无法设置调用代理的顺序。
5. 如果电话呼叫从启用了Location-Based路由的直接路由网关路由到队列，则不支持会议模式。
...
- iPhone Skype for Business客户端 (版本 6.16.0 及更高版本) 。
- iPad Skype for Business客户端 (版本 6.16.0 及更高版本) 。
...
1. 选择下面的“**运行测试**”，这将填充Microsoft 365 管理中心中的诊断。
