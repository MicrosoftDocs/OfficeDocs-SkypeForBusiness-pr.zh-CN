...
- [Microsoft 团队控件 For Security 和合规性](https://www.youtube.com/watch?v=Km4T4hMM__k) (10:54 分钟) 
...
### <a name="advanced-threat-protection-atp"></a> (ATP) 的高级威胁防护
...
适用于 Windows 和 Mac 的 Microsoft 团队桌面客户端支持新式身份验证。 新式身份验证通过平台 (ADAL) 到 Microsoft Office 客户端应用程序，基于 Azure Active Directory 身份验证库将登录引入了登录。
...
团队有大量信息可帮助您处理合规性领域，包括频道、聊天和附件、保留策略、数据丢失保护 (DLP) 、聊天和法律封存、用于频道、聊天和文件、审核日志搜索以及使用 Microsoft Intune 的移动应用程序管理。 我们提供了有关以下所有主题的一些信息，您可以转到 [Microsoft 365 合规中心](https://compliance.microsoft.com) 来管理这些设置。

### <a name="information-barriers"></a>信息障碍

信息障碍是由团队管理员在原地安排的策略，可使用户或组在没有业务需求时与其他人进行通信，从而使用户或组无法与另一个 (进行通信，或者阻止) 他们执行此操作的规章原因，还允许你设置与以下内容相关的策略，如查阅和电子数据展示 () 下文所述。 这些策略可以影响1:1 聊天、群组聊天或团队级的用户。
...
### <a name="data-loss-prevention-dlp"></a> (DLP) 的数据丢失防护

Microsoft 团队中的数据丢失防护 (DLP) 以及 Microsoft 365 或 Office 365 的较大 DLP 故事，可在保护敏感文档和数据时绕过业务准备工作。 无论您是否关注邮件或文档中的敏感信息，DLP 策略都能够帮助确保您的用户不会与错误的用户共享此敏感数据。
...
电子查询（电子查询或电子数据展示）是识别、收集和出具电子存储信息 (ESI) 的电子方面，可响应法律花色或调查中的生产请求。 功能包括 "案例管理"、"保存"、"搜索"、"分析" 和 "工作组数据导出"。 这包括聊天、消息传递和文件、会议和通话摘要。 对于团队会议和通话，将创建会议和通话中发生的事件的摘要，并将其提供给电子数据展示。
