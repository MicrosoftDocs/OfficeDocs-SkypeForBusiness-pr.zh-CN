...
直接路由仅受 Microsoft 团队支持。 要通过直接路由放置或接收 PSTN 呼叫, 您需要配置必要的策略以确保在团队中接收传入呼叫。 你可以通过将用户配置为仅限团队模式或通过分配 TeamsCallingPolicy 和 TeamsInteropPolicy 将团队配置为首选呼叫客户端, 来配置用户以将团队设置为其首选客户端进行呼叫。

|询问你自己|操作 |
|:------------|:-------|
|如何将团队设置为首选客户端进行呼叫？ | 有关详细信息, 请参阅[将 Microsoft 团队设置为用户的首选调用客户端](direct-routing-configure.md#set-microsoft-teams-as-the-preferred-calling-client-for-users)。|
|||
