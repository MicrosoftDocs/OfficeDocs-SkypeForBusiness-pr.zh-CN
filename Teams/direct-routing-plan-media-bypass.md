...
媒体旁路利用称为交互式连接的协议在 SBC 上 (ICE) 团队客户端和 ICE 精简。 这些协议使直接路由能够使用最直接的媒体路径来获得最佳质量。 冰和 ICE Lite 是 WebRTC 标准。 有关这些协议的详细信息，请参阅 RFC 5245。
...
下图显示了启用媒体旁路时的呼叫流，客户端是内部客户端，并且客户端可以访问 SBC 的公共 IP 地址 (直接媒体) ： 

- 路径的箭头和数值与 [Microsoft 团队通话流](https://docs.microsoft.com/microsoftteams/microsoft-teams-online-call-flows)相符。

- SIP 信号始终采用路径4和 4 " (，具体取决于流量) 的方向。 媒体保持本地，并采用路径5b。
...
- 对于媒体旁路，Microsoft 使用传输中继的版本，需要在团队传输中继和 SBC (之间打开端口 50 000 到 59 999，我们计划移动到仅需要3478和3479端口) 的版本。


下图显示了启用媒体跳过后的呼叫流，客户端是外部的，并且客户端无法访问会话边界控制器的公共 IP 地址 (媒体由团队传输中继) 中继。
...
语音应用程序的媒体路径 | 都 | 绝对 | 
 (B2BUA 中可以执行转换) \* | 是 | 否，仅在终结点之间中继音频 | 
全球实例数和位置 | 总共10个：在美国东部和西部2在阿姆斯特丹和都柏林中的2个;2在中国香港和新加坡中;2在日本;澳大利亚东部和东南部的2 | 名

IP 范围包括：
- 52.112.0.0/14 (IP 地址从52.112.0.1 到 52.115.255.254) 
- 52.120.0.0/14 (IP 地址从52.120.0.1 到 52.123.255.254) 

\* 代码解释解释： 

- 媒体处理器为 B2BUA，这意味着它可以更改编解码器 (例如，从团队客户端到 MP 的，以及 MP 和 SBC) 之间的711。
...
### <a name="requirements-for-direct-media-traffic-between-the-teams-client-and-the-sbc"></a>团队客户端和 SBC)  (直接媒体通信的要求 

客户端必须有权访问指定的端口 (请参阅 SBC 的公共 IP 地址上的表) 。 
...
媒体处理器始终位于语音应用程序和 Web 客户端的媒体路径中 (例如，在 Edge 或 Google Chrome) 中的团队客户端。 要求与非绕过配置相同。
