...
ms.locfileid: "58595284"
---
# <a name="implement-quality-of-service-qos-in-microsoft-teams"></a>在 Microsoft Teams 中 (QoS) 服务质量Microsoft Teams

Microsoft Teams 中的服务质量 (QoS) 允许对网络延迟敏感的实时网络流量 (例如，语音或视频流) 在不太敏感的 (（如下载新应用，其中额外下载秒数不是大型交易) ）的流量前"直接换行"。 QoS Windows组策略对象和基于端口的访问控制列表来识别并标记实时流中的所有数据包。 这有助于网络为语音、视频和屏幕共享流提供专用的网络带宽部分。
...
- RTT (延迟的往返时间) – 媒体数据包到达目的地需要很长时间，导致对话中双方之间的明显延迟，导致人员相互交谈
...
1. 实现 QoS 设置：
   1. 在使用组策略对象或 GPO (客户端) [设置客户端设备端口范围和标记](QoS-in-Teams-clients.md)。
   ...
   3. [设置希望如何处理会议媒体Teams流量](meeting-settings-in-teams.md#set-how-you-want-to-handle-real-time-media-traffic-for-teams-meetings)。
...
网络的流量拥塞将极大地影响媒体质量。 带宽不足会导致性能下降和用户体验不佳。 随着Teams和使用量的增长，请使用报告、每用户呼叫分析和呼叫[](use-call-analytics-to-troubleshoot-poor-call-quality.md)质量仪表板[ (CQD) ](turning-on-and-using-call-quality-dashboard.md)来识别问题，然后使用 QoS 和选择性带宽添加进行调整。

### <a name="vpn-considerations"></a>VPN 注意事项

QoS 仅在呼叫者之间的所有链接上实现时，才能正常使用。 如果在内部网络上使用 QoS，并且用户从远程位置登录，则只能在内部托管网络中设置优先级。 尽管远程位置可以通过实现虚拟专用网络 (VPN) 来接收托管连接，但 VPN 本身会增加数据包开销，并创建实时流量的延迟。 建议避免通过 VPN 运行实时通信流量。
...
当网络流量进入路由器时，流量将放入队列。 如果未配置 QoS 策略，则只有一个队列，并且所有数据都被视为优先级相同的"先入先出"。 这意味着语音流量 (对延迟非常敏感) 可能会卡在流量后面，在这些流量上，延迟几毫秒不是问题。
...
可以通过基于端口的标记、使用访问控制列表和 (ACL) 网络路由器上实现 QoS。 基于端口的标记是最可靠的方法，因为它适用于混合Windows、Mac 和 Linux 环境，并且最容易实现。 移动客户端不提供使用 DSCP 值标记流量的机制，因此需要此方法。  
...
尽管基于端口的标记可以跨平台工作，但它只会在 WAN 边缘标记 (不会一路标记到客户端计算机，) 产生管理开销。 有关实现此方法的说明，请参阅路由器制造商提供的文档。
...
尽管此方案完全有效，但它仅适用于已加入域的Windows客户端。 任何未加入域的设备Windows客户端不会启用 DSCP 标记。 其他客户端（例如运行 macOS 的客户端）具有硬编码的标记，并且始终标记流量。
...
我们建议在终结点上结合使用 DSCP 标记和路由器上基于端口的 ACL（如果可能）。 使用 GPO 捕获大多数客户端，同时使用基于端口的 DSCP 标记可确保移动、Mac 和其他客户端仍获得 QoS 处理 (至少部分) 。
...
如果以前已部署 Skype for Business（包括 QoS 标记和端口范围），并且现在正在部署。 Teams，Teams将遵守现有配置，并使用与客户端相同的端口范围和Skype for Business标记。 在大多数情况下，不需要其他配置。
...
在Teams，应主动管理不同工作负荷使用的 QoS 源端口，并根据需要进行调整。 引用"选择每种媒体类型[](#choose-initial-port-ranges-for-each-media-type)的初始端口范围"中的表，端口范围是可调整的，但 DSCP 标记不可配置。 实现这些设置后，你可能会发现给定媒体类型需要更多或更少的端口。 应该[使用](use-call-analytics-to-troubleshoot-poor-call-quality.md)基于用户的呼叫分析和呼叫质量仪表板[ (CQD) ](turning-on-and-using-call-quality-dashboard.md)来做出在 Teams 实施后调整端口范围的决策，并定期根据需求变化进行调整。
...
如果之前已在本地Skype for Business Server，可能需要重新检查 QoS 策略。 调整策略以匹配已验证的端口范围设置，为用户Teams。
