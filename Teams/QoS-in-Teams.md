...
audience: admin
description: 了解如何为组织的服务质量网络做好准备， (QoS) Microsoft Teams。
...
ms.locfileid: "58732931"
---
# <a name="implement-quality-of-service-qos-in-microsoft-teams"></a>在 Microsoft Teams 中实现服务质量 (Qo) S Microsoft Teams

Microsoft Teams 中的服务质量 (QoS) 允许对网络延迟敏感的实时网络流量 (例如，语音或视频流) 在不太敏感的 (例如下载新应用（其中额外下载秒数不是很大交易) ）的流量前"切线"。 QoS Windows组策略对象和基于端口的访问控制列表来识别并标记实时流中的所有数据包。 这有助于网络为语音、视频和屏幕共享流提供专用的网络带宽部分。
...
若要使 QoS 生效，必须在整个组织中应用一致的 QoS 设置。 路径中无法支持 QoS 优先级的任何部分都可能会降低通话、视频和屏幕共享的质量。 这包括将设置应用到所有用户电脑或设备、网络交换机、路由器到 Internet，以及Teams服务。

_图 1.组织网络与服务Microsoft 365 Office 365关系_
...
1. 实现 QoS 设置：
   1. 在使用组策略对象 (GPO) [设置客户端设备端口范围和标记](QoS-in-Teams-clients.md)。
   2. 在路由器 (请参阅制造商文档) 或其他网络设备。 这可能包括基于端口的访问控制列表 (ACL) 定义 QoS 队列和 DSCP 标记，或者所有这些 ACL。

      > [!IMPORTANT]
      > 我们建议使用客户端源端口以及"任意"的源和目标 IP 地址实施这些 QoS 策略。 这会捕获内部网络上传入和传出的媒体流量。  

   3. [设置如何处理会议媒体Teams流量](meeting-settings-in-teams.md#set-how-you-want-to-handle-real-time-media-traffic-for-teams-meetings)。

5. [通过分析网络上](#validate-your-qos-implementation)流量Teams QoS 实现。

准备实施 QoS 时，请记住以下准则：

- 最佳选择是Microsoft 365路径。
- 关闭端口只会导致质量下降。
- 不建议在两者之间设置任何障碍，例如代理。
- 限制跃点数：
  - 客户端到网络边缘 – 3 到 5 个跃点
...
有关配置防火墙端口的信息，请转到Office 365 [URL 和 IP 范围](office-365-urls-ip-address-ranges.md)。

## <a name="make-sure-your-network-is-ready"></a>确保网络已准备就绪

如果你正在考虑 QoS 实现，你应该已经确定了你的带宽要求和其他 [网络要求](prepare-network.md)。
  
网络的流量拥塞将极大地影响媒体质量。 带宽不足会导致性能下降和用户体验不佳。 随着Teams和使用量的增长，请使用报告、每用户呼叫分析和[](use-call-analytics-to-troubleshoot-poor-call-quality.md)呼叫质量仪表板[ (CQD) ](turning-on-and-using-call-quality-dashboard.md)来识别问题，然后使用 QoS 和选择性带宽添加进行调整。

### <a name="vpn-considerations"></a>VPN 注意事项

QoS 仅在呼叫者之间的所有链接上实现时，才能正常使用。 如果在内部网络上使用 QoS，并且用户从远程位置登录，则只能在内部托管网络中设置优先级。 尽管远程位置可以通过实现虚拟专用网络 (VPN) 来接收托管连接，但 VPN 本身会增加数据包开销，并造成实时流量的延迟。 建议避免通过 VPN 运行实时通信流量。
...
当网络流量进入路由器时，流量将放入队列。 如果未配置 QoS 策略，则只有一个队列，并且所有数据都被视为优先级相同的"先入先出"。 这意味着语音流量 (对延迟非常敏感) 可能会卡在流量后面，如果延迟几毫秒也不会是问题。
...
可以通过基于端口的标记实现 QoS，使用访问控制列表 (网络路由器) ACL。 基于端口的标记是最可靠的方法，因为它适用于混合Windows、Mac 和 Linux 环境，并且最容易实现。 移动客户端不提供使用 DSCP 值标记流量的机制，因此需要此方法。  
...
尽管基于端口的标记可以跨平台工作，但它只标记 WAN 边缘 (流量，而不是一路标记到客户端计算机) 产生管理开销。 有关实现此方法的说明，请参阅路由器制造商提供的文档。

### <a name="insert-dscp-markers"></a>插入 DSCP 标记

还可使用组策略对象 (GPO) 实现 QoS，以指示客户端设备在 IP 数据包标头中插入 DSCP 标记，将其标识为特定的流量类型 (例如语音) 。 可以将路由器和其他网络设备配置为识别这一点，将流量放在一个单独的、优先级更高的队列中。

尽管此方案完全有效，但它仅适用于已加入域Windows客户端。 任何未加入域的设备Windows不会启用 DSCP 标记。 其他客户端（例如运行 macOS 的客户端）具有硬编码的标记，并且始终标记流量。
...
我们建议在终结点上结合使用 DSCP 标记和路由器上基于端口的 ACL（如果可能）。 使用 GPO 捕获大多数客户端，同时使用基于端口的 DSCP 标记将确保移动、Mac 和其他客户端仍将获得 QoS 处理 (至少部分) 。

DSCP 标记可以比作邮政戳记，它向邮政工作人员指示交付的紧急性，以及如何以最佳方式对它进行排序以加快交付。 将网络配置为优先处理实时媒体流后，丢失的数据包和延迟的数据包应大大减少。

一旦网络内的所有设备使用相同的分类、标记和优先级，则通过更改分配给用于每种流量类型的队列的端口范围的大小，可以减少或消除延迟、丢弃数据包和抖动。 从Teams的角度来看，最重要的配置步骤是数据包的分类和标记。 但是，若要使端到端 QoS 成功，还需要仔细将应用程序的配置与基础网络配置保持一致。 完全实施 QoS 后，持续管理就是根据组织的需求和实际使用情况调整分配给每种流量类型的端口范围的问题。

## <a name="choose-initial-port-ranges-for-each-media-type"></a>选择每种媒体类型的初始端口范围

DSCP 值告知相应配置的网络为数据包或流提供什么优先级，无论 DSCP 标记是由客户端分配还是网络本身基于 ACL 设置分配。 每个媒体工作负荷获取其自己的唯一 DSCP 值 (其他服务可能允许工作负荷共享 DSCP 标记，Teams 不) 以及用于每种媒体类型的已定义和单独端口范围。 其他环境可能有现有的 QoS 策略，可帮助确定网络工作负荷的优先级。

不同实时流式处理工作负荷的端口范围的相对大小设置专用于该工作负荷的总可用带宽的比例。 要返回到我们之前的邮政类比：带"Air Mail"戳记的信函可能在一小时内到达最近的机场，而标有"批量邮件"标记的小包裹可以等待一天，然后再在一系列卡车上陆。
...
- 所有客户端（包括移动客户端Teams设备）都将使用这些端口范围，并且将受你实施的任何使用这些源端口范围的 DSCP 策略的影响。 将继续使用动态端口的唯一客户端是基于浏览器的客户端 (允许参与者使用其浏览器加入会议) 。
...
如果以前已部署 Skype for Business Online，包括 QoS 标记和端口范围，并且现在正在部署。 Teams，Teams将遵守现有配置，并使用与客户端相同的端口范围和Skype for Business标记。 在大多数情况下，不需要其他配置。

> [!NOTE]
> 如果通过组策略使用应用程序名称 QoS 标记，则必须Teams.exe作为应用程序名称。

### <a name="implement-qos-in-teams-on-windows-using-powershell"></a>使用 PowerShell 在 Teams 上Windows QoS
...
## <a name="managing-source-ports-in-the-teams-admin-center"></a>管理管理中心Teams端口

在Teams中，应主动管理不同工作负荷使用的 QoS 源端口，并根据需要进行调整。 引用"选择每种媒体类型[](#choose-initial-port-ranges-for-each-media-type)的初始端口范围"中的表，端口范围是可调整的，但 DSCP 标记不可配置。 实现这些设置后，你可能会发现给定媒体类型需要更多或更少的端口。 应该[使用](use-call-analytics-to-troubleshoot-poor-call-quality.md)基于用户的呼叫分析和呼叫质量仪表板[ (CQD) ](turning-on-and-using-call-quality-dashboard.md)来做出在端口范围实施后调整端口Teams的决策，并定期根据需求变化进行调整。

> [!NOTE]
> 如果已基于 Skype for Business Online 的源端口范围和 DSCP 标记配置 QoS，则相同的配置将应用于 Teams 并且无需对映射进行进一步客户端或网络更改，不过，您可能需要设置[Teams](meeting-settings-in-teams.md#set-how-you-want-to-handle-real-time-media-traffic-for-teams-meetings)中使用的范围，以匹配为 Skype for Business Online 配置的范围。

如果以前已在本地Skype for Business Server，可能需要重新检查 QoS 策略。 调整策略以匹配已验证的端口范围设置，为用户提供高质量的Teams。
...
最好是捕获网络出口点的流量。 可以在交换机或路由器上使用端口镜像来帮助实现此要求。
