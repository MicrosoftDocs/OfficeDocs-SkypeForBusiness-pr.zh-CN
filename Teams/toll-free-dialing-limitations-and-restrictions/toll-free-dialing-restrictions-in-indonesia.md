---
title: 印度尼西亚的免费拨号限制
ms.author: crowe
author: CarolynRowe
manager: serdars
ms.reviewer: mikedav, oscarr
ms.topic: reference
ms.assetid: 4cec1b2d-80b4-4cc0-b250-19449c7b136b
ms.tgt.pltfrm: cloud
ms.service: msteams
ms.collection:
- M365-collaboration
- M365-voice
search.appverid: MET150
audience: Admin
appliesto:
- Skype for Business
- Microsoft Teams
ms.localizationpriority: medium
ROBOTS: NOINDEX, NOFOLLOW
f1.keywords:
- NOCSH
ms.custom:
- Calling Plans
- seo-marvel-mar2020
description: 了解如何在印度尼西亚拨打免费电话号码，包括可用性、有线/固定线路和移动网络可用性以及限制。
ms.openlocfilehash: 55e17f036261ce7f03417b8cde4a8cb403f3bffe
ms.sourcegitcommit: 556fffc96729150efcc04cd5d6069c402012421e
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 08/26/2021
ms.locfileid: "58612891"
---
# <a name="toll-free-dialing-restrictions-in-indonesia"></a>印度尼西亚的免费拨号限制

[查找其他国家/地区](../toll-free-dialing-limitations-and-restrictions.md)，或了解有关免费拨号限制和限制的更多内容。


|**详细信息**|**说明**|
|:-----|:-----|
|是否提供免费呼叫？  <br/> |是  <br/> |
|拨号格式 ()   <br/> | 001 803 CCC XXXX <br/>  007 803 CCC XXXX <br/> |
|国家/地区 ISO 代码  <br/> |ID  <br/> |
|涵盖的岛屿/区域  <br/> |不适用  <br/> |
|未涵盖的岛屿/区域  <br/> |不适用  <br/> |
|有线/固定线路可用性  <br/> |是  <br/> |
|移动网络可用性  <br/> |部分。 请参阅注释。  <br/> |
|注释、限制和访问问题  <br/> |在更大的范围内提供两种号码格式。 数字格式 007 803 CCC XXXX 格式可从 PT Telkom 网络（包括 Telkomsel 和 XL 移动版）访问。 号码格式 001 803 CCC XXXX 可通过 Indosat 网络访问。 对于移动设备：矩阵手机 (前缀 0855、0858、0815 和 0816) 。  <br/> |
   
## <a name="related-topics"></a>相关主题

[免费拨号限制](../toll-free-dialing-limitations-and-restrictions.md)

[获取 Skype for Business 和 Microsoft Teams 的服务电话号码](../getting-service-phone-numbers.md)

  
