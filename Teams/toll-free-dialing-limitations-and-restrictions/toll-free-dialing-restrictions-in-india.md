---
title: 印度的免费拨号限制
ms.author: crowe
author: CarolynRowe
manager: serdars
ms.reviewer: mikedav, oscarr
ms.topic: reference
ms.assetid: d9e0809c-633d-4ee9-891b-ff4282789da5
ms.tgt.pltfrm: cloud
ms.service: msteams
ms.collection:
- M365-collaboration
- Teams_ITAdmin_Help
- M365-voice
search.appverid: MET150
appliesto:
- Skype for Business
- Microsoft Teams
localization_priority: Normal
ROBOTS: NOINDEX, NOFOLLOW
f1keywords: None
ms.custom:
- Calling Plans
description: 这将帮助您了解如何拨打每个国家/地区的免费电话号码。 选择国家/地区后, 将向您转到特定于国家/地区的页面, 其中包含特定于免费服务可用的详细信息、限制和限制。 拨号格式或格式将向你显示每个国家/地区内所需的访问代码, 以拨打免费电话号码。
ms.openlocfilehash: 5226df97db101088e5a843b6a130dd36c16eb460
ms.sourcegitcommit: 4b8350e5bb2ef138dcc0204d764bdf85bae539ba
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 05/21/2019
ms.locfileid: "34334852"
---
# <a name="toll-free-dialing-restrictions-in-india"></a>印度的免费拨号限制

[查找其他国家或地区](../toll-free-dialing-limitations-and-restrictions.md), 或了解有关免费拨号限制和限制的详细信息。


|**详细信息**|**说明**|
|:-----|:-----|
|是否可以免费通话？  <br/> |是  <br/> |
|拨号格式  <br/> |000 800 XXX XXXX  <br/> |
|国家/地区 ISO 代码  <br/> |IND  <br/> |
|已覆盖的岛/地区  <br/> |不适用  <br/> |
|未覆盖的岛/区域  <br/> |不适用  <br/> |
|有线/固定线路可用性  <br/> |部分。 请参阅注释。  <br/> |
|移动网络可用性  <br/> |部分。 请参阅注释。  <br/> |
|批注、限制和访问问题  <br/> |可从以下网络中访问: 循环手机, BSNL-已修复 (所有地区, 但 Bhopal), Vodafone, 构思/更加生动 (所有地区, 但 Punjab 更加生动), MTS 印度/Shyam (固定), &amp; MTNL-固定的手机、信任 (所有地区, 但 Ahmedabad、Bhopal、Bhuneswer, Jaipur, Lucknow, Meerut), TTML &amp; TTSL (固定&amp; Postpaid), Tata DoCoMo, HFCL Infotel/Ping 移动电话、手机手机、Uninor、Videocon 手机、BSNL、Bharti/Airtel 网络。  <br/> 从以下网络不可用的访问权限: TTSL (预付)、Etisalat DB 电信和 Jio。  <br/> |
   
## <a name="related-topics"></a>相关主题
[免费拨号限制](../toll-free-dialing-limitations-and-restrictions.md)

[获取 Skype for Business 和 Microsoft Teams 的服务电话号码](/microsoftteams/getting-service-phone-numbers)

  
 