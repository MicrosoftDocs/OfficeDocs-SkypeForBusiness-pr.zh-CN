...
"允许 **现在开会** " 是每用户策略，在会议开始之前应用。 此设置控制用户是否可以在团队频道中启动计划外会议。 如果启用此设置，用户可以选择 " **开会** " 按钮启动计划外会议或在频道中安排会议。 默认值为 True。
...
如果关闭此功能，用户将无法进行计划。 在 Outlook 中创建新会议时，团队会议。 例如，在 Windows 上的 Outlook 中，" **新建团队会议** " 选项不会显示在功能区中。

### <a name="allow-channel-meeting-scheduling"></a>允许频道会议安排

使用现有的 AllowChannelMeetingScheduling 策略控制可在团队频道日历上创建的事件类型。 这是每用户策略，在会议开始之前应用。 此设置控制用户是否可以在团队频道中安排会议。 默认情况下，此设置处于打开状态。

如果此策略处于关闭状态，则用户将无法创建新的频道会议。 但是，现有频道会议可由事件的组织者编辑。
...
- 将禁用频道日历标题上的 "**添加新事件**" 按钮。
  ![频道日历标题上的按钮已禁用](media/add-new-event-disabled.png)

- 用户无法在频道日历上拖动和选择时间段来创建频道会议。
...
这是每用户策略，在会议开始之前应用。 此设置控制用户是否可以启动未计划的私人会议。  默认情况下，此设置处于打开状态。
...
### <a name="mode-for-ip-audio"></a>IP 音频的模式

这是每个用户的策略。 此设置控制是否可以在会议和群组通话中打开音频。 下面是此设置的值。

|设置值 |行为  |
|---------|---------|
|**已启用传出和传入音频**    |会议中允许传出和传入音频。 这是默认设置。 |
|**已禁用**     |会议中的传出和传入音频处于关闭状态。     |

如果为用户设置为 " **已禁用** "，则该用户仍可以计划和组织会议，但不能使用音频。 要加入会议，他们必须通过公共交换电话网络 (PSTN) 拨入，或者通过电话进行会议呼叫和加入会议。 未分配任何策略的会议参与者 (例如，匿名参与者) 将此设置为默认情况下 **启用的传出和传入音频** 。 在团队移动客户端上，如果禁用此设置，则用户必须通过 PSTN 拨入会议。
...
这是每个用户的策略。 此设置控制是否可以在会议和群组通话中打开视频。 下面是此设置的值。

|设置值 |行为  |
|---------|---------|
|**已启用传出和传入视频**    | 会议中允许发送和接收视频。 这是默认设置。 |
|**已禁用**     | 会议中的传出和传入视频处于关闭状态。 在团队移动客户端上，用户无法在会议中共享视频或照片。 <br><br>请注意，如果 " **ip 音频模式** " 已禁用，则 " **Ip 视频" 模式** 也将保持禁用状态。  |
...
这是每个组织单位和每用户策略的组合。 视频是会议的关键组件。 在某些组织中，管理员可能希望更好地控制哪些用户的会议有视频。 此设置控制在用户托管的会议、1:1 和用户启动的组通话中是否可以打开视频。 在团队移动客户端上，此设置控制用户是否可以在会议中共享照片和视频。

通过启用此策略设置的用户组织的会议，如果参与者也启用了策略设置，则会议参与者允许会议中的视频共享。 未分配任何策略的会议参与者 (例如，匿名参与者和联盟参与者) 继承会议组织者的策略。

> [!NOTE]
> 请记住，此设置控制传出视频，而 " **IP 视频模式** " 设置控制传出和传入视频。 若要了解详细信息，请参阅 [哪些 IP 视频策略设置优先？](#which-ip-video-policy-setting-takes-precedence) 和 [管理会议参与者的音频/视频](#manage-audiovideo-for-meeting-participants)。

| 团队桌面和 web 客户端 |团队移动客户端  |
|:-------:|:-------:|
...
#### <a name="which-ip-video-policy-setting-takes-precedence"></a>哪种 IP 视频策略设置优先

对于用户，视频的最严格的策略设置优先。 下面是一些示例。

|允许 IP 视频|IP 视频模式|会议体验|
|---------|---------|---------|
|组织者： **开**<br><br>参与者： **开** |参与者： **已禁用**        |" **IP 视频模式** " 设置优先。 分配了此策略的参与者无法打开或查看其他人共享的视频。|
|组织者： **开**<br><br>参与者： **开** |参与者： **已启用传出和传入视频**          |分配了此策略的参与者可以打开或查看其他人共享的视频。         |
|组织者： **开**<br><br>参与者： **关** |参与者： **已启用传出和传入视频**         |" **允许 IP 视频** " 设置优先。 参与者只能查看传入视频，不能发送传出视频。         |
|组织者： **开**<br><br>参与者： **关** |参与者： **已禁用**         |" **IP 视频模式** " 设置优先。 参与者无法查看传入或传出视频。|
|组织者： **关闭**    |       |" **允许 IP 视频** " 设置优先，因为它对于组织者而言处于关闭状态。 任何人都无法在分配了此策略的用户组织的会议中打开视频。         |

### <a name="manage-audiovideo-for-meeting-participants"></a>管理会议参与者的音频/视频

|如果需要 .。。  |设置以下策略设置  |
|---------|---------|
|对会议中的参与者禁用音频和视频  |IP 音频的模式： **已禁用**<br> IP 视频模式： **已禁用**<br>允许使用 IP 视频： N/A       |
|仅为会议中的参与者启用传入视频和音频  |IP 音频模式： **启用传出和传入音频**<br> IP 视频模式： **启用传出和传入视频**<br>允许 IP 视频： **关闭**       |
|在会议中禁用参与者的视频 (参与者仅具有音频) |  IP 音频模式： **启用传出和传入音频**<br> IP 视频模式： **已禁用**<br>允许使用 IP 视频： N/A
|为会议中的参与者启用音频和视频    |适用于 IP 音频的模式： **已启用传出和传入音频** (默认) <br> 适用于 IP 视频的模式：默认情况下 **启用的传出和传入视频** () <br>允许 IP **视频： (** 默认)     |
...
对于团队移动客户端的用户，在会议期间共享照片和视频的功能也由 " **允许 IP 视频** 或 **ip 视频模式** " 设置确定。 根据优先的策略设置，无法使用共享视频和照片的功能。 这不会影响屏幕共享，您可以使用单独的 [屏幕共享模式](#screen-sharing-mode) 设置进行配置。 此外，你可以设置 [团队移动策略](https://docs.microsoft.com/powershell/module/skype/new-csteamsmobilitypolicy) ，以防止移动用户通过手机网络连接使用 IP 视频，这意味着它们必须使用 WiFi 连接。
...
> [!Note]
> 此功能仍在开发中。 屏幕共享是每个参与者的策略。

此设置控制是否允许在用户的会议中共享桌面和/或窗口共享。 未分配任何策略的会议参与者 (例如，匿名、来宾、B2B 和联盟参与者) 继承会议组织者的策略。
...
这是每个组织者策略，允许 leaderless 电话拨入式会议会议。 此设置控制在没有经过身份验证的组织中，拨入用户是否可以加入会议。 默认情况下，此设置处于关闭状态，这意味着拨入用户将在大厅中等待，直到组织中的经过身份验证的用户加入会议。

> [!NOTE]
> 如果此设置处于关闭状态，并且电话拨入式用户首先加入会议，并且位于会议厅中，则组织用户必须加入与团队客户的会议，以便从会议厅中为用户提供会议。 没有可用于在用户中拨打的会议厅控件。
...
这是每个组织者的策略。 此设置控制通过电话拨入的用户是否直接加入会议或在会议厅中等待，而不管 "是否 **自动允许人员** " 设置。 默认情况下，此设置处于关闭状态。 当此设置关闭时，拨入用户将在大厅中等待，直到组织用户通过团队客户加入会议并准许他们。 如果启用此设置，当组织用户加入会议时，拨入用户将自动加入会议。
