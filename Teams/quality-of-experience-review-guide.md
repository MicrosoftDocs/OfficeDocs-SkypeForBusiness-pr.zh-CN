...
-   **服务管理：** 此类别包含两个部分：
    ...
    -   第二个是组织必须管理的任务以确保能够可靠地访问该服务，例如，在向服务添加基础结构时，更新构建信息和维护新 Microsoft 365 或 Office 365 IP 地址的防火墙。
...
在 cloud-first world 中，你必须执行某些服务管理任务来保持高质量的用户体验。 这些任务范围包括：确保有足够的带宽来联系服务，但没有饱和 internet 链接，验证所有托管网络区域的服务质量（QoS）是否存在，最后，在防火墙上的[Microsoft 365 和 Office 365 IP 范围](https://aka.ms/o365ips)内保持最高。
...
2. 打开 Azure PowerShell 命令窗口并运行以下脚本，在出现提示时输入 Microsoft 365 或 Office 365 凭据：
...
若要筛选单个详细报表，请将筛选器添加 ``Is Teams`` 到报表，并将其设置为 True 或 False。 有关详细信息，请参阅本部分后面的[编辑报告](#editing-reports)。
...
|             |           |
| ------------|-----------|
| ![数字1的图标，引用上一个屏幕截图中的标注](media/qerguide-image-callout1.png "下列") | 从左窗格中选择维度、度量值和筛选器。 指向现有值将显示 "关闭" 按钮（**X**），您可以选择删除该值。<ul><li>通过选择维度或度量值，可以通过编辑 "**标题**" 字段来更改标题。 您也可以通过选择顶部窗格中的蓝色向上或向下箭头来更改顺序。</li><li>选择标题旁边的 "（" **+** ）将打开用于添加新维度、度量值或筛选器的对话框。</li><li>在 "**查找**字段" 中输入维度、度量值或筛选器的前几个字母以筛选列表，以便更轻松地进行搜索。</li></ul> |
...
2. 使用 Microsoft 365 或 Office 365 管理凭据进行身份验证。
...
> [!IMPORTANT]
> 某些 VPN 实现不会准确报告子网信息。 这是因为有一个32位子网提供了 VPN 客户端。  正如上一节中所述，CQD 无法正确识别32位子网。  若要在 CQD 中准确地标识 VPN 子网，请在生成文件中将 VPN 字段设置为1。
...
![图表显示使用 TCP 图25的音频流百分比 ](media/qerguide-image-audiostreamswithtcp.png)
 _（使用 tcp 使用的音频流）_
...
### <a name="network-connectivity-principles"></a>网络连接原理

在开始规划 Microsoft 365 或 Office 365 网络连接的网络之前，请务必了解安全管理 Microsoft 365 或 Office 365 流量并获得最佳性能的连接原则。 以下文章将帮助您了解安全优化网络连接的最新指南：
