...
<td>
<p>通过组织内部呼叫（ (VPN、WiFi、有线或) 等）来分解指标</p>
<p>通过生成或网络分解指标</p>
<p>VPN 调用</p>
...
本文重点介绍使用呼叫质量仪表板 (CQD) Online 作为主要工具来报告并调查每个区域，并特别强调音频，以最大程度地采用和影响。 对网络进行的任何改进来改善音频体验也将直接转换为视频和桌面共享方面的改进。

为了加快评估速度，提供了两个特策划的 [CQD](https://aka.ms/qertemplates) 模板：一个模板用于管理所有网络，另一个模板仅针对托管 (内部) 筛选。 尽管"所有网络"模板报表配置为显示建筑物和网络信息，但仍可在收集和上传建筑物信息时使用。 将建筑物信息上传到 CQD 后，服务可添加自定义建筑物、网络和位置信息，同时从外部子网内部进行区分，从而增强报告功能。 有关详细信息，请阅读建筑物 [映射](CQD-building-mapping.md)。
...
呼叫数据提供使用情况指标，但不一定会导致通话质量不佳的根本原因。 通过查看流方向，可以识别一些因素，例如不在托管网络上呼叫、来自非员工 (例如供应商或其他网络中某人的呼叫) 。 在这些情况下，如果其他人的网络连接不佳，则整个呼叫将标记为差。 不能对外部因素执行任何操作，因此此数据没有帮助。
...
此指标的计算公式为：未能设置的流总数除以在 CDR 中提交成功的呼叫详细信息记录的 (总数) ：
...
本部分讨论一些用于评估服务运行状况的核心服务指标。 通过持续评估和推动努力，使这些指标低于其定义的目标，有助于确保用户体验一致、可靠的呼叫质量。 首先，请使用下表中建议的目标。 根据需要调整目标以满足业务目标。
...
分析用户体验比分析科学更具艺术性，因为此处收集的指标不一定总意味着网络或服务存在问题，而只是表示用户感知了问题。 CQD 包括内置调查机制（对 RMC (呼叫) 评分）以帮助评估总体用户体验。 RMC 将让用户从用户的角度深入了解以下问题：
...
-   **网络：** 音频质量侧重于 PSR (性能) 、TCP 使用情况、有线和无线子网，以及识别 HTTP 代理和 VPN 的使用
...
CQD 中的许多维度和度量值被分类为第一或第二个。 CQD 不使用调用方/被调用方字段，这些字段已重命名为第一个和第二个，因为调用方和被调用方之间存在中间步骤。 以下逻辑确定将涉及的终结点标记为第一个：
...
有关确定第一个或第二个终结点（二者相同）详细信息，请参阅 [CQD 中可用的维度和度量值](./dimensions-and-measures-available-in-call-quality-dashboard.md)。

### <a name="stream-vs-call"></a>流与通话

你需要了解调用和流的区别，以正确选择要在 CQD 中查看的维度或度量值。 尽管 CQD 的主要焦点是流，但基于调用的度量也可用。
...
### <a name="good-poor-and-unclassified-calls"></a>良好、较差和未分类的调用

调用被分类为良好、较差或未分类。 让我们花些时间更详细地讨论每个对话。

-   **好或差：** 良好或较差的呼叫包括包含一组完整服务指标的呼叫，服务生成并接收了完整的 QoE 报告。 本文前面介绍了确定流是好还是 [差](#poor-stream-rate)。

-   **未分类：** 未分类的流不包含一组完整的服务指标。 这些呼叫可以是短时间调用（通常少于 60 秒），其中无法计算平均值，并且未生成 QoE 报告。 调用未分类的最常见原因是数据包利用率很少或没有。 例如，参与者在静音时加入会议，从不说话。 参与者正在接收媒体，但不传输媒体。 如果不传输媒体，CQD 将不会使用任何指标来分类终结点的出站媒体流。
...
评估整个组织的音频质量状态的下一步是调查 PSR (、TCP 和代理) 的低流速率。 必须记住，CQD 数据不会提供特定的根本原因，而是提供可能的问题领域，让你开始与相应的团队进行协作，开展修正活动。 
...
PSR 百分比用于指示组织是否满足给定重点区域定义的指标目标。 必须注意，即使高级百分比在定义的目标内，单个子网或建筑物也可能不符合定义的目标，因此需要进一步调查。 例如，如果 4 月的整体音频 PSR 百分比为 2%，满足示例目标，则单个建筑物和子网可能仍然体验不佳，具体取决于该 2% 的总体分布情况。 
...
尽管使用的尺寸在报表之间可能略有不同，但每个报告将包含针对总流、总较差流、PSR 和由于原因导致的质量不佳的度量值。 已针对每个感兴趣的领域创建报告：会议、双方、PSTN 呼叫、VPN 和会议室。 托管网络模板包括其他报表，以利用通过建筑物文件上传的位置信息。
...
将补救工作集中在具有最大流量的建筑物或子网上，因为这将最大化影响，并有助于快速改善用户体验。 使用抖动、数据包丢失和往返时间 (RTT) 度量来了解导致质量不佳的原因 (因为存在多个问题) ：

-   **抖动**：媒体数据包以不同的速度到达，这会导致扬声器发出声音机器人的声音。
...
如果发现 TCP 使用趋势正在增加或高于每月正常使用量，请花些时间通过子报表进行调查，以查找任何可能需要修正的建筑物或网络。 理想情况下，需要托管网络上尽可能少的基于 TCP 的音频会话。

#### <a name="tcp-vs-udp"></a>TCP 与 UDP

此报告确定基于 VBSS 的基于音频、视频和视频的屏幕共享的 TCP 与 UDP 使用情况报告量 (VBSS) 。 

![显示使用 TCP 与 UDP 的流量的报告](media/qerguide-image-tcpvsudp.png)

##### <a name="analysis"></a>分析

尽管希望 TCP 使用率尽可能低，但是，在正常的部署中可能会看到一些 TCP 使用情况。 TCP 本身不会导致较差的调用，因此提供流速率以帮助确定 TCP 使用率是否导致质量不佳。 
...
此报告标识了影响 TCP 用量的特定建筑物和子网。 此外，还包括一个额外的报告，用于标识调用中使用的 Microsoft 中继 IP，以帮助隔离缺少的防火墙规则。 将修正工作集中在 TCP 流量最大的建筑物上，以最大程度地提高影响。
...
> [!IMPORTANT]
> 除非排除联合参与者数据，否则这些报告将包括来自联合终结点的客户端遥测数据。 若要排除联合终结点，必须为设置为组织的租户 ID 的第二个 **租户 ID** 添加 [查询筛选器](CQD-data-and-reports.md#how-to-find-your-tenant-id)。 通常，可以使用 [URL](CQD-data-and-reports.md#url-filters) 筛选器排除联合参与者遥测数据。
