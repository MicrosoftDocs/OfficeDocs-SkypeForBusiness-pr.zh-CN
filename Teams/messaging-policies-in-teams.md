...
- 已**读回执**"已读" 回执允许收件人在1:1 和群组聊天20人或更少的人阅读其邮件时, 收到聊天消息的发件人。 邮件已读回执删除有关邮件是否已被阅读的 uncertainly, 并改善团队沟通。  
    ...
    - 对**所有人启用**这意味着租户中的每个人都将启用该功能, 而不选择将其关闭。 请注意, 为整个租户**** 设置收据的唯一方式是仅为整个租户设置一个消息策略 (名为 "Global (组织范围默认)" 的默认策略), 或在所有邮件策略中使用租户对收据使用相同的设置。 当**为所有人**启用该功能时, "已读回执" 功能最有效。
