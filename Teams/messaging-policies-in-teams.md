...
- **删除已发送的消息** 使用此设置可让用户删除他们在聊天中发送的单个消息。
- **删除聊天** 使用此设置，用户可以选择从聊天列表中删除整个对话。 这只会删除执行操作的用户的聊天内容;聊天中的其他用户仍可查看所有消息。 删除聊天的用户不会在其活动源或已删除聊天的搜索结果中看到任何现有消息;此用户也将从已删除的聊天中删除。 系统将提示用户确认删除聊天的决定。 默认情况下，此设置处于打开状态。 如果关闭此设置，用户将不再看到其聊天的删除选项。
...
- **在对话中使用 Giphy**  如果打开 Giphys，用户可以在与其他人的聊天对话中包括 Giphys。 Giphy 是一个在线数据库和搜索引擎，允许用户搜索和共享动态 GIF 文件。 为每个 Giphy 分配内容分级。 除了启用此设置外，还需要启用 [可选连接体验](/deployoffice/privacy/manage-privacy-controls#policy-setting-for-optional-connected-experiences) ，以允许 Giphys 参与对话。
- **Giphy 内容分级**
