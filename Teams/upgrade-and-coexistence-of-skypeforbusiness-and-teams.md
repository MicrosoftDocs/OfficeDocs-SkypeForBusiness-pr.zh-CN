...
当您的部分用户准备若要为其日常通信和协作使用仅团队需要您可以开始将这些用户升级到团队，通过为其启用**仅团队**模式。

如果不为整个组织将移动到团队可行，可以首先在**群岛**共存模式下试验旁 for Business 的 Skype 的团队。 （即**Skype for Business 使用团队协作**和**Skype for Business with 团队协作和会议**） 的其他共存模式是可用逐渐完全在接下来的几个月，您还可以通过启动完全采用团队作为一个组的协作解决方案首先保持为贵组织的统一的通信解决方案的业务的 Skype。 这是 Microsoft 的建议的路径 （本地或混合） 的业务服务器和客户的 Skype 使用向工作组其轨迹将包括长共存的很复杂的客户。
...
|仅团队 |一些用户需要停留在 Skype for Business<br><br>正在将业务 Online 用户您 Skype 升级到团队时保持 for Business 的 Skype 内部部署用户 Skype 业务服务器<br><br>您可能已经部署了群岛模式中的用户并准备好停用的用户组的业务的 Skype |减少 for Business （内部部署服务器操作、 外包合同等） 的 Skype 变量成本<br><br>至少一些用户尽可能快地转到的完整的团队体验|通过提供只有一个客户端使用的互操作性与 Skype 中的用户仅业务、 Skype 团队协作 Business、 Business 团队协作和会议的 Skype 限制用户混淆情况|互操作性仅支持基本聊天和调用 Skype 商业和团队之间的互操作的升级方案的桌面共享和多方聊天和呼叫|
