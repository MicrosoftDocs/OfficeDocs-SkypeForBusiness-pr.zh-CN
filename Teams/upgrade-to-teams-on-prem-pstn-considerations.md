...
- [Teams 客户端体验和共存模式的一致性](teams-client-experience-and-conformance-to-coexistence-modes.md)
 ...
 > [!NOTE]
 > - 仅在电话系统仅Teams模式为用户帐户分配了一个Teams策略时，才Teams策略。  
 > - 只有在电话系统 SfB Skype for Business为用户帐户分配了一个Teams策略时，才支持将 Teams 与应用一起使用。 
 > - 电话系统为用户帐户分配了一个使用Teams升级策略时，不支持此策略。
 > - 系统不会迁移来自 Skype for Business 的任何呼叫转发、团队呼叫组和委派设置，并且需要重新重新创建Teams。
...
- [在本地使用 Skype for Business 的用户企业语音，](#from-skype-for-business-server-on-premises-with-enterprise-voice-to-microsoft-calling-plan)该用户将移动到联机状态，使用 Microsoft 呼叫计划。  将该用户迁移到 Teams 需要将用户的本地 Skype for Business 帐户移动到云，并协调该移动与 A) 该用户电话号码的端口移动到 Microsoft 呼叫计划或 B) 从可用区域分配新的订阅者号码。
...
1. 确保为用户分配了一个Teams许可证。 默认情况下，分配许可证或Microsoft 365 Office 365时，Teams启用，因此，除非以前禁用了 Teams 许可证，否则不需要任何操作。
...
在此方案中，用户已使用 Skype for Business Online，但其 PSTN 连接位于本地，使用混合模式Skype for Business Server云连接器版本。 使用 PSTN 功能将这些用户迁移到 TeamsOnly 模式意味着支持直接路由，其中 PSTN 中继通过本地会话边界控制器 (SBC) 直接连接到云中的直接路由服务。
...
3. 如果需要，请为这些Teams配置各种策略 (例如 TeamsMessagingPolicy、TeamsMeetingPolicy 等) 。 这随时都可以完成，但是，如果你想要确保用户在升级时具有正确的配置，最好在用户升级到 TeamsOnly 模式之前这样做。

4. 为语音迁移准备选择用户： 
   - 如有必要，请分配Teams许可证。  假设用户已在本地Skype for Business中正常运行，则用户已拥有计划 2 Skype for Business以及 Microsoft 电话 系统。 让这两个计划保持启用状态，Skype for Business Online 计划 2 许可证。  
   - 分配所需的 OnlineVoiceRoutingPolicy。 

5. 升级用户：这些步骤应进行协调。 

   - 在 Microsoft 365 或 Office 365 中，使用 Grant-CsTeamsUpgradePolicy (将用户升级到 TeamsOnly) 。
   - 在 SBC 上，配置语音路由，通过向直接路由而不是本地中介服务器发送呼叫来启用传入呼叫。
...
4. 如果需要，请为这些Teams配置各种策略 (例如 TeamsMessagingPolicy、TeamsMeetingPolicy 等) 。 这随时都可以完成，但是，如果你想要确保在用户升级时具有正确的配置，最好在用户升级到 TeamsOnly 之前这样做。

5. 如有必要，Microsoft 365或Office 365许可证。  用户应同时拥有 Teams Skype for Business Online 计划 2 以及 电话系统。 如果Skype for Business计划 2，请重新启用它。  

6. 升级用户：这些步骤应进行协调。 

   - 使用本地 Skype for Business 工具，Move-CsUser -MoveToTeams 开关运行。 如果使用不支持 -MoveToTeams 开关的 Skype for Business Server 版本，请首先运行 Move-CsUser，然后在租户远程 PowerShell 或 Teams 管理控制台中分配 TeamsOnly 模式。
...
在此方案中，用户仍位于本地Skype for Business，其 PSTN 连接也在本地。 使用 PSTN 功能将这些用户迁移到 TeamsOnly 模式意味着将用户移动到云，或者将其号码从旧运营商移植到 Microsoft 呼叫计划，或者为用户分配新号码。 
...
3. 如果需要，请为这些Teams配置各种策略 (例如 TeamsMessagingPolicy、TeamsMeetingPolicy 等) 。 这随时都可以完成，但是，如果你想要确保在用户升级时具有正确的配置，最好在用户升级到 TeamsOnly 之前这样做。 

4. 如有必要，Microsoft 365或Office 365许可证。用户应同时拥有 Teams Skype for Business Online 计划 2 以及 电话系统。 如果Skype for Business计划 2，请重新启用它。  
...
无论使用直接路由还是 Microsoft 呼叫计划，用户都必须在 Azure AD 中拥有 EnterpriseVoiceEnabled=true，用户必须具有 PSTN 功能。  EnterpriseVoiceEnabled ("已启用 EV") 是本地目录和云 (不存在) 策略) 的属性。 云中的值对于云Teams。  启用 EV 的方式设置为 true 的确切逻辑取决于以下方案： 

- 如果用户在本地 Skype for Business Server 中启用了 EV，并且使用 Move-CsUser 将用户移动到云之前，向用户分配了 电话系统 许可证，则联机用户将预配为 EV-enabled=true。 

- 如果为现有 TeamsOnly 或 Skype for Business Online 用户分配了 电话系统 许可证，则默认情况下，启用 EV 的用户未设置为 true。  如果在分配本地用户许可证之前将本地用户移到云中，电话系统这种情况。 在任一情况下，管理员必须指定以下 cmdlet： 
