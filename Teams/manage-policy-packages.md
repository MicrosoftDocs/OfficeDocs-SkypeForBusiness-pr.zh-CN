...
|小型企业用户 (Business Voice)  |创建一个应用设置策略，其中包括用于业务语音体验的应用。|
...
可以通过"策略包"页或直接进入Microsoft Teams 管理中心的策略页来编辑策略的设置。
