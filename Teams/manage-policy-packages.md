...
策略包允许你Teams组织中特定人员集允许或限制的特定功能。 每个策略包Teams用户角色设计，并且包括预定义的策略和策略设置，这些策略和策略设置支持该角色的典型协作和通信活动。
...
|使用 (远程学习工具的教育)     |创建一组适用于主要学生的策略，在远程学习中最大化学生的安全和协作。      |
...
|使用呼叫计划 (Teams 电话的中小型企业)  |创建一个应用设置策略，其中包含具有Teams 电话计划体验的应用。|
|小型企业和中型企业用户无需 (呼叫Teams 电话套餐)  |创建与具有通话计划体验的Teams用户 (和Teams 电话计划体验的用户) 。
...
- [Teams EDU 管理员管理策略包](policy-packages-edu.md)
