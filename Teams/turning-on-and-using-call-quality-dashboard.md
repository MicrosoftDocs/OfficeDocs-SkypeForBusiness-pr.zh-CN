...
这两个选项卡提供了在其终结点到终结点方案中发生的流的其他详细信息。 "服务器-客户端" 选项卡具有四个可折叠的部分, 表示媒体流将流经的四个方案。
...
1. 在 "**租户数据上载**" 页面上, 使用下拉菜单选择要上载的数据文件类型。 "文件" 数据类型表示文件的内容 (例如, "建筑物" 指的是 IP 地址和建筑物的映射以及其他地理信息, "终结点" 指的是将终结点名称映射到终结点 "生成/模型/类型 ..."。 信息)。 目前, 我们支持为 cqd.teams.microsoft.com 上载 "建筑物" 和 "终结点" 数据类型 (在预览阶段和尚未正式提供), cqd.lync.com 仅支持上载 "建筑物" 数据类型。 后续版本中将添加更多数据类型。
...
2. 选择文件数据类型后, 单击 "**浏览**" 以选择数据文件。
   ...
   - 上载到 cqd.teams.microsoft.com 的文件具有展开的行限制 1000000, 以使查询性能更快。 我们也可能对 cqd.lync.com 施加该限制。
...
6. 请注意, 如果你选择使用多个生成数据文件或多个终结点数据文件, 则某些报表的运行速度将较慢。

### <a name="tenant-data-file-format-and-structure"></a>租户数据文件格式和结构
<a name="BKMKTenantDataFile"> </a>

### <a name="building-data-file"></a>生成数据文件
CQD 使用生成数据文件的方法是首先从展开 "网络 + NetworkRange" 列中派生出子网列, 然后将子网列联接到呼叫记录的第一个子网/第二个子网列, 以显示建筑物/城市/国家/地区 .。。 信息. 你上载的数据文件的格式必须满足以下情况才能在上载之前通过验证检查。
