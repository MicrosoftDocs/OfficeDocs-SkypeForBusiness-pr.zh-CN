...
f1.keywords:
- NOCSH
description: 快速入门指南，介绍在 Microsoft Teams 中配置呼叫计划，以便让一组用户正常运行。
...
ms.locfileid: "58593146"
---
# <a name="quick-start-guide-configuring-calling-plans-in-microsoft-teams"></a>快速入门指南：在 Microsoft Teams 中配置通话套餐

本指南将帮助你设置并运行一组用户，以便他们可以在 Teams 中浏览呼叫Teams。

阅读 2017 年 12 月 12 日发布的有关在 Teams 中调用计划的公告[：Intelligent Communications](https://aka.ms/ipyqus)在 Teams

> [!NOTE]
> 我们建议，在此快速入门指南的同时，阅读电话系统计划，并FastTrack计划[](calling-plan-landing-page.md)并推动成功推出。 [](https://aka.ms/cloudvoice)

通过添加呼叫计划（Skype for Business 支持Microsoft 365和 Office 365 功能），你现在可以使用 Teams 通过公用电话交换网 (PSTN) 拨打和接听电话，或者从陆地电话和移动电话拨打和接听电话。
...
若要在Teams 中启用"呼叫"选项卡，用户需要在 Teams 中启用 1：1 呼叫，并且使用支持 1：1 Teams的 Teams 客户端。 若要了解如何在 Teams 中管理 1：1 Teams，请阅读[Set-CsTeamsCallingPolicy。](/powershell/module/skype/set-csteamscallingpolicy) 若要了解哪些客户端支持呼叫，请阅读 Microsoft Teams[的限制和Microsoft Teams。](./limits-specifications-teams.md)

> [!NOTE]
> 目前，语音邮件在"呼叫"选项卡中不可用，除非为用户启用了 PSTN 呼叫。 

## <a name="prerequisites-for-enabling-the-dial-pad-in-teams"></a>在键盘中启用 **拨号盘** Teams
若要在"**拨号盘**"选项卡中启用Teams并允许用户拨打和接听 PSTN 呼叫，需要为用户预配电话系统和呼叫计划。 若要了解如何设置呼叫计划，请阅读 [设置呼叫计划](./set-up-calling-plans.md)。
...
为了控制传入呼叫 (和聊天) 是否进入 Teams 或 Skype for Business，管理员使用 TeamsUpgradePolicy，使用[Microsoft Teams](https://aka.ms/teamsadmincenter)管理中心或与 Skype for Business cmdlet 进行远程[Windows PowerShell](/powershell/module/skype)会话。


TeamsUpgradePolicy 的默认配置是"群岛模式"，其设计目的是确保现有业务工作流在部署期间不会Teams中断。 默认情况下，VoIP、PSTN 和向用户的联合呼叫将继续路由到 Skype for Business，直到更新策略以启用到 Teams 的入站呼叫。  收件人在岛屿模式下时：

 - 源自客户端的传入 VOIP Skype for Business始终位于接收者的客户端Skype for Business。
 - 如果发送方和接收方位于同一租户Teams，源自 Teams Teams 的传入 VOIP 呼叫将登陆 *到该租户*。
 - 传入的联合 VOIP (无论哪个客户端) PSTN 呼叫始终位于接收者的客户端Skype for Business。
...
**说明**
 ...
 - 已预配 电话系统 和呼叫计划许可证以与 Skype for Business Online (一同使用的用户（例如，已为其分配值为 OnlineVoiceRoutingPolicy) ）的用户将在 Teams 中启用"呼叫"选项卡，并且可以从 Teams 拨打出站 PSTN 呼叫，而无需管理员执行任何管理操作。
