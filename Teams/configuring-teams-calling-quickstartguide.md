...
通过添加呼叫计划- Microsoft 365 和 Office 365 功能，Skype for Business 现在可以使用 Teams 通过公用电话交换网 (PSTN) 拨打和接听电话呼叫，或者从陆地电话和移动电话拨打和接听电话。
...
若要在Teams 中启用"呼叫"选项卡，用户需要在 Teams 中启用 1：1 调用，并且使用支持 1：1 Teams的 Teams 客户端。 若要了解如何在 Teams 中管理 1：1 Teams，请阅读[Set-CsTeamsCallingPolicy。](/powershell/module/skype/set-csteamscallingpolicy) 若要了解哪些客户端支持调用，请阅读 Microsoft Teams[的限制和Microsoft Teams。](./limits-specifications-teams.md)
...
此外，对于Teams用户，必须确保在呼叫策略中启用了"允许Teams呼叫"。 有关详细信息[，Teams](./manage-teams-skypeforbusiness-admin-center.md)新管理中心Microsoft Teams管理帐户。
> [!NOTE]
...
TeamsUpgradePolicy 的默认配置是"群岛"模式，该模式可确保现有业务工作流在部署期间不会Teams中断。 默认情况下，VoIP、PSTN 和用户的联合呼叫将继续路由到 Skype for Business，直到更新策略以启用入站呼叫到 Teams。  收件人在岛屿模式下时：

 - 源自客户端的传入 VOIP Skype for Business始终位于接收者的 Skype for Business客户端。
 - 如果发送方和接收方位于同一Teams，则源自 Teams 的传入 VOIP 呼叫将登陆 *到该租户*。
 - 传入的联合 VOIP (无论哪个客户端) PSTN 呼叫始终位于接收者的客户端Skype for Business客户端。
 
为了确保传入的 VOIP 和 PSTN 呼叫始终进入用户的 Teams 客户端，将用户的共存模式更新为 TeamsOnly (这意味着，请为其分配 TeamsUpgradePolicy 的"UpgradeToTeams"实例。  有关共存模式和 TeamsUpgradePolicy 详细信息，请参阅迁移和互操作性指南，了解将 Teams[与](./migration-interop-guidance-for-teams-with-skype.md)Skype for Business
