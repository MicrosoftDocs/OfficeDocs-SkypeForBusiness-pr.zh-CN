...
> [!NOTE]
> 我们建议，在此快速入门指南的并行下，阅读电话系统计划，并[](calling-plan-landing-page.md)FastTrack计划并推动成功推出。 [](https://aka.ms/cloudvoice)

通过添加呼叫计划（ Skype for Business 支持Microsoft 365和 Office 365 功能），你现在可以使用 Teams 通过公共电话交换网 (PSTN) 拨打和接听电话，或者从陆地电话和移动电话拨打和接听电话。
...
若要在Teams 中启用"呼叫"选项卡，用户需要在 Teams 中启用 1：1 呼叫，并且使用支持 1：1 Teams的 Teams 客户端。 若要了解如何在 Teams 中管理 1：1 Teams，请阅读[Set-CsTeamsCallingPolicy。](/powershell/module/skype/set-csteamscallingpolicy) 若要了解哪些客户端支持调用，请阅读客户端[的限制和Microsoft Teams。](./limits-specifications-teams.md)

> [!NOTE]
> 目前，语音邮件在"呼叫"选项卡中不可用，除非为用户启用了 PSTN 呼叫。 

## <a name="prerequisites-for-enabling-the-dial-pad-in-teams"></a>在键盘中启用 **拨号盘** Teams
若要在"**拨号盘**"选项卡Teams并允许用户拨打和接听 PSTN 呼叫，需要为用户预配电话系统呼叫计划"。 若要了解如何设置呼叫计划，请阅读 [设置呼叫计划](./set-up-calling-plans.md)。
此外，对于Teams用户，必须确保在呼叫策略中启用了"允许Teams呼叫"。 有关详细信息[Teams，](./manage-teams-skypeforbusiness-admin-center.md)请参阅在Microsoft Teams管理中心期间管理管理员。
> [!NOTE]
...
TeamsUpgradePolicy 的默认配置是"群岛模式"，其设计目的是确保现有业务工作流在部署期间不会Teams中断。 默认情况下，VoIP、PSTN 和向用户的联合呼叫将继续路由到 Skype for Business，直到更新策略以启用对 Teams 的入站呼叫。  收件人在岛屿模式下时：
 ...
 - 如果发送方和接收方位于同一租户中Teams源自 Teams 的传入 VOIP 呼叫将登 *陆到该租户*。
...
**说明**
 ...
 - 已预配 电话系统 和呼叫计划许可证以用于 Skype for Business Online (（例如，已为其分配值为 OnlineVoiceRoutingPolicy) ）的用户将在 Teams 中启用"呼叫"选项卡，并且可以从 Teams 进行出站 PSTN 呼叫，而无需管理员执行任何管理操作。
