...
通过添加呼叫计划-Microsoft 365 和 Office 365 功能（由 Skype for Business 支持）-现在，您可以使用 Teams 通过公用电话交换网 (PSTN) 拨打和接听电话呼叫，或者从陆地电话和移动电话接听电话呼叫。
...
此外，对于Teams用户，必须确保在呼叫策略中启用了"允许Teams呼叫"。 有关详细信息[Teams，请参阅在Microsoft Teams管理中心期间](./manage-teams-skypeforbusiness-admin-center.md)管理帐户。
> [!NOTE]
...
为了控制传入的 (和聊天) 是否进入 Teams 或 Skype for Business，管理员使用 TeamsUpgradePolicy，使用[Microsoft Teams](https://aka.ms/teamsadmincenter)管理中心或通过 Skype for Business cmdlet 使用远程[Windows PowerShell](/powershell/module/skype)会话。


TeamsUpgradePolicy 的默认配置是"群岛模式"，它旨在确保现有业务工作流在部署期间不会Teams中断。 默认情况下，VoIP、PSTN 和用户的联合呼叫将继续路由到 Skype for Business，直到更新策略以启用入站呼叫到 Teams。  收件人在岛屿模式下时：

 - 源自客户端的传入 VOIP Skype for Business始终位于接收者的客户端Skype for Business。
 - 如果发送方和接收方位于同一租户中Teams源自 Teams 的传入 VOIP 调用将登 *陆到该租户*。
 - 传入的联合 VOIP (无论哪个客户端) PSTN 呼叫始终位于接收者的客户端Skype for Business中。
