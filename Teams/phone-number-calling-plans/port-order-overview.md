...
- Calling Plans
description: 大致了解什么是端口订单，以及如何将电话号码从服务提供商传输到Teams。
...
如果你目前有电话服务提供商或运营商，并且已经为用户或服务提供电话号码，则需要创建一个“*端口订单*”，将这些电话号码传输到Microsoft Teams。 移植号码后，可以将这些电话号码分配给用户和服务，例如会议桥) 、自动助理和呼叫队列的音频会议 (。
  
将电话号码移植到Teams后，Microsoft 将成为你的服务提供商，你可以断开服务与旧服务提供商或运营商的连接。

查看本文中的信息以熟悉数字移植。 之后，应准备好创建端口订单并传输电话号码。 有关分步说明，请参阅[将电话号码转到Teams](transfer-phone-numbers-to-teams.md)。
...
目前，Microsoft Teams管理中心的[移植向导](transfer-phone-numbers-to-teams.md)支持获取英国、美国和加拿大的电话号码。 若要获取其他国家和地区的电话号码，可以 [手动提交端口订单](manually-submit-port-order.md)。
...
- Skype for Business混合电话号码。 如果你想转移这些数字，请通过电子邮件向我们 <ptn@microsoft.com>发送电子邮件。
...
- 专用于收发传真的电话号码。
    ...
    如果你有用于传真的现有专用电话号码，*则可以将* 这些号码转移到Teams但传真服务不会继续按预期工作。 即使你拥有电话系统、国内呼叫计划或国际通话套餐的许可证，Teams客户也无法使用传真服务。

    如果将电话号码移植到Teams，则可以将此电话号码分配给组织中的用户，而不是将其用于传真。
...
将电话号码移植到Teams时，可以选择转移所有号码或部分电话号码。
  
- **全端口** 此时会将所有号码从当前服务提供商转移到Teams。 当系统询问要转接的电话号码时， *必须包括* (BTN) 的计费电话号码以及帐户上所有其他电话号码。

    例如，假设 BTN  *是 +1 425-555-1234*  ，你想要移植所有 25 个电话号码 (*+1 425-555-1235 到 1259*) 。 按照下面的说明转网号码时，你应该输入： **+14255551234 - +14255551259** 。

- **部分端口** 此时，你只需将一些电话号码从当前服务提供商转移到Teams。 如果要移植绑定到同一 BTN 的一些电话号码，则不得 *包含* BTN 以及帐户上所有其他电话号码。
...
- 可用于Microsoft 365或Office 365中音频会议的服务号。

下面是有关如何为每种类型的数字提交数字移植请求的详细信息：
  
- **电话** 不同承运商提供的数字需要对每个承运商的数字发出唯一的移植请求。

- 包含区域代码（例如：800、844、855、866、877 和 888）的 **免费号码** 不能包含在包含其他类型的号码的号码移植请求中。 若要移植这些免费号码，必须 [手动提交端口订单](manually-submit-port-order.md)。 无法将这些数字移植到Microsoft Teams管理中心。 有关详细信息，请参阅[为你的组织管理电话号码](../manage-phone-numbers-for-your-organization/manage-phone-numbers-for-your-organization.md)。
...
若要检查端口订单的状态，请在Microsoft Teams管理中心的左侧导航中，转到 **“语音** > **电话数字**，然后单击 **”订单历史记录**”。 状态 **列中** 列出了每个端口订单状态。
...
## <a name="can-i-port-out-my-numbers-from-teams-to-a-different-phone-service-provider-or-carrier"></a>是否可以将号码从Teams移植到其他电话服务提供商或运营商？

若要将号码从Teams移植到其他运营商，必须向新运营商提交请求。 还需要在Microsoft Teams管理中心设置移植 PIN。

若要定义移植 PIN，请在Microsoft Teams管理中心的左侧导航中，转到 **语音** > **电话数字**，在页面右上角选择 **“管理移植 PIN**”，然后输入一个 10 位数的 PIN。
