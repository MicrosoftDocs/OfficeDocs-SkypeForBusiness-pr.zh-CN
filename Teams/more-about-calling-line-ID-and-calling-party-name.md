...
CallerID （通常称为），实际上包含两个面向用户的可识别信息片段：
    ...
    - 调用方名称 (通常称为 CNAM) ，最多可长达15个字符。 

进行通话时，CLID (电话号码) 将路由到目的地的运营商 (也称为终止载波) 。 通话的 CNAM 信息可能与呼叫一起发送，也可能不会与呼叫一起发送，因为这取决于国家是如何实现的 CNAM (（如果所有) 都是如此）。 与通话的 CNAM 传递的可靠性因作为中间和/或终止运营商处理呼叫的国家和运营商而异。 
...
CNAM 中的不一致可能由中间或终止运营商刷新权威数据库中的 CNAM 信息引起的，如美国的情况。 在没有 CNAM 权威数据库的国家/地区，单个运营商的做法还可能导致与通话的 CNAM 信息不完整的问题。 Microsoft 目前不支持除美国之外的其他国家/地区的原始 CNAM 信息。 "
