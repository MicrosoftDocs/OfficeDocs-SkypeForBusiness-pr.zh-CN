...
description: 了解用户生成的调试、媒体Microsoft Teams桌面日志、日志的查找位置，以及它们如何帮助进行监视和故障排除。
appliesto:
- Microsoft Teams
...
|日志文件名  |说明  |
|---------|---------|
|Teams.msrtc-0-s1039525249.blog     | 包含与媒体堆栈有关的信息。 这包括通道状态，例如分辨率、使用的解码器、编码器、已发送和接收的帧数，以及基于相机和视频的 (VBSS) 会话状态。         |
