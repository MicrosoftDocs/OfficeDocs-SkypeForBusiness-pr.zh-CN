...
>[!Note]
>管理紧急呼叫方式存在一些差异，具体取决于是使用 Microsoft 呼叫计划、接线员连接还是直接路由进行[PSTN 连接](pstn-connectivity.md)。 本文中介绍了这些注意事项。

## <a name="emergency-address-validation"></a>紧急地址验证

若要向用户或网络标识符分配紧急地址，必须确保紧急地址标记为"已验证"。 地址验证可确保地址是合法的，并且分配后无法对其进行修改。 

如果使用地址映射搜索功能在 Teams 中定义紧急地址，该地址会自动标记为已验证。 由于如果地址的格式或表示形式发生更改，则不能修改已验证的紧急地址，因此必须使用更新的格式创建新 &mdash; 地址。
...
以下部分介绍如何管理接线员呼叫连接呼叫。 若要了解接线员连接是适合你的企业的解决方案，请参阅[PSTN 连接选项](pstn-connectivity.md)。

### <a name="emergency-call-enablement-for-operator-connect"></a>为接线员启用紧急呼叫连接

每个接线员连接自动启用紧急呼叫。 紧急呼叫将自动路由到运营商连接运营商为给定号码。

租户管理员为运营商用户设置注册地址连接取决于运营商将其上传到客户库存时分配给号码的功能。 根据此设置，租户管理员可能（也可能不需要）设置、修改 &mdash; &mdash; 或删除用户的紧急位置。 
...
对于直接路由，必须使用 Teams 紧急呼叫路由策略 (TeamsEmergencyCallRoutingPolicy) 来定义紧急号码及其关联的路由目标，为用户定义紧急呼叫策略。  (当前，直接路由用户不支持已注册的紧急位置。) 
...
直接路由的动态紧急呼叫提供基于客户端当前位置配置和路由紧急Teams的功能。 执行自动路由到相应的公共安全应答点 (PSAP) 或通知安全服务台人员的能力因用户使用国家/地区Teams而异。
...
> [!NOTE]
> Teams客户端不再在紧急号码前面加"+"号;即 +911。 因此，Teams紧急呼叫将不再发送 911 号码前面的"+"。 请确保语音路由模式反映此更改。
...
在美国，有许多经过认证的紧急路由服务提供商 (ERSP) ，可基于呼叫者的位置自动路由紧急呼叫。
...
会话边界控制器 (SBC) ELIN 应用程序中包括紧急 (标识) 号。 如果 SBC ELIN 应用程序集成到直接路由部署中，则必须在 ELIN 应用程序中配置紧急地址和关联的电话号码，然后将 ELIN 记录上传到相应 PSTN 中的紧急呼叫数据库。 Teams ELIN 标识符的紧急位置必须与 ELIN 应用程序中的位置匹配。
