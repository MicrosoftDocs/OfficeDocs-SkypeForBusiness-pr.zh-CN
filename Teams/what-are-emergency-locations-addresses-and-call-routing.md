...
ms.locfileid: "62881517"
---
# <a name="manage-emergency-calling"></a>管理紧急呼叫

本文介绍管理紧急呼叫&mdash;时需要知道的概念包括有关紧急地址、动态紧急地址和紧急呼叫路由的信息。 本文使用以下术语：

- **紧急地址** - 街道&mdash;地址：贵组织的营业地点的物理地址或街道地址。
...
- **紧急位置** - 位置是具有可选&mdash;位置的市政地址。 如果你的企业具有多个物理位置，则你可能需要多个紧急位置。 

  创建紧急地址时，会自动为此地址创建唯一的位置 ID。 例如，如果向&mdash;&mdash;紧急地址添加位置，则向建筑物地址添加楼层时，会为紧急地址和地点的组合创建位置 ID。  本示例将存在两个位置 ID：一个针对市/区地址;一个地址用于已加入的市政地址和关联位置。
...
>[!Note]
>根据 PSTN 连接使用的是 Microsoft 呼叫计划、接线员连接或直接路由，紧急呼叫管理方式存在一[些差异](pstn-connectivity.md)。 本文中介绍了这些注意事项。

## <a name="emergency-address-validation"></a>紧急地址验证

若要向用户或网络标识符分配紧急地址，必须确保紧急地址标记为"已验证"。 地址验证可确保地址是合法的，并且分配后无法对其进行修改。 

如果使用地址管理中心中的地址映射搜索功能Teams紧急地址，该地址会自动标记为已验证。 由于如果地址的格式&mdash;或表示形式发生更改，则不能修改已验证的紧急地址，因此必须使用更新的格式创建新地址。
...
- &mdash;对于欧洲、中东和非洲 (EMEA) 等其他国家/地区) &mdash;当您从 Microsoft 365 获取电话号码或者从另一服务提供商或运营商转移电话号码时，需要提供紧急位置。
...
- 如果Teams位于租户定义的动态紧急位置，则来自该客户端的紧急呼叫会自动路由到提供该地理位置的 PSAP。
...
**加拿大、爱尔兰** 和英国首先会筛选紧急呼叫，以确定用户的当前位置，然后再将呼叫连接到相应的调度中心。

**法国、德国** 以及西班牙的紧急呼叫将直接路由到 PSAP，无论呼叫方的位置如何，都提供与号码关联的紧急地址。
...
## <a name="considerations-for-operator-connect"></a>运算符的注意事项连接

以下部分介绍如何为接线员和连接呼叫。 若要了解接线员连接是适合你的企业的解决方案，请参阅 [PSTN 连接选项](pstn-connectivity.md)。

### <a name="emergency-call-enablement-for-operator-connect"></a>为接线员启用紧急呼叫连接

每个接线员连接自动启用紧急呼叫。 紧急呼叫将自动路由到运营商连接运营商为给定号码。

租户管理员为运营商用户设置注册地址连接取决于运营商将其上传到客户库存时分配给号码的功能。 根据此设置，租户&mdash;&mdash;管理员可以设置、修改或删除用户的紧急位置，也可能不需要。 

### <a name="dynamic-emergency-calling-for-operator-connect"></a>接线员呼叫动态紧急连接
...
- 如果Teams用户的用户客户端未在美国动态获取紧急地址，则注册的紧急地址用于帮助屏幕和路由呼叫。 但是，在将呼叫方连接到相应的 PSAP 之前，将筛选呼叫以确定是否要求更新的地址。

加拿大支持将紧急呼叫路由到的动态位置，但美国除外：所有紧急呼叫都将在传输到 PSAP 之前进行国内筛选。

有关详细信息，请参阅 [计划和配置动态紧急呼叫](configure-dynamic-emergency-calling.md)。

### <a name="emergency-call-routing-for-operator-connect"></a>接线员呼叫紧急连接

当Teams接线员连接拨打紧急号码时，呼叫路由到 PSAP 的方式取决于以下条件：
...
- 如果Teams客户端位于租户定义的动态紧急位置：
   ...
   - 在美国，来自该客户端的紧急呼叫会自动路由到提供该地理位置的 PSAP。
   - 加拿大的所有紧急呼叫都将通过国家呼叫中心进行筛选，然后再将呼叫转接到为该地理位置服务的 PSAP。
...
对于直接路由，必须使用紧急呼叫路由策略来定义紧急呼叫Teams紧急呼叫[](manage-emergency-call-routing-policies.md)策略来定义紧急号码及其关联的路由目标。  (当前，直接路由用户不支持已注册的紧急位置。) 
...
直接路由的动态紧急呼叫提供根据客户端的当前位置配置和路由紧急Teams的功能。 自动路由到相应的公共安全应答点 (PSAP) 或通知安全服务台人员的能力因使用安全Teams国家/地区而异。
...
直接路由的紧急呼叫路由策略引用在线 PSTN 使用情况，该策略必须具有适当的直接路由配置，以正确将紧急呼叫路由到 (PSTN) 。 具体而言，必须确保紧急拨号字符串有 OnlineVoiceRoute。 有关详细信息，请参阅 [配置直接路由](direct-routing-configure.md)。 
...
在美国，有许多经过认证的紧急路由服务提供商 (ERSP) ，这些提供商可以基于呼叫者的位置自动路由紧急呼叫。

- 如果紧急路由服务提供商集成到直接路由部署中，具有动态获取位置的紧急呼叫将自动路由到提供该位置的 (PSAP) 安全应答点。

-  没有动态获取位置的紧急呼叫首先经过筛选，以确定用户的当前位置，然后根据更新的位置将呼叫连接到相应的调度中心。

有关详细信息，请参阅通过 [直接路由认证的会话边界控制器](direct-routing-border-controllers.md)。


#### <a name="emergency-location-identification-number-applications"></a>紧急位置标识号应用程序

会话边界控制器 (SDC) ELIN 应用程序中包括紧急 (标识) 号。 如果 SBC ELIN 应用程序集成到直接路由部署中，则必须在 ELIN 应用程序中配置紧急地址和关联的电话号码，然后将 ELIN 记录上传到相应 PSTN 中的紧急呼叫数据库。 Teams ELIN 标识符的紧急位置必须与 ELIN 应用程序中的位置匹配。
...
使用 Teams 紧急呼叫策略 (TeamsEmergencyCallingPolicy) 来配置在紧急呼叫期间应通知哪些人以及如何通知他们：仅聊天、参加会议和静音，或者已进入和静音但能够取消静音。 您还可以指定要呼叫和加入紧急呼叫的用户或组的外部 PSTN 号码。 请注意，不允许 PSTN 群取消静音。
