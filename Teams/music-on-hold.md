...
> [!NOTE]
> 你负责独立清除和保护使用任何音乐或音频文件的一切必要权限Microsoft Teams服务。 这可能包括来自所有相关权限持有者的音频文件中任何音乐、音效、音频、品牌、名称和其他内容的知识产权和其他权利。 持有者可能包括艺术家、执行者、表演者、艺术家、歌曲作者、作曲者、录制标签、音乐发布者、联盟、协会、权利保护者、集体管理组织，以及拥有、控制或许可音乐版权、声音效果、音频和其他知识产权的其他任何方。
...
1. 确保在"Teams呼叫策略音乐 PSTN 呼叫者已设置为"已启用Teams保留。 
...
- 音乐当用户进行咨询转移时，Teams保留不可用。

- 如果为Teams用户启用了Location-Based，音乐无法向调用方播放"保持"。

-   音乐只有当调用的 Teams用户使用以下版本的客户端之一时，"保持"Teams可用：
...
- 自定义音乐保留"不适用于为共享线路外观配置的用户 (委派) 以及何时使用呼叫保留。 将音乐保持状态。
