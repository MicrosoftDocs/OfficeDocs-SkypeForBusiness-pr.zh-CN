...
请注意，在其他情况下，调用方也可以侦听“保留”音乐;例如，当呼叫进入云呼叫队列或呼叫由Microsoft Teams用户停靠时。 本文中提到的功能不涵盖或控制这些方案。
...
如果Teams用户有一个Teams调用策略，音乐 on Hold 设置为“已禁用”，则当Teams用户暂停呼叫时，将不播放任何音乐。
...
> [!NOTE]
> 你负责独立清除和保护所有必要的权限和权限，以便将任何音乐或音频文件用于Microsoft Teams服务。 这可能包括所有相关权利人对音频文件中任何音乐、声音效果、音频、品牌、名称和其他内容的知识产权和其他权利。 持有者可以包括艺术家、演员、表演者、音乐家、词曲作者、作曲家、唱片公司、音乐出版商、工会、公会、权利协会、集体管理组织以及拥有、控制或许可音乐版权、音效、音频和其他知识产权的任何其他各方。
...
3. 创建引用自定义音频文件的Teams调用保留策略，并将其分配给Teams用户。
...
### <a name="reference-the-audio-file-in-a-teams-call-hold-policy"></a>在Teams呼叫保留策略中引用音频文件

上传音频文件后，在创建或设置Teams呼叫保留策略时，需要使用该文件的 ID 引用Teams调用保留策略中的文件。 例如：

```PowerShell
C:\> New-CsTeamsCallHoldPolicy -Identity "CustomMoH1" -Description "Custom MoH using CustomMoH1.mp3" -AudioFileId $AudioFile.Id
```

创建新的Teams调用保留策略后，可以使用Grant-CsTeamsCallHoldPolicy将其授予用户，如下所示：
...
| 按住 1：1 Teams呼叫 | -音乐保留<br>-保留时自定义音乐 | -音乐保留<br>-保留时自定义音乐 | -音乐保留<br>-保留时自定义音乐 | -音乐保留<br>-保留时自定义音乐 | -音乐保留<br>-保留时自定义音乐 |
| 在 1：1 PSTN 呼叫时保留协商传输 |-音乐保留<br>-保留时自定义音乐 || -音乐保留<br>-保留时自定义音乐 | -音乐保留<br>-保留时自定义音乐 | -音乐保留<br>-保留时自定义音乐 |
| 在 1：1 Teams呼叫时保留协商转移 |-音乐保留<br>-保留时自定义音乐 || -音乐保留<br>-保留时自定义音乐 | -音乐保留<br>-保留时自定义音乐 | -音乐保留<br>-保留时自定义音乐 |
...
- 如果为调用的Teams用户启用了Location-Based路由，则无法对调用方播放“保留”音乐。

- 对于配置共享行外观 (委派) 以及何时使用呼叫寄存的用户，暂留自定义音乐不可用。 将播放“保留”标准音乐。

- 在某些情况下，直接路由媒体旁路呼叫将转换为非媒体旁路，以便在“保留”上播放音乐，呼叫将保留为非媒体旁路，直到呼叫终止。
