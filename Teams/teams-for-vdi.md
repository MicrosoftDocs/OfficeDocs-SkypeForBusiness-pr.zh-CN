...
在Teams环境中使用资源可能稍有不同，Teams非虚拟化环境中使用虚拟环境。 例如，某些高级功能在虚拟化环境中可能不可用，并且视频分辨率可能有所不同。
...
## <a name="teams-on-vdi-components"></a>Teams VDI 组件上
...
目前，Teams Azure 虚拟桌面、Citrix 和 VMware (AV) VDI 上的音频/视频优化。 查看本部分中的信息，确保满足正确功能的所有要求。
...
VMware Horizon 是一个新式平台，用于跨混合云安全交付虚拟桌面和应用。 为了提供出色的最终用户体验，VMware Horizon 为用户提供媒体Teams。 此优化可跨虚拟桌面和应用提高整体工作效率，并增强使用虚拟桌面进行呼叫和会议时Teams。

可以从 VMware 下载页下载最新版本的 [VMware](https://customerconnect.vmware.com/downloads/#all_products) Horizon。 默认情况下，所需的媒体优化组件是 Horizon Agent 和 Horizon 客户端的一部分，无需安装任何其他插件来使用优化功能Teams。
...
使用每台计算机安装时，将禁用自动更新。 这意味着，若要更新 Teams，必须卸载当前版本以更新到较新版本。 通过按用户安装，将启用自动更新。 对于大多数 VDI 部署，我们建议使用Teams安装来部署 VDI。
...
在Teams设置中使用配置文件缓存管理器还需要配置文件缓存管理器，Teams运行时数据同步。 高效的数据同步可确保在用户会话 (缓存用户的数据、配置文件或) 等用户特定信息。 确保同步这两个文件夹中的数据：<br>
...
Microsoft 365 企业应用版不支持每台计算机安装 Teams。 若要使用每台计算机安装，必须从Teams中排除Microsoft 365 企业应用版。 请参阅[将 Teams 桌面应用部署到 VM](#deploy-the-teams-desktop-app-to-the-vm)和如何通过 Teams[排除Microsoft 365 企业应用版](#how-to-exclude-teams-deployment-through-microsoft-365-apps-for-enterprise)部分。

#### <a name="how-to-exclude-teams-deployment-through-microsoft-365-apps-for-enterprise"></a>如何通过Teams排除Microsoft 365 企业应用版

若要详细了解Teams Microsoft 365 企业应用版，请参阅如何从 Teams 的新安装中排除[Microsoft 365 企业应用版和使用组](/DeployOffice/teams-install#how-to-exclude-microsoft-teams-from-new-installations-of-microsoft-365-apps)策略来控制[Teams。](/DeployOffice/teams-install#use-group-policy-to-control-the-installation-of-microsoft-teams)
...
2. 运行以下命令之一，将 MSI 安装到 VDI VM：
    ...
    - 按用户安装 (默认) 
        ...
        此过程是默认安装，用于Teams %AppData% 用户文件夹。 此时，黄金映像设置已完成。 Teams安装时，无法正常使用基于用户的安装。
    ...
    - 每台计算机安装
        ...
        此过程Teams 64 位操作系统上的"程序文件" (x86) 文件夹和 32 位操作系统上的"程序文件"文件夹。 此时，黄金映像设置已完成。 非Teams需要针对每台计算机安装一个服务。

        下一个交互式登录会话Teams请求凭据。

        > [!NOTE]
        > 这些示例还使用 **ALLUSERS=1** 参数。 设置此参数时，Teams Machine-Wide"安装程序"会显示在"控制面板"中的"程序和功能"中，&应用的"Windows 设置"中，供计算机的所有用户使用。 然后，所有用户都可以卸载Teams管理员凭据的用户。
...
3. 从 VDI VM 卸载 MSI。 有两种方法可以卸载Teams。
    ...
    - PowerShell 脚本：可以使用此[PowerShell](scripts/powershell-script-deployment-cleanup.md)脚本卸载Teams并删除Teams的 Teams 文件夹。 针对计算机上安装了 Teams的每个用户配置文件运行脚本。
    - 命令行：运行以下命令。
      ...
      此过程从"Teams文件" (x86) "文件夹或"程序文件"文件夹中卸载文件，具体取决于操作系统环境。
...
![显示 VDI Teams的示意图。](media/teams-on-vdi-architecture.png)

> [!IMPORTANT]
> 如果当前运行 Teams 而不在 VDI 中执行 AV 优化，并且使用尚不支持用于优化的功能 (如在应用共享) 时授予并控制，必须设置虚拟化提供程序策略以关闭 Teams 重定向。 这意味着Teams不会优化媒体会话。 若要了解如何设置策略以关闭重定向Teams，请与虚拟化提供商联系。
...
### <a name="migrate-from-skype-for-business-on-vdi-to-teams-on-vdi"></a>从 VDI Skype for Business迁移到 VDI Teams VDI 上
...
Teams浏览器上的应用无法取代 VDI Teams AV 优化的桌面应用。 聊天和协作体验如期工作。 当需要媒体时，某些体验可能无法满足用户在 Chrome 浏览器上的期望：
...
如果你的组织只想在 Teams 使用聊天和协作功能，你可以设置用户级策略以关闭呼叫和会议Teams。
...
[**会议策略**](meeting-policies-overview.md)：Teams包括内置的 AllOff 会议策略，其中所有会议功能都已关闭。 将 AllOff 策略分配给组织中在虚拟化环境中使用Teams的所有用户。
...
如果 VDI 上的 Teams 与聊天和协作的现有实现已设置用户级策略以关闭呼叫和会议功能，并且要迁移到具有 AV 优化的 Teams，则必须设置策略，为 VDI 用户启用 Teams 的呼叫和会议功能。
...
[**会议策略**](meeting-policies-overview.md)：Teams中的会议策略控制用户可以创建的会议类型和可供组织中用户安排的会议参与者使用的功能。 Teams包括内置的 AllOn 会议策略，其中所有会议功能都打开。 若要启用所有会议功能，请分配 AllOn 策略。 或者，创建自定义会议策略以打开你需要的会议功能并为其分配用户。
...
当用户使用 VDI 策略设置 -DisableAudioVideoInCallsAndMeetings $true登录到 Teams on VDI 时，他们应该能够：
...
- 在 Citrix 环境中，如果用户在运行 Teams 时与虚拟机断开连接，Teams 更新可能会导致用户在重新连接时为 AV 保持未优化状态。 建议用户在从 Citrix Teams之前退出配置，以避免这种情况。
