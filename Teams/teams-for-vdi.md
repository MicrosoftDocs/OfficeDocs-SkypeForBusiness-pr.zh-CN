...
已Teams领先的虚拟化解决方案提供商验证了该桌面应用。 对于多个市场提供商，建议咨询虚拟化解决方案提供商，确保满足最低要求。
  
目前，Teams Azure 虚拟桌面、Citrix 和 VMware (AV) 优化的 VDI 上的应用。 查看本部分中的信息，确保满足正确功能的所有要求。
...
可以在 Citrix 下载站点 下载最新版本的 Citrix 虚拟应用 [和桌面](https://www.citrix.com/downloads/citrix-virtual-apps-and-desktops/)。  (需要先登录。) 所需的组件默认捆绑到 Citrix Workspace 应用 [ (CWA) ](https://www.citrix.com/downloads/workspace-app/) 和虚拟交付代理 (VDA) 中。 无需在 CWA 或 VDA 上安装任何其他组件或插件。
...
VMware Horizon 是一个新式平台，用于跨混合云安全交付虚拟桌面和应用。 为了提供出色的最终用户体验，VMware Horizon 为 Teams。 此优化可跨虚拟桌面和应用提高整体工作效率，并增强使用 Teams 进行呼叫和会议时用户体验。

可以从 VMware 下载页下载最新版本的 [VMware](https://my.vmware.com/web/vmware/downloads/#all_products) Horizon。 默认情况下，所需的媒体优化组件是 Horizon Agent 和 Horizon 客户端的一部分，无需安装任何其他插件来使用优化功能Teams。

若要获取最新的要求以及如何为用户配置媒体优化Teams，请参阅[此 VMware 网站](https://docs.vmware.com/en/VMware-Horizon/2006/horizon-remote-desktop-features/GUID-F68FA7BB-B08F-4EFF-9BB1-1F9FC71F8214.html)。

## <a name="install-or-update-the-teams-desktop-app-on-vdi"></a>在 VDI Teams或更新桌面应用

可以使用 MSI Teams按计算机安装或按用户安装来部署适用于 VDI 的桌面应用。 决定使用哪种方法取决于是使用持久性设置还是非持久性设置，以及组织的关联功能需求。
...
|参数  |工作站操作系统  |服务器操作系统  |
|---------|---------|---------|
|vCPU   |    2 个核心     |  4、6 或 8<br>必须了解 NUMA (配置) 不一致的内存访问，并相应地配置 VM。     |
...
在Teams设置中使用缓存管理器还需要配置文件缓存管理器，Teams同步。 高效的数据同步可确保在用户的会话 (缓存用户的数据、配置文件) 设置等用户特定信息。 确保同步这两个文件夹中的数据：<br>
...
Teams每个用户Microsoft 365 企业应用版安装一次。 有关详细信息，请参阅在[VDI Teams或更新桌面](#install-or-update-the-teams-desktop-app-on-vdi)应用部分。

#### <a name="teams-deployments-through-microsoft-365-apps-for-enterprise-updates"></a>Teams更新Microsoft 365 企业应用版部署

Teams添加到现有安装的 Microsoft 365 企业应用版。 由于Microsoft 365 企业应用版仅Teams用户安装，请参阅在[VDI Teams桌面应用](#install-or-update-the-teams-desktop-app-on-vdi)部分。
...
Microsoft 365 企业应用版不支持每台计算机安装 Teams。 若要使用每台计算机安装，必须排除Teams计算机Microsoft 365 企业应用版。 请参阅[将 Teams 桌面应用部署到 VM](#deploy-the-teams-desktop-app-to-the-vm)和如何Teams[部署Microsoft 365 企业应用版](#how-to-exclude-teams-deployment-through-microsoft-365-apps-for-enterprise)部分。

#### <a name="how-to-exclude-teams-deployment-through-microsoft-365-apps-for-enterprise"></a>如何通过Teams排除Microsoft 365 企业应用版

若要详细了解Teams Microsoft 365 企业应用版，请参阅如何从 Teams 的新安装中排除[Microsoft 365 企业应用版和使用组](/DeployOffice/teams-install#how-to-exclude-microsoft-teams-from-new-installations-of-office-365-proplus)策略来控制[Teams。](/DeployOffice/teams-install#use-group-policy-to-control-the-installation-of-microsoft-teams)
...
1. 使用Teams之一下载与 VDI VM 操作系统匹配的 MSI 包：
    ...
    > [!NOTE]
    > 对于政府云，请参阅Microsoft Teams[安装](msi-deployment.md)Microsoft Endpoint Configuration Manager，了解 MSI 文件的下载链接。
...
2. 运行以下命令之一，将 MSI 安装到 VDI VM：
    ...
    - 按用户安装 (默认) 
        ...
        此过程是默认安装，用于Teams %AppData% 用户文件夹。 此时，黄金映像设置已完成。 Teams在非永久性设置上，无法正常使用按用户安装。
    ...
    - 每台计算机安装
        ...
        此过程Teams 64 位操作系统上的 Program Files (x86) 文件夹和 32 位操作系统上的 Program Files 文件夹。 此时，黄金映像设置已完成。 非Teams要求每台计算机安装一个客户端。

        下一个交互式登录会话Teams请求凭据。

        > [!NOTE]
        > 这些示例还使用 **ALLUSERS=1** 参数。 设置此参数时，Teams Machine-Wide"安装程序"会显示在"控制面板"中的"程序和功能"中，&的"应用"Windows 设置"功能"中供计算机所有用户使用。 然后，所有用户都可以卸载Teams管理员凭据的用户。
...
3. 从 VDI VM 卸载 MSI。 有两种方法可以卸载Teams。
    ...
    - 命令行：运行以下命令。
      ...
      此过程从"Teams文件" (x86) 或"程序文件"文件夹中卸载) ，具体取决于操作系统环境。
...
> [!IMPORTANT]
> 如果当前运行 Teams 而不在 VDI 中执行 AV 优化，并且使用尚不支持用于优化的功能 (如在应用共享) 时提供并控制，必须设置虚拟化提供程序策略以关闭 Teams 重定向。 这意味着Teams不会优化媒体会话。 若要了解如何设置策略以关闭重定向Teams，请与虚拟化提供商联系。
...
### <a name="migrate-from-skype-for-business-on-vdi-to-teams-on-vdi"></a>从 VDI Skype for Business迁移到 VDI Teams VDI 上的 VDI
...
Teams浏览器上的应用无法替代 VDI Teams AV 优化的桌面应用。 聊天和协作体验如期工作。 当需要媒体时，某些体验可能无法满足用户在 Chrome 浏览器上的期望：

- 音频和视频流式处理体验可能并非最佳。 用户可能会遇到延迟或质量降低的情况。
- 设备设置在浏览器设置中不可用。
- 设备管理通过浏览器进行处理，需要在浏览器站点设置中设置多个设置。
- 设备设置可能还需要在设备管理中Windows设置。

## <a name="teams-on-vdi-with-chat-and-collaboration"></a>Teams聊天和协作在 VDI 上聊天

如果你的组织只想在 Teams 使用聊天和协作功能，你可以设置用户级策略，以在 Teams 中关闭呼叫和Teams。
...
[**会议策略**](meeting-policies-in-teams.md)：Teams中的会议策略控制用户可以创建的会议类型以及组织中用户安排的会议参与者可用的功能。 Teams包括内置的 AllOn 会议策略，其中所有会议功能都打开。 若要启用所有会议功能，请分配 AllOn 策略。 或者，创建自定义会议策略以打开你需要的会议功能并为其分配用户。
...
若要禁用回退模式，将值设置为 **1。** 若要仅启用音频，将值设置为 **2。** 如果该值不存在或设置为 **0** 或零 (，则) 回退模式。

此功能在 Teams 1.3.00.13565 及更高版本中可用。

## <a name="disable-audio-and-video-settings-for-vdi"></a>禁用 VDI 的音频和视频设置

TeamsVDI 策略在 Microsoft Teams 模块中提供。 这些策略处于活动状态，在未优化的 VDI 环境中强制实施。
...
当具有 VDI 策略设置 -DisableAudioVideoInCallsAndMeetings 的用户$true登录到 VDI 上的 Teams 时，他们应该能够：
...
- 在 Citrix 环境中，如果用户在运行 Teams 时从虚拟机断开连接，Teams 更新可能会导致用户在重新连接时为 AV 保持未优化状态。 建议用户在从 Citrix 虚拟机Teams之前退出配置，以避免这种情况。
...
- 快速租户切换可能会导致 VDI 上与调用相关的问题。 重新启动客户端将缓解这些问题。
...
然后重启 VDA。 有关详细信息，请参阅此 Citrix 支持文章排查针对 Teams[的 HDX 优化](https://support.citrix.com/article/CTX253754)问题。
