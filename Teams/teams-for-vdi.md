...
|AllowPrivateCalling| 控制呼叫应用是否在团队客户端左侧的应用栏中可用，以及用户是否可以在私人聊天中看到呼叫和视频通话选项。 |设置为 False 可从团队左侧的应用栏中删除呼叫应用，并删除私人聊天中的呼叫和视频通话选项。|
...
|ScreenSharingMode | 确定允许用户在通话或会议中共享其屏幕的模式。 | 设置为 "已禁用"，禁止用户共享其屏幕。 |
...
| AllowOutlookAddIn |确定用户是否可以在 Outlook 桌面客户端中安排团队会议。 | 设置为 False 可禁止用户在 Outlook 桌面客户端中安排团队会议。 |
...
| AllowPowerPointSharing |确定用户的会议中是否允许 PowerPoint 共享。 |设置为 False 可禁止用户在会议中共享 PowerPoint 文件。 |
| AllowWhiteboard | 确定用户的会议中是否允许白板。 | 设置为 False 以在会议中禁用白板。 |
| AllowTranscription |确定用户的会议中是否允许实时和/或后期会议标题和转录。 | 设置为 False 将禁止会议中的脚本和字幕。 |
| AllowSharedNotes | 确定是否允许用户接受共享笔记。 | 设置为 False 以禁止用户接受共享笔记。 |
| MediaBitRateKB |确定会议中音频/视频/应用共享传输的媒体比特率。 | 建议值为5000（5 MB）。 您可以根据组织的需要对其进行更改。 |

#### <a name="create-and-assign-a-meeting-policy"></a>创建和分配会议策略

1. 以管理员身份启动 Windows PowerShell 会话。
1. 连接到 Skype Online 连接器。
...
1. 将 AllOff 内置策略选项应用于将在虚拟化环境中使用团队的所有用户。
...
1. 运行以下命令，将 MSI 安装到 VDI VM （或完成更新）。
...
1. 运行以下命令以从 VDI VM 卸载 MSI （或准备更新它）。
