...
目前，Teams Azure 虚拟桌面、Citrix 和 VMware (VDI) AV) VDI 上的应用。 查看本部分中的信息，确保满足正确功能的所有要求。
...
#### <a name="how-to-exclude-teams-deployment-through-microsoft-365-apps-for-enterprise"></a>如何通过 Teams 排除Microsoft 365 企业应用版
...
2. 运行以下命令之一，将 MSI 安装到 VDI VM：
    ...
    - 按用户安装 (默认) 
        ...
        此过程是默认安装，它Teams %AppData% 用户文件夹。 此时，黄金映像设置已完成。 Teams在非永久性设置上，无法正常使用按用户安装。
    ...
    - 每台计算机安装
        ...
        > [!NOTE]
        > 这些示例还使用 **ALLUSERS=1** 参数。 设置此参数时，Teams Machine-Wide安装程序会显示在"控制面板"中的"程序和功能"中，&应用的"Windows 设置"中，供计算机所有用户使用。 然后，所有用户都可以卸载Teams管理员凭据。
...
3. 从 VDI VM 卸载 MSI。 有两种方法可以卸载Teams。
    ...
    - PowerShell 脚本：可以使用此[PowerShell](scripts/powershell-script-deployment-cleanup.md)脚本卸载Teams并删除Teams的 Teams 文件夹。 针对计算机上安装了Teams的每个用户配置文件运行脚本。
    - 命令行：运行以下命令。
...
[**会议策略**](meeting-policies-in-teams.md)：Teams包括内置的 AllOff 会议策略，其中所有会议功能都已关闭。 将 AllOff 策略分配给组织中在虚拟化环境中使用Teams的所有用户。
