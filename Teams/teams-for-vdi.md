...
在Teams环境中使用资源可能稍有不同，Teams非虚拟化环境中使用。 例如，某些高级功能在虚拟化环境中可能不可用，并且视频分辨率可能不同。
...
目前，Teams Azure 虚拟桌面、Citrix 和 VMware (AV) VDI 上的应用。 查看本部分的信息，确保满足正确功能的所有要求。
...
Azure 虚拟桌面为 VDI 上的Teams AV 优化。 若要详细了解要求和安装，请参阅[在 Azure 虚拟Teams使用应用程序](/azure/virtual-desktop/teams-on-wvd)。
...
可以从 VMware 下载页下载 [最新版本的 VMware](https://customerconnect.vmware.com/downloads/#all_products) Horizon。 默认情况下，所需的媒体优化组件是 Horizon Agent 和 Horizon 客户端的一部分，无需安装任何其他插件来为 Teams。
...
可以使用 MSI Teams每台计算机安装或按用户安装部署适用于 VDI 的桌面应用。 决定使用哪种方法取决于是使用持久性设置还是非持久性设置，以及组织的关联功能需求。
...
Microsoft 365 企业应用版不支持每台计算机安装 Teams。 若要使用每台计算机安装，必须从Teams中排除Microsoft 365 企业应用版。 请参阅[将 Teams 桌面应用部署到 VM](#deploy-the-teams-desktop-app-to-the-vm) 和[如何Teams部署Microsoft 365 企业应用版](#how-to-exclude-teams-deployment-through-microsoft-365-apps-for-enterprise)部分。
...
若要详细了解如何为用户准备网络Teams，请参阅[准备组织的网络以Teams](prepare-network.md)。
...
如果你的组织希望仅使用 Teams 中的聊天和协作功能，你可以设置用户级策略以在 Teams 中关闭呼叫和Teams。
...
[**会议策略**](meeting-policies-overview.md)Teams包括内置的 AllOff 会议策略，其中所有会议功能都已关闭。 将 AllOff 策略分配给组织中在虚拟化环境中Teams的所有用户。
...
以下示例演示如何使用 [Grant-CsTeamsCallingPolicy 将 AllowCalling](/powershell/module/skype/grant-csteamscallingpolicy) 调用策略分配给用户。
...
当具有 VDI 策略设置 -DisableAudioVideoInCallsAndMeetings $true登录到 Teams on VDI 时，他们应该能够：
...
有关Teams VDI 的已知问题，请参阅[组织中Teams支持。](/MicrosoftTeams/troubleshoot/teams-welcome)
...
然后重启 VDA。 若要了解有关详细信息，请参阅此 Citrix 支持文章[排查 HDX 优化问题Teams](https://support.citrix.com/article/CTX253754)。
