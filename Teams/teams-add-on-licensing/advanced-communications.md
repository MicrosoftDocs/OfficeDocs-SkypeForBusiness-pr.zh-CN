...
- **定制和自定义会议**：使用自定义品牌会议大厅等功能，为面向内部和面向自定义的方案促进会议标准化。 在组织的部门中灵活实施。
- **管理组织通信**：监视、跟踪和分析用户和设备上的数据，以确保顺利体验。
- **自定义策略包的定制** 体验：用于为组织中具有类似角色的用户捆绑自己的策略集。 通过使用管理中心或 PowerShell 添加要捆绑的策略类型和策略，创建自己的Teams包。 每个用户都需要许可证才能接收自定义策略包分配。 

高级通信作为任何付费Microsoft 365订阅的附加内容在所有 Microsoft 365 或 Office 365 中提供。 高级通信中包含的功能不会添加到其他Microsoft 365或Office 365计划中，并且不能单独购买。 若要获取功能，必须购买附加内容。
...
从 2020 年 8 月 4 日开始提供 60 天试用期。 可以通过管理中心获取Microsoft 365试用版。
