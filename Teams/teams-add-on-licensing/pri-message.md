---
title: '团队消息中的优先级通知 '
ms.author: tonysmit
author: tonysmit
manager: serdars
ms.reviewer: mikedav
ms.topic: reference
ms.service: msteams
search.appverid: MET150
ms.collection:
- M365-collaboration
audience: Admin
appliesto:
- Microsoft Teams
localization_priority: Normal
ms.custom:
- Licensing
- LIL_Placement
description: 了解团队消息中的优先级通知。
ms.openlocfilehash: 4e39af4c400fef5b44e0994d95821ea9005edbba
ms.sourcegitcommit: 1329ddd54d2a22c493677581c838eb6b3456bacc
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 01/06/2020
ms.locfileid: "40947072"
---
# <a name="messaging-policies-licensing"></a>消息策略许可

在2020年4月1日之前，Microsoft 团队中的无限制优先级通知可供所有用户使用。 此促销已延长为2020年4月31日（2019）的原始结束日期。 4月1日之后，许可用户将能够根据其订阅条款发送优先级通知。

如6月2019日宣布的，当用户有权访问 "优先级通知" 功能时，用户可以将其标记为 "紧急邮件"。 将使用优先级通知发送紧急邮件，通知收件人每隔两（2分钟）分钟长达20分钟，或者收件人阅读邮件。 此功能最大程度地提高了邮件的处理可能性。

IT 管理员将获得有关优先级通知使用情况的报告，以支持相应的用户许可。 无论许可证如何，用户都可以通过优先级通知收到不受限制的紧急邮件。 [了解有关优先级通知的详细信息](https://docs.microsoft.com/MicrosoftTeams/messaging-policies-in-teams)。

|字段| |每个用户最多发送5条消息| 发送每个用户每月无限长的消息|
|---|---|---|---|
|Enterprise||Office 365 F1/Microsoft 365 F1<br> Office 365 E1 | Office 365 E3/Microsoft 365 E3 <br>Office 365 E5/Microsoft 365 E5
|教育版 ||Office 365 A1/Microsoft 365 A1|Office 365 A3/Microsoft 365 A3<br> Office 365 A5/Microsoft 365 A5
|中小型       ||Office 365 商业协作版|Office 365 商业高级版/Microsoft 365 商业版|
|||||

<!-- |Government| | |
||GCC|Office 365 F1 / Microsoft 365 F1<br> Microsoft 365 G1|Office 365 G3 / Microsoft 365 G3 <br> Microsoft 365 G5|
||GCC-High| Office 365 F1 / Microsoft 365 F1<br> Office 365 G1| Office 365 G3 / Microsoft 365 G3 Microsoft 365 G5|
||DoD| Office 365 F1 / Microsoft 365 F1<br>Office 365 G1|Office 365 G3 / Microsoft 365 G3| -->
