...
> [!NOTE]
> 请参阅适用于使用加载项许可证的团队功能的 [团队加载项许可](https://docs.microsoft.com/microsoftteams/teams-add-on-licensing/microsoft-teams-add-on-licensing) 。 你还可以找到有关你需要购买哪些许可证以及如何购买这些许可证的信息 (，具体取决于你的计划) ，以便用户可以获得音频会议、免费电话号码等功能，以及呼叫组织外部电话号码的功能。 确定所需的用户功能后，为他们分配许可证。
...
- 如果您使用的是本地公共交换电话网络 (PSTN) 混合用户的连接，则只需分配电话系统许可证。 不要分配呼叫计划许可证。
...
使用 Microsoft 365 管理中心将许可证分配给单个用户或一次一组小型用户。 你可以在 " **许可证** " 页面上为最多20个用户 (分配许可证) 或 " **活动用户** " 页面。 你选择的方法取决于你是要为特定用户管理产品许可证还是管理特定产品的用户许可证。

有关分步说明，请参阅 [向用户分配许可证](https://docs.microsoft.com/microsoft-365/admin/manage/assign-licenses-to-users)。

如果需要为大量用户（如成百上千用户）分配许可证，请 [在 Azure Active Directory 中使用基于 Powershell 或基于组的许可 (AZURE AD) ](https://docs.microsoft.com/azure/active-directory/users-groups-roles/licensing-groups-assign)。  
...
2. 安装适用于 Windows PowerShell 的 Microsoft Azure Active Directory 模块：
    1. 打开提升了权限的 Windows PowerShell 命令提示符 (以管理员身份) 运行 Windows PowerShell。
    2. 运行以下命令：
        ```powershell
...
3. 在 Windows PowerShell 命令提示符处，运行以下脚本以向你的用户分配许可证，其中 \<CompanyName:License> 是你的组织名称和要分配的许可证的标识符。 例如，litwareinc： MCOMEETADV。
    ...
    若要在不调用计划) 许可证的情况下分配 Microsoft Business Voice (，请在脚本中使用以下语法：
...
| 产品名称| SKU 部件名称 |
|--------------|---------------|
| Microsoft 企业版 E5 (与电话系统)  | ENTERPRISEPREMIUM |
| 不带音频会议的 Microsoft 企业版 E5 ()  | ENTERPRISEPREMIUM_NOPSTNCONF |
...
| Microsoft Business Voice (无需通话计划)  | BUSINESS_VOICE_DIRECTROUTING  |
| Microsoft Business Voice (没有通话计划) 美国| BUSINESS_VOICE_DIRECTROUTING _MED |
| 音频会议 | MCOMEETADV | 
|  (按分钟支付的音频会议) </br>*需要设置和启用通信信用点数。* | MCOMEETACPEA |
| 电话系统 | MCOEV |
| 国内和国际通话计划 | MCOPSTN2 |
| 国内呼叫计划 (每个用户每月每月3000分钟，对于欧盟国家/地区，每个用户/月1200分钟)  | MCOPSTN1 |
| 每个国家/地区的国内通话计划 (每个用户/月120分钟)  </br>*此计划在美国不可用。* | MCOPSTN5 |
| 每个国家/地区的国内通话计划 (每个用户/月240分钟)  </br>*此计划在美国不可用。* | MCOPSTN6 |
| 通信点数 | MCOPSTNPP |
