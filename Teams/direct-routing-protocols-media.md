...
ICE 完整实现终结点将始终是控制终结点，并且将按照"常规"提名来选择用于媒体流的最终候选项。 ICE Lite 终结点可以使用提名来结束用于媒体和完整的呼叫建立的路径。

注意：如果使用对等终结点（发送 183 个临时答案）进行分贝，SBC 必须准备好响应来自多个终结点的检查，并且如果提名发生在 200OK 之前，还要响应来自多个终结点的提名。 提名可以在 200OK 之前或之后进行，具体取决于 ICE 状态机在用户应答的最终路径和时间上的收敛。 SBC 必须能够处理这两种情况。
...
目前，没有仅Teams DTLS 的客户端;但 Google 已宣布，在某些时间点，他们将停止支持 SDES。
