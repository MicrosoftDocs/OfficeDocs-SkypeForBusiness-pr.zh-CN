...
| [在个人预览版中将策略包](#assign-a-policy-package-to-a-group) (组)    |将多个策略分配给组织中具有相同或类似角色的一组用户。 例如，将策略包分配给安全组或通讯组列表中的所有用户。 |Microsoft Teams PowerShell 模块 (powerShell) PowerShell cmdlet 即将Teams管理中心|
...
1. 在管理中心左侧导航Microsoft Teams，转到策略页。
2. 单击策略名称左侧，选择要分配的策略。
3. 选择“管理用户”。
...
现在，我们从用户中删除"员工事件"策略。 这意味着用户不再直接分配Teams直播策略，并且将继承分配给用户所属组的供应商实时事件策略。
