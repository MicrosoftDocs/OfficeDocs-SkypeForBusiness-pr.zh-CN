...
f1.keywords:
- NOCSH
description: 了解如何在 SBC (配置一个) 控制器，为 Microsoft 合作伙伴和/或 PSTN 运营商提供多个租户。
...
Microsoft 不管理运营商。 Microsoft 提供 PBX (Microsoft 电话 System) 和 Teams 客户端。 Microsoft 还认证手机，并认证可与 Microsoft 电话 系统一Microsoft 电话的 SDC。 在选择运营商之前，请确保你的选择具有经过认证的 SBC，并可以端到端地管理语音质量。
...
*请确保你了解 DNS 基础知识以及如何在 Microsoft 365 或 Office 365 中管理域名。在 [继续下一步之前Microsoft 365或Office 365获取](https://support.office.com/article/Get-help-with-Office-365-domains-28343f3a-dcee-41b6-9b97-5b0f4999b7ef)帮助。*
...
- Woodgrove Bank、Contoso 和 Adventure Works 是三个拥有或Microsoft 365域但Office 365接收 Adatum 电话服务的客户。

子域 **必须与** 将为客户配置的中继的 FQDN 名称与"联系人"标头中的 FQDN 匹配，在将邀请发送到 Microsoft 365 或 Office 365。 
...
SBC 需要证书来验证连接。 对于 SBC 托管方案，运营商需要使用 CN 和/或 SAN 请求.base_domain (*\* 例如 \* .customers.adatum.biz) 。* 此证书可用于对从单个 SBC 提供的多个租户的连接进行身份验证。
...
6. 下一页上，清除所有值 (，除非要使用 Exchange、SharePoint 或 Teams/Skype for Business) 的域名，请单击"下一步"，然后单击 **"完成**"。  确保新域位于"设置完成"状态。

    ![显示状态为"设置已完成"的域的屏幕截图。](media/direct-routing-14-sbc-setup-complete.png)

### <a name="activate-the-domain-name"></a>激活域名

注册域名后，需要通过添加至少一个具有 电话系统 许可证的用户，并分配一个包含与所创建基域匹配的 SIP 地址的 FQDN 部分的 SIP 地址来激活它。

> [!NOTE]
> 运营商租户必须至少保留一个电话系统租户许可证，以避免删除Skype for Business配置。 
...
需要为每个客户创建唯一的子域名称。 本示例将在租户中创建一个子 sbc1.customers.adatum.biz 域名为 woodgrovebank.us。
...
-    派生的中继，不需要注册。 它只是从运营商中继中添加的所需主机名。 它从运营商中继派生其所有配置参数。 无需在 PowerShell 中创建派生的中继，并且与运营商中继的关联基于 FQDN 名称 (请参阅下面的) 。

**预配逻辑和示例**

-    运营商只需使用) 命令在运营商域 (中继中设置和管理Set-CSOnlinePSTNGateway中继。 在以上示例中，它是 adatum.biz;
...
-    Sbc1.customers.adatum.biz – 客户租户中的派生中继，无需在 PowerShell 中创建。  只需在联机语音路由策略中将派生的中继的名称添加到客户租户中，而无需创建它 (在 TAC 中设置 Teams-Voice-Direct Routing-Voice 路由字段 SDC 注册的) 下的语音路由策略时，请使用派生的中继 FQDN。
...
若要从运营商托管模型的当前实现迁移到新模型，运营商需要为客户租户重新配置中继。 使用以下方法从客户租户中删除中继Remove-CSOnlinePSTNGateway (将中继留在运营商租户) -
