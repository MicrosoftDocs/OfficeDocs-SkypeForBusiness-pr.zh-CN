...
3. 单击" **下一步**"。
4. 在示例中，租户已 adatum.biz 已验证的域名。 向导不会要求进行其他验证，customers.adatum.biz 已注册名称的子域。 但是，如果添加以前未验证的 FQDN，则需要完成验证过程。 验证过程 [如下所述](#add-a-subdomain-to-the-customer-tenant-and-verify-it)。
...
需要为每个客户创建唯一的子域名称。 本示例将在租户中创建一个 sbc1.customers.adatum.biz 域名为 woodgrovebank.us。
