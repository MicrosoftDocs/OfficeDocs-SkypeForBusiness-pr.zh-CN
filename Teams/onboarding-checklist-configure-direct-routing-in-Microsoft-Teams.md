...
|db-9|验证 SBC 配对|针对 `Get-CsOnlinePSTNGateway` 为特定网站配对的每个 SBC 运行 cmdlet，并验证**启用**的参数是否显示值**True**。<br><br>使用 SBC 管理接口验证 SBC 是否收到对传出 SIP 选项的**200 "确定"** 响应。|||
|10|验证用户配置|验证用户帐户是使用目录同步直接在 Microsoft 365 或 Office 365 中创建还是同步到 Microsoft 或 Office。<br><br>验证是否为用户分配了必要的许可证。<br><br>对于直接路由的 PSTN 连接，你的用户必须托管在 Skype for Business Online 中，并且已启用 Microsoft 团队。||[为用户启用直接路由服务](https://docs.microsoft.com/microsoftteams/direct-routing-configure#enable-users-for-direct-routing-service)|
