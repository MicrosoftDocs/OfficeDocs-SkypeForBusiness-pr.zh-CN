...
> [!IMPORTANT]
> 向导应用的策略将满足大多数客户Teams 教育版的需求。 该向导使用建议用于学生 (设置来调整一组核心策略的"全局") "组织范围默认策略"定义，并应用于学生。 该向导还会创建一组自定义策略并将其分配给教师和教职员工。 大多数Teams 教育版客户在运行此向导后不需要使用其他策略分配方法。 只有在希望 *手动为学生* 、教师和教职员工创建和管理策略时，才使用其他策略分配方法。
...
- **学生**：该向导使用适合保护学生安全的新默认设置来调整 (组织范围内每个策略区域的默认) 策略定义。 这可确保当前学生和所有新学生获得限制性最强的策略集。
- **教师和教** 职员工：该向导为向导涵盖的每个策略区域创建一组自定义策略定义，其设置专门针对教师和教职员工的需求。 然后，它将策略定义分配给你选择的教师和教职员工组。 这样，教师和教职员工可以获取一组更宽松的策略，帮助他们获得成功。

只需运行向导一次。 新学生自动获取向导 (组织范围的默认) 策略定义，并自动为添加到所选组的教职员工分配自定义策略。
...
1. 如果你是新用户，Teams向导会自动启动。 否则，你随时都可以从仪表板启动向导。 在 Microsoft Teams 管理中心的左侧导航中，转到"主页"，然后在"安全学习环境简易策略设置"磁贴中，选择"快速 **设置"。**
...
> [!IMPORTANT]
> **只有在运行向导** 之前，将现有策略分配给学生或教师和教职员工时，才完成此步骤。 如果尚未使用Teams除向导创建的策略外没有任何现有策略，请跳过此步骤并转到步骤 2。
...
目前，该向导在运行时应用我们建议的策略。 必须知道，当新策略在 Teams 中可用时，向导会自动更新) 全局 (组织范围的默认) 设置。 
...
|仅更改教师和教职员工组   |<ul><li>**学生**：不会更改应用于 (全局策略) 组织范围的默认策略定义。</li><li>**教师和教** 职员工：自定义策略定义分配给新的教师和教职员工组，从以前的教师和教职员工组中删除。</li></ul>         |
...
- [使用远程Teams确保学生安全](https://support.microsoft.com/office/keeping-students-safe-while-using-teams-for-distance-learning-f00fa399-0473-4d31-ab72-644c137e11c8)
