...
这些报告使用来自呼叫质量仪表板 [数据存储](CQD-Power-BI-query-templates.md) 的数据。 报告允许组织报告自动助理和呼叫队列正在处理的呼叫数。  这些报告还提供有关调用队列中代理性能的见解。

## <a name="prerequisites"></a>先决条件

### <a name="power-bi-desktop"></a>Power BI Desktop
需要安装Power BI Desktop。 可以从 Microsoft Store Windows[安装它](https://aka.ms/pbidesktopstore)。
...
|目录搜索方法总计                  |AADirectorySearchMethod<br>TotalCallCount  |AADirectorySearchMethod abs_search_dtmf或abs_search_name    |
...
|名称                                    |数据类型                |说明                            |
|:---------------------------------------|:------------------------|:--------------------------------------|
|AA 名称                                 |文本                     |附加到 自动助理 的资源帐户自动助理<br><br>如果完整资源帐户名称 aa_test@microsoft.com 则此值为：**aa_test** |
...
|AAChainDuration                         |十进制数           |汇总：总和<br>通话持续时间（以自动助理                     |
|AAChainIndex                            |文本                     |                                                                         |
...
|AATransferAction                        |文本                     |呼叫转接目标类型 - 可能的值：<br><br>***§ 应用程序 - 语音应用程序实体**_<br>§ external_pstn<br>_*_§ hunt_group - 调用队列实体_*_<br>_*_§ orgaa - 组织自动助理实体_**<br>§ shared_voicemail<br>§ unknown<br>§ user |
|调用类型 <sup>1</sup>                   |文本                     |调用类型 - 可能的值：<br><br>§ 外部<br>§ 内部         |
...
|名称                                    |数据类型                |说明                            |
|:---------------------------------------|:------------------------|:--------------------------------------|
|调用队列标识                     |文本                     |附加到呼叫队列的资源帐户的名称<br><br>如果完整资源帐户名称 cq_test@microsoft.com 则此值为：**cq_test** |
...
|呼叫队列呼叫结果                  |文本                     |调用队列调用最终状态 - 可能的值：<br><br>§ agent_joined_conference<br>§ 已拒绝<br>§ disconnected<br>§ 错误<br>§ 失败<br>§ 无效<br>§ overflown<br>§ timed_out<br>§ transferred_to_agent |
|调用队列标识                     |文本                     |附加到呼叫队列的资源帐户的名称<br><br>如果完整资源帐户名称 cq_test@microsoft.com 则此值为：**cq_test** |
|调用队列目标类型                  |文本                     |***调用重定向目标类型 -- 可能的值：***<br><br>§ ApplicationEndpoint<br>§ 邮箱<br>§ 其他<br>§ 用户 |
|调用类型 <sup>1</sup>                   |文本                     |调用类型 - 可能的值：<br><br>§ 外部<br>§ 内部           |
...
|名称                                    |数据类型                |说明                              |
|:---------------------------------------|:------------------------|:----------------------------------------|
|***已放弃的调用百分比***                 |百分比               |度量值：已放弃的调用数/总调用数    |
...
|呼叫计数                              |全数             |汇总：总和<br>通话的数量                  |
|呼叫队列呼叫结果                  |文本                     |调用队列调用最终状态 - 可能的值：<br><br>§ agent_joined_conference<br>§ 已拒绝<br>§ disconnected<br>§ 错误<br>§ 失败<br>§ 无效<br>§ overflown<br>§ timed_out<br>§ transferred_to_agent |
|调用队列最终状态操作           |文本                     |调用队列最终操作 - 可能的值：<br><br>§ 断开连接 (timed_out调用) <br>§ disconnect_with_busy (overflown 调用) <br>§ failed_to_accept_call<br>§ forward<br>§ shared_voicemail<br>§ other<br>§ 语音邮件 |
|调用队列标识                     |文本                     |附加到呼叫队列的资源帐户的名称<br><br>如果完整资源帐户名称 cq_test@microsoft.com 则此值为：**cq_test** |
|日期                                    |日期/时间                |呼叫队列呼叫开始日期和时间 (utc)  (UTC)    |
|IsAbandoned                             |True/false               |如果代理未应答调用，则为 True           |
...
|代理和 (呼叫队列) 总呼叫持续时间   |代理 (呼叫队列) 的总持续时间（秒）     |
...
|名称                                    |数据类型                |说明                                         |
|:---------------------------------------|:------------------------|:---------------------------------------------------|
|代理名称                              |文本                     |用户 UPN<br>如果输入完整的用户名 **user@microsoft.com** 则此值为： **user** |
|平均呼叫持续时间 (秒)           |十进制数           |汇总：总和<br>呼叫队列调用的平均持续时间（以秒数） |
|呼叫计数                              |全数             |汇总：总和<br>代理处理的调用数                    |
|通话持续时间 (分钟)                   |全数             |汇总：总和<br>呼叫队列呼叫的总呼叫持续时间（分钟）  |
|呼叫队列名称                         |文本                     |附加到呼叫队列的资源帐户的名称<br><br>如果完整资源帐户名称 cq_test@microsoft.com 则此值为：**cq_test** |
|日期                                    |日期                     |                                                    |
