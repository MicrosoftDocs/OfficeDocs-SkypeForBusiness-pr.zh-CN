...
|已放弃的呼叫                     |fCallQueueAnalytics -> %Abandoned 调用<br>fCallQueueAnalytics ->呼叫计数<br>fCallQueueAnalytics -> Date<br>fCallQueueAnalytics -> IsAbandoned |IsAbandoned 为 True |
...
|名称                                    |数据类型                |说明                            |
|:---------------------------------------|:------------------------|:--------------------------------------|
|调用队列标识                     |文本                     |附加到呼叫队列的资源帐户的名称<br><br>如果完整资源帐户名称 cq_test@microsoft.com则此值为 **：cq_test** |
...
|呼叫计数                              |全数             |汇总：总和<br>通话的数量                  |
|呼叫队列呼叫结果                  |文本                     |调用队列调用最终状态 - 可能的值：<br><br>§ agent_joined_conference<br>§ 已拒绝<br>§ disconnected<br>§ 错误<br>§ 失败<br>§ 无效<br>§ overflown<br>§ timed_out<br>§ transferred_to_agent |
|调用队列最终状态操作           |文本                     |调用队列最终操作 - 可能的值：<br><br>§ disconnect<br>§ disconnect_with_busy<br>§ failed_to_accept_call<br>§ forward<br>§ shared_voicemail<br>§ other<br>§ 语音邮件 |
|调用队列标识                     |文本                     |附加到呼叫队列的资源帐户的名称<br><br>如果完整资源帐户名称 cq_test@microsoft.com则此值为 **：cq_test** |
|日期                                    |日期/时间                |呼叫队列呼叫开始日期和时间 (utc)  (utc)    |
|IsAbandoned                             |True/false               |如果代理未应答调用，则为 True           |
...
|呼叫队列名称                         |文本                     |附加到呼叫队列的资源帐户的名称<br><br>如果完整资源帐户名称 cq_test@microsoft.com则此值为 **：cq_test** |
|日期                                    |日期                     |                                                    |
