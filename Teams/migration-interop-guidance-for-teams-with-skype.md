...
ms.locfileid: "60865460"
---
# <a name="coexistence-modes---reference"></a>共存模式 - 参考

> [!Important] 
> - 2021 年 7 月 31 日停用 Skype for Business Online 后，不再能够将共存模式（TeamsOnly）分配给云中的用户。 与停用之前的情况一样，可以分配本地Skype for Business Server用户除 *TeamsOnly* 外的任何模式。 
...
- *传入路由*：在哪个客户端 (Teams或Skype for Business) 进行传入聊天和通话？ 
- *状态* 发布：根据其他用户在用户或用户中的活动向其他用户显示Teams Skype for Business？ 
- *会议计划*：哪种服务用于安排新会议并确保在会议Outlook？ TeamsUpgradePolicy 不控制会议加入。 用户始终 *可以加入* 任何会议，无论是Skype for Business会议还是Teams会议。
...
下面列出了这些模式。 云用户必须是 TeamsOnly，本地用户必须是 TeamsOnly 外的任何模式。 
</br>
</br>

|模式|通话和聊天|会议计划<sup>1</sup>|Teams &频道|用例|
|---|---|---|---|---|
|**TeamsOnly <sup>2</sup>**</br>*只有在用户没有本地帐户的情况下，才能Skype for Business Server*|Teams|Teams|是|正在升级的最终状态。 除非检测到混合配置，否则新租户的默认值。|
|孤岛|任一|任一|是|混合组织的默认配置。 允许单个用户并排评估这两个客户端。 聊天和通话可以登陆任一客户端，因此用户必须始终运行这两个客户端。 为了避免令人困惑或回归的 Skype for Business 体验，Skype for Business 将继续处理外部 (联合) 通信、PSTN 语音服务和语音应用程序、Office 集成和若干其他集成。|
|SfBWithTeamsCollabAndMeetings<sup>2</sup>|Skype for Business|Teams|是|"会议第一"。 主要面向本地组织Teams会议功能（如果尚未准备好将呼叫转移到云）。|
...
|模式|解释|
|---|---|
|**孤岛**</br> (本地) |用户同时运行Skype for Business和Teams运行。 此用户：</br><ul><li>可以在客户端或客户端中Skype for Business聊天Teams VoIP 呼叫。 注意：本地Skype for Business的用户无法从 Teams 启动，Skype for Business访问另一个用户，无论收件人的模式如何。<li>接收由&客户端Skype for Business发起 VoIP Skype for Business聊天。<li>接收由&客户端Teams用户发起的 VoIP Teams聊天（如果他们位于同一 *租户中*）。<li>接收由&客户端Teams用户发起的 VoIP Skype for Business（如果他们位于联合 *租户 中*）。 <li>具有 PSTN 功能，如下所述：<ul><li>当用户位于本地Skype for Business且具有企业语音时，PSTN 呼叫始终在 Skype for Business 中发起和Skype for Business。<li>当用户位于 Skype for Business Online 上并拥有 Microsoft 电话 System 时，用户始终通过以下方式发起和接收 PSTN Skype for Business：<ul><li>无论用户具有 Microsoft 呼叫计划，还是通过 Skype for Business 云连接器版本 或本地部署将 Skype for Business Server (混合语音连接到 PSTN) 。<li>**注意：电话系统群岛模式不支持直接路由。**</ul></ul><li>在服务中接收 Microsoft 呼叫队列和自动助理Skype for Business：<ul><li>电话"呼叫队列"和"自动助理"的号码不能电话系统"直接路由"号码。</ul></ul><li>可以在 Teams 或 Skype for Business (中安排会议，并且默认情况下会看到这两个插件) 。<li>可以加入任何Skype for Business或Teams会议;会议将在相应的客户端中打开。</ul>|
|**SfBOnly**</br> (本地) |用户仅Skype for Business。 此用户：</br><ul><li>只能从聊天发起聊天Skype for Business呼叫。<li>无论发起在何处，Skype for Business客户端中接收任何聊天/呼叫，除非发起程序是Teams本地Skype for Business用户。*<li> 只能安排Skype for Business会议，但可以Skype for Business Teams会议。 </br> \** 不建议将群岛模式与本地用户结合使用，而不要与 SfBOnly 模式下的其他用户结合使用。 如果Teams本地 Skype for Business 的用户向 SfBOnly 用户发起呼叫或聊天，则 SfBOnly 用户不可访问，并收到错过的聊天或呼叫电子邮件。*|
|**SfBWithTeamsCollab**</br> (本地) |用户同时运行Skype for Business和Teams运行。 此用户：</br><ul><li>具有 SfBOnly 模式下用户的功能。<li>仅Teams频道和频道 (组) ;聊天/通话/会议安排被禁用。</ul>|
|**SfBWithTeamsCollab </br> AndMeetings**</br> (本地) |用户同时运行Skype for Business和Teams运行。 此用户：<ul><li>具有 SfBOnly 模式下用户的聊天和通话功能。<li>为Teams组协作启用 (- 包括频道对话) ;聊天和通话已禁用。<li>只能安排Teams会议，但可以Skype for Business Teams会议。</ul>|
|**TeamsOnly**</br> (云用户) |用户仅Teams。 此用户：<ul><li>无论从何处启动，Teams客户端接收任何聊天和呼叫。<li>只能从对话发起聊天Teams通话。<li>只能在会议中安排Teams，但可以加入Skype for Business Teams会议。<li>可以继续使用 IP Skype for Business。<br><br>*不建议将 TeamsOnly 模式与群岛模式下的其他用户结合使用，除非Teams饱和;也就是说，所有群岛模式用户都主动使用和监视Teams Skype for Business客户端。如果 TeamsOnly 用户发起与群岛用户的通话或聊天，该呼叫或聊天将登陆该群岛用户Teams客户端;如果群岛用户不使用或监视Teams，该用户将显示为脱机状态，并且 TeamsOnly 用户无法访问该用户。*</ul> |
|||
