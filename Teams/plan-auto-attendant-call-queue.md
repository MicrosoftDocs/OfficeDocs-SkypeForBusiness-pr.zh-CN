...
在呼叫队列中将资源帐户用于呼叫线路 ID 用途时，或者当自动助理或呼叫队列将呼叫转接到外部电话号码时，资源帐户必须具有 电话系统 虚拟用户许可证，并且分配有以下任一许可证：
...
- Microsoft 电话版本 1449/1.0.94.2020051601 或更高版本
...
- 3 (3) 将用户重定向到公司目录的另一个自动助理。 公司目录自动助理允许呼叫者通过拨打其姓名或分机号呼叫组织中个人。
...
- 先描述服务，然后按一个选项 (例如：对于 Sales 按 1) ，而不是按其他方式 (例如 按 1 表示"销售额) "。
...
1. 从组织外部直接拨号，获取自动助理和呼叫队列所需的服务号码。 这可能包括 [从另一提供商转移号码](phone-number-calling-plans/transfer-phone-numbers-to-teams.md) 或 [请求新的服务号码](getting-service-phone-numbers.md)。
