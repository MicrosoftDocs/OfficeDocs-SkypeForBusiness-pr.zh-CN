...
对于每个呼叫队列，可以指定队列中的代理是否可以选择不接听呼叫，以及是否应当根据呼叫中的状态指示将呼叫路由到Teams。
...
如果你的代理使用 Microsoft Teams 应用进行呼叫队列调用，则需要在 TeamsOnly 模式下。
...
- 第一 (1) 键将呼叫者重定向到销售呼叫队列。 此呼叫队列连接到包含分配到队列的销售团队的团队。
...
- 首先描述服务，然后是按 (选项，例如：对于 Sales 按 1) ，而不是按其他方式 (例如 按 1 表示"销售额) "。
