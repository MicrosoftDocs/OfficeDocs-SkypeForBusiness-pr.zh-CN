...
菜单提示可以通过使用文本到语音 (系统生成的提示) 上传录制的音频文件创建。 语音识别接受语音命令进行免提导航，但呼叫者也可使用电话键盘导航菜单。
...
*会议模式* 是呼叫队列中的一个选项，可显著减少将 VOIP Teams PSTN 呼叫连接到代理所花的时间。 若要使会议模式正常工作，呼叫队列中的代理必须使用以下客户端之一：
...
- 先描述服务，然后按 (选项，例如：对于"销售") 1，而不是按其他方式 (例如 按 1 表示"销售) "。
