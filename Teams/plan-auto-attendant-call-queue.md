...
ms.locfileid: "62039885"
---
# <a name="plan-for-teams-auto-attendants-and-call-queues"></a>规划Teams自动助理和呼叫队列

自动助理允许您设置菜单选项，以便根据呼叫者输入路由呼叫。 菜单选项，如"对于销售，请按 1。  对于"服务按 2"，对于自动助理，让组织提供一系列选项，引导呼叫者快速到达其目的地，而无需依赖人工接线员来处理传入呼叫。
...
对于每个呼叫队列，可以指定队列中的代理是否可以选择不接听呼叫，以及是否应当根据呼叫队列中的状态指示将呼叫路由Teams。
...
- 每个自动助理和每个呼叫队列的资源帐户
- 免费Microsoft Teams 电话 - 每个资源帐户的虚拟用户许可证
- 要直接拨打的每个资源帐户连接至少一个[Microsoft](getting-service-phone-numbers.md)服务号码、接线员号码、直接路由号码或混合号码
 - 服务号码可能是收费或免费号码
...
如果代理使用 Microsoft Teams 应用进行呼叫队列调用，则需要在 TeamsOnly 模式下。

在呼叫队列中使用资源帐户进行呼叫行 ID 时，或者当自动助理或呼叫队列将呼叫转接到外部电话号码时，资源帐户必须具有 Teams 电话 虚拟用户许可证，并且分配有以下任一许可证：

- 分配的 [呼叫](calling-plans-for-office-365.md) 计划许可证和电话号码
- 已[连接](operator-connect-plan.md)电话号码的接线员
- 使用 [直接路由](manage-voice-routing-policies.md) (时，可以选择使用联机语音路由策略) 

> [!NOTE]
> 如果分配给资源帐户的呼叫计划被禁用或删除，则使用通信信用额度（[](what-are-communications-credits.md)如果在租户 (中可用，但不分配给资源帐户) ）。 如果没有呼叫计划或通信积分，则调用将失败。
>
> 自动助理和呼叫队列的直接路由服务号码仅支持Microsoft Teams和呼叫代理。
> 
> 不支持在呼叫计划、连接和直接路由中继之间传输。
> 
...
- 您是否希望在你的组织中 [启用](call-park-and-retrieve.md) 呼叫停放和检索以帮助人员或部门之间的呼叫协调？
...
如果需要，可以针对每个队列使用这些选项的组合。 具有电子邮件地址的组可用于语音邮件。 使用 Teams 提供许多优势，包括代理之间的共享文件存储和聊天、可接收语音邮件的公用邮箱，以及可扩展的平台，可包含与业务线应用程序或 Power Apps 的集成。

建议在开始配置之前，选择将呼叫代理添加到队列的策略。

如果有现有的自动助理和呼叫队列基础结构，并且要迁移到 Teams，则需要制定计划，将现有电话号码转移到新的自动助理和呼叫队列。 您可能需要创建转口 [订单，以便](phone-number-calling-plans/port-order-overview.md) 从其他提供商移动号码。 建议暂时获取一个或多个新电话号码，测试自动助理和呼叫队列流，然后再切换当前使用的电话号码。
...
- **基于拨号选项重定向** - 可以指示呼叫者在分配给其电话键盘上的号码的选项（0-9）之间选择。 可以为每个拨号键分配呼叫路由目标。
...
- **组织中的人** - 组织中能够接收语音呼叫的人。 这可以是联机用户，或者使用 Skype for Business Server 在本地托管的用户。
...
从队列中重定向的呼叫可以发送到上面列出的任何呼叫路由目标，操作员除外。  (呼叫队列没有运算符，但可以将呼叫者重定向到与为自动助理.) 
...
- 三 (3) 密钥将用户重定向到公司目录的另一个自动助理。 公司目录自动助理允许呼叫者通过拨打其姓名或分机号呼叫组织中个人。
...
- 首先描述服务，然后是按 (选项，例如：对于"销售"按 1) ，而不是按其他方式 (例如 按 1 表示"销售) "。
...
2. 获取[Teams 电话创建的每个](teams-add-on-licensing/virtual-user.md)资源帐户的虚拟用户许可证。 这些许可证是免费的，因此如果你决定在将来对资源帐户进行更改，我们建议额外获取一些许可证。

3. [为要创建的每个](manage-resource-accounts.md) 自动助理和呼叫队列创建资源帐户。 为每个帐户分配Teams 电话 - 虚拟用户许可证和服务编号（可选）。
...
7. 如果计划允许按分机号码拨叫，请确保已将用户的分机号码添加到其Azure Active Directory个人资料。
