...
共享Teams设备（如会议室设备或公用区域电话）不能对通常应用于个人设备的注册和合规性使用相同的要求。 将个人设备身份验证要求应用到共享设备将导致以下登录问题：
...
例如，如果需要多重身份验证才能访问 Teams，则用户需要干预才能完成身份验证。 共享设备不支持多重身份验证。 同样，如果帐户配置为每隔 X 天重新进行身份验证，则共享设备在没有用户干预的情况下无法解决质询。
...
如果要将共享设备注册到 Intune 中，可以将设备符合性配置为条件访问中的控件，以便只有合规的设备可以访问企业资源。 Teams配置基于设备符合性的条件访问策略的设备。 有关详细信息，请参阅条件[访问：要求符合或混合Azure AD设备](/azure/active-directory/conditional-access/howto-conditional-access-policy-compliant-device)。
