...
- **为临时会议保留可用的会议空间。** 使用触摸屏面板，您可以在现场为临时会议保留可用的会议空间，然后从会议室或Teams设备加入该Microsoft Teams 会议室Surface Hub会议。 
...
若要详细了解通过客户面板认证的Teams，请参阅[当前认证的Teams面板](teams-ip-phones.md#currently-certified-teams-panels)。
...
如果参与规划、部署和管理 Teams面板设备，则本部分适合你。 本部分不面向这些面板的Teams用户。
...
>
>- 如果已在会议Microsoft Teams 会议室中部署了一个 Teams 面板，则无需购买单独的许可证来部署面板。 管理员使用与同一空间的Microsoft Teams 会议室登录面板设备。
>
