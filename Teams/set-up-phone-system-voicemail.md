...
有关邮件加密的信息，请参阅 [电子邮件加密](/microsoft-365/compliance/email-encryption)。

> [!NOTE]
> 只有当呼叫来自内部用户时，我们才对语音邮件应用加密。 如果呼叫来自外部用户，则语音邮件不会加密。

若要设置受保护的语音邮件，请执行下列操作：

1. 转到 <https://admin.microsoft.com> ，使用具有全局管理员权限的帐户登录。
2. 选择 **"全部显示**"，然后转到"**管理中心**  >  **Exchange"。**
3. 在"Exchange管理中心"中，选择"**邮件流**  >  **规则"。**
4. 选择 **+** **"添加**"，然后选择 **"Office 365 邮件加密和权限保护"。**
5. 提供新邮件流规则的名称，然后在"如果应用此规则 **"** 下选择"邮件 **属性**  >  **包括邮件类型** \> **"语音邮件"。** 选择"**确定"。**
6. 在 **"执行以下操作"** 下，选择 **"Office 365 邮件加密邮件应用权限保护"，** 然后选择"选择 **一个"。** 在 **"RMS 模板"** 下，选择 **"不转发"。** 选择 **"确定**"，然后选择"**保存"。**

    > [!NOTE]
    > 如果 **RMS 模板** 列表为空，则需要设置消息加密。 有关设置消息加密的信息，请参阅以下文章：
    >
    > - [设置新的消息加密功能](/microsoft-365/compliance/set-up-new-message-encryption-capabilities)
...
我们针对你的用户提供有关管理其语音邮件设置的信息，以及你的语音信箱中的其他呼叫Teams：
