...
如果决定将现有资源帐户上的许可证从 **Teams 电话标准版** 许可证切换到虚拟用户许可证，则需要获取免费的虚拟用户许可证，然后按照Microsoft 365 管理中心中的步骤 [将用户移动到其他订阅](/microsoft-365/admin/manage/assign-licenses-to-users#move-users-to-a-different-subscription)。

> [!WARNING]
> 始终删除完整的Teams 电话标准版许可证，并在同一许可证活动中分配虚拟用户许可证。 如果删除旧许可证，请保存帐户更改，添加新许可证，然后再次保存帐户设置，资源帐户可能不再按预期运行。 如果发生这种情况，建议为虚拟用户许可证创建新的资源帐户，并删除损坏的资源帐户。

## <a name="skype-for-business-server-2019"></a>Skype For Business Server 2019

有关托管在 Skype For Business Server 2019 上的资源帐户，这些帐户可用于云呼叫队列和云自动助理，请参阅[计划云呼叫队列](/SkypeforBusiness/hybrid/plan-call-queue)或 [Plan Cloud 自动助理](/SkypeForBusiness/hybrid/plan-cloud-auto-attendant)。 直接路由) 上托管的混合实现 (数字使用本地Skype for Business Server 2019 服务器上的 [New-CsHybridApplicationEndpoint](/powershell/module/skype/new-cshybridapplicationendpoint) cmdlet 进行配置。
...
> [!NOTE]
> 如果希望呼叫队列或自动助理可由 Skype For Business Server 2019 用户搜索，则应在 Skype For Business Server 2019 上创建资源帐户，因为联机资源帐户不会同步到 Active Directory。 当 sipfederationtls 的 DNS SRV 记录解析为 Skype for Business Server 2019 时，**必须** 使用 SfB Management shell 在 Skype For Business Server 2019 上创建资源帐户并同步到 Azure AD。
