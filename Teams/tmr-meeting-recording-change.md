...
ms.locfileid: "60769750"
---
# <a name="use-onedrive-for-business-and-sharepoint-or-stream-for-meeting-recordings"></a>将OneDrive for Business SharePoint流或流用于会议录制

> [!NOTE]
> 从经典流Teams到 OneDrive 和 SharePoint (ODSP) 会议录制的存储更改已在 2021 年 8 月 30 日完成。 现在，所有录制都存储在 ODSP 中。 此更改将覆盖 RecordingStorageMode 策略，并且修改 PowerShell 中的设置不再具有任何影响。

|日期&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;|事件&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;                                                                                                                                                                                                                                                                                                             |
|:-----------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|2020 年 10 月 5 日<br> *（完成）* &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;| 启用会议Teams将会议录制保存到OneDrive for Business SharePoint而不是Microsoft Stream (经典) |
|从 2021 年 1 月 7 日开始推出<br> *（完成）* &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;|所有Teams会议录制都将保存到 OneDrive for Business 和 SharePoint，除非你通过修改组织的 Teams 会议策略并显式将其设置为"流式传输"来延迟此 **更改**。 将策略报告视为 Stream 是不够的。 需要将策略值显式设置为 **Stream**。|
...
|从 2021 年 8 月 16 日开始以增量方式推出 <br> *（完成）* &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;|**所有客户（企业版、教育版和 GCC）**<br>无法将新的会议录制保存到Microsoft Stream (经典) ;所有客户都会自动将会议录制保存到OneDrive for Business SharePoint，即使他们将会议策略更改为Teams流式传输。<br><br> 为了更好地控制组织中的更改，建议客户在对更改感到满意时选择加入，而不是等待更改发生。 |
...
5. 使用[Set-CsTeamsMeetingPolicy](/powershell/module/skype/set-csteamsmeetingpolicy)设置Teams会议策略，以从流存储转换为OneDrive for Business SharePoint。
...
|频道会议                            |频道成员         |Teams SharePoint通道的位置。 **注意**：基于 IP 的限制不支持SharePoint录制上载到会议。 建议使用 [Azure 条件访问](/azure/active-directory/conditional-access/overview)。 |单击"录制"的成员对录制具有编辑权限。 <br /> <br />每个其他成员的权限都基于频道SharePoint权限。|
...
默认情况下，所有录制文件将转到OneDrive"录制"的用户 **的帐户。** 对于频道会议，录制内容将始终转到SharePoint的录制网站。 管理员不能更改录制内容存储位置。

**如何处理以前员工的录音？**

由于视频与 OneDrive for Business 和 SharePoint 中的其他任何文件一样，因此处理员工离职后的所有权和保留期将遵循[OneDrive for Business SharePoint过程](/onedrive/retention-and-deletion)。
...
视频将在视频播放器或OneDrive for Business SharePoint，具体取决于您访问文件的位置。
...
**如果用户没有存储空间或存储空间或OneDrive for Business，或者存储配额已满SharePoint将在哪里进行录制？**
