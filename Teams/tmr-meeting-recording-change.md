...
|从 2021 年 1 月 7 日开始推出<br> *（完成）* &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;|所有Teams会议录制都将保存到 OneDrive for Business 和 SharePoint，除非你通过修改组织的 Teams 会议策略并显式将其设置为流式传输来延迟此 **更改。** 将策略报告视为 Stream 是不够的。 需要将策略值显式设置为 **Stream**。|
...
5. 使用[Set-CsTeamsMeetingPolicy](/powershell/module/skype/set-csteamsmeetingpolicy)将 Teams 会议策略设置为从流存储转换到OneDrive for Business SharePoint。
...
你的视频将在视频播放器或OneDrive for Business SharePoint，具体取决于你访问文件的位置。
...
**如果用户没有存储或存储配额OneDrive for Business，SharePoint记录将在哪里？**
