...
[为什么在只筛选 Teams 时，我在 CQD 中看到了 Skype for Business 信息？](#why-am-i-seeing-skype-for-business-information-in-cqd-when-ive-filtered-for-teams-only)
...
> [!IMPORTANT]
> 从 2020 年 7 月 1 日开始，较旧的 CQD (CQD.lync.com) 使用来自最新 CQD (CQD 的数据。Teams.microsoft.com) 。 旧 CQD 数据不再可用，并且无法导出建筑物或报表数据。 你仍然可以使用 CQD.lync.com (Skype for Business 管理中心) ，但我们将很快关闭对 CQD.lync.com 的访问权限，因此你应该移动到 CQD。Teams.microsoft.com，可以这样做。


如果你尝试比较旧版 Skype for Business 门户 (cqd.lync.com) 中的旧 CQD 与 Teams 管理中心 (cqd.teams.microsoft.com) 中的最新 CQD 之间的数据，你很快就会注意到数据不匹配。 这是因为最新的 CQD 报告有关许多其他调用方案。 如果你仍在使用来自旧版 CQD 的报告，请使用本文来帮助你解释这些报告 [：Skype for Business Server](/skypeforbusiness/management-tools/call-quality-dashboard/call-quality-dashboard)的呼叫质量仪表板。
...
### <a name="why-am-i-seeing-skype-for-business-information-in-cqd-when-ive-filtered-for-teams-only"></a>为什么在只筛选 Teams 时，我在 CQD 中看到了 Skype for Business 信息？
...
根据客户的方案，如果 SFB 2019 用于数据连接器) ，则 CQDv3 将包括 SFB 2019 本地呼叫 (、Skype 机器人呼叫 (AA、CVI、VDI) 、实时事件和 PSTN 呼叫。 客户可用的方案/功能，但其数据不在 CQD V2 中。
