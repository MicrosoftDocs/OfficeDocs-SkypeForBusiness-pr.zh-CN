...
获取电话系统[-](../teams-add-on-licensing/virtual-user.md)虚拟用户许可证（如果还没有）。 为计划设置的每个呼叫队列和自动助理获取一个。 这些许可证是免费的，因此我们建议获取一些额外的许可证，以防你决定在将来对设置进行更改。
...
2. 选择 **想要使用的** 路由方法。 这会确定代理从队列接收调用的顺序。 我们建议使用 **串行路由或****轮循机制**。 从以下选项中进行选择：
    ...
    - **最长空闲** 时间将每次调用路由到空闲时间最长的代理。  (状态为"离开"超过 10 分钟的代理未包含在内。) 
