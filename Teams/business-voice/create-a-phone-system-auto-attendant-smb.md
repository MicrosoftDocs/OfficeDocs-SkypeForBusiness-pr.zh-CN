...
从组织外部直接拨号，获取自动助理所需的服务号码。 这可能包括[转移来自其他提供商的号码或](../phone-number-calling-plans/transfer-phone-numbers-to-teams.md)[请求新的服务号码](../getting-service-phone-numbers.md)。

获取 [计划创建的](../teams-add-on-licensing/virtual-user.md) 每个自动助理的电话系统 - 虚拟用户许可证。 这些许可证是免费的，因此我们建议额外获取一些许可证，以防你决定在将来更改设置。
...
3. 在"**添加资源帐户"** 窗格中，填写 **显示名称**、**用户名**，然后选择"自动助理 **"作为资源****帐户类型**
...
如果需要通过电话号码访问此自动助理，则向资源帐户分配该号码。

1. 在 Teams 管理中心的"资源帐户"页上，选择要为其分配服务编号的资源帐户，然后单击"分配 **/取消分配"。**
...
1. 在 Teams 管理中心，展开 **"语音**"，单击 **"自动助理**"，然后单击"添加 **"。**

2. 在顶部的框中键入自动助理的名称。

3. 如果要指定一个运算符，请指定调用该运算符的目标。 这是可选 (，但建议) 。 你可以设置 **"接线员** "选项，允许呼叫者离开菜单，与指定的人员通话。

4. 指定此自动助理的时区。 如果为营业时间创建单独的呼叫流，则时区用于计算营业时间。

5. 为此自动助理指定语言。 此语言将用于系统生成的语音提示。

6. 选择是否要启用语音输入。 启用后，每个菜单选项的名称将成为语音识别关键字。 例如，呼叫者可以说"一个"来选择映射到键 1 的菜单选项，也可以说"销售"以选择名为"销售"的菜单选项。
...
# <a name="step-3brcall-flow"></a>[步骤 3 <br> 调用流](#tab/call-flow)
...
3. 如果希望呼叫者使用拨号键进行导航，然后在"设置菜单选项"下，选择当呼叫者按拨号键时要发生的情况。  (如果将此自动助理创建为公司目录，将拨号键选项留空。) 
    ...
    - **接线** 员 - 为自动助理定义的接线员。 定义运算符是可选的。 可以将运算符定义为此列表中的任何其他目标。
    ...
    - **语音** 命令 - 定义呼叫者为访问此选项而提供的声音命令（如果启用了语音输入）。 它可以包含多个单词，例如"客户服务"或"运营与企业"。 
...
4. 如果要使用此自动助理作为公司目录，请在"目录搜索"下选择 **"按名称拨号"。**  启用此选项时，呼叫者可以说出用户的姓名或在电话键盘上键入用户名。 任何具有电话系统许可证的联机用户都是符合条件的用户，可以使用"按名字拨叫"进行找到。 

     (可以选择"按分机 **拨叫**"，但必须在 Azure Active Directory.) 
...
# <a name="step-4brafter-hours"></a>[步骤 <br> 4：数小时后](#tab/after-hours)

可以针对每个自动助理设置营业时间。 如果没有设置营业时间，所有日期以及每天的所有时间均视为营业时间，因为默认情况下设置为全天候时间表。 可以设置营业时间，同时设置一天中的休息时间，所有未设置为营业时间的营业时间均视为非营业时间。 您可以为非营业时间设置不同的传入呼叫处理选项和问候语。

根据自动助理和呼叫队列的配置方式，可能只需为具有直接电话号码的自动助理指定非营业时间呼叫路由。

如果希望为非营业时间呼叫者单独路由呼叫，请指定每天的营业时间。 例如 **，单击"** 添加新时间"以指定给定天的多个小时集，以指定午餐休息时间。

!["工作时间"和"时间设置"的屏幕截图](../media/auto-attendant-business-hours.png)

指定营业时间后，请选择数小时的呼叫路由选项。 提供的选项与在步骤 **3**- 呼叫流中指定的营业时间呼叫路由相同。
...
你可以让呼叫自动助理在节假日与其他日期以不同方式路由。  (如果不希望对假日使用不同的呼叫流，可以跳过此步骤。) 



自动助理可以针对你设置的每个假日设置呼叫流。 可以为每个自动助理添加最多 20 个计划假日。
...
拨号 *范围* 定义当呼叫者使用按名字拨叫或按分机拨叫时，哪些用户在目录中可用。 默认情况下 **，"所有联机用户** "包括组织中具有电话系统许可证的联机用户。
...
1. 单击 **"** 添加帐户"，然后搜索要添加的帐户。 单击 **"添加**"，然后单击"**添加"。**
