...
可以使用 Microsoft Teams 配置设计器Surface Hub管理Windows上的Microsoft Intune设置Microsoft Endpoint Manager。 若要Windows设置，Microsoft Intune配置设计器或Teams知识。 有关这些选项的详细信息，请参阅以下文章：
...
Windows如果只有几个设备，并且可以轻松访问Surface Hub设计器是一个不错的选择。 如果有许多 Surface Hub，或者它们位于远程位置，请使用 Microsoft Intune Microsoft Endpoint Manager（如果已部署在你的组织中）。 无论选择哪种方法，都需要创建 XML 配置文件，以更改Teams设置Surface Hub。
...
| `<SurfaceHubSettings>`  | `<BluetoothAdvertisementEnabled>`         |           | 确定Surface Hub是否播发它可用于蓝牙连接。<br>接受的值 `true` ：、 `false`                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `<CoordinatedMeetings>` | `<TrustedAccounts>`                       |           | 这是每个 Teams 会议室设备或设备的 UPN 的逗号分隔列表Surface Hub设备应接受会议加入请求，或者应发送到哪些会议加入请求。<br>接受的值：字符串                                                                                                                                                                                                                                                                                                                         |
| `<CoordinatedMeetings>` | `<Settings>`                              |           | 包含协调会议的配置音频和视频配置元素                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `<Settings>`            | `<Audio>`                                 |           | 控制视频Teams的音频Surface Hub。                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
使用 Teams 配置设计器Surface Hub或Windows在 Microsoft Endpoint Manager Microsoft Intune 中应用或更新 Microsoft Endpoint Manager。
...
> [!IMPORTANT]
> 如果已使用预配包Teams配置Surface Hub，并想要更改它，则需要先删除现有的预配包。 有关详细信息，请参阅[删除配置设计器](#remove-a-provisioning-package-created-by-windows-configuration-designer)创建的Windows包。
...
3. 在Surface Hub上，打开"开始"菜单，选择"**所有应用**"，**然后选择"设置**
...
如果你的 Surface Hub 是使用 Microsoft 终结点Microsoft Intune管理中的设置管理的，则你可以使用它Teams Surface Hub 应用这些设置。 将创建新的配置文件，然后将上面创建的 XML 文件粘贴到该文件中。

> [!IMPORTANT]
> Surface Hub 需在设备组中，以便Microsoft Intune标识要应用配置文件的设备。 若要了解如何创建设备组，请参阅添加 [组以组织用户和设备](/mem/intune/fundamentals/groups-add)。

执行下列操作以创建配置文件，以将Teams应用到 Surface Hub：
...
如果使用 Teams 配置Surface Hub创建的预配包向 Windows 应用了该设置，请使用以下步骤删除该包及其设置：

1. 在Surface Hub上，打开"开始"菜单，选择"**所有应用**"，**然后选择"设置**
...
删除包含会议协调会议设置的配置文件后Surface Hub，使用以下步骤重置Teams应用程序Surface Hub：
