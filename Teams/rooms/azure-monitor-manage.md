...
当应用在事件日志中记录Microsoft Teams 会议室信息时，每隔五分钟将以下事件说明插入Windows说明。 该Microsoft Monitoring Agent这些记录传递给 Azure Monitor 中的 Log Analytics，它将这些记录分析到使用 Azure Monitor 部署监视Microsoft Teams 会议室[仪表板中创建的](azure-monitor-deploy.md)仪表板。 使用仪表板，可根据需要查看各个日志条目，以确定操作步骤。
