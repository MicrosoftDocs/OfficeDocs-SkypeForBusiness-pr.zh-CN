...
在创建 Microsoft 365 资源帐户之前，请检查它需要哪种类型的许可证：

- **Teams 会议** 如果要将资源帐户与共享设备（例如 Microsoft Teams 会议室或 Teams 显示器）与热桌面相关联，并使用它加入 Teams 会议，以便与会者可以使用它通过它呈现视频和音频，则需要会议室许可证。 有关会议室许可的详细信息，请参阅 [Teams 会议室许可](rooms-licensing.md)。

- **PSTN 调用** 如果希望资源 (调用公共交换电话网络或 PSTN 呼叫) 拨打或从外部电话号码拨打或接听呼叫，则需要 Microsoft 365 电话系统或Microsoft 365 商务语音许可证。 只需在以下概述中完成步骤 1。 然后，有关详细信息，请参阅 [Microsoft Teams 加载项许可证](../teams-add-on-licensing/microsoft-teams-add-on-licensing.md) 。

- 如果仅使用资源帐户来预订资源&mdash;，请邀请资源加入会议，并让其自动接受或拒绝不需要向资源帐户分配许可证的邀请&mdash;，并且只需在以下概述中完成步骤 1。  

## <a name="overview"></a>概述

**步骤 1 -** [创建新的资源帐户](#create-a-resource-account)。 或者，如果会议室邮箱已存在，并且要将其转换为资源帐户，则可以 [修改现有的 Exchange 会议室邮箱](?tabs=existing-account#create-a-resource-account)。

**步骤 2 -**  然后，为 Teams 会议 [配置帐户](#configure-mailbox-properties) 。

**步骤 3 -**  如果资源帐户要与共享设备关联，例如 Teams 显示热桌面， [请关闭密码过期](#turn-off-password-expiration)。

**步骤 4 -**  最后， [分配会议室许可证](#assign-a-meeting-room-license) ，以便帐户可以访问 Microsoft Teams。
...
若要更改资源邮箱的设置，请参阅 [配置邮箱属性](#configure-mailbox-properties) 或使用 Exchange 管理中心。
...
如果你不在 Exchange 混合配置中，则可以继续执行下一步， [配置邮箱属性](#configure-mailbox-properties)。
...
>
> 仅当资源帐户适用于 Windows 上的Teams 会议室、Android 上的Teams 会议室、Surface Hub 上的Teams 会议室或具有热桌面的 Teams 显示时，才继续执行下一部分。

## <a name="configure-mailbox-properties"></a>配置邮箱属性

在 Exchange PowerShell（联机或本地）中，在会议室邮箱上配置以下设置以改善会议体验：
...
3. 然后，用于`Get-AzureADSubscribedSku`检索 Microsoft 365 或Office 365组织的可用 SKU 列表。
...
2.  使用 `Set-MsolUser` cmdlet 将使用位置分配给资源帐户。 这决定了哪些许可证 SKU 可用。
    ...
    然后，可以使用`Get-MsolAccountSku`它来检索 Microsoft 365 或Office 365组织的可用 SKU 列表。
...
若要验证帐户创建和许可证分配，请使用创建的帐户登录到任何 Teams 客户端。
