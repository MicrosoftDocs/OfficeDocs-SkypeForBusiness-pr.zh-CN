...
description: 阅读本主题，了解如何使用 Microsoft Teams 会议室 或 Microsoft 365 部署Office 365，其中Teams或Skype for Business Exchange联机。
...
阅读本主题，了解如何使用 Microsoft Teams 会议室 或 Microsoft 365 Office 365 部署 Microsoft Teams 或 Skype for Business Exchange 联机。
...
使用 Microsoft Teams 会议室 或 Microsoft 365 部署Office 365之前，请确保已满足要求。 有关详细信息，请参阅Microsoft Teams 会议室[要求](requirements.md)。

若要Skype for Business，必须具有以下项：

- Skype for Business Online (计划 2，或Enterprise计划或) 计划Microsoft 365或Office 365计划。 该计划需要允许电话拨入式会议功能。
...
2. 在 Exchange Online PowerShell 中，创建新的会议室邮箱或修改现有的会议室邮箱。 默认情况下，会议室邮箱没有关联的帐户，因此创建或修改会议室邮箱时，需要添加帐户，以便使用 Skype 会议室系统 v2 进行身份验证。
...
4. 连接 MS Online PowerShell，通过 `Connect-MsolService -Credential $cred` 运行PowerShell cmdlet。 有关 Active Directory 的详细信息，请参阅[Azure ActiveDirectory (MSOnline) 1.0。](/powershell/azure/active-directory/overview?view=azureadps-1.0)
...
6. 设备帐户需要具有有效的Microsoft 365或Office 365许可证，Exchange Microsoft Teams Skype for Business无效。 如果你有许可证，则需为设备帐户分配使用位置—此位置确定可供帐户使用的许可证 SKU。 可以使用 `Get-MsolAccountSku` <!-- Get-AzureADSubscribedSku --> 检索组织或组织的可用 SKUS Microsoft 365 Office 365，如下所示：
...
7. 若要在本地启用设备帐户Skype for Business帐户，请确保环境满足以下要求中[Microsoft Teams 会议室要求](requirements.md)。
