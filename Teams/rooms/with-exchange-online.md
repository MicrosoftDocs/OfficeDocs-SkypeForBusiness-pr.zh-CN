...
description: 请阅读本主题，了解如何使用 Microsoft Teams 会议室 部署Exchange Online。
ms.openlocfilehash: e6eb3253d7edb999ba74d28ef9a6d8ae835ac16d
ms.sourcegitcommit: 8f999bd2e20f177c6c6d8b174ededbff43ff5076
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 01/15/2022
ms.locfileid: "62055482"
---
# <a name="deploy-microsoft-teams-rooms-with-exchange-online"></a>使用 Exchange Online 部署 Microsoft Teams Rooms

请阅读本主题，了解如何使用 Microsoft Teams 会议室 部署Exchange Online。
  
如果组织混合了一些服务，其中一些托管在本地，一些托管联机，则配置将取决于每个服务的托管位置。 本主题介绍使用联机托管Microsoft Teams 会议室的Exchange混合部署。 由于这种类型的部署存在许多不同的变体，因此无法提供所有这些变体的详细说明。 以下过程适用于许多配置。 如果该过程不适用于你的设置，我们建议使用 Windows PowerShell实现此处介绍的相同最终结果，并用于其他部署选项。
...
若要使用 Microsoft Teams 会议室 部署Exchange Online，请执行以下步骤。 请确保拥有运行 cmdlet 所需的正确权限。 
   ...
   > [!NOTE]
   >  本部分[Windows PowerShell Azure Active Directory cmdlet](/powershell/azure/active-directory/overview?view=azureadps-1.0)的 Azure Active Directory 模块 (例如，Set-MsolUser) 已在为 Microsoft Teams 会议室 设置帐户中进行了测试。 其他 cmdlet 可能正常工作，但是，它们尚未在此特定方案中进行测试。
...
2. 将显示名称 ( - ) cmdlet (Set-Mailbox 或 New-Mailbox) 中的"名称- 标识"键入到"全名"框中，将别名键入到"用户 **登录** 名称"框中。 单击" **下一步**"。
