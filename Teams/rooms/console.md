...
仅当已创建和测试必要的 Microsoft 团队或 Skype for business 和 Exchange 帐户时，才应执行这些步骤，如[部署 Microsoft 团队聊天室](rooms-deploy.md)中所述。 你将需要[Microsoft 团队会议室要求](requirements.md)中所述的硬件和软件。 本主题包括以下部分：
...
> [!NOTE]
> Microsoft 团队聊天室仅适用于正确配置的 Microsoft 团队或 Skype for business 环境，在此环境中，设备帐户按照[部署 Microsoft 团队聊天室](rooms-deploy.md)中的说明正确设置。
...
> [!IMPORTANT]
> 用于创建 Microsoft 团队聊天室安装媒体的 Windows 10 计算机必须位于与目标安装媒体相同或更高版本的 Windows 上。
...
2. 将控制台置于管理员模式下（请参阅[管理员模式和设备管理](rooms-operations.md#AdminMode)）。
...
你可以将 Microsoft 团队聊天室控制台加入到你的域。 Microsoft 团队聊天室控制台应放置在电脑工作站的单独 OU 中，因为许多工作站策略与 Microsoft 团队聊天室不兼容。 常见的示例是密码强制策略，可防止 Microsoft 团队聊天室自动启动。 有关 GPO 设置管理的信息，请参阅[管理 Microsoft 团队聊天室](rooms-operations.md)。
  
### <a name="to-join-microsoft-teams-rooms-to-a-domain"></a>将 Microsoft 团队会议室加入域

1. 从管理员帐户登录到控制台（请参阅[管理员模式和设备管理](rooms-operations.md#AdminMode)）。
