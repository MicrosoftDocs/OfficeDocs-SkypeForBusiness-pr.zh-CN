...
Microsoft Teams 会议室设备支持一键式体验，用于加入第三方在线会议，也称为直接来宾加入。 启用后，可以使用 Teams 会议室设备加入 Cisco WebEx 和 Zoom 上托管的会议，就像加入 Microsoft Teams 中托管的会议一样简单。

在从 Teams 会议室设备加入第三方会议之前，需要执行以下操作：

1. 配置 Teams 会议室设备的 Exchange Online 会议室邮箱，处理第三方会议的邀请。
2. 请确保您的组织没有任何阻止您连接到第三方会议服务的策略。
...
若要启用一触式加入体验，会议邀请中需要显示并阅读来自第三方会议的会议加入链接信息。 如果您的组织使用 [Office 365](https://docs.microsoft.com/microsoft-365/security/office-365-security/atp-safe-links) 高级威胁防护安全链接功能，或者如果您使用第三方解决方案扫描所有传入和传出 URL 中的威胁，则可能会更改会议加入 URL，使 Teams 会议室设备无法识别会议。 若要确保不会发生这种情况，需要将第三方会议服务的 URL 添加到 ATP 安全链接"不重写"列表或第三方 URL 重写异常列表。
...
3. 转到"会议 **"** 选项卡，选择 **Cisco WebEx** 和 **/或缩放**。
