...
---
<a name="powershell-script-sample---create-a-company-wide-team-in-microsoft-teams"></a><span data-ttu-id="58cb6-103">PowerShell 脚本示例 - 在 Microsoft Teams 中创建公司范围的团队</span><span class="sxs-lookup"><span data-stu-id="58cb6-103">PowerShell Script Sample - Create a company-wide team in Microsoft Teams</span></span>
-------------------------------------------------------------------------

<span data-ttu-id="58cb6-104">此 PowerShell 脚本用于在 Teams 中创建公司范围的公共团队。</span><span class="sxs-lookup"><span data-stu-id="58cb6-104">This PowerShell script creates a public, company-wide team in Teams.</span></span> <span data-ttu-id="58cb6-105">该脚本使用 MicrosoftTeams PowerShell 模块（当前在 beta 版中提供）创建公司范围的公共团队。</span><span class="sxs-lookup"><span data-stu-id="58cb6-105">The script uses the MicrosoftTeams PowerShell module (currently in beta) to create a public, company-wide team.</span></span> <span data-ttu-id="58cb6-106">它还使用 AzureAD PowerShell 模块提取你的租户中的用户列表。</span><span class="sxs-lookup"><span data-stu-id="58cb6-106">It also uses the AzureAD PowerShell module to fetch the list of users in your tenant.</span></span> 

<span data-ttu-id="58cb6-107">有关使用此 PowerShell 脚本的完整说明，请查看我们的教程[使用 PowerShell 在 Teams 中创建公司范围的团队](../Company-wide-team-creation-powershell.yml)。</span><span class="sxs-lookup"><span data-stu-id="58cb6-107">For complete instructions on using this PowerShell script, check out our tutorial, [Create a company-wide team in Teams using PowerShell](../Company-wide-team-creation-powershell.yml).</span></span>

<span data-ttu-id="58cb6-108">如果你不了解 PowerShell 并需要入门帮助，请参阅 [Azure PowerShell 概述](https://docs.microsoft.com/powershell/azure/overview?view=azurermps-5.1.1)。</span><span class="sxs-lookup"><span data-stu-id="58cb6-108">If you're new to PowerShell and need help getting started, see [Overview of Azure PowerShell](https://docs.microsoft.com/powershell/azure/overview?view=azurermps-5.1.1).</span></span>


## <a name="sample-script"></a><span data-ttu-id="58cb6-109">示例脚本</span><span class="sxs-lookup"><span data-stu-id="58cb6-109">Sample script</span></span>

> [!TIP]
> <span data-ttu-id="58cb6-110">PowerShell 脚本文件是扩展名为 .PS1 的文本文件。</span><span class="sxs-lookup"><span data-stu-id="58cb6-110">A PowerShell script file is a text file with a .PS1 extension.</span></span> <span data-ttu-id="58cb6-111">要使用此脚本，请单击“**复制**”，然后将代码粘贴到文本文件中。</span><span class="sxs-lookup"><span data-stu-id="58cb6-111">To use this script, click **Copy** and then paste the code into a text file.</span></span> <span data-ttu-id="58cb6-112">使用文件名 CreateCompanyWideTeam.ps1 保存该文件，你即得到一个 PowerShell 脚本。</span><span class="sxs-lookup"><span data-stu-id="58cb6-112">Save it with the filename CreateCompanyWideTeam.ps1, and you've got a PowerShell script.</span></span>
...
## <a name="script-explanation"></a><span data-ttu-id="58cb6-113">脚本说明</span><span class="sxs-lookup"><span data-stu-id="58cb6-113">Script explanation</span></span>


<span data-ttu-id="58cb6-114">有关使用此 PowerShell 脚本的完整说明，请查看我们的教程[使用 PowerShell 在 Teams 中创建公司范围的团队](../Company-wide-team-creation-powershell.yml)</span><span class="sxs-lookup"><span data-stu-id="58cb6-114">For complete instructions on using this PowerShell script, check out our tutorial, [Create a company-wide team in Teams using PowerShell](../Company-wide-team-creation-powershell.yml)</span></span>

<span data-ttu-id="58cb6-115">此脚本有 5 个部分（按从上到下的顺序列出）：</span><span class="sxs-lookup"><span data-stu-id="58cb6-115">There are 5 sections to this script (listed in order from top to bottom):</span></span>
1. <span data-ttu-id="58cb6-116">**文档和变量定义**</span><span class="sxs-lookup"><span data-stu-id="58cb6-116">**Documentation and variable definitions**</span></span>
2. <span data-ttu-id="58cb6-117">**Function Create-Team**：创建团队（如果需要）。</span><span class="sxs-lookup"><span data-stu-id="58cb6-117">**Function Create-Team**: Creates a team if necessary.</span></span> <span data-ttu-id="58cb6-118">如果你未指定 GroupID，则此部分将使用指定的 TeamName 和 TeamDescription 创建团队。</span><span class="sxs-lookup"><span data-stu-id="58cb6-118">If you don't specify a GroupID, this section will create the team with the specified TeamName and TeamDescription.</span></span>
3. <span data-ttu-id="58cb6-119">**Function Add-Channels**：如果你指定了频道，则在此提供频道说明，然后创建频道。</span><span class="sxs-lookup"><span data-stu-id="58cb6-119">**Function Add-Channels**: If you've specified channels, this is where the channel description gets requested, and then the channel gets created.</span></span> 
4. <span data-ttu-id="58cb6-120">**Function Add-Members**：在此部分进行所有成员管理。</span><span class="sxs-lookup"><span data-stu-id="58cb6-120">**Function Add-Members**: This does all member management.</span></span> <span data-ttu-id="58cb6-121">此部分连接到 AzureAD、提取一组用户，然后将其添加到团队（如果你未提供成员列表）。</span><span class="sxs-lookup"><span data-stu-id="58cb6-121">This section connects to AzureAD,  fetches a set of users, and adds them to the team if you don’t provide a member list.</span></span> <span data-ttu-id="58cb6-122">此部分处理添加所有者和成员以及不添加已属于团队的成员的逻辑。</span><span class="sxs-lookup"><span data-stu-id="58cb6-122">It handles the logic of adding owners and members, and not adding members who already belong to the team.</span></span> 
5. <span data-ttu-id="58cb6-123">**Main program**：这是各功能的执行顺序。</span><span class="sxs-lookup"><span data-stu-id="58cb6-123">**Main program**: This is the order in which the functions get executed.</span></span> 
