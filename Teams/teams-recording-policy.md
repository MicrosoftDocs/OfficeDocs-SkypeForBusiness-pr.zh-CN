---
title: 有关调用&会议的基于 Teams 策略的录制简介
author: cabailey
ms.author: cabailey
manager: laurawi
ms.date: 05/11/2020
...
description: 了解用于调用&会议的基于 Teams 策略的录制
f1.keywords:
- CSH
ms.custom:
- Adopt
...
ms.locfileid: "66915210"
---
# <a name="introduction-to-teams-policy-based-recording-for-callings--meetings"></a>有关通话&会议的基于 Teams 策略的录制简介

通过基于策略的录制，采用 Microsoft Teams 电话和会议的组织可以使用管理策略规定，当应根据相关公司或法规策略的要求自动记录和捕获呼叫和联机会议以供后续处理和保留时。

增强了 Teams，以支持第三方录制解决方案的集成，包括平台功能、用户体验和管理接口，这些解决方案需要提供端到端解决方案来配置、管理、录制、存储和分析 Teams 通信。 增强功能包括通信平台 API 和用于录制的事件，其中提供：

- 跨设备和音频、视频、屏幕共享和聊天的所有受支持的终结点进行无缝、高质量的媒体捕获。

- 支持 Teams 用户之间的交互捕获，以及支持在 Teams、Teams 移动、Skype for Business、PSTN (调用终结点) 

- 用于合规性记录的新管理策略，包括与现有 Teams 管理呼叫和会议工具和策略的集成

可以在Microsoft 365 A3/A5/E3/E5/商业高级版和Office 365 A3/A5/E3/E5 用户上启用合规性记录。 

合规性记录解决方案集成功能也在 Ignite 2019 [的合规性录制和 Microsoft Teams 会话](https://myignite.microsoft.com/archives/IG19-VCE40)中进行了评审。
...
| 类型                   | 方便 (常规 Teams 录制)  | 组织 - 受监管的 (合规性记录)  |
| ---------------------- | ------------------ | --------------- |
| 引发              | 用户               | 管理员 (系统)   |
| 目标                 | 按呼叫/会议 | 按用户        |
| 存储所有者          | 用户               | 合规性      |
...
Teams 提供各种功能 [，](./cloud-recording.md) 方便和功能性地记录会议和直播活动。 组织记录意味着使采用 Teams 电话和会议的组织能够通过管理策略规定呼叫和联机会议应根据相关公司或法规策略的要求自动记录和捕获呼叫和联机会议以供后续处理和保留。 此策略下的用户会意识到，他们与 Teams 的数字交互正在录制中，但无法禁用录制，并且在交互完成后将无法访问录制。 记录成为可用于电子数据展示、法律保留和其他公司保留用途的合规性和法律人员的组织存档的一部分。
...
<td>通信管理员</td>
<td><ul>
<li><p>了解为何以及如何向 Teams 用户/终结点应用/强制执行录制策略。</p></li>
<li><p>为组织配置和维护 Teams 录制策略。</p></li>
<li><p>监视和排查 Teams 呼叫和会议的录制相关问题。</p></li>
...
<td>合规性官员</td>
<td><ul>
<li><p>按照在适当的区域边界中履行合规性义务所需的方式收集所有 Teams 通信。</p></li>
...
符合性记录解决方案与 Teams 集成，如下图所示：

> [!div class="mx-imgBorder"]
> ![显示团队自定义应用设置的屏幕截图。](media/hp-compliance-recording-for-teams-calling-and-meetings.jpg "图像显示发送和接收 Teams 会议或呼叫时的流。")

> [!NOTE]
> 此解决方案专门用于在 Teams 中启用基于策略的合规性记录。 不支持此解决方案的任何其他使用。

## <a name="recorder"></a>录音机

合规性记录解决方案的核心组件是记录器。
记录器构建为基于 Azure 的可缩放服务， (机器人) [使用 Microsoft 的通信平台](/graph/cloud-communications-concept-overview) ，并在 Microsoft Graph 中注册为应用程序。 该记录器提供与 Teams 呼叫和会议 [通信平台 API](/graph/api/resources/communications-api-overview) 的直接交互，并为媒体引入提供终结点。
...
记录器服务的确切实现因合作伙伴而异，但必须设计为支持多个记录器，以便实现部署的高可用性和地理分布，以减少从 Teams 到记录器的延迟。 此外，预计记录器本身的设计考虑到复原能力和冗余。

合作伙伴在提交解决方案进行认证之前，必须先确认 Microsoft Graph 通信 API 和 SDK 的最低必需版本，以确保支持合规性记录集成的所有要求。
...
Azure 和 Windows VM 要求仅适用于 Teams 机器人组件，这意味着合作伙伴可以实现所选平台的其余部分，前提是他们可以满足合规性录制的相关性能和功能要求。
...
使用 Teams 客户端体验启用对通知的支持。 体验可以是视觉对象或音频。
...
- SIP 手机
- Skype for Business
- 音频会议 (电话拨入号码或用户选择的语言) 中的音频通知
- PSTN 呼叫者 (Teams 用户的默认语言) 中的音频通知

> [!NOTE]
> 会议模式呼叫队列不支持合规性录制。 请使用传输模式呼叫队列。
> 如果用户遇到 Internet 中断，并且正在使用 SBA 发出和接收 PSTN 呼叫，则合规性录制将不起作用。

## <a name="compliance-recording-for-teams-certification-programs"></a>Teams 认证程序的合规性记录

除了发布允许合作伙伴开发 CCaaS 解决方案和将 CCaaS 解决方案与 Teams 集成的公开可用 API 外，我们还开发了 Microsoft Teams 认证计划的合规性记录，为客户提供保证，确保每个参与合作伙伴的解决方案都经过测试和验证，以提供他们期望从 Microsoft 解决方案获得的质量、兼容性和可靠性。  

以下合作伙伴已为 Microsoft Teams 认证其解决方案。<br/><br/>
...
<br/>
以下合作伙伴正在为 Microsoft Teams 认证其解决方案。<br/><br/>
