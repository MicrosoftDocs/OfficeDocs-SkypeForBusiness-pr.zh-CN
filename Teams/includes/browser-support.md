Teams 完全支持以下 Internet 浏览器，存在特别提出的通话和会议例外情况。 此表适用于在台式计算机上运行的操作系统。 
...
|Microsoft Edge RS2 或更高版本     |完全支持，但无传出共享<sup>3</sup>         |完全支持，不排除传出共享         |
...
<sup>2</sup> 浏览器上的 Teams 会议仅限于单个流;当前扬声器的传入视频源或屏幕共享。

<sup>3</sup> Edge RS2 或更高版本不支持通过 HTTP 代理发送实时音频和视频流量。
...
>
> 在移动设备上，建议使用 Teams 应用。 Teams 应用可从 Android 和 iOS 商店获得。
