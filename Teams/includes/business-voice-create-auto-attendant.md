...
需要为每个自动助理分配一个电话系统 - 虚拟用户许可证。 购买 Business Voice 时，还收到许多电话系统 - 虚拟用户许可证，因此可能不需要请求更多。 但是，如果将来需要更多信息，可以按照 电话系统 - 虚拟用户许可证 中的说明获取[它们](../teams-add-on-licensing/virtual-user.md)。
...
4. 如果要使用此自动助理作为公司目录，请在"目录搜索"下，选择"**按名称拨号"。** 启用此选项后，呼叫者可以说出用户的姓名或在电话键盘上键入用户名。 任何具有"按电话系统在线用户都是符合条件的用户，可以使用"按姓名拨叫"找到。 
