...
若要避免不恰当或误导性的名称，或只是为了提供团队命名方式的更多结构，可以使用当前处于预览状态Microsoft 365) 组 (策略：

-   **前缀后缀命名策略** 可以使用前缀或后缀来定义团队和组 (的) 约定，例如 **GRP_US_My Group_Engineering。** 前缀和后缀可以是固定字符串或用户属性 (例如 **[部门]**) ，根据创建团队的用户添加到名称中。
-   **自定义阻止字词** 可以上传一组字词，阻止特定组织的用户使用他们创建的团队的名称。 例如，可以阻止 **将术语 CEO**、 **工资** 和 **人力资源** 用于不应用于的组的团队名称中。
...
> [!Note]
> 如果团队是使用另一个系统 (（例如 学校数据同步) ）的输入自动创建的，请验证输入数据是否符合配置的命名策略;如果没有，团队创建会失败。

## <a name="can-i-see-who-created-a-team"></a>我能否查看团队的创建人？

若要了解创建特定团队的人，请参阅在 审核日志 [中搜索Microsoft Teams](audit-log-events.md)。
...
-   学校数据同步为 Exchange Online SharePoint Online 创建 Microsoft 365 组、Microsoft Teams 和 OneNote 课堂笔记本、Intune for Education 学校组、名单和单一登录 (SSO) 许多其他第三方应用程序集成。 有关详细信息，请[浏览学校数据同步](/schooldatasync/overview-of-school-data-sync)。
-   使用 PowerShell 可以创建团队和频道，并自动配置设置。 有关详细信息[，Microsoft Teams PowerShell](/powershell/module/teams/?view=teams-ps)。
-   可以使用 Microsoft Graph API (beta 版) 创建、配置、克隆和存档团队。 有关详细信息，请参阅使用 [Microsoft Graph API 处理](/graph/api/resources/teams-api-overview) Microsoft Teams。
...
建议你首先考虑在学校Teams或季度结束后如何处理这些学生的数据：是删除数据，还是使学生即使在完成课程后也能够使用这些数据。 你需要记住学校日历，这样你设置的任何策略都不要与假日冲突。 可以使用以下工具实现策略：

-   **保留策略：** 使用此功能删除超过指定年龄的所有数据，以确保从聊天中删除旧数据， (或部分用户的聊天) 频道。 还可以将Teams配置为保留内容，以便无法将其删除。 有关详细信息，请参阅[适用于该策略的保留Microsoft Teams](https://techcommunity.microsoft.com/t5/Microsoft-Teams-Blog/Retention-policies-for-Microsoft-Teams/ba-p/178011)。
...
> [!IMPORTANT]
> 使用Microsoft 365组过期策略Azure Active Directory Premium P1一个或多个组的成员的每个唯一用户分配Microsoft 365许可证。

## <a name="are-there-team-templates-for-my-faculty-members-to-use-when-creating-a-team"></a>创建团队时，是否有供教职员工使用的团队模板？

是。 创建新团队时，用户可以从现有模板中选择"创建团队"，Teams 所有者也可使用 [Graph API (beta) ](/graph/api/resources/teams-api-overview) 从可用模板创建新团队。

## <a name="what-tasks-can-i-automate-via-powershell-or-graph"></a>可以通过 PowerShell 或 powerShell 自动执行哪些Graph？
...
有关详细信息，请参阅组[Microsoft 365访问](https://support.office.com/article/Guest-access-in-Office-365-Groups-bfc7a840-868f-4fd6-a390-f347bf51aff6#PickTab=Manage)。
...
有关详细信息，请参阅在审核日志[中搜索Microsoft Teams](audit-log-events.md)。

## <a name="teams-evolves-so-quickly-how-can-i-stay-up-to-date"></a>Teams发展如此迅速。 如何保持最新状态？

我们建议使用以下资源获取最新Teams：

-   [Microsoft Teams](https://support.office.com/article/What-s-new-in-Microsoft-Teams-d7092a6d-c896-424c-b362-a472d5f105de)
-   [Microsoft Teams博客](https://techcommunity.microsoft.com/t5/Microsoft-Teams-Blog/bg-p/MicrosoftTeamsBlog)