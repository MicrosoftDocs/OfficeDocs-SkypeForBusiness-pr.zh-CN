---
title: 适用于 IT 管理员的 Microsoft 教育常见问题解答
author: SerdarSoysal
ms.author: serdars
manager: serdars
ms.topic: article
...
---
# <a name="microsoft-education-faq-for-it-admins"></a>适用于 IT 管理员的 Microsoft 教育常见问题解答
...
一般情况下，我们建议不要阻止任何人创建团队。 如果每个人都可以创建团队，则 Teams 更有可能被广泛采用。 教职员工、教师或学生可以使用 Teams 创建学习组或特殊兴趣组。 此功能可帮助在教室内外接受 Teams。
...
每学期或四分之一开始，你将需要新的团队。 采取自动方法自动创建这些团队、使用正确的用户填充这些团队并设置正确的权限可能是有意义的：

- 学校数据同步可以为 Exchange Online 和 SharePoint Online 创建Microsoft 365 组、Microsoft Teams 和 OneNote 课堂笔记本的课堂团队、教育Intune的学校组，以及适用于许多其他第三方应用程序的名册和单Sign-On (SSO) 集成。 详细了解 [学校数据同步概述](/schooldatasync/overview-of-school-data-sync)。
- 借助 PowerShell，可以创建团队和频道，并自动配置设置。 有关详细信息，请参阅 [Microsoft Teams PowerShell](/powershell/module/teams/?view=teams-ps)。
- 可以使用当前处于预览) 中的 Microsoft 图形 API (创建、配置、克隆和存档团队。 有关详细信息，请参阅[使用 Microsoft 图形 API与 Microsoft Teams 合作](/graph/api/resources/teams-api-overview)。
...
- **保留策略：** 使用此策略可以删除您指定的年龄超过的所有数据，以确保从聊天 (中删除旧数据，) 和频道的所有或某些用户。 还可以将 Teams 配置为保留内容，以便无法将其删除。 有关详细信息，请参阅 [Microsoft Teams 的保留策略](https://techcommunity.microsoft.com/t5/Microsoft-Teams-Blog/Retention-policies-for-Microsoft-Teams/ba-p/178011)。
- **到期策略：** 将团队配置为在设置日期后过期。 在到期前 30 天，团队的所有所有者都会收到通知，要求续订其团队，否则会将其删除。 尽管管理员可以再恢复 30 天已删除的团队。 此设置有助于确保已停用未使用的团队。 详细了解 [Microsoft 365 组过期策略](https://support.office.com/article/office-365-group-expiration-policy-8d253fe5-0e09-4b3c-8b5e-f48def064733)。

- **存档团队：** 此设置将团队置于只读模式。 仍然可以浏览和搜索它们，但没有人可以添加任何新帖子。 [存档或还原团队](https://support.office.com/article/archive-or-restore-a-team-dc161cfd-b328-440f-974b-5da5bd98b5a7)说明团队所有者如何存档团队;团队所有者还可以使用[图形 API (预览) ](/graph/api/resources/teams-api-overview)存档或还原团队。
...
是。 用户可以在创建新团队时 **从现有模板中选择“创建** 团队”，Teams 所有者还可以使用 [图形 API (预览)](/graph/api/resources/teams-api-overview)从可用模板创建新团队。

## <a name="what-tasks-can-i-automate-via-powershell-or-graph"></a>可以通过 PowerShell 或 Graph 自动执行哪些任务？

[Microsoft 图形 API (预览) ](/graph/api/resources/teams-api-overview)可以执行以下任务：
...
[PowerShell](/powershell/module/teams/?view=teams-ps) 可以执行以下任务：
...
> [!TIP]
> 图形 API和 PowerShell cmdlet 不断添加功能。 请确保经常检查 [Microsoft 图形 API (预览版) ](/graph/api/resources/teams-api-overview)和 [PowerShell](/powershell/module/teams/?view=teams-ps) 文章以获取功能增强功能。  
...
有关 Teams 策略的详细信息，请参阅 [管理组织的 Microsoft Teams 设置](enable-features-office-365.md)。
