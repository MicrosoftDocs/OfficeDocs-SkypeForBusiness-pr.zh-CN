...
2. 演练每个步骤并完成建议的更新：
   ...
   10. **安装和配置监控代理**： 此步骤安装 64 位版本的 Microsoft Azure 监视器代理和配置要连接到您的日志分析工作区的代理。
       ...
       -   有关获取操作管理套件工作区 ID 和主关键字的详细信息，请参阅[Configure 测试设备的 Azure 监控](azure-monitor-deploy.md#configure-test-devices-for-azure-monitoring)。
       -   验证选中的**SR v2 – Microsoft 监控代理包**和**禁用 64-bit 文件系统重定向**。
       -   有关监视的 Microsoft 团队聊天室部署运行状况的详细信息，请参阅[规划 Microsoft 团队聊天室管理使用 Azure 监视器](azure-monitor-plan.md)、[使用 Azure 监视器部署 Microsoft 团队聊天室管理](azure-monitor-deploy.md)和[管理 Microsoft团队 Azure 监视器聊天室设备](azure-monitor-manage.md)。
