...
本文介绍 Microsoft 团队聊天室设备使用的默认设置的远程管理, 包括应用自定义主题。
  
通过更新主 XML 文件并将副本发送到你管理的控制台，你可以更改远程管理的设备的默认设置。 本文讨论如何创建此文件，并关联到有关如何将其按需放置在远程管理的设备上的讨论。 使用此方法, 还可以在 Microsoft 团队聊天室控制台上实现自定义主题。
  
## <a name="create-an-xml-configuration-file"></a>创建 XML 配置文件

下表介绍了此示例 SkypeSettings (文件名必需) 配置文件中所示的元素。
...
如果 XML 文件的格式不正确 (变量值的类型不正确、元素的顺序不正确、元素未闭合等), 则将应用找到错误所在位置的设置, 然后在处理期间忽略文件的其余部分。 XML 中的任何未知元素都将忽略。 如果忽略某个参数，它在设备上将保持不变。 如果参数值无效, 则它的前一个值保持不变。
...
|\<ContentCameraEnhancement>  | 布尔 &#x2777; | |当设置为 true (默认值) 时, 将对内容相机图像进行数字增强: 将检测白板边缘并选择相应的缩放, 使墨迹线条得到增强, 并且在白板上书写的人员将变为透明。  <br><br> 如果想要在不使用笔绘制白板的情况下向会议参与者发送原始视频源, 而使用摄像头来显示粘滞便笺、海报或其他媒体, 请将此值设置为 false。  |
| \<主题\>  |容器 |第一 &#x2776;  |可以使用 XML 文件应用的一项功能是为组织设置自定义主题。 你可以指定主题名称、背景图像和颜色。 |
|\<ThemeName\> |字符串 &#x2778;  || 用于识别客户端上的主题。 主题名称选项包括“默认”、提供的预设主题之一或“自定义”。 <br/>  自定义主题名称应始终使用“*Custom*”这个名称。可以在控制台上将客户端 UI 设置为“默认”或预设主题之一，但应用自定义主题只能由管理员远程设置。<br/>  预设主题包括： <br/>  默认 <br/>  蓝色波浪 <br/>  数字丛林 <br/>  追梦人 <br/>  莱姆汁 <br/>  完美像素 <br/>  路线图 <br/>  夕阳 <br/>  若要禁用当前主题, 请对 ThemeName 使用 "无主题"。  |
...
&#x2777; boolean 标志可以是下列任一项: true、false、0或1。 将布尔值或数字值保留为空可能会导致 XML 格式不正确, 并防止对设置进行更改。
  
 &#x2778; 如果存在字符串参数, 则为空且为空, 则在设备上清除该参数。
  
## <a name="manage-console-settings-using-an-xml-configuration-file"></a>使用 XML 配置文件管理控制台设置

在启动时, 如果 Microsoft 团队聊天室控制台在位置**C:\Users\Skype\AppData\Local\Packages\Microsoft.SkypeRoomSystem_8wekyb3d8bbwe\LocalState**找到名为 SKYPESETTINGS 的 xml 文件, 则它将应用配置设置由 XML 文件指示, 然后删除 XML 文件。
...
- [为 Microsoft 团队聊天室配置组策略](room-systems-v2-operations.md#GroupPolicy)

- [使用 PowerShell 的远程管理](room-systems-v2-operations.md#RemotePS)和[配置文件项目](https://technet.microsoft.com/library/cc772536%28v=ws.11%29.aspx)

你可以使用你喜欢的任何方法，只要你能传输文件并在控制台设备上触发重启即可。 该文件必须是可读、可写且可通过设备的本地用户帐户进行删除 (最好是该帐户拥有且拥有授予该用户的完全权限)。 如果文件权限设置不正确，软件可能无法应用设置，成功处理后无法删除文件，甚至可能崩溃。
  
## <a name="custom-theme-images"></a>自定义主题图像

<a name="Themes"> </a>

自定义主题图像文件必须放置在**C:\Users\Skype\AppData\Local\Packages\Microsoft.SkypeRoomSystem_8wekyb3d8bbwe\LocalState**中, 在\<CustomThemeImageUrl\>变量中输入文件名和扩展名。
  
图像文件应正好3840X1080 像素, 并且必须是以下文件格式之一: jpg、jpeg、png 和 bmp。 如果你的组织需要自定义图像, 图形设计器可以使用[自定义主题 Photoshop 模板](https://go.microsoft.com/fwlink/?linkid=870441)。 它包含有关在主题图像中放置各种元素的位置以及控制台和显示器中显示哪些区域的更多详细信息。
