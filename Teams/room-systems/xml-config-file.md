...
下表说明了此示例 SkypeSettings (这是一个必需的文件名) 配置文件中所示的元素。 
...
如果 XML 文件格式错误（即变量值类型错误、元素无序、元素未闭合，等等），则在处理过程中将应用到发现错误时为止的设置，并忽略文件的其余部分。 XML 中的任何未知元素都将忽略。 如果忽略某个参数，它在设备上将保持不变。 如果参数的值无效, 则它的前一个值保持不变。
...
在启动时, 如果 Microsoft 团队聊天室控制台在位置**C:\Users\Skype\AppData\Local\Packages\Microsoft.SkypeRoomSystem_8wekyb3d8bbwe\LocalState**找到名为 SKYPESETTINGS 的 xml 文件, 它将应用配置设置由 XML 文件指示, 然后删除 XML 文件。
...
自定义主题图像文件必须放置在**C:\Users\Skype\AppData\Local\Packages\Microsoft.SkypeRoomSystem_8wekyb3d8bbwe\LocalState**中, 只需在\<CustomThemeImageUrl\>变量中输入文件名和扩展名。
  
该图像文件应为 3840X1080 像素，并且必须为以下文件格式之一：jpg、jpeg、png 和 bmp。 如果你的组织需要自定义图像, 图形设计器将发现我们的[自定义主题 Photoshop 模板](https://go.microsoft.com/fwlink/?linkid=870441)很有用。 它包含有关在主题图像中放置各种元素的位置以及控制台和显示器中显示哪些区域的更多详细信息。
