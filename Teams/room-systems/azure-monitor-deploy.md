...
本文介绍如何使用 Azure 监视器设置和部署 Microsoft 团队聊天室设备的集成端到端管理。

你可以在 Azure 监视器中配置日志分析，以提供可帮助你管理 Microsoft 团队聊天室设备的基本遥测和警报。 随着管理解决方案的成熟，你可能会决定部署其他数据和管理功能来创建更详细的设备可用性和性能视图。
...
1. [验证 Log Analytics 配置](azure-monitor-deploy.md#validate_LogAnalytics)
2. [配置日志分析管理设置的测试设备](azure-monitor-deploy.md#configure_test_devices)
3. [映射自定义字段](azure-monitor-deploy.md#Custom_fields)
4. [在日志分析中定义 Microsoft 团队聊天室视图](azure-monitor-deploy.md#Define_Views)
5. [定义警报](azure-monitor-deploy.md#Alerts)
6. [配置用于监视的所有设备](azure-monitor-deploy.md#configure_all_devices)
7. [配置其他 Azure 监视器解决方案](azure-monitor-deploy.md#Solutions)

> [!IMPORTANT]
> 尽管使用最少的配置，Azure 监视器日志分析可以监视运行 Windows 操作系统的计算机，但仍有一些 Microsoft 团队聊天室-在开始向所有 Microsoft 团队部署代理之前需要执行的特定步骤会议室设备。
> 因此，我们强烈建议你按照正确的顺序执行所有配置步骤，以进行受控设置和配置。 最终结果的质量非常多取决于初始配置的质量。

## <a name="validate-log-analytics-configuration"></a>验证 Log Analytics 配置
<a name="validate_LogAnalytics"> </a>

你需要具有 Log Analytics 工作区才能开始从 Microsoft 团队聊天室设备收集日志。 工作区是具有自己的数据存储库、数据源和解决方案的唯一日志分析环境。 如果你已有一个现有的 Log Analytics 工作区，则可以使用它监视你的 Microsoft 团队会议室部署，或者，你可以创建特定于你的 Microsoft 团队聊天室监视需求的专用 Log Analytics 工作区。

如果需要创建新的 Log Analytics 工作区，请按照文章在[Azure 门户中创建 Log Analytics 工作区](https://docs.microsoft.com/azure/azure-monitor/learn/quick-create-workspace)中的说明进行操作

> [!NOTE]
> 若要对 Azure 监视器使用日志分析，你需要具有活动的 Azure 订阅。 如果您没有 Azure 订阅，您可以创建[一个免费试用订阅](https://azure.microsoft.com/free)作为起始点。

### <a name="configure-log-analytics-to-collect-microsoft-teams-rooms-event-logs"></a>配置日志分析以收集 Microsoft 团队聊天室事件日志

日志分析仅收集在设置中指定的 Windows 事件日志中的事件。 对于每个日志，仅收集具有选定严重性的事件。

你需要配置日志分析以收集监视 Microsoft 团队聊天室设备和应用程序状态所需的日志。 Microsoft 团队会议室设备使用**Skype 会议室系统**事件日志。
...
你需要准备日志分析，以便能够监视 Microsoft 团队聊天室-相关事件。 首先，你需要将 Microsoft Monitoring agent 部署到你具有物理访问权限的一个或两个 Microsoft 团队聊天室设备，并且让这些测试设备生成一些数据并将其推送到 Log Analytics 工作区。

### <a name="install-microsoft-monitoring-agents-to-test-devices"></a>安装 Microsoft Monitoring agent 以测试设备

通过使用[将 Windows 计算机连接到 Azure 中的 Log Analytics 服务](https://docs.microsoft.com/azure/azure-monitor/platform/agent-windows)中提供的说明，将 Microsoft Monitoring agent 部署到测试设备。 本文提供有关为 Windows 部署 Microsoft Monitoring Agent 的步骤的详细信息，获取用于获取连接到的 Microsoft 团队聊天室设备的 Log Analytics***工作区 ID***和***主密钥***的说明你的 Azure 监视器部署和验证代理连接到 Log Analytics 实例的步骤。

### <a name="generate-sample-microsoft-teams-rooms-events"></a>生成 Microsoft 团队会议室事件示例

将 Microsoft Monitoring agent 部署到测试设备后，请验证 Azure 监视器是否收集了所需的事件日志数据。

> [!NOTE]
> 在安装 Microsoft Monitoring agent 后重新启动设备，并确保 Microsoft 团队聊天室会议应用已启动，以便它可以在事件日志中生成新事件。

1.  登录到[Microsoft Azure 门户](https://portal.azure.com)并转到日志分析并选择你的工作区。

2.  列出由 Microsoft 团队聊天室设备生成的检测信号事件：
    1.  选择您的工作区并转到 "**日志**" 并使用查询来检索将具有 Microsoft 团队聊天室的自定义域的检测信号记录。
    2.  示例查询：`Event | where Source == "SRS-App" and EventID == 2000`

3.  请确保查询返回包含由 Microsoft 团队聊天室会议应用生成的事件的日志记录。

4.  生成硬件问题，并验证是否在 Azure 日志分析中记录所需的事件。
    1.  拔掉测试 Microsoft 团队聊天室系统上的其中一个外围设备。 这可能是相机、话筒、麦克风或前置房间显示
    2.  等待10分钟，以便在 Azure 日志分析中填充事件日志。
    3.  使用查询列出硬件错误事件：`Event | where Source == "SRS-App" and EventID == 3001`

5.  生成应用程序问题，并验证是否已记录所需的事件。
    1.  修改 Microsoft "团队聊天室应用程序配置"，然后键入不正确的会话初始协议（SIP）地址/密码对。
    2.  等待10分钟，以便在 Azure 日志分析中填充事件日志。
...
使用自定义域从事件日志中提取特定数据。 你需要定义稍后将用于你的磁贴、仪表板视图和通知的自定义字段。 查看[日志分析中的自定义字段](https://docs.microsoft.com/azure/azure-monitor/platform/custom-fields)，并熟悉概念，然后再开始创建自定义字段。
...
5. 使用*表 1*中所示的映射。 当您定义新字段时，日志分析将自动追加** \_CF**字符串。

> [!IMPORTANT]
> 请记住，所有 JSON 和 Log Analytics 字段都区分大小写。
> 
> 请注意下表中每个自定义域所需的查询。 需要使用正确的日志分析查询才能成功提取自定义域值。
> 
 ![自定义字段定义](../media/Deploy-Azure-Monitor-5.png "自定义字段定义")

**表1**

| **JSON 字段**                   | **日志分析自定义字段** | **事件 ID** | **要与提取一起使用的查询**                   |
|:---------------------------------|:-------------------------------|:-------------|:-------------------------------------------------------|
...
## <a name="define-the-microsoft-teams-rooms-views-in-log-analytics"></a>在日志分析中定义 Microsoft 团队聊天室视图
<a name="Define_Views"> </a>

收集数据并映射自定义字段后，可以使用 "视图设计器" 开发包含各种磁贴的仪表板，以监视 Microsoft 团队会议室事件。 使用 "视图设计器" 创建以下图块。 有关详细信息，请参阅[使用日志分析中的视图设计器创建自定义视图](https://docs.microsoft.com/azure/azure-monitor/platform/view-designer)
...
你可以导入 Microsoft 团队聊天室仪表板，并快速开始监视你的设备。 执行以下步骤导入仪表板：

1.  获取 omsview 仪表板文件[SkypeRoomSystems_v2](https://go.microsoft.com/fwlink/?linkid=835675) 。
2.  登录到[Microsoft Azure 门户](https://portal.azure.com)并转到日志分析并选择你的工作区。
3.  打开**视图设计器**。
4.  选择 "**导入**"，然后选择 " **SkypeRoomSystems_v2** " 文件。
5.  选择 "**保存**"。
...
当 Microsoft 团队聊天室控制台遇到问题时，Azure 监视器可引发通知以通知管理员。

Azure 监视器包括以固定时间间隔通过计划的日志搜索运行的内置警报机制。 如果日志搜索的结果符合某些特定条件，则会创建一条警告记录。

然后，该规则可以自动运行一个或多个操作，主动通知你警报或调用另一个进程。 警报的可能选项如下：
-   发送电子邮件
-   通过 HTTP POST 请求调用外部进程
-   在 Azure 自动化服务中启动 runbook

请参阅[在 Azure 监视器中记录警报](https://docs.microsoft.com/azure/azure-monitor/platform/alerts-unified-log)，了解有关 Azure 监视器中的警报的详细信息。

> [!NOTE]
> 以下示例将在 Microsoft 团队会议室设备生成硬件或应用程序错误时发送电子邮件警报。

### <a name="configure-an-email-alert-for-microsoft-teams-rooms-hardware-issues"></a>配置 Microsoft 团队会议室硬件问题的电子邮件通知

配置在过去一小时内遇到硬件问题的 Microsoft 团队聊天室设备进行检查的警报规则。
1.  登录到[Microsoft Azure 门户](https://portal.azure.com)并转到日志分析并选择你的工作区。

2. 导航到 Log Analytics 工作区并选择 "**通知**"，然后选择 "**新建警报规则**"
...
### <a name="configure-an-email-alert-for-microsoft-teams-rooms-application-issues"></a>为 Microsoft 团队聊天室应用程序问题配置电子邮件通知
...
![示例 Azure 监视器警报电子邮件](../media/Deploy-Azure-Monitor-6.png "示例 Azure 监视器警报电子邮件")

## <a name="configure-all-devices-for-azure-monitoring"></a>配置用于 Azure 监视的所有设备
<a name="configure_all_devices"></a>配置仪表板和警报后，你可以在所有 Microsoft 团队聊天室设备上设置和配置 microsoft Monitoring agent，以完成你的监视部署。

虽然你可以在每台设备上手动安装和配置 Microsoft Monitoring agent，但我们强烈建议你充分利用现有软件部署工具和方法。

如果你是首次构建 Microsoft 团队聊天室设备，你可能希望在生成过程中包含 Microsoft Monitoring agent 设置和配置步骤。 有关详细信息，请参阅[使用命令行安装代理](https://docs.microsoft.com/azure/azure-monitor/platform/agent-windows#install-the-agent-using-the-command-line)。

### <a name="deploying-microsoft-monitoring-agent-by-using-a-group-policy-object-gpo"></a>使用组策略对象（GPO）部署 Microsoft Monitoring agent

如果你在实现 Azure 监视之前已部署 Microsoft 团队聊天室设备，你可以使用提供的脚本来使用 Active Directory 组策略对象设置和配置代理。

1.  创建共享网络路径并向 "**域计算机**" 组授予 "读取" 访问权限。

2.  从下载 Windows 的 Microsoft Monitoring Agent 64 位版本<https://go.microsoft.com/fwlink/?LinkID=517476>

3.  将安装程序包的内容提取到网络共享中。
    1.  打开命令提示符窗口，然后执行**MMASetup-AMD64/c**
    2.  指定刚刚创建的共享，然后提取内容。

4.  创建新的组策略对象，并将其分配给 Microsoft 团队聊天室计算机帐户所在的组织单位。

5.  配置 PowerShell 执行策略：
    1.  编辑新创建的组策略对象并导航到 "计算机\\配置\\策略" \\管理模板\\ windows 组件 windows PowerShell
    2.  启用 "**打开脚本执行**" 和 "设置**执行策略**" 以**允许本地脚本**。

6.  配置启动脚本：
    1.  复制以下脚本并将其另存为 Install-MMAgent。
    2.  修改 WorkspaceId、WorkspaceKey 和 SetupPath 参数以匹配您的配置。
    3.  编辑相同的组策略对象并导航到 "计算机\\配置\\ " 策略\\ Windows 设置脚本（启动/关闭）
    4.  双击以选择 "**启动**"，然后选择 " **PowerShell 脚本**"。
    5.  选择 "**显示文件**"，然后将**Install-MMAgent**文件复制到该文件夹。
    6.  选择 "**添加**"，然后单击 "**浏览**"。
    7.  选择您刚刚复制的 ps1 脚本。

7.  Microsoft 团队会议室设备应在第二次重启时安装和配置 Microsoft Monitoring agent。
...
> [!NOTE]
> 如果需要重新配置代理、将其移动到其他工作区或在初始安装后修改代理服务器设置，可以参阅[管理和维护日志分析代理](https://docs.microsoft.com/azure/azure-monitor/platform/agent-manage)的文章。

## <a name="additional-solutions"></a>其他解决方案
<a name="Solutions"> </a>

Azure 监视器通过其[解决方案库](https://docs.microsoft.com/azure/azure-monitor/insights/solutions)提供内置管理解决方案，进一步帮助你监视你的环境。 强烈建议你将[警报管理](https://docs.microsoft.com/azure/azure-monitor/platform/alert-management-solution)和[Azure 日志分析代理运行状况](https://docs.microsoft.com/azure/azure-monitor/insights/solution-agenthealth)解决方案添加到你的工作区。

> [!NOTE]
> 代理运行状况解决方案可帮助你在你的环境中识别过时或损坏的 Microsoft Monitoring agent，并且警报管理解决方案提供有关在给定时间段内引发的警报的详细信息。
