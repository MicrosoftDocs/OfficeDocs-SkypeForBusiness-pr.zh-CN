---
title: 使用Microsoft Teams和Bookings应用进行虚拟访问
author: lanachin
...
ms.reviewer: ''
description: 了解如何在Teams中使用Bookings应用计划、管理和进行虚拟访问。
...
|行业 | 示例 |
|---------|---------|
|金融服务    |  远程销售和服务的虚拟访问<br/>计划和管理银行关系经理、财务顾问和声明调整器的约会，仅举几例，为客户提供更高的效率和便利性。  |
|零售   | 虚拟拟合和咨询 <br/>安排和管理销售人员、产品专家和设计顾问的约会，以便与客户进行虚拟配件和咨询。   |
|医疗   |  患者护理的虚拟访问 <br/>安排和管理护理团队成员的约会，与患者或其他医疗保健提供商会面，讨论医疗问题。   |

本文概述了如何使用Teams中的Bookings应用计划、管理和执行虚拟访问。
...
创建特定的约会类型来表示组织提供的服务，并定制预订体验。 然后，计划程序可以使用约会类型来安排访问。
...
如果你是Bookings管理员，则每次预订此约会类型时，最多可以链接四个表单供与会者填写。 例如，可能需要与会者在加入访问之前完成注册表单。 若要链接窗体，请选择 **"链接窗体**"。 输入窗体的 URL，然后选择 **"链接**"。  (如果这是第一次链接窗体，系统会提示你创建一个Microsoft 365组来存储表单。 选择 **"创建组** "以创建组。 只需为预订日历执行此操作一次。) 
...
当计划程序使用约会类型来安排访问时，他们可以选择包括表单、删除表单或添加你链接到约会类型的任何其他表单。 与会者必须先填写表格，然后才能加入访问。
...
发送给与会者的电子邮件确认包括会议链接和附件，以便他们可以将虚拟约会添加到日历中。 工作人员还会收到电子邮件确认和会议邀请。 如果约会中包含表单，Bookings管理员和计划程序可以在访问前查看与会者是否已完成表单，并可以查看与会者的响应。
...
Microsoft Teams管理中心的["虚拟访问"使用情况报告](../teams-analytics-and-reports/virtual-visits-usage-report.md)为管理员提供了组织中Teams虚拟访问活动的概述。 此报表显示虚拟约会的详细分析，包括Bookings访问。

可以查看关键指标，例如大厅等待时间和访问持续时间。 使用此信息可深入了解使用趋势，帮助优化虚拟访问以提供更好的业务成果。

## <a name="related-articles"></a>相关文章

- [管理浏览器上Teams虚拟访问的联接体验](browser-join.md)
