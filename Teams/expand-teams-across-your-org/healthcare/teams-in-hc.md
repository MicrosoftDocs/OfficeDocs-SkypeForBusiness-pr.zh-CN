...
应用 Teams 策略包来定义可在 Teams 中执行的不同角色。 例如，指定用于：
...
- 聊天、发布消息和交流。 你的团队可就需要关注的不同患者展开持久对话。
- 与运行状况团队的成员通话并会面。 通过 Teams 音频、视频、屏幕共享、录音和脚本功能，安排单个会议或使用频道会议来管理日常会议。
- 存储和共享文件和文档。 运行状况团队是单个虚拟团队的一部分，该团队可处理 Office 文档并协作处理文档。
