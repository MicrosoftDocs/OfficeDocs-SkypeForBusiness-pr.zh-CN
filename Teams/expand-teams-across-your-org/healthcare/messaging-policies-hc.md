...
- ![数字1的图标, 引用上一个屏幕截图](../../media/sfbcallout1.png)中的标注**读取回执**已读回执使聊天消息的发件人能够知道他们的邮件何时被收件人阅读。 使用此设置指定 "已读" 回执是由用户控制、为所有人启用还是 "已禁用"。 邮件已读回执在医疗保健组织中很重要, 因为他们删除了有关是否已阅读邮件的 uncertainly。

  对于医疗保健应用程序, 为每个人选择 "**用户控制**的" 或 "**已启用**"。 请注意, 在使用 "**已启用的 for everyone** " 设置时, 为整个租户设置收据的唯一方式是仅为整个租户设置一个消息策略 (名为 "Global (组织范围默认)" 的默认策略) 或使用所有消息租户中的策略对收据使用相同的设置。

  > [!NOTE]
  > 在大型群组聊天中使用已读回执时 (例如, 超过100用户), 收据邮件可能会影响实际邮件, 并导致聊天用户不满。 您需要让用户知道这一点。 较小的群组聊天 (或许20个用户或更少) 能够更好地使用此功能。
