...
ms.locfileid: "61611329"
---
# <a name="virtual-visits-with-teams---integration-into-cerner-ehr"></a>使用证书进行虚拟Teams - 集成到 Cerner EHR

借助 Microsoft Teams EHR) 连接器 (电子健康记录，医生可以轻松与患者发起虚拟访问，或者直接从 Cerner EHR 系统咨询 Microsoft Teams 中的其他提供商。 基于 Microsoft 365 云Teams，借助支持符合 HIPAA、HITECH 认证等要求的单一中心内聊天、视频、语音和医疗保健工具，Teams轻松、安全地进行协作和通信。
...
本文介绍如何设置和配置 EHR Teams以与 Cerner 平台集成。 此外，还概述了 Cerner EHR Teams虚拟访问体验。
...
- 仅在生产 EHR Microsoft Teams环境中测试时，才 (EHR 连接器独立产品/服务) 。
...
> [!IMPORTANT]
> 这些步骤必须由组织中全局Microsoft 365管理员完成。  
...
接下来，若要设置集成，Microsoft 365管理员从 Cerner 添加快速运行状况互操作性资源 (FHIR) 基 URL 并指定环境。 根据需要配置多个 FHIR 基 URL，具体取决于组织的需求和要测试的环境。
...
> [!NOTE]
> 用户管理员Microsoft 365登录配置门户以查看集成记录并根据需要更改配置设置。
