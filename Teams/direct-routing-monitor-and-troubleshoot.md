...
如果您遇到故障排除问题，您可以使用 SBC 供应商或 Microsoft 打开支持案例。 
...
如果 sbc1.contoso.com 在前面所述的时间间隔内未发送 SIP 选项，则会将其降级。 接下来，sbc2.contoso.com 尝试通话。 如果 sbc2 无法送达呼叫，则会在生成故障之前再次尝试 sbc1.contoso.com （已降级）。 

如果一个路线中的两个（或更多） SBCs 被视为完好且相等，则会应用 Fisher Yates 无序播放，以便在 SBCs 之间分配通话。

## <a name="monitor-call-quality-analytics-dashboard-and-sbc-logs"></a>监控通话质量分析仪表板和 SBC 日志 
 
在某些情况下，特别是在初始配对过程中，可能存在与 SBCs 或直接路由服务的错误配置相关的问题。 
