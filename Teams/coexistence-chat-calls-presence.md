...
ms.locfileid: "54351152"
---
# <a name="coexistence-with-skype-for-business"></a>与 Skype for Business 共存

Skype for Business 和 Teams 客户端与用户之间的共存和互操作性由 TeamsUpgrade 模式定义，如将 Teams 与 Skype for Business 一起用于[组织的迁移和互操作性指南中所述](migration-interop-guidance-for-teams-with-skype.md)。
...
如果路由选项仍然可用，则从 Teams中预先存在的持久线程启动的呼叫或聊天将按照与该线程相同的方式路由。

如果 Teams 中预先存在的持久线程是本机线程 (即路由到 Teams) ，则来自该线程的其他聊天消息和调用将转到Teams。 如果它是一个互操作线程 (即路由到 Skype for Business) ，则其他聊天消息和呼叫Skype for Business (假设路由选项) 。

> [!NOTE]
> Teams 中的现有线程可能不再可路由，例如，当线程是现在升级到 Teams 的用户的互操作线程时。 由于它是作为互操作线程创建的，因此该线程将路由到Skype for Business，但该用户不再能够Skype for Business聊天和通话。 在这种情况下，线程将被禁用，不允许进一步通信。

### <a name="from-skype-for-business"></a>从Skype for Business

Skype for Business超过 10 分钟时，会话线程不会保留。SIP 会话超时。 在 SIP 会话过期之前，Skype for Business会话中现有线程的聊天和调用将按照与线程相同的方式路由。 来自 Skype for Business 中超过 SIP 会话超时的现有线程的调用和聊天将路由到远程方 Skype for Business，不管原始线程来自另一方一侧的哪个客户端。
...
例如，如果发起方聊天或通话应位于目标的 Skype for Business 客户端上，则 Skype for Business 客户端应显示给发起方。 如果它应位于目标Teams客户端上，则应该Teams客户端状态。
