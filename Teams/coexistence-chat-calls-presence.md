...
> [!NOTE]
> 如果接收方和发件人都在 TeamsOnly 升级模式中，则对话将是一个本机聊天体验，包括所有丰富的消息传递和呼叫功能。 若要了解详细信息，请阅读 [团队中的外部 (联盟) 用户的本机聊天体验](native-chat-for-external-users.md)。 如果其中一个对话参与者未处于 TeamsOnly 升级模式，则对话将保持带有纯文本消息的互操作体验。
...
下表显示了给定模式下的客户端 (三个最左列) 的客户端接收呼叫，具体取决于发起方的模式、所选客户端以及其 Skype for Business 客户端 (本地或 online) 。

在下面的表中： 
- **SfB \* *表示以下任何模式： _SfBOnly *、 *SfBWithTeamsCollab*、 *SfBWithTeamsCollabAndMeetings*。

- *斜体文本* 突出显示互操作对话。

- **不可能** 表示不可能聊天或呼叫的情况。 在这些情况下，发信方必须使用 Skype for Business。 这是 Microsoft 对本地/混合客户的说明性指导的原因之一是使用孤岛以外的模式 (通常 SfBWithTeamsCollab) 作为升级到团队的升级起点。

**表1a：租户新聊天或呼叫路由到孤岛模式收件人**

| <br/><br/> 众 | 创 <br/><br/> 客户端 | <br/><br/> SfB &nbsp; 托管 |<br/><br/>路线-->| 收信 <br/><br/> 孤岛  |
|--- |--- |--- |--- |--- |
...
| <br/><br/> 众   | 创 <br/><br/> 客户端 | <br/><br/> SfB &nbsp; 托管 |<br/><br/>路线--> |   收信 <br/><br/> SfB\*   |
|--- |--- |--- |---   |--- |
...
| <br/><br/> 众   | 创 <br/><br/> 客户端 | <br/><br/> SfB &nbsp; 托管 |<br/><br/>路线-->|   收信 <br/><br/> TeamsOnly  |
|--- |--- |--- |--- | --- |
...
> [!NOTE]
> 团队联合身份验证的当前实现基于 Skype for business federation，因此它利用互操作性基础结构 (该基础结构需要发起方的租户为纯在线或 Skype for business 混合) ，并提供与本机线程相比的一组精简功能。 我们预计将来会向团队联盟提供本机团队，此时该线程将是本机团队并提供完整功能。

下表描述了 (三个最左侧的列) 的客户端接收呼叫的客户端，具体取决于发起人的模式、所选客户端，以及其 Skype for Business 客户端 (本地或 online) 。
...
如果团队中预先存在的持久性线程是本机线程 (即路由到团队) ，则其他聊天消息和来自该线程的调用将转到团队。 如果是互操作线程 (即路由到 Skype for business) ，则其他聊天消息和呼叫将转到 Skype for business (再次假设路由选项可用) 。
...
* 如果用户处于 TeamsOnly 模式，则任何其他用户 (无论是在团队中还是在 Skype for business) ，都可以看到 TeamsOnly 用户的团队是否存在
* 如果用户处于任何 SfB \* 模式 (SfbOnly、SfbWithTeamsCollab、SfbWithTeamsCollabAndMeetings) ，则任何其他用户 (无论是在团队中还是在 skype for business) ，都可以看到 SfB \* 用户的 skype for business 是否存在
* 如果用户处于 " (" 或 "旧版") 模式中，则团队中的联机状态和 Skype for business 中的状态是独立的 (这些值不需要匹配) ，并且其他用户将看到孤岛用户的一种或其他状态，具体取决于它们是位于同一租户还是在联合租户中以及使用的客户端
    * 来自团队，同一租户中的任何其他用户将看到孤岛用户的团队状态;这与上面的租户内路由表对齐
    * 从团队中，联合租户中的任何其他用户将看到孤岛用户的 Skype for business 状态;这与上面的联盟路由表对齐
    * 在 Skype for Business 中，任何其他用户都会看到孤岛用户的 Skype for business 存在 (租户和联合) ;这与上面的路由表对齐
...
下表介绍了将由观察者查看的发布者的状态，具体取决于 Publisher 的模式以及新线程) 的观察程序 (的客户端模式。

**表3：租户状态 (新线程) **

|观察 <br/><br/>客户端|<br/><br/>路线--> |<br/><br/>孤岛 |Publisher <br/><br/>SfB\* |<br/>仅限团队|
|--- |--- |--- |--- |---|
...
下表介绍了将由观察者查看的发布者的状态，具体取决于 Publisher 的模式和) 的新线程的观察程序 (的客户端。 在实践中，观察程序的客户在此阶段不会有任何不同的联盟。

**表4：新线程) 的联合状态 (**

|观察 <br/><br/> 客户端 |<br/><br/>路线-->|<br/><br/> 孤岛  |Publisher <br/><br/> SfB\* |<br/><br/> 仅限团队 |
|--- |--- |--- |--- |---|
|Skype for Business |&boxv; |Skype for Business  | Skype for Business  | Teams  |
