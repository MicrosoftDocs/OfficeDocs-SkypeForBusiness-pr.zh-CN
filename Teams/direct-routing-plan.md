...
description: 了解Microsoft 电话直接路由如何允许将受支持的客户提供的会话边界控制器与 SBC (连接到) 系统Microsoft 电话系统。
ms.openlocfilehash: 9ce64ef16f7dcfed97c81a7a3fd1e77a21c9acc2
ms.sourcegitcommit: b0bb7db41856ee377dbe4ca8c9dff56385bf120d
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 12/17/2021
ms.locfileid: "61562982"
---
...
- 将几乎任何 PSTN 中继与 Microsoft 电话系统一同使用。 
...
为同一用户混合使用呼叫计划和直接路由连接是可选的，但 (例如，当用户分配了 Microsoft 呼叫计划，但想要使用 SBC) 路由某些呼叫时。 最常见的方案之一是调用第三方 PBX。  使用第三方 PBX 时，所有呼叫（对连接到该 PBX 的电话的呼叫除外）都使用 Microsoft 呼叫计划进行路由，但连接到第三方 PBX 的电话的呼叫将转到 SBC，因此停留在企业网络中，而不是 PSTN 内。 

有关许可的详细信息电话系统，请参阅[从计划选项Office](https://products.office.com/compare-all-microsoft-office-products?tab=2)[和计划选项](/office365/servicedescriptions/office-365-platform-service-description/office-365-plan-options)。 
...
### <a name="leg-between-sbc-and-cloud-media-processor-or-microsoft-teams-client"></a>在 SBC 和云媒体处理器或客户端Microsoft Teams。
适用于媒体绕过案例和非绕过情况。

会话边界控制器和云媒体处理器 (之间没有媒体旁路) 或 Teams 客户端与 SBC (（如果启用了媒体旁路）之间的直接路由接口) 可以使用以下编解码器：

- 从 SBC (云媒体处理器的非媒体旁路) ：SILK、G.711、G.722、G.729
