...
description: 了解如何Microsoft 电话系统直接路由将受支持的客户提供的会话边界控制器与 SBC (连接) 连接到 Microsoft 电话 系统。
...
Microsoft 电话系统直接路由允许将支持的、客户提供的会话边界控制器 (SBC) 连接到Microsoft 电话系统。  例如，使用此功能，可以配置本地公用电话交换网 (PSTN) 与 Microsoft Teams 客户端的连接，如下图所示： 
...
> [!Important]
> Cloud Connector Edition 将于 2021 年 7 月 31 日与 Skype for Business Online 一起停用。 组织升级到 Teams 后，请了解如何使用直接路由 将本地电话网络Teams[网络](direct-routing-landing-page.md)。 

使用直接路由，可以将 SBC 连接到几乎任何电话中继或和第三方 PSTN 设备进行互连。 使用直接路由可以： 

- 将几乎任何 PSTN 中继与 Microsoft 电话 系统一起使用。 
...
使用直接路由时，当用户参与计划的会议时，拨入号码由需要适当许可的 Microsoft 音频会议服务提供。  拨出时，Microsoft 音频会议服务使用需要适当许可的在线呼叫功能进行呼叫。  (注意如果用户没有 Microsoft 音频会议许可证，则呼叫通过直接路由路由。) 有关详细信息，请参阅使用 Teams[的联机会议](https://products.office.com/microsoft-teams/online-meeting-solutions)。 
...
|Microsoft 365或Office 365组织|用于Microsoft 365 Office 365用户Microsoft Teams配置和连接 SBC 的 Microsoft 365 或组织。|
|用户注册机构|用户必须Microsoft 365或Office 365。<br/>如果您的公司具有与 Microsoft 365 或 Office 365 的混合Skype for Business或 Lync 环境，则您无法为本地用户启用 Teams 中的语音。<br/><br/>若要检查用户的注册机构，请使用以下 Skype for Business Online PowerShell cmdlet：<br/><code>Get-CsOnlineUser -Identity \<user> \| fl HostingProvider</code> <br/><br/>cmdlet 的输出应显示：<br/><code>HostingProvider : sipfed.online.lync.com</code>|
...
|SBC (FQDN) 完全限定的域名|SBC 的 FQDN，其中 FQDN 的域部分是组织或组织Microsoft 365域Office 365之一。 有关详细信息，请参阅 [SBC 域名](#sbc-domain-names)。|
...
一Teams用户可以启动一对一呼叫Teams PSTN 或 Teams Teams呼叫并添加 PSTN 参与者。 此方案称为临时会议。 呼叫采用的路径取决于升级呼叫的用户是否分配有 Microsoft 音频会议许可证：
...
直接路由还支持获得 Microsoft 呼叫计划许可的用户。 Microsoft 电话计划的系统可以使用直接路由接口路由某些调用。 但是，用户的电话号码必须在线获取或移植到 Microsoft。  

为同一用户混合使用呼叫计划和直接路由连接是可选的，但 (例如，当用户分配有 Microsoft 呼叫计划，但想要使用 SBC) 路由某些呼叫时。 最常见的方案之一是调用第三方 PBX。  使用第三方 PBX 时，所有呼叫（对连接到该 PBX 的电话的呼叫除外）都使用 Microsoft 呼叫计划进行路由，但连接到第三方 PBX 的电话的呼叫将转到 SBC，因此停留在企业网络中，而不是 PSTN 内。 

有关许可电话系统，请参阅从计划选项[和](https://products.office.com/compare-all-microsoft-office-products?tab=2)Office[获取最大功能](/office365/servicedescriptions/office-365-platform-service-description/office-365-plan-options)。 
...
公司可能在一个租户中具有多个 SIP 地址空间。 例如，公司可能 contoso.com 作为 SIP 地址空间，fabrikam.com 第二个 SIP 地址空间。 一些用户具有 user@contoso.com，而一些用户具有 user@fabrikam.com。 
...
该证书需要将 SBC FQDN 作为 CN (公用名) 或者将主题可选名称 (SAN) 字段中。 证书应直接从证书颁发机构颁发，而不是从中间提供商颁发。
...
直接路由 SIP 接口仅信任由证书颁发机构 (证书颁发机构) Microsoft 受信任的根证书计划一部分的 CA 签名的证书。 确保 SBC 证书由程序的 CA 签名，并且证书的扩展密钥使用情况 (EKU) 包括服务器身份验证。
了解更多信息：

[计划要求 - Microsoft 受信任的根计划](/security/trusted-root/program-requirements) 
...
在会话边界控制器和云媒体处理器 (之间（没有媒体旁路) ）之间或者 Teams 客户端与 SBC (（如果启用了媒体旁路）之间使用直接路由接口) 可以使用以下编解码器：
...
仅适用于非媒体旁路情况。 使用"媒体旁路"，媒体Teams客户端和 SBC 之间流动。
