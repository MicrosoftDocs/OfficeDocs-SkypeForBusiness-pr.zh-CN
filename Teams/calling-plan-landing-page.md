...
您已完成 " [入门](get-started-with-teams-quick-start.md)"。 你已在整个组织中部署了具有[聊天、团队、频道和应用](deploy-chat-teams-channels-microsoft-teams-landing-page.md)的 Teams。 也许你已将 [会议部署 & 会议](deploy-meetings-microsoft-teams-landing-page.md)。 现在，你已准备好添加云语音工作负载，你已决定将 Microsoft Phone 系统与通话计划配合使用，连接到公共交换电话网络 (PSTN) 。 
...
每个用户是否都有直接向内拨号 () 电话号码？ | 若要获取电话号码，请参阅 [步骤3：获取电话号码](set-up-calling-plans.md#step-3-get-phone-numbers)。 |
|||
...
使用 Microsoft 365 或 Office 365 中的呼叫计划，组织中的每个用户都需要具有唯一的直接向内拨号 () 电话号码和相应的已验证的紧急地址。 您还可以在紧急地址内指定紧急地点 (例如，办公室号码或楼层号) 。 
