...
ms.locfileid: "60605198"
---
# <a name="manage-emergency-voice-routing-policies-for-direct-routing"></a>管理直接路由的紧急语音路由策略

如果已在你的组织中部署了[直接](direct-routing-landing-page.md)路由，可以使用 Microsoft Teams 中的紧急语音路由策略来设置紧急号码并指定紧急呼叫的路由方式。 紧急语音路由策略确定是否为分配了该策略的用户启用增强型紧急服务、用于呼叫紧急服务 (例如，美国) 的 911 号码，以及如何路由对紧急服务的呼叫。 

> [!Note]
> **请注意，这些语音路由策略仅适用于直接路由 -它们不适用于呼叫计划或运营商连接。**

若要管理紧急语音路由策略，可Microsoft Teams管理中心中的语音紧急  >  Windows PowerShell。 可以将策略分配给用户和网络 [站点](cloud-voice-network-settings.md)。
