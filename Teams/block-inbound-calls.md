...
在下面的示例中，租户管理员希望阻止来自号码范围 1 (312) 555-0000 到 1 (312) 555-9999 的所有调用。 创建数字模式，以便匹配范围中带 + 前缀的数字和范围中不带 + 前缀的数字。 你不需要在电话号码中包括符号和 () ，因为系统在匹配之前会剥离这些符号。  若要启用数字模式，Enabled **参数设置为** True。 若要禁用此特定数字模式，将 参数设置为 False。
...
**PhoneNumber** 参数是必需的，应是不带任何其他字符的数字字符串，例如 +、- 或 () 。 如果租户中阻止了数字，则生成的 **IsNumberBlocked** 参数返回值为 True;参数返回 False（如果未阻止）。
...
在这些示例中，可以看到电话号码 1 (312) 555-8884 被阻止，因为它位于上述阻止范围内，而电话号码 1 (312) 555-8883 允许呼叫，因为它应该基于上面创建的豁免。
