...
电话系统直接路由和呼叫计划支持阻止来自公共电话交换网和 PSTN (的) 。 此功能允许定义一个租户全局数字模式列表，以便可以针对列表检查每个传入到租户的 PSTN 呼叫的来电显示是否匹配。 如果进行了匹配，则拒绝传入呼叫。
...
使用"新建"、"获取"、"设置"和"**删除**  - **CsInboundBlockedNumberPattern** cmdlet"来管理数字模式。 可以使用这些 cmdlet 管理给定的模式，包括切换给定模式的激活。
...
可以使用"新建"、"获取"、"设置"和"删除  - **CsTenantBlockNumberExceptionPattern** cmdlet"向阻止的编号模式添加异常。
...
本示例需要 **PhoneNumber** 和 **Tenant** 参数。 **PhoneNumber** 参数应为不带任何其他字符（如 + 或 -）的数字字符串。 在 TRPS 中 **，Tenant 参数** 是可选的。 如果租户中阻止了数字，则生成的 **isNumberBlocked** 参数返回值 True;如果未阻止，则返回 False。
