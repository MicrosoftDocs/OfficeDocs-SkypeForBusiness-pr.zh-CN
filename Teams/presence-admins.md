...
用户可以手动将其当前状态设置为某些选项，其状态将反映给所有其他用户。 更多用户状态详细信息也会自动更新。 这些更改基于用户活动（可用、离开）、Outlook 日历状态（在会议中）或团队应用状态（正在进行的调用），以及列表中缩进的状态。 15分钟不活动超时，在这种情况下，当前状态将重置为 "离开"。

用户将收到在团队中发送给他们的所有聊天消息，而不考虑其状态。 如果用户向其发送消息时用户处于脱机状态，则在用户下次联机时，将在团队中显示聊天消息。 如果用户处于 "请勿打扰" 状态，则用户仍将收到聊天消息，但不会显示标题通知。

用户接收除 "请勿打扰" 状态之外的所有状态的呼叫，传入呼叫将送达其语音邮件。 如果收件人阻止呼叫方，呼叫将不会送达，并且呼叫者将收件人的状态显示为 "脱机"。

通过转到团队中的 "**设置** > "**隐私**，用户可以将人员添加到其优先级访问列表。 具有优先级访问权限的人员可以与用户联系，即使用户处于 "请勿打扰" 状态也是如此。
