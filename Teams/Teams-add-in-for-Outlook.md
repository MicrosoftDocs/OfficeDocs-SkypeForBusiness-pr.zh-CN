...
- 团队会议加载项要求主用户安排会议的 Exchange 邮箱。 确保你在 Outlook 配置文件中配置了至少一个 Exchange 邮箱，并使用它来安排团队会议与加载项。 有关 Exchange 要求，请参阅 [exchange 和团队如何进行交互](https://docs.microsoft.com/microsoftteams/exchange-teams-interact)。
...
- 请按以下步骤操作：
  ...
  - 重启 Outlook 桌面客户端。  (确保 Outlook 未在管理员模式下运行。 ) 
