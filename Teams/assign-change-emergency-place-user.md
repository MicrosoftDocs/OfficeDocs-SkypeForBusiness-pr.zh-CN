...
将电话号码分配给用户时，每个活动电话号码必须具有关联的紧急位置。  (在 Office 365 中获取电话号码时或转移电话号码时，将关联该地址。) 将号码与紧急位置关联时，还可以添加一个位置，在物理位置内提供更精确的位置。 位置可以是用户所在的楼层、建筑物侧楼或办公室号码。 对于给定的紧急位置，你可以有无限数量的地点，并且如果用户移动到其他办公室或建筑物，你可以更改位置。 例如，如果用户从 34 楼移到 35 楼。
...
3. 在" **编辑** "窗格的 **"紧急位置"下**，执行下列操作之一：
    ...
    - 若要更改已分配给用户的位置，请单击 **"X"** 以删除现有位置和位置，搜索并选择要分配的位置。
