...
| 日期 | 版本 | 更新 |
|------- | -------------------- | ------------------------------ |
| 2021 年 11 月 | [3.0.0](https://www.powershellgallery.com/packages/MicrosoftTeams/3.0.0) |<li>不久Microsoft Teams 3.x.x 系列中的 PowerShell 模块版本将成为唯一支持的版本。 所有早期版本都将位于弃用路径上。</li><li> [中断性更改] Get-CsOnlineUser 和 Get-CsOnlineVoiceUser：以下更改适用于 TeamsOnly 租户：<ul><li>这些 cmdlet 现已从早期实现迁移到较新的 API。</li><li> (-Identity 参数) ：对于 TeamsOnly 租户，不再与 Teams相关的属性已弃用。 某些属性也已重命名/替换 —请参阅[Get-CsOnlineUser](/powershell/module/skype/get-csonlineuser?view=skype-ps)和[Get-CsOnlineVoiceUser。](/powershell/module/skype/get-csonlinevoiceuser?view=skype-ps)</li><li>Get-CsOnlineUser (-Filter 参数) ：基于 AssignedPlans 和 EnterpriseVoice 的筛选格式已修改。 基于 TeamsOnly 用户的使用情况 (可筛选属性) 暂时受限 — 请参阅[Get-CsOnlineVoiceUser。](/powershell/module/skype/get-csonlinevoiceuser?view=skype-ps)</li><li>这些 cmdlet 将逐步推出，在 CY21 结束推出之前，某些租户不会遇到这些更改。</ul></li><li>[中断性变更]Get-CsTenant：不再与 Teams 相关的属性已弃用 TeamsOnly 租户 - 请参阅[Get-CsTenant。](/powershell/module/skype/get-cstenant?view=skype-ps)</li><li> 发布所有 75+ Grant-Cs \<Name\> Policy cmdlet (所有参数集) 。 它们应类似于其远程处理对应项。 将逐步推出新式版本，因此某些租户将看到早期版本的远程处理 (直到推出完成) 。</li><li> 发布所有 75+ Remove-Cs \<Name\> Policy cmdlet 的现代化版本。 它们应类似于其远程处理对应项。 将逐步推出新式版本，因此某些租户将看到早期版本的远程处理 (直到推出完成) 。</li><li> Releases [Set \| Remove]-CsPhoneNumberAssignment cmdlets。 这些 cmdlet 将逐步推出，因此，某些租户将收到一条错误消息，其中引用了在推出完成 () 之前可以使用的现有 cmdlet。</li><li> 为 [Set \| New]-CsTeamsEmergencyCallingPolicy cmdlet 发布新参数 EnhancedEmergencyServiceDisclaimer。<li> 版本 [获取 \| 添加 \| 删除]-TeamChannelUser cmdlet。</li><li> Export-CsOnlineAudioFile cmdlet。</li><li> 修复了 [获取导入 \| \| 删除]-CsOnlineAudioFile cmdlet 的错误处理。</li><li>错误处理Get-Team修复。 如果提取团队数据失败，则输出错误消息。</li><li>修复了Connect-MicrosoftTeams - AccessToken 增加了到期时间的差异。</li>
| 2021 年 11 月 | [2.6.2-preview](https://www.powershellgallery.com/packages/MicrosoftTeams/2.6.2-preview) |<li>发布所有 75+ Grant-Cs \<Name\> Policy cmdlet (所有参数集) 。 它们应与它们的远程处理对应方类似。 将逐渐推出新式版本，未进行测试的租户将看到较旧的远程处理版本。</li><li>发布所有 75+ Remove-Cs \<Name\> Policy cmdlet 的现代化版本。 它们应与它们的远程处理对应方类似。 将逐渐推出新式版本，未进行测试的租户将看到较旧的远程处理版本。</li><li>Set-CsUserCallingSettings cmdlet。 此 cmdlet 最终会在 GA 模块中发布。 在预览版模块中发布试用版。</li><li>Export-CsOnlineAudioFile cmdlet。</li><li>修复了 [获取导入 \| \| 删除]-CsOnlineAudioFile cmdlet 的错误处理。</li>
...
| 2021 年 9 月 | [2.5.2-preview](https://www.powershellgallery.com/packages/MicrosoftTeams/2.5.2-preview) |<li>注意：从此版本开始，发行说明也将随模块本身一起在 PowerShell 库中发布，以减少发行说明的延迟。</li><li>Releases [Get \| Set \| Grant New \| \| Remove]-CsTeamsEnhancedEncryptionPolicy cmdlets。</li><li>删除 [获取 \| 设置 \| 新 \| 删除]-CsTenantBlockedNumberExceptionPattern cmdlet。</li><li>修复了当模块Microsoft Teams另一个自定义 PowerShell 模块的嵌套模块时，该模块查找失败的 bug。 现在，Microsoft Teams cmdlet 可用，即使它是另一模块的嵌套模块。</li><li>Releases [获取 \| 新的 \| 完整 \| 清除]-CsOnlineTelephoneNumberOrder cmdlet。</li><li>发布 Get-CsOnlineTelephoneNumberCountry Get-CsOnlineTelephoneNumberType cmdlet。</li><li>修复了在尝试登录错误后重试Connect-MicrosoftTeams登录时发生的失败。</li><li>修复Add-TeamChannelUserRemove-TeamChannelUser通道的故障和故障。</li>
| 2021 年 8 月 | [2.5.1](https://www.powershellgallery.com/packages/MicrosoftTeams/2.5.1) |<li>现在，Connect-MicrosoftTeams的访问令牌登录使用统一的令牌数组，而不是每个特定于资源的令牌的单独参数。 可在此处找到更多详细信息[：连接-MicrosoftTeams。](/powershell/module/teams/connect-microsoftteams)</li><li>修复了 Cloudshell 中Connect-MicrosoftTeams登录失败的问题。 现在默认使用用户的登录标识，而不是提示重新身份验证。</li><li>TeamsUnassignedNumberTreatment cmdlet 现已可用。</li><li>Get-CsOnlineDialInConferencingBridge Set-CsOnlineDialInConferencingBridge cmdlet 现已从旧实现迁移到较新的 API。</li><li>仅已发布带 -identity Get-CsTenant Get-CsOnlineUser (-identity 参数) 版本。 与它们的远程处理对应对象相比，它们不再发出已弃用的属性，并且有一些格式更改。</li><li>注意：New-Team相关的更新已从 2.5.0 还原，并且已提供以前的版本以避免任何中断性更改。</li>|
