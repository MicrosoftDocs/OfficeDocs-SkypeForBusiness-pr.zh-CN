...
| 日期 | 版本 | 更新 |
|------- | -------------------- | ------------------------------ |
| 2021 年 12 月 | [3.0.1-preview](https://www.powershellgallery.com/packages/MicrosoftTeams/3.0.1-preview) |<li>发布最新版本的 Grant-CsTeamsFilesPolicy 和 Remove-CsTeamsFilesPolicy。 它们应类似于其远程处理对应项。 将逐步推出新式版本。因此，某些租户将看到较旧的远程处理 (，直到推出完成) 。</li><li> Releases [New \| Set \| Remove]-CsUserCallingDelegate cmdlets。 这些 cmdlet 最终会在 GA 模块中发布。 在预览版模块中发布试用版。</li><li>Releases [Get \| Remove]-CsTeamsShiftsConnectionErrorReport cmdlets。 这些 cmdlet 最终会在 GA 模块中发布。 在预览版模块中发布试用版。</li><li>Get-CsTeamsShiftsConnectionOperation cmdlet。 此 cmdlet 最终会在 GA 模块中发布。 在预览版模块中发布试用版。</li><li>New-CsTeamsShiftsConnectionBatchTeamMap cmdlet。 此 cmdlet 最终会在 GA 模块中发布。 在预览版模块中发布试用版。</li><li>Remove-CsTeamsShiftsScheduleRecord cmdlet。 此 cmdlet 最终会在 GA 模块中发布。 在预览版模块中发布试用版。</li>
| 2021 年 11 月 | [3.0.0](https://www.powershellgallery.com/packages/MicrosoftTeams/3.0.0) |<li>不久Microsoft Teams 3.x.x 系列中的 PowerShell 模块版本将成为唯一支持的版本。 所有早期版本都将位于弃用路径上。</li><li> [中断性更改] Get-CsOnlineUser 和 Get-CsOnlineVoiceUser：以下更改适用于 TeamsOnly 租户：<ul><li>这些 cmdlet 现已从早期实现迁移到较新的 API。</li><li> (-Identity 参数) ：对于 TeamsOnly 租户，不再与 Teams相关的属性已弃用。 某些属性也已重命名/替换 —请参阅[Get-CsOnlineUser](/powershell/module/skype/get-csonlineuser?view=skype-ps)和[Get-CsOnlineVoiceUser。](/powershell/module/skype/get-csonlinevoiceuser?view=skype-ps)</li><li>Get-CsOnlineUser (-Filter 参数) ：基于 AssignedPlans 和 EnterpriseVoice 的筛选格式已修改。 基于 TeamsOnly 用户的 (使用情况，可筛选属性也暂时受限) ，请参阅[Get-CsOnlineVoiceUser。](/powershell/module/skype/get-csonlinevoiceuser?view=skype-ps)</li><li>这些 cmdlet 将逐步推出，在 CY21 结束推出之前，某些租户不会遇到这些更改。</ul></li><li>[中断性变更]Get-CsTenant：不再与 Teams 相关的属性已弃用 TeamsOnly 租户 - 请参阅[Get-CsTenant。](/powershell/module/skype/get-cstenant?view=skype-ps)</li><li> 发布所有 75+ Grant-Cs \<Name\> Policy cmdlet (所有参数集) 。 它们应类似于其远程处理对应项。 将逐步推出新式版本，因此某些租户将看到较早的远程 (，直到推出完成) 。</li><li> 发布所有 75+ Remove-Cs \<Name\> Policy cmdlet 的现代化版本。 它们应类似于其远程处理对应项。 将逐步推出新式版本，因此某些租户将看到较早的远程 (，直到推出完成) 。</li><li> Releases [Set \| Remove]-CsPhoneNumberAssignment cmdlets。 这些 cmdlet 将逐渐推出，因此，某些租户将收到一条错误消息，其中引用了现有的 cmdlet，在推出完成 (之前，这些 cmdlet) 。</li><li> 为 [Set \| New]-CsTeamsEmergencyCallingPolicy cmdlet 发布新参数 EnhancedEmergencyServiceDisclaimer。<li> 版本 [获取 \| 添加 \| 删除]-TeamChannelUser cmdlet。</li><li> Export-CsOnlineAudioFile cmdlet。</li><li> 修复了 [获取导入 \| \| 删除]-CsOnlineAudioFile cmdlet 的错误处理。</li><li>错误处理Get-Team修复。 如果提取团队数据失败，则输出错误消息。</li><li>修复了Connect-MicrosoftTeams - AccessToken 增加了到期时间的差异。</li>
