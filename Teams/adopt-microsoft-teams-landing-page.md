---
title: 采用 Microsoft Teams
ms.reviewer: karuanag
description: 了解可用于帮助你在组织中规划和部署 Microsoft 团队的采纳资源。
ms.topic: overview
author: serdarsoysal
ms.author: serdars
manager: serdars
audience: admin
ms.service: msteams
ms.collection:
- Teams_ITAdmin_Adopt
- M365-collaboration
f1.keywords:
- CSH
ms.custom:
- ms.teamsadmincenter.dashboard.helparticle.adoption
- Adopt
search.appverid: MET150
appliesto:
- Microsoft Teams
ms.openlocfilehash: 9e731bbd4cd407b61de27db58d9b71b8b4f2c766
ms.sourcegitcommit: 532205e5a3c28b44b86cd4d1376ebee9590b8266
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 10/30/2020
ms.locfileid: "48816092"
---
# <a name="adopt-microsoft-teams"></a>采用 Microsoft Teams

> [!NOTE]
> 有关转换到远程学习和资源以帮助入门的概述，请参阅我们的[远程学习主页](https://www.microsoft.com/education/remote-learning)。

如果你是小型企业版，或者想要推出从聊天开始的团队，团队 & 频道和会议，请使用我们的规范性 [入门指南](get-started-with-teams-quick-start.md)，它旨在让你快速开始使用并运行。 如果你是具有混合或本地 Skype for business 配置的大型组织，或者你想要 (例如 Microsoft 365 或 Office 365 通话计划或电话系统) ）推出语音功能，则可以从我们的入门指南开始，但你需要以下其他指南，请参阅 [更深的采纳指南](#deeper-adoption-guidance)。

|               |               |               |               |               |               |
| :-------------| :-------------| :-------------| :-------------| :-------------| :-------------|
| <img src="https://docs.microsoft.com/office/media/icons/clock-teams.svg" alt="clock" width="40 px" height="40 px"> | [快速入门](/MicrosoftTeams/teams-adoption-quick-start-checklist) | <img src="https://docs.microsoft.com/office/media/icons/chat.svg" alt="chat bubbles" width="40 px" height="40 px"> | [团队和频道简介](/MicrosoftTeams/teams-adoption-understand-teams-and-channels) | <img src="https://docs.microsoft.com/office/media/icons/task-checklist-planning-teams.svg" alt="checklist" width="40 px" height="40 px"> | [规划您的第一个团队](/MicrosoftTeams/teams-adoption-your-first-teams) |
| <img src="https://docs.microsoft.com//office/media/icons/best-practices-teams.svg" alt="best practices" width="40 px" height="40 px"> | [为您的拥护人士准备](/MicrosoftTeams/teams-adoption-create-champions-program) | <img src="https://docs.microsoft.com/office/media/icons/education-tutorial-teams.svg" alt="prize ribbon" width="40 px" height="40 px"> | [培训](https://docs.microsoft.com/MicrosoftTeams/training-microsoft-teams-landing-page) | <img src="https://docs.microsoft.com/office/media/icons/toolbox.svg" alt="wrench" width="40 px" height="40 px"> | [工具和下载](/microsoftteams/adopt-tools-and-downloads) |

### <a name="deeper-adoption-guidance"></a>[更深入的采纳指南](#deeper-adoption-guidance)

采纳计划很容易复杂，具体取决于你的环境。 对于大规模部署，请逐步查看以下指南，以确保你的组织能够顺利过渡到团队。

|               |               |               |               |               |               |
| :-------------| :-------------| :-------------| :-------------| :-------------| :-------------|
| <img src="https://docs.microsoft.com/office/media/icons/circle-number-1-teams.svg" alt="circle number one" width="40 px" height="40 px"> | **[开始](/MicrosoftTeams/teams-adoption-phase1)** <br/> [入门](/MicrosoftTeams/teams-adoption-get-started) <br/><br/> [了解团队和频道](/MicrosoftTeams/teams-adoption-understand-teams-and-channels) <br/><br/> [创建第一个团队](/MicrosoftTeams/teams-adoption-your-first-teams) <br/><br/> [评估文化和组织准备情况](/MicrosoftTeams/teams-adoption-assess-readiness) | <img src="https://docs.microsoft.com/office/media/icons/circle-number-2-teams.svg" alt="circle number 2" width="40 px" height="40 px"> | **[试验](/MicrosoftTeams/teams-adoption-phase2-experiment)** <br/> [创建冠军计划](/MicrosoftTeams/teams-adoption-create-champions-program) <br/><br/> [管理快速入门](/MicrosoftTeams/teams-adoption-governance-quick-start)<br/><br/> [定义使用场景](/MicrosoftTeams/teams-adoption-define-usage-scenarios) <br/><br/> [登记早期采用者并收集反馈](/MicrosoftTeams/teams-adoption-onboard-early-adopters) <br/><br/> [登记支持](/MicrosoftTeams/teams-adoption-onboard-support) | <img src="https://docs.microsoft.com/office/media/icons/circle-number-3-teams.svg" alt="circle number 3" width="40 px" height="40 px"> | **[调整](/MicrosoftTeams/teams-adoption-phase3-enable)** <br/> [定义结果和成功](/MicrosoftTeams/teams-adoption-define-outcomes) <br/><br/> [优化反馈和报告](/MicrosoftTeams/teams-adoption-optimize-feedback-and-reporting) <br/><br/> [提高认识度并实施培训](/MicrosoftTeams/teams-adoption-drive-awareness) <br/><br/> [安排服务运行状况检查](/MicrosoftTeams/teams-adoption-schedule-service-health-reviews) |
