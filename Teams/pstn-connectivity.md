...
- [**运算符连接**](#teams-phone-with-operator-connect)。 使用运营商连接，如果你的现有运营商是 Microsoft Operator 连接 计划的参与者，他们可以管理 PSTN 呼叫和会话边界控制器 (SDC) 。 

- [**直接路由**](#teams-phone-with-direct-routing)，通过将会话边界控制器连接到 SBC (，)  (使用自己的 PSTN) Teams 电话。


还可以选择一组选项，用于为复杂环境设计解决方案，或管理多步骤迁移。

请注意，选择的选项会影响某些Teams 电话功能的配置方式。 有关详细信息，请参阅 [本文稍后的配置](#configuration-considerations) 注意事项。
...
- 不需要部署或维护&mdash;本地部署，因为调用计划在Microsoft 365。
...
使用运营商连接，如果你的现有运营商是 Microsoft Operator 连接计划的参与者，他们可以管理将 PSTN 呼叫引入 Teams。 运营商管理 PSTN 呼叫服务和会话边界控制器 (SDC) ，从而节省硬件购买和管理费用。
...
有关运营商运营商权益和要求连接，有关参与此计划的运营商列表，请参阅计划运营商[连接。](operator-connect-plan.md) 若要了解如何配置操作员服务连接，请参阅[配置操作员](operator-connect-configure.md)连接。
...
- 您可以选择配置和管理此选项，也可以由运营商或合作伙伴合作伙伴配置和管理 (询问您的运营商或合作伙伴是否提供此选项) 。

- 可以在电话设备（&mdash;例如第三方 PBX&mdash; 和模拟设备）之间配置互操作性，Teams 电话。
...
- 与第三方运营商的合同。
   (除非部署为一个选项，为使用呼叫计划.Teams 电话的用户提供第三方 PBX、模拟设备或其他电话设备) 
...
在某些国家和地区，绕过 PSTN 运营商降低长途呼叫费用非法。 Location-Based直接 (LBR) 允许根据用户的地理位置限制Teams免收费。 若要详细了解如何计划和配置 LBR，请参阅以下文章：
...
- 对于呼叫计划，每个用户会自动启用紧急呼叫，并且需要具有与其分配的电话号码相关联的已注册紧急地址。 动态紧急 (呼叫基于支持Teams客户端) 位置。  

- 对于接线连接，会自动为每个用户启用紧急呼叫，并且要求具有与其分配的电话号码相关联的已注册紧急地址，但只能由运营商合作伙伴设置。 动态紧急 (呼叫基于支持Teams客户端) 位置。
...
- [云语音功能的网络设置Microsoft Teams概念和术语](cloud-voice-network-settings.md)
