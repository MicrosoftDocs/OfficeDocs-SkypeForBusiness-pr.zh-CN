...
除了已分配给您的会议桥的电话号码，您还可以 [获取其他服务号码](/microsoftteams/getting-service-phone-numbers) ， (用于从其他位置) 的音频会议的收费电话号码，然后将其分配给会议桥，以便为您的用户展开覆盖范围。
...
取消为会议网桥分配的服务电话号码时，用户将不能再使用该电话号码加入任何会议。 由于电话号码已更改，因此如果任何人都可以将电话号码作为其默认号码，请务必将其设置为默认号码 (如果有) 和更新其现有会议邀请，然后再从音频会议桥中取消分配电话号码。
...
- 使用 **Set-CsOnlineDialInConferencingUserDefaultNumber** cmdlet 可基于用户的原始默认电话号码或位置来更改其默认收费电话号码或免费电话号码。
  ...
  ```
    ...
    > [!NOTE]
    > 上面使用的位置需要与在 Microsoft 365 管理中心中设置的用户 (s) 的联系人信息匹配。
...
若要获取有关桥 (s) 的详细信息，请运行以下 Powershell：
```PowerShell
