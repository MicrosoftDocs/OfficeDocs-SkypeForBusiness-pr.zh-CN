...
> [!NOTE]
> [!注释] 重置会议 ID 后，会向用户发送包含新会议 ID 的电子邮件。 此电子邮件将发送到主要电子邮件地址（在许多情况下为 Microsoft 365 或 Office 365 邮箱）。 电子邮件包含新的会议 ID、默认拨入电话号码 (以及) 现有会议的说明。 
...
- Teams 服务创建一个 7-9 位数的会议 ID。 不能更改其长度。
