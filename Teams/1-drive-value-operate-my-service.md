...
<tr><td><img src="media/audio_conferencing_image9.png" alt="An icon depicting the next steps"/><br/>下一步</td><td><ul><li>完整查看操作指南。</li><li>实施与组织的目标相一致的运营策略，以提供云语音工作负荷的质量和可靠性。</li><li>查看["监视呼叫质量"。](monitor-call-quality-qos.md)</li><li> 实施运营策略，定期执行体验质量评审，确保云语音部署以峰值功能运行。</li></ul></td></tr>
</table>
...
Microsoft Teams将跨团队或Microsoft 365技术Office 365，为团队合作提供中心。 示例包括：
...
本指南介绍管理服务的主要Teams方面。 很可能，你已针对所依赖的支持技术Teams计划。 如果没有，则还需要为本地和联机部署中的 (组件建立) 计划。 这将帮助确保用户享受优质、可靠的应用程序体验Teams。
...
必须了解服务的总体运行状况Microsoft Teams，以便主动提醒组织中其他人任何影响该服务的事件。 如上所述，Teams依赖于其他 Microsoft 365 或 Office 365 服务，例如 Azure Active Directory、Exchange Online、SharePoint Online 和 OneDrive for Business。 因此，监视依赖服务的运行状况也同样重要。
...
| Microsoft 365 或 Office 365 Service Communications API | 以编程方式Microsoft 365或Office 365运行状况。   | 当需要集成第三方监视工具或想要构建自己的解决方案时，请使用 。 |
...
Apple iOS Microsoft 365 Android Office 365 Admin Apple iOS、Android 和 Windows (电脑和移动设备) 。 该应用向服务管理员提供有关服务运行状况和即将进行的更改的信息。 该应用支持推送通知，在发布公告后几乎可以立即提醒你。 这可帮助你随时了解状态、运行状况以及即将对服务进行的任何更改。 通知支持使它成为建议管理员的监视工具。 有关详细信息，请参阅：
...
可以通过利用服务通信 API 来监视服务运行状况和即将Office 365，以编程Office 365访问服务运行状况和更改。 使用此 API 创建自己的监视工具，或连接现有的监视工具Office 365服务通信，这可能会简化监视环境的方法。 有关详细信息，请参阅Office 365[开发人员Enterprise应用](https://developer.microsoft.com/office)。
...
更新Teams自动向用户推出。 用户将始终拥有最新的客户端和功能，这些功能在 Teams服务中。 无法管理向用户推出 Teams更新，因此，通过有效的通信、培训和采用计划管理更改至关重要。 如果你的用户了解变化、了解好处并能够利用新功能，他们将能够更快适应并 &mdash; 欢迎更改。

### <a name="monitoring-for-change"></a>监视更改

更改管理的第一步是监视计划用于管理Teams。 监视这些更改的最佳来源是Office 365路线图，其中[](https://products.office.com/business/office-365-roadmap)列出了当前处于开发阶段、正在向客户推出或已完全启动的功能。 可以使用提供的筛选器Teams特定功能，也可以将路线图下载到 Excel 文件进行进一步分析。 对于每个功能，路线图提供了简短说明以及预期的发布日期。
...
有关规划使用 Teams 管理更改[Teams，请参阅为用户创建更改管理Microsoft Teams。](change-management-strategy.md)
...
初始试点开始后，必须定期测量实际使用情况Teams节奏。 这使组织能够深入了解实际使用情况如何与"构想"阶段预测的使用情况保持一致。 虽然本部分重点介绍Teams使用情况，但此部分应是测量和评估整体使用情况的Office 365工作的一部分。
...
报告仪表板Teams中的使用情况数据。 Teams使用情况数据可在三个不同的报告中找到。 第一份报告提供跨产品视图，其中显示用户如何使用 Office 365 中的各种服务进行通信和Office 365。 可在此处找到此报告[：Office 365活动用户报告](https://support.office.com/article/Office-365-Reports-in-the-Admin-Center-Active-Users-FC1CF1D0-CD84-43FD-ADB7-A4C4DFA8112D)
...
此外，" **报表读取** 者"角色适用于需要访问报表，但不执行任何需要管理员级权限的任务的用户。 可分配此角色，向利益干系人提供使用情况报告，以监视和推动采用。 有关可用不同角色的信息，请参阅[关于Office 365角色。](https://support.office.com/article/About-Office-365-admin-roles-da585eea-f576-4f55-a1e0-87090b6aaa9d)
...
1.  通过执行体验质量评审来验证质量 (请参阅[改进](monitor-call-quality-qos.md)和监视 Teams 的呼叫质量，) 。
...
| 测量使用情况 (驱动器值阶段)                            | 在部署完成后Teams"驱动器值"阶段 (评估) 。 按需要解决使用问题。 | Biweekly  |               |
...
可以通过许多不同的方法评估用户情绪。 这些方式可能包括电子邮件调查、当场或电话式面试，或者直接在用户或Teams Yammer。 有关详细信息，请参阅中[用户反馈方法](best-practices-feedback.md)Microsoft Teams。

还可使用全行业方法评估用户情绪，称为 npS (net promotor) ，如下一部分所述。

### <a name="nps"></a>NPS 

NPS (分数) 是一个全行业客户会员指标，也是评估用户情绪的一个好方法。 可通过提出两个问题计算 NPS："你向同事推荐Teams的可能性有多大？"，后跟自由格式问题"为什么？"
...
许多核心规划元素会转到优化、调整大小以及修正网络基础结构，以确保为服务提供高质量、Microsoft Teams路径。 我们的网络准备指南中介绍了规划 [任务和](3-envision-evaluate-my-environment.md#network-readiness) 要求。 由于升级、扩展或其他业务要求，网络通常会随时间而发展。 在网络规划活动中，Teams要求非常重要。
...
| 根据对 IP 和 URL Office 365网络 | 对适用的网络组件（ (防火墙、代理服务器、VPN、客户端防火墙等）进行更新) 以反映对 Office 365 URL 和[IP 地址范围的更改](/microsoft-365/enterprise/urls-and-ip-address-ranges)。                                                                                                                                                              | 根据需要              |               |
...
| 网络故障排除和修正                        | 协助Teams支持人员、服务所有者和关键利益干系人排查和解决与连接、可靠性Teams质量相关的问题。 网络元素可以包括：<ul><li>防火墙</li><li>VPN</li><li>有线和Wi-Fi网络</li><li>Internet 连接和 ExpressRoute</li><li>DNS</li></ul>    | 根据需要              |               |
...
Microsoft Teams终结点可以定义为任何 PC、Mac、平板电脑或移动设备 (运行) 客户端的其他任何Teams设备。 术语 *终结点* 不仅包括设备本身，还包括用户如何连接到设备，例如，通过使用设备的内置麦克风或扬声器、耳机或优化耳机。 部署终结点后，不得忘记终结点。 终结点Teams需要持续维护。 以下部分介绍要关注的特定领域。
...
客户端Teams基础软件平台的最低要求。 这些要求可能会随着时间的推移而改变，因此，必须监视它们以发现更改。 例如，Teams客户端具有最低 iOS 版本。 如果客户端使用 Internet 浏览器，则浏览器也需要保持最新。 可在获取客户端的客户端中找到支持的平台[Microsoft Teams。](get-clients.md)
...
Wi-Fi驱动程序可能有问题。 例如，驱动程序可能在访问点之间具有非常激进的漫游行为，这会引发不必要的接入点切换，从而导致通话质量差。 性能不佳Wi-Fi驱动程序可能通过体验质量评审发现 (请参阅改进和监视 Teams 的呼叫质量) 。 [](monitor-call-quality-qos.md) 必须实施质量驱动的流程，以监视新的用户Wi-Fi，并确保在将新驱动程序部署到一般用户群体之前已经过测试。
...
1.  **验证服务运行状况：** 用户可能遇到的问题可能与对服务及其依赖服务Teams产生负面影响的事件相关。 第一步，建议确认没有活动的服务问题。 请参阅[如何检查Office 365运行状况](https://support.office.com/article/How-to-check-Office-365-service-health-932AD3AD-533C-418A-B938-6E44E8BC33B0)。
    请记得检查从属服务的状态 (例如，Exchange、SharePoint OneDrive for Business) 。 上一部分监视服务运行状况中更详细地讨论了 [服务运行状况的监视](#monitor-service-health)。

2.  **验证客户端连接：** 连接问题会导致服务中的功能或登录Teams。 我们建议 (新站点或) 验证服务连接的位置。 确保每个Office 365遵循以下 URL 和[IP](/microsoft-365/enterprise/urls-and-ip-address-ranges)地址范围指南。 可以利用 Microsoft [网络评估工具](https://www.microsoft.com/download/details.aspx?id=53885) 执行连接测试，验证媒体端口已正确打开，以便实现云语音功能。 网络就绪性指南中提供了有关如何运行连接测试 [的详细](3-envision-evaluate-my-environment.md#network-readiness) 步骤。
...
随着组织的发展或合约，必须针对当前和未来的需求规划许可。 除了云语音Teams许可证外，还有一个基础 (电话系统音频会议) 。 [](here-s-what-you-get-with-phone-system.md) [](https://products.office.com/skype-for-business/audio-conferencing)
...
默认情况下，邮箱在 Exchange Online的所有用户都有权创建Microsoft 365组，因此，拥有 Microsoft Teams。 如果希望更严格控制和限制新团队[ (从而](assign-roles-permissions.md#permissions-to-create-teams)创建新的 Microsoft 365 组) ，可以将组创建和管理权限委派给一组管理员。 如果组织想要使用此选项，请参阅本文中所述的过程，以允许用户提交由分配的团队处理的请求。
