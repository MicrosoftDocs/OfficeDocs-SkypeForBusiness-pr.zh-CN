...
3. 对于每个团队或组 ID，请运行以下 PowerShell 脚本来标识所有相关的专用频道网站，其中 是团队 `$groupID` 的组 ID。
...
由应用在 Teams、1：1 聊天和 1xN 聊天中生成的卡内容存储在邮箱中，可以搜索。 卡 *是* 一个 UI 容器，用于提供简短的内容。 卡片可以有多个属性和附件，并且可以包含可以触发卡片操作的按钮。 有关详细信息，请参阅 [卡片](/microsoftteams/platform/task-modules-and-cards/what-are-cards)
