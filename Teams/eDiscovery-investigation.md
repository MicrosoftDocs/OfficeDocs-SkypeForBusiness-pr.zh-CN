...
也可Microsoft Teams工作流 搜索和保留某些Advanced eDiscovery[内容](/microsoft-365/compliance/overview-ediscovery-20)。 电子数据展示提供一系列搜索、保留和导出功能，Advanced eDiscovery为合规性管理员提供了更多工具来识别数据源和分析其内容。

### <a name="advanced-ediscovery-custodian-workflow-for-teams-content"></a>Advanced eDiscovery内容的管理员Teams工作流

管理员可能是各种团队的成员。 您可以捕获Teams监管员的相关内容。 有关监管员工作流的说明，请参阅[将监管员添加到Advanced eDiscovery案例。](/microsoft-365/compliance/add-custodians-to-case)

添加管理员后，单击"下一 **步"** 按钮，然后单击" **添加"** 按钮。 随后会显示一个窗口，提示你选择其他位置，该窗口会显示管理员的所有成员身份及其SharePoint的相应管理员网站位置。 从所有这些数据源和团队中，你可以选择要用于电子数据展示的内容，然后将该用户以及你标识的所有数据源放在保留状态。

可以选择是否包括其Exchange内容、OneDrive或两者。 Exchange内容包括用户邮箱中所有应用程序内容，例如其电子邮件、Teams存储在其邮箱中的应用程序内容，等等。 OneDrive内容不仅包括用户的内容，还包括 OneDrive 中存储的所有 Teams 内容，例如 1：1 聊天、1：N 聊天和聊天中共享的文件。
...
如果没有特定用户指定为监管员，您可以保留整个数据源。 有关保留详细信息，请参阅在 Advanced eDiscovery[中管理保留](/microsoft-365/compliance/managing-holds)。

为保留内容Teams保留时，您可以选择要包括在保留中的所有位置。 即使用户正在删除或更改内容，保留也将保留该内容的所有以前版本的副本。

还可使用可选查询，根据关键字、日期范围、作者和许多其他条件设置保留条件。 如果未指定关键字，则来自该数据源的所有内容都将受到保留。

### <a name="advanced-ediscovery-searches"></a>Advanced eDiscovery搜索

Teams搜索内容。 有关搜索详细信息，请参阅在搜索中[收集Advanced eDiscovery。](/microsoft-365/compliance/collecting-data-for-ediscovery) 即使一条消息与搜索查询匹配，搜索也将返回整个对话。

创建搜索查询时，可以选择监管员，以便搜索已选择的所有源。 还可以搜索非监管源，例如Teams未映射到用户的托管网站。 还可使用可选查询缩小搜索范围，缩小搜索Teams范围。
...
完成搜索后，可以单击"添加结果 **以审阅集** "按钮，并将其添加到审阅集。 有关评审集详细信息[，请参阅本文](/microsoft-365/compliance/managing-review-sets)稍后的Advanced eDiscovery管理审阅集[和](#review-sets-workflow)审阅集工作流。

#### <a name="normal-review-sets-and-conversation-review-sets"></a>普通审阅集和对话审阅集

将搜索添加到评审集时，可以从普通审阅集或对话评审集进行选择。

普通评审集类似于导出;它提供 `.msg` 单个文件用于Teams内容，并且以基本视图显示内容。 计划以后使用其他软件工具重新处理文件时，通常使用普通评审集。
...
添加到评审集时，窗口的"集合选项"部分下有若干选项作为复选框提供，包括"对话检索选项"和"Teams **对话"。** 如果启用这些选项，则作为审阅Teams一部分的任何单个邮件也将随周围其他消息一起显示，以便进行上下文处理。 例如，如果查询是特定的，因此只返回一条消息，则启用这些选项还会返回多个消息，导致和跟踪与查询匹配的消息。

许多逻辑条件用于确定其他消息是否为匹配查询的消息提供上下文。 例如，对于Teams，启用这些选项将检索父消息以及所有子消息，因为消息的线程处理方式。
...
可以通过单击"审阅集"选项卡查看现有审阅集 **或创建新** 审阅集。有关评审集详细信息，请参阅管理审阅 [Advanced eDiscovery。](/microsoft-365/compliance/managing-review-sets)

除了文档，您还可以向审阅集Teams电子邮件、Yammer和其他内容。 在评审集内，还可以执行许多可在其他上下文中执行的操作，例如搜索内容和创建自定义查询。 这些操作仅适用于已添加到评审集的项。

" **管理评审集"** 按钮提供了其他选项，例如分析、摘要报告、已添加的负载集数等。

若要访问数据的可视化效果和图表，**请单击右上角的**"单个结果 \> 搜索结果配置文件视图"。 可以单击这些图表中的"拨片"以交互方式选择要查询的内容类型。 例如，可以选择仅查询Teams内容。 也可以保存这些查询，就像保存手动写入的查询一样。

#### <a name="summary-view-text-view-and-annotate-view"></a>摘要视图、文本视图和批注视图

如果单击评审Teams对话，它将显示"摘要"视图，它将整个 Teams对话显示为可单独交互的消息列表。 单击邮件右侧向下箭头可显示一个上下文菜单，允许您查看消息详细信息或下载单个 `.msg` 文件。 单击消息详细信息会显示元数据摘要或消息的完整元数据。

若要下载 PDF，请单击摘要视图右上角的下载按钮。

单击"**文本视图**"选项卡以显示对话中已提取文本的Teams视图。 此纯文本内容适合导出，可以使用其他软件工具轻松使用它。

单击"批注 **视图"选项卡** 访问批注功能。 此选项卡以类似于对话Teams格式显示内容，但还有其他编辑选项。 有一个铅笔工具，可用于做笔记、在邮件上绘图或执行精细的划掉以用于修订。 还有一个 **"** 区域修订"工具，可用于绘制一个矩形，用于将该区域划掉并标记为"修订"。
...
"批注视图"选项卡 **的底部** 是" **标记文档** "按钮，它显示标记面板。 在此面板中，可以将标记应用到对话中Teams邮件。 可以将聊天标记为响应性或非响应性、特权或非特权、是否包含"有趣项"、是否应该包含在导出中，以及是否需要进一步评审。 还可以管理和应用其他可自定义标记。

#### <a name="action-menu"></a>操作菜单

在"审阅集"窗口中，可以通过单击"操作 **导出"导出** \> **内容**。 导出时有许多选项可用。

若要导出包含所有消息的所有元数据Teams，请单击以选中"加载 **文件"** 复选框。 若要在文件中包括已应用到内容的任何标记，请单击以选中"标记 **"** 复选框。

使用 **"本机文件** "选项导出其本机格式的文件。 你可以选择将对话导出为一个文件，或者将单个聊天消息导出到它们自己的单独文件中。

" **文本文件** "选项允许保存纯文本版本的内容。 若要详细了解如何获取审阅Teams对话的纯文本视图，请参阅上面的摘要视图、文本视图和[批注](#summary-view-text-view-and-annotate-view)视图。
