...
- **许可证激活日期**：激活许可证的日期。 如果你的帐户尚未设置，这将显示为"订阅 **挂起激活"**。
- **许可证**：购买的许可证数。
...
> [!NOTE]
> 如果全局管理员想要管理由另一个全局管理员购买的订阅，则需要在同一计费帐户中。 可以通过选择订阅中的应用，为另一个全局管理员授予对所购买的订阅Microsoft 365 管理中心。 从该页转到" **查看计费配置文件** > **""选择计费帐户** > **""分配角色** > **""其他全局管理员"**。

### <a name="invoice-billing"></a>发票计费

- 发票计费作为一些事务的支付选项提供。
- 首次使用发票计费时需要信用审核，审批最多需要 24 到 48 小时。 在信用检查完成之前，发票计费不可用。 可以使用信用卡下单，或稍后在信用审核获得批准后重试。
- 发票计费仅适用于全局管理员或同时具有服务管理员Teams计费管理员权限的管理员。
