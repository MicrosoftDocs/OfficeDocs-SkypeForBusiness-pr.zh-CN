...
RSC 权限允许团队所有者授予应用访问和修改团队数据的许可。 RSC 权限是细化Teams特定权限，用于定义应用可在特定团队中执行哪些操作。 RSC 权限的示例包括创建和删除频道、获取团队设置以及创建和删除频道选项卡的功能。 

RSC 权限在应用清单中定义，而不是在Azure AD。 将应用添加到团队时，需要授予对 RSC 权限的许可。 有关详细信息，请参阅[RSC (特定于资源) 。 ](/microsoftteams/platform/graph-api/rsc/resource-specific-consent)
...
3. 在 **"Microsoft Graph RSC** (权限) "下，查看应用请求的 RSC 权限。
