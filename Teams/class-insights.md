...
假设主管也负责教学，则他们既是教师又是教育领导者，并且可以访问见解的两种视图（适用于教师和教育领导者）。 在这里，见解可帮助教育领导者为教师和学生提供支持。 在组织级别，IT 管理员需要连接学生信息系统，并为每个角色分配访问与其相关的学校或部门的权限。
...
例如，校长只能看到其学校的课堂，或者部门负责人只能看到部门的课堂。 学生数据在课堂、部门、学校和地区级别进行汇总，我们（基于每个用户的权限）在每个级别提供见解。教师和教育领导者都可以放大并查看各个学生的数据。
...
教师最了解他们的学生。 见解提供的信息可帮助教师在数字学习场景中 **为学生提供支持**。 它复制了通常可在亲身体验中获得的见解。 例如，假设某个学生在特定时期内没有活动，或者上周未按时完成所有作业。 该数据将呈现给教师，以便提供适当的提示或签入该名学生。 教师有责任与学生以及学生的家人或监护人互动，以确定所发现的任何活动或非活动的根本原因。
...
* 快速启动并运行 - [获取见解单页 PDF](https://aka.ms/insights/start)。
...
默认情况下，Insights 处于打开状态，这使我们能够收集 Teams 中学生活动的分析，以便在见解中使用。 你可以决定选择退出 Insights，在这种情况下，我们 *删除为 Insights 收集的所有数据* 并停止收集任何未来数据。 如果重新启用 Insights，我们将从重新启用时开始收集数据。

* 要关闭或打开 Insights，请打开 [SDS 管理中心](https://sds.microsoft.com/)，转到“**设置**” > “**管理教育版 Insights**”。 关闭或打开 **为 Insights 收集活动数据** 以禁用或启用 Insights。

:::image type="content" source="media/insights-settings.png" alt-text="SDS 管理中心页面的屏幕截图，可在其中使用设置选项打开或关闭 Insights。":::
