...
若要防止当会议拨出发送到电话号码且被呼叫者无法接听呼叫时语音邮件系统连接到会议，你可以设置 Teams 以请求被呼叫者确认他们加入会议。 如果被呼叫者无法接听呼叫，并且呼叫由语音邮件系统应答，则语音邮件系统将不会连接到会议，因为它不会提供加入会议的确认。

启用此功能后，接听电话拨出或呼叫我的人必须通过在传统或移动电话上按 1 确认他们想要加入会议。
