...
拨号计划范围确定可以应用拨号计划的层次结构级别。 范围是以外的其他业务服务器的本地部署的 Skype 中。 客户端可通过用户登录 Skype for Business Online 时自动提供的配置设置获得相应的拨号计划。 作为管理员，你可以使用 Remote PowerShell 管理和分配拨号计划范围级别。
