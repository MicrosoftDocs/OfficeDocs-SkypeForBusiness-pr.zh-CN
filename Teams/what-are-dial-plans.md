...
拨号计划的作用域确定可应用拨号计划的层次结构级别。范围与在本地部署 Skype for business 服务器不同。客户通过预配设置来获取相应的拨号计划，用户可在用户登录到团队或 Skype for Business Online 时自动提供这些设置。作为管理员，你可以使用远程 PowerShell 管理和分配拨号计划范围级别。
