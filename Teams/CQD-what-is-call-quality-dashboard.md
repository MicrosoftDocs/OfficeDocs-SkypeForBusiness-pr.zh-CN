...
- seo-marvel-apr2020
description: 了解 CQD (呼叫质量) 以及如何使用它来查看有关会议质量的报告，以及如何在 Microsoft Teams。
ms.openlocfilehash: 92b7bdcd5acaa86c530f5d0380666b2ebce3eed8
ms.sourcegitcommit: 556fffc96729150efcc04cd5d6069c402012421e
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 08/26/2021
ms.locfileid: "58593266"
---
...
最新版本的 CQD 具有近实时 [的 (NRT) ](CQD-data-and-reports.md)数据源，这意味着呼叫记录在通话结束后 30 分钟内在 CQD 中可用。
...
例如，使用 CQD，可以确定用户使用每用户调用分析 () 观察到的用户通话质量不佳) 是由于网络问题也影响许多其他用户。 CQD 捕获单个呼叫体验和使用 Teams 或 Skype for Business。 使用 CQD 时，总体模式可能会变得明显，因此网络工程师可以就呼叫质量进行明智的评估。 CQD 提供呼叫质量指标报告，让你深入了解总体呼叫质量、服务器-客户端流、客户端流和语音质量[SLA。](https://go.microsoft.com/fwlink/p/?linkid=846252) 
  
![通话质量仪表板的屏幕截图。](media/teams-difference-between-call-analytics-and-call-quality-dashboard-image3.png)

在 CQD 中，建议上传建筑物和终结点信息，这样Location-Enhanced报表来分析用户建筑物内的呼叫质量和可靠性。 可以评估数据，以确定问题是隔离给单个用户还是影响较大用户段。 若要在 CQD 中启用建筑物或终结点特定的视图，[管理员必须在](CQD-upload-tenant-building-data.md)"CQD 租户数据"页上上传Upload信息。

![通话质量仪表板的"呼叫质量"Location-Enhanced屏幕截图。](media/teams-difference-between-call-analytics-and-call-quality-dashboard-image4.png)

不要错过我们的["管理](quality-of-experience-review-guide.md)呼叫和会议质量"一文，其中为负责管理 Teams 服务质量的管理员或支持工程师提供了深入Teams。

## <a name="legacy-version-of-cqd-cqdlynccom"></a>旧版 CQD (CQD.lync.com) 

当前版本的 CQD (https://CQD.Teams.microsoft.com) 已替换旧版 CQD https://CQD.lync.com) (。 你仍然可以使用 CQD.lync.com (管理中心Skype for Business中提供) ，但截至 2020 年 7 月 1 日，它使用的是 CQD 提供的数据。Teams.microsoft.com，你无法再查看或修改建筑物或查询旧 CQD (CQD.lync.com) 。 如果尚未从数据库迁移此数据 CQD.lync.com，请记录支持票证。

> [!IMPORTANT]
> 自 2021 年 7 月 31 日起，我们将停用旧版 CQD (CQD.lync.com) 。 该日期之后，会自动重定向到 CQD。Teams.microsoft.com 访问 CQD.lync.com，任何未分配建筑物或查询数据都将丢失。
