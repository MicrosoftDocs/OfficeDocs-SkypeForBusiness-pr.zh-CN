...
如果呼叫计划不可用，或者你计划使用现有的 PSTN 连接提供商，则需要将企业语音部署（或使用现有本地部署或云连接器版本的混合语音部署）转换为 Microsoft 电话 系统直接路由。 若要将用户升级到 Teams，请参阅直接路由的其他电话系统[注意事项](./direct-routing-landing-page.md)。