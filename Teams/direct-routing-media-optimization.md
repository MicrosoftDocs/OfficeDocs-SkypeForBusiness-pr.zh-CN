...
# <a name="local-media-optimization-for-direct-routing"></a>用于直接路由的本地媒体优化
 ...
 (PSTN) 语音的公共交换电话网络被视为一种业务关键型应用程序，对语音质量有很高的预期。 直接路由允许你控制媒体流量流，以适应全球各地的各种企业的多种网络拓扑和本地电话设置。 

直接路由的本地媒体优化允许你通过以下方式管理语音质量：

-   控制团队客户端和客户会话边界控制器之间媒体通信流的方式 (SBCs) 。
-   在企业网络子网的边界内保持媒体的本地。
-   允许团队客户端和 SBCs 之间的媒体流，即使 SBCs 位于具有专用 Ip 的企业防火墙之后，也不会直接显示到 Microsoft。

本地媒体优化支持两种方案：

- 通过集中的 SBC 将所有本地中继集中的 SBC 连接到主会话启动协议， (SIP) 主干-向公司的所有本地分支机构提供电话服务。
...
- **在欧洲** ，Contoso 拥有大约30个国家/地区的办事处。 每个 office 都有其自己的专用分支 Exchange (PBX) 。 
...
- **在 APAC 地区** ，Contoso 在不同的国家/地区有多个办事处。 
  ...
  在许多国家/地区，公司仍有时间划分的多路复用 (TDM) 本地分支机构中的中继。 TDM 中继的集中化不是 APAC 地区的一个选项，因此无法切换到 SIP。 假设 APAC 区域中有超过50个 Contoso 分支办事处，其中有数百个网关 (SBCs) 。 在这种情况下，不能将所有网关与直接路由接口配对，因为缺少公共 IP 地址和/或本地 internet breakouts。 此外，某些国家/地区强加了无法在没有本地 PSTN 网络连接的情况下满足的法规要求。

根据业务需求，Contoso 实施了两个解决方案，其中包含用于直接路由的本地媒体优化：

- **在欧洲** ，所有中继都是集中式的，并且在中心 SBC 和用户之间基于用户位置进行媒体流。 

  - 如果用户连接到企业网络的本地子网 (也就是说，用户是内部) ，在中心 SBC 的内部 IP 和用户的团队客户端之间的媒体流。 
...
- **在 APAC 区域中** ，集中的代理 SBC 与 Microsoft Direct 路由进行了配对，后者定向直接路由接口和本地分支机构中的下游 SBCs 之间的媒体。 
  ...
  本地分支办公室中的下游 SBCs 不直接在 APAC 中直接显示，但它们的配对方式是使用 Set-CSOnlinePSTNGateway cmdlet 在 Microsoft Phone 系统中创建虚拟网络拓扑。 如果可能，媒体始终保持本地。 外部用户在团队客户端与代理 SBC 的公共 IP 之间有媒体流动。


## <a name="central-sbc-with-centralized-trunks"></a>具有集中中继的中心 SBC

若要构建一个解决方案，其中 PSTN 服务通过单个中心 SBC 和连接的集中 SIP 主干一起提供给所有本地分支机构，Contoso 租户管理员对一位 SBC (centralsbc.contoso.com) 服务;SBC 有一个与之连接的集中 SIP 中继。 
...
下图显示了当用户不在本地且未连接到企业网络时的通信流 (也就是说，用户的设备通过移动设备或公共 Wi-fi) 连接到 Internet。 用户通过团队进行直接路由通话：
...
若要构建一个解决方案，其中 PSTN 服务在 APAC 区域中的所有本地分支机构中均提供，其中，TDM 中继的集中不是一个选项，Contoso 管理员对直接路由服务中的 SBC (proxysbc.contoso.com) （也称为代理 SBC）。 

之后，Contoso 管理员添加了一些下游的 SBCs，指示可以通过代理 SBC proxysbc.contoso.com 访问它们。 下游 SBCs 不具有公共 Ip，但可以将它们分配给语音路由。 下表显示了示例网络参数和配置。

当用户位于下游 SBC 所在的本地分支机构中时，媒体流量将直接在用户和本地下游 SBC 之间流动。 如果用户在公共 internet) 上的 office (外部，则媒体将从用户流向代理 SBC 的公共 IP，并将其代理到相关下游 SBC (s) 。
...
-  代理 SBC 仅通过外部 IP 地址对电话系统可见，并路由下游 SBC (中的流，在此情况下，越南) 到手机系统的本地 SBC。 
...
下图显示了当用户超出公司网络边界时的通信流。 用户不在本地 (不在企业网络) 的边界内。 用户通过团队将直接路由电话拨入到越南的电话号码。 

- 用户的团队客户端通过 REST API 直接与电话系统通信，但在呼叫过程中生成的媒体首先排在新加坡的代理 SBC 的外部 IP 地址。 

- 根据配置和语音策略 (参阅 [配置本地媒体优化](direct-routing-media-optimization-configure.md) 以获取详细信息) ，代理 SBC 将该流重定向到越南的下游 SBC。 
...
为了区分本地媒体优化模式，租户管理员需要使用 Set-CSonlinePSTNGateway cmdlet 将-BypassMode 参数设置为每个 SBC 的 "Always" 或 "OnlyForLocalUsers"。 有关详细信息，请参阅 [配置本地媒体优化](direct-routing-media-optimization-configure.md)。  
...
德国的 SBC 可以在 "始终绕过" 模式下进行配置。 无论用户的位置如何，用户都将通过 SBC 的内部 IP 地址直接连接到 SBC (例如从华北到德国;有关参考) ，请参阅下图。
...
将阿姆斯特丹中的 SBC 配置为德国本地下游 SBC 的代理 SBC。 用户与本地 SBC 的企业网络在同一子网中的德国位于德国。 SBCs (代理和下游) 都配置为始终绕过模式。 联机语音路由策略指定，如果在德国通话中 (区号 + 49) 应将它们路由到德国的本地 SBC。 所有其他通话--在德国的 SBC 出现故障时，应以阿姆斯特丹的形式将其路由到代理 SBC。 下表总结了示例配置。 
...
- 代理 SBC 仅通过外部 IP 地址对电话系统可见，并路由下游 SBC (中的流，在这种情况下，德国的本地 SBC) 到电话系统。 
...
将阿姆斯特丹中的 SBC 配置为德国本地下游 SBC 的代理 SBC。 SBCs (代理和下游) 都配置为始终绕过模式。 法国的内部用户（位于本地分支机构）正在与德国进行直接路由呼叫。 联机语音路由策略指定通过区号 + 49) 拨打德国 (应路由到德国的本地 SBC。 所有其他通话--在德国的 SBC 出现故障的情况下，所有通话都应以阿姆斯特丹的形式路由到代理 SBC。 下表总结了示例配置。 
...
例如，在 APAC 区域中，假设 Contoso 拥有不同国家/地区的多个办事处。 对于许多国家/地区，由于公司在许多本地分支机构中仍然有 TDM 中继，因此无法切换到 SIP。 TDM 中继的集中化不是 APAC 地区的一个选项。 此外，在 APAC 区域中，有数百个50多个网关的多个 Contoso 分支办事处 (SBCs) 。 
...
假定已将新加坡中的 SBC 配置为适用于越南和印度尼西亚本地下游 SBCs 的代理 SBC。 用户在与本地 SBC 位于同一位置的越南中。 联机语音路由策略指定在越南 (中，区号为 + 84) 的电话应路由到越南的本地 SBC。 所有其他呼叫-如果越南中的 SBC 失败，则在越南的调用中应路由到新加坡的代理 SBC。 下表总结了示例配置。 
...
- 代理 SBC 仅通过外部 IP 地址对电话系统可见，并路由下游 SBC (中的流，在这种情况下，越南) 到手机系统的本地 SBC。 
...
假定已将新加坡中的 SBC 配置为适用于越南和印度尼西亚本地下游 SBCs 的代理 SBC。 在印度尼西亚的内部用户（位于本地分支机构中）是向越南进行直接路由呼叫。 联机语音路由策略指定对越南 (区号 + 84) 的调用应路由到越南的本地 SBC。 所有其他呼叫（如果越南中的 SBC 失败），应将对越南的调用路由到新加坡的代理 SBC。 已将新加坡中的代理 SBC 设置为 "始终绕过" 模式，并且越南中的本地 SBC 在 "本地用户" 模式下设置为 "仅适用于本地用户" 模式。 下表总结了示例配置。 
...
以下是当前在本地媒体优化中存在的已知问题的列表。 Microsoft 正在努力解决这些问题。

| 问题 | 规避 |
| :--- | :--- |
| 团队客户端公共 IP 与客户信任的 IP 列表匹配时，不会将团队客户端标识为 **内部** 团队客户端。 | 本地媒体优化要求团队客户端子网与租户配置的[网络子网](https://docs.microsoft.com/powershell/module/skype/new-cstenantnetworksubnet?view=skype-ps)相匹配|
| 当团队客户端被标识为内部时，呼叫升级会导致呼叫中断。| 在直接路由 SBC 上禁用本地媒体优化。|
