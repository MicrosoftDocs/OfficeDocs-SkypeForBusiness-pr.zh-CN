...
作为管理员，您获取的呼叫分析的所有功能的完全访问权限。 此外，您可以分配支持人员的 Azure Active Directory 角色。 团队 communications 支持专家角色分配用户应具有有限的呼叫分析视图。 团队 communications 支持工程师角色分配用户需要访问呼叫分析的全部功能。 这两个权限级别防止对其余的 Microsoft 团队 & Skype 业务管理中心的访问。
...
- **团队服务管理员或团队 communications 管理员**： 和团队 & Business Admin Center 的 Skype 调用分析中，您有权访问所有信息。
    
- **团队 communications 支持专员**： 请参阅一组有限的呼叫分析中的数据。 您可以解决呼叫，但将交给会议问题的团队 communications 支持工程师。 您无需访问业务 Admin Center 团队 & Skype 的其余部分。
    
- **团队 communications 支持工程师**： 请参阅呼叫分析中的所有可用的数据并帮助解决问题呼叫和会议。 您无需访问业务 Admin Center 团队 & Skype 的其余部分。
...
2. 转到 Office 365 管理中心，并使用工作或学校帐户登录。 在 web 浏览器前往*https://adminportal.services.skypeforbusiness.com*。

3. 选择**管理中心** > **团队 & Skype**。 
    
4. 在**仪表板**，在**用户搜索**，开始键入名称或您想要解决或选中**查看用户**的用户列表，请参阅其呼叫用户的 sip 地址。
    
    ![在工作组 & Skype 业务管理中心的用户搜索框的呼叫分析的屏幕截图。](media/use-call-analytics-to-troubleshoot-image-1.png)
  
5. 从列表中选择用户。

6. 选择**呼叫历史记录**，，然后选择您想要解决该呼叫或会议。
    
    ![屏幕快照显示了用户的呼叫历史记录页。](media/use-call-analytics-to-troubleshoot-image-2.png)
  
7. 选择**高级**选项卡，然后查找的黄色和红色项目指示质量欠佳的呼叫质量或连接问题。
