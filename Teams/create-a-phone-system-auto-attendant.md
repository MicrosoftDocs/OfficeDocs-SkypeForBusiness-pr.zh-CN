...
> [!TIP]
> 本文适用于大型组织。 如果你的组织是一家小型企业，请改为阅读 [“设置自动助理 - 小型企业”教程](/microsoftteams/business-voice/create-a-phone-system-auto-attendant-smb) 。
...
要用于 Dial By Extension 的用户需要在 Active Directory (中定义的以下电话属性之一中指定扩展，并通过Azure AD 连接) 或Azure Active Directory进行同步。  (有关详细信息，请参阅 [单独或批量添加用户](/microsoft-365/admin/add-users/add-users) 。) 

- OfficePhone/PhoneNumber (AD 和 Azure AD) 
- HomePhone (AD) 
- 移动/移动电话 (AD 和Azure AD) 
- OtherTelephone (AD) 
